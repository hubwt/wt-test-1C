...
Функция текстДляКарточекНоменклатуры()
	текстзапроса ="ВЫБРАТЬ
	              ...
	              |			ИЗ
	              |				РегистрСведений.дт_МестаХраненияНоменклатуры.СрезПоследних(, ) КАК МестаХранения
	              |			
	              |			СГРУППИРОВАТЬ ПО
	              ...
	              |	ВТ_ПредВывод.КолвоОстаток КАК КолвоОстаток,
...
Функция текстДляСпискаСотрудников() 
	...
	|	ВТ_ДолжностиНовые.Отдел КАК ДолжностьНов,
	|	ВТ_ДолжностиНовые.Должность КАК ОтделНов,
...
Функция ПолучитьВыданыеgetissue(Запрос)
	...
	ЗапросКоличествоПоСотруднику.Текст = "ВЫБРАТЬ
	                                     ...
	                                     |	ПродажаЗапчастейТаблица.Ссылка.ОтветственныйКладовщик
	                                     |ИЗ
	                                     |	Документ.ПродажаЗапчастей.Таблица КАК ПродажаЗапчастейТаблица
	                                     |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК ИндНомер
	                                     |		ПО ПродажаЗапчастейТаблица.Партия = ИндНомер.индкод
	                                     ...
	                                     |	И ПродажаЗапчастейТаблица.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	                                     |	И ПродажаЗапчастейТаблица.СтатусТовара
	                                     |	И ПродажаЗапчастейТаблица.Ссылка.ОтветственныйКладовщик В(&Сотрудники)
										 |  И ИндНомер.Стеллаж.Владелец В (&Склад)
	                                     |
	                                     |СГРУППИРОВАТЬ ПО
	                                     |	ПродажаЗапчастейТаблица.Ссылка.ОтветственныйКладовщик
...
Функция ПолучитьПотеряшкиОбщgetlostall(Запрос)  
	...
	//ЗапросТоваров = Новый запрос;
	//ЗапросТоваров.Текст = "ВЫБРАТЬ
	//                      |	НАЧАЛОПЕРИОДА(ЛогИндНомера.дата, ДЕНЬ) КАК Дата,
	...
	//                      |
	//                      |СГРУППИРОВАТЬ ПО
	//                      |	НАЧАЛОПЕРИОДА(ЛогИндНомера.дата, ДЕНЬ)
	//                      |;
	//                      |
	//                      |////////////////////////////////////////////////////////////////////////////////
	//                      |ВЫБРАТЬ
	...
	//                      |////////////////////////////////////////////////////////////////////////////////
	//                      |ВЫБРАТЬ
	//                      |	ЕСТЬNULL(ВТ_Потери.Потери, 0) - ЕСТЬNULL(ВТ_Нашли.Нашли, 0) КАК Потерянные,
	//                      |	ЕСТЬNULL(ВТ_Нашли.Нашли, 0) - ЕСТЬNULL(ВТ_Потери.Потери, 0) КАК Найденные,
	//                      |	НАЧАЛОПЕРИОДА(ДанныеПроизводственногоКалендаря.Дата, ДЕНЬ) КАК Дата
	//                      |ИЗ
	//                      |	РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеПроизводственногоКалендаря
	//                      |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Нашли КАК ВТ_Нашли
	//                      |		ПО ДанныеПроизводственногоКалендаря.Дата = ВТ_Нашли.Дата
	//                      |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Потери КАК ВТ_Потери
	//                      |		ПО (ВТ_Потери.Дата = ДанныеПроизводственногоКалендаря.Дата)
	//                      |ГДЕ
	...
	//                                     |////////////////////////////////////////////////////////////////////////////////
	//                                     |ВЫБРАТЬ
	//                                     |	СУММА(ВТ_Результат.Потерянные) КАК Потерянные,
	//                                     |	СУММА(ВТ_Результат.Найденные) КАК Найденные,
	//                                     |	ВТ_Результат.ОтветственныйНашли КАК ОтветственныйНашли,
	//                                     |	ВТ_Результат.ОтветственныйПотери КАК ОтветственныйПотери
	//                                     |ИЗ
	//                                     |	ВТ_Результат КАК ВТ_Результат
	//                                     |
	//                                     |СГРУППИРОВАТЬ ПО
	//                                     |	ВТ_Результат.ОтветственныйНашли,
	//                                     |	ВТ_Результат.ОтветственныйПотери";
	//
	...
	//МассивСтатусовОбш = новый массив;
	СтруктураСтатусовобщ = Новый структура;	
	//СтруктураСтатусовобщ.Вставить("date",Строка(НачалоДня(ТекущаяДата())));
	...
	//МассивСтатусовОбш.Добавить(СтруктураСтатусов);
...
Функция СоздатьЗадачуcreatework(Запрос)
	...
	///+ГомзМА 26.02.2024
	Если число(Массив.level) = 1 тогда 
		ПоставитьПриоритетЗадачи(Документы.Задача.НайтиПоНомеру(Новаязадача.номер), Справочники.Сотрудники.НайтиПоКоду(Массив.executor).Пользователь);
	Иначеесли число(Массив.level) = 3 тогда
		ПоставитьПриоритетЗадачиСрочно(Документы.Задача.НайтиПоНомеру(Новаязадача.номер), Справочники.Сотрудники.НайтиПоКоду(Массив.executor).Пользователь);
	КонецЕсли;
	///-ГомзМА 26.02.2024
	Попытка
		Если НаборЗаписей.Количество() = 1 Тогда
			ТекстЛога = " Для вас создана задача - " + Новаязадача.номер + " " +Новаязадача.Наименование ;
		...
		ТекстЛога = "Для " + Строка(Исполнитель) + " создана задача - " + Новаязадача.номер+ " " +Новаязадача.Наименование;	
		если число(Массив.level) = 3 тогда
			ТекстЛога = "❗️❗️СРОЧНО❗️❗️ " + ТекстЛога ;	
		КонецЕсли;
...
Процедура ПоставитьПриоритетЗадачи(Задача, Исполнитель)
	...
	Запись.Ответственный = Исполнитель;
...
Процедура ПоставитьПриоритетЗадачиСрочно(Задача, Исполнитель)
	...
	ЗапросСписокЗадач.УстановитьПараметр("Ответственный", 	 Исполнитель);
...
Процедура СдвинутьПорядокЗадачКНачалу(НомерПриоритета, Исполнитель)
...
Процедура СдвинутьПорядокЗадачПлюсОдин(КонечныйПриоритет, НачальныйПриоритет, Исполнитель)
...
Процедура СдвинутьПорядокЗадачККонцу(КонечныйПриоритет, НачальныйПриоритет, Исполнитель)
...
Функция СменаПриоритетаЗадачиChangePriorityTask(Запрос)
	...
	|	РегистрСведений.ПриоритетыЗадач КАК ПриоритетыЗадач
	|ГДЕ
	|	ПриоритетыЗадач.Ответственный = &Исполнитель
	|	И ПриоритетыЗадач.Приоритет > 0";
	
	ЗапросКоличесвоЗадач.УстановитьПараметр("Исполнитель", Документы.Задача.НайтиПоНомеру(Запрос.ПараметрыURL["id"]).Исполнитель);
	...
	Если ЗаписьРегистраСведений.Выбран() И ЗаписьРегистраСведений.Задача <> Документы.Задача.ПустаяСсылка() Тогда
		Если ЗаписьРегистраСведений.Приоритет > Приоритет Тогда
			СдвинутьПорядокЗадачПлюсОдин(ЗаписьРегистраСведений.Приоритет, Приоритет, ЗаписьРегистраСведений.Ответственный);
		Иначе 
			СдвинутьПорядокЗадачККонцу(Приоритет, ЗаписьРегистраСведений.Приоритет, ЗаписьРегистраСведений.Ответственный);
		КонецЕсли;
...
Функция СменаОтветственногоЗадачиChangeExecutorTask(Запрос)
	...
	Испонитель = Справочники.Сотрудники.НайтиПоКоду(Массив.executor).Пользователь; 
	...
	Пока выборка.Следующий() Цикл 
		...
		Попытка
			ЗакрытьВремяЗадачи(Выборка.ссылка,ОбъектЗадачи.исполнитель);
			ОбъектЗадачи.исполнитель = Испонитель;
...
Функция ПолучитьСписокКарточекТоваровGetProductsList(Запрос)
	...
	запросТовара = Новый Запрос;
		...
		запросТовара.Текст =  СтрШаблон(текстДляКарточекНоменклатуры(), Формат(?(Число(Запрос.ПараметрыURL["count"])>0,Запрос.ПараметрыURL["count"],10000), "ЧГ="));
		запросТовара.УстановитьПараметр("наименование", "%00%" );
		запросТовара.УстановитьПараметр("НачинаяСЗаписи", 0);
	...
	ИндКоды = тз.ВыгрузитьКолонку("код");
	Фотки = ПолучениеФото(ИндКоды);
	итер = 0;
...
Функция ОтчётКонвейерПоСотрудникуpipelinereportperson(Запрос) 
	...
	//	СтруктураИнфо.Вставить("count",выборка.Количество);
	//	СтруктураИнфо.Вставить("date",Строка(выборка.Дата));
	//	СтруктураИнфо.Вставить("person",Строка(выборка.Ответственный));
	//	МассивДанных.Добавить(СтруктураИнфо); 
	//	
	//КонецЦикла;
