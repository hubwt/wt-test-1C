...
НаборЗаписей = РегистрыСведений.Telegram_Пользователи.СоздатьНаборЗаписей();  			
	...
	НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() = 1 Тогда
			ТекстЛога = " Для вашего направления создана задача - " + Новаязадача.номер + " " +Новаязадача.Наименование + символы.ПС + "Автор: " + Строка(Ответственный);
...
КонецЦикла;
		...
		ТекстЛога = "Для " + Строка(Новаязадача.Отдел) + " создана задача - " + Новаязадача.номер+ " " +Новаязадача.Наименование + символы.ПС + "Автор: " + Строка(Ответственный);	
		если число(Массив.level) = 3 тогда
			ТекстЛога = "❗️❗️СРОЧНО❗️❗️ " + ТекстЛога ;	
		КонецЕсли;
...
Функция ПолучитьСтатистикуПродажgetsalesstatistics(Запрос)
	...
	Для каждого Менеджер Из ВыборкаСотрудников Цикл
		...
		Конверсия = Формат(?(КоличествоПродаж <> 0, КоличествоЗаявок / КоличествоПродаж, 0), "ЧДЦ=2");
	...
	ОбщаяКонверсия = Формат(?(ОбщееКоличествоПродаж <> 0, ОбщееКоличествоЗаявок / ОбщееКоличествоПродаж, 0), "ЧДЦ=2");
...
МассивКатегорий= Новый Массив;
  ...
  ЗапросОргСтруктуры.Текст ="ВЫБРАТЬ
                            ...
                            |ИТОГИ ПО
                            |  Ссылка ТОЛЬКО ИЕРАРХИЯ" ;
  ЗапросОргСтруктуры.УстановитьПараметр("Ссылка",Справочники.Подразделения.НайтиПоКоду("000000105"));
