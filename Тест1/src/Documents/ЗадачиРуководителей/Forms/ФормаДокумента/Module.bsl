...
Процедура ДобавитьЧек(Команда)
	...
	Иначе
		Этаформа.Записать();
		ОткрытьФормуМодально("РегистрСведений.ЧекЛистыРуководителей.ФормаЗаписи",
...
Процедура УдалитьЧекНаСервере(ТекСтрока)
	...
	Запись = РегистрыСведений.ЧекЛистыРуководителей.СоздатьМенеджерЗаписи();
...
Процедура СписокЧекВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе
		...
		КлючЗаписиРегистра = Новый ("РегистрСведенийКлючЗаписи.ЧекЛистыРуководителей", МассивКонструктораКлюча);
		ОткрытьФормуМодально("РегистрСведений.ЧекЛистыРуководителей.Форма.ФормаЗаписи", Новый Структура("Ключ", КлючЗаписиРегистра));
	КонецЕсли;
	ОбновлениеДиаграммы();
	Элементы.СписокЧек.Обновить();
КонецПроцедуры
...
Процедура ОбновлениеДиаграммы()
	...
	|	ЧекЛистыРуководителей.Задача КАК Задача,
	|	ЧекЛистыРуководителей.Выполнено КАК Выполнено
	|ИЗ
	|	РегистрСведений.ЧекЛистыРуководителей КАК ЧекЛистыРуководителей
	|ГДЕ
	|	ЧекЛистыРуководителей.Задача = &Задача";
...
Процедура ПовышениеПулаЧеков(ТекСтрока)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЧекЛистыРуководителей.Задача КАК Задача,
	               |	ЧекЛистыРуководителей.Ответственный КАК Ответственный,
	               |	ЧекЛистыРуководителей.Выполнено КАК Выполнено,
	               |	ЧекЛистыРуководителей.Описание КАК Описание,
	               |	ЧекЛистыРуководителей.Дата КАК Дата,
	               |	ЧекЛистыРуководителей.Порядок КАК Порядок
	               |ИЗ
	               |	РегистрСведений.ЧекЛистыРуководителей КАК ЧекЛистыРуководителей
	               |ГДЕ
	               |	ЧекЛистыРуководителей.Задача = &Задача
	               |	И ЧекЛистыРуководителей.Порядок > &Порядок
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Порядок";
	Запрос.УстановитьПараметр("Задача",Объект.Ссылка);
...
Процедура ПовыситьПорядокНаСервере(ТекСтрока)
	...
	|	РегистрСведений.ЧекЛистыРуководителей КАК ЧекЛистыРуководителей
	|ГДЕ
	|	(ЧекЛистыРуководителей.Порядок = &Порядок
	|			ИЛИ ЧекЛистыРуководителей.Порядок = &Порядок - 1)
	|    И ЧекЛистыРуководителей.Задача = &Задача
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок";
