...
Процедура ПереключитьПометкуКнопкиРедактирование()
	...
	// Волков И.О. 23.11.2023 ++ 
	РольРазборщик = ПроверкаНаРольРазборщик();
	
	Если РольРазборщик Тогда 
...
Функция ПроверкаНаРольРазборщик()
	
	РольРазборщик = Ложь;
	
	Если РольДоступна("Разборщик") Тогда 
	     Возврат Истина;
	КонецЕсли;
	
	Возврат РольРазборщик;
