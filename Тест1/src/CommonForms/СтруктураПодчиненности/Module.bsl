...
Функция ПредставлениеОбъектаДляВыводаВОтчет(Выборка)
	...
	Если ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Тогда
		// ++ obrv 04.08.20
		// Удалить время из представления
		Если СтрНайти(ПредставлениеОбъекта, ":", НаправлениеПоиска.СКонца) <> 0 Тогда
			Индекс = СтрНайти(ПредставлениеОбъекта, " ", НаправлениеПоиска.СКонца);
			Если Индекс <> 0 Тогда
				ПредставлениеОбъекта = Лев(ПредставлениеОбъекта, Индекс - 1);
			КонецЕсли;
		КонецЕсли;
		// -- obrv 04.08.20
		...
		Если (Выборка.СуммаДокумента <> 0) И (Выборка.СуммаДокумента <> NULL) Тогда
			...
			// ++ obrv 04.08.20
			Если Выборка.Валюта = NULL Тогда
				ПредставлениеОбъекта = ПредставлениеОбъекта + "руб.";
			КонецЕсли;
			// -- obrv 04.08.20
		КонецЕсли;
	Иначе
	...
	// ++ obrv 04.08.20
	Если ЗначениеЗаполнено(Выборка.ДополнительныйРеквизит1) Тогда
		ПредставлениеОбъекта = ПредставлениеОбъекта + " (" + Выборка.ДополнительныйРеквизит1 + ")";
	КонецЕсли;
	// -- obrv 04.08.20
