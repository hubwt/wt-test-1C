...
Процедура УстановитьПараметр(код)
	...
	// Параметр код - это ссылка 
	Партии.Параметры.УстановитьЗначениеПараметра("Товар", код);
	//ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Партии, "Товар", код,,, Истина);
	// -- obrv 24.07.18
КонецПроцедуры
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	УстановитьПараметр(Элементы.Список.ТекущаяСтрока);
	// -- obrv 24.07.18
	Если Элементы.Список.ТекущиеДанные.Состояние = ПредопределенноеЗначение("Перечисление.Состояние.Новый") Тогда
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ПродажаЗапчастей") Тогда
		...
		ДобавитьВПродажуСерв(ВыбранноеЗначение,Элементы.Список.ТекущаяСтрока, 
						Элементы.Партии.ТекущиеДанные.КолвоОстаток,Элементы.Партии.ТекущиеДанные.Машина,
						Элементы.Партии.ТекущиеДанные.Цена,Элементы.Партии.ТекущиеДанные.наценкаФ,
						Элементы.Партии.ТекущиеДанные.Код, Отказ);
...
&НаСервере
Процедура ДобавитьВПродажуСерв(ВыбранноеЗначение, СсылкаНом,Колво,Машина,Цена,цена2,укод, Отказ = Ложь)
...
Процедура ОткрытьСайт(Команда)
	// Вставить содержимое обработчика.
	ОбщДействия.ПоискКодаНаСайте( ЭтаФорма.Элементы.Список.ТекущиеДанные.Код);
КонецПроцедуры
...
Процедура ОткрытьПродажу(Команда)
	...
	рез = НайтиПоступление(Элементы.Партии.ТекущиеДанные.Машина, Элементы.Партии.ТекущиеДанные.Код);
	Если рез <> Неопределено Тогда
		ОткрытьФорму("Документ.ПоступлениеЗапчастей.Форма.ФормаДокумента", Новый Структура("Ключ,НоСтроки", рез.Ссылка, Рез.НоСтроки), ЭтаФорма)
	КонецЕсли
КонецПроцедуры

&НаСервере
Функция НайтиПоступление(Машина, Код)
	//Товар = Справочники.Номенклатура.НайтиПоКоду(Код);
	запрос = Новый Запрос;
	запрос.Текст = "ВЫБРАТЬ
	               |	ПоступлениеЗапчастей.НомерСтроки КАК НомерСтроки,
	               |	ПоступлениеЗапчастей.Ссылка КАК Ссылка
	               |ИЗ
	               |	Документ.ПоступлениеЗапчастей.Таблица КАК ПоступлениеЗапчастей
	               |ГДЕ
	               |	ПоступлениеЗапчастей.Ссылка.Машина = &Машина
	               |	И ПоступлениеЗапчастей.Код = &Товар
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Ссылка УБЫВ";
	...
	Если Выборка.Следующий() Тогда
		...
		возврат Новый Структура("Ссылка,НоСтроки", Выборка.Ссылка, Выборка.НомерСтроки);
...
Процедура ПартииВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// ++ obrv 24.01.18
	//Струк.Вставить("машина",Элементы.Партии.ТекущиеДанные.Машина.Наименование);
	Струк.Вставить("машина",Элементы.Партии.ТекущиеДанные.Машина);
	// -- obrv 24.01.18
	Струк.Вставить("Цена",Элементы.Партии.ТекущиеДанные.НаценкаФ);
