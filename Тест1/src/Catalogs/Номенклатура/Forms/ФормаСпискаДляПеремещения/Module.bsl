...
&НаСервере
Процедура ПереучестьДетальНаСервере(НашаДеталь)
	Отказ = Ложь;
	...
	списание    = ПолучитьДокументПоступления();
...
Функция ПолучитьДокументПоступления()
	Запрос = новый  Запрос;
	Запрос.Текст =   "ВЫБРАТЬ
	...
	|ГДЕ
	|	СписаниеЗапчастей.Дата > &Дата
	|	И СписаниеЗапчастей.Номер = &Номер";
	Запрос.УстановитьПараметр("Дата", НачалоГода(ТекущаяДата()));
	Запрос.УстановитьПараметр("Номер", "000000431");
...
&НаКлиенте
Процедура ОткрытьСтраницуНаСайте(Команда)
	
	///+ГомзМА 13.09.2023
	ИндКод = ЭтаФорма.Элементы.Партии.ТекущиеДанные.индкод;
	ОбщДействия.ОткрытьСтраницуНаСайте(ИндКод);
	///-ГомзМА 13.09.2023
