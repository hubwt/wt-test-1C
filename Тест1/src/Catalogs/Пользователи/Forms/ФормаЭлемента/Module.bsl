...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ПрочитатьНастройкиСогласования(ЭтаФорма, ТекущийОбъект);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаписатьНастройкиСогласования(ЭтаФорма, ТекущийОбъект);
...
Процедура ПрочитатьНастройкиСогласования(Форма, ТекущийОбъект)
	
	Форма.Согласование_ЗаявкаНаРасход_ВидыРасходов.Очистить();
	
	Настройки = ТекущийОбъект.НастройкиСогласования.Получить();
	
	Если Настройки = Неопределено 
		ИЛИ НЕ ТипЗнч(Настройки) = Тип("Структура") Тогда
			
		Возврат
	КонецЕсли;
	
	Если Настройки.Свойство("ЗаявкаНаРасход") Тогда
		
		НастройкаДокумента = Настройки.ЗаявкаНаРасход;
		...
		НастройкаДокумента.Свойство("СуммаМаксимальная", Форма.Согласование_ЗаявкаНаРасход_СуммаМаксимальная);
		
		ВидыРасходов = Неопределено;
		НастройкаДокумента.Свойство("ВидыРасходов", ВидыРасходов);
		
		Если ТипЗнч(ВидыРасходов) = ТИп("Массив") Тогда
			Форма.Согласование_ЗаявкаНаРасход_ВидыРасходов.ЗагрузитьЗначения(ВидыРасходов);
		КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ЗаписатьНастройкиСогласования(Форма, ТекущийОбъект)
	
	Настройки = Новый Структура();
	НастройкиДокумента  = Новый Структура();
	НастройкиДокумента.Вставить("ВидыРасходов", Форма.Согласование_ЗаявкаНаРасход_ВидыРасходов.ВыгрузитьЗначения());
	НастройкиДокумента.Вставить("СуммаМаксимальная", Форма.Согласование_ЗаявкаНаРасход_СуммаМаксимальная);
	
	Настройки.Вставить("ЗаявкаНаРасход", НастройкиДокумента);
	
	ТекущийОбъект.НастройкиСогласования = Новый ХранилищеЗначения(Настройки);
