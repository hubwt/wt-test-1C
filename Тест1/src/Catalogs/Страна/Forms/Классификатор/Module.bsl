...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Мета = Метаданные.Справочники.Страна;
	...
	Элементы.КлассификаторКонтекстноеМенюИзменить.Видимость = ПравоДоступа("Изменение", Метаданные.Справочники.Страна);
	
	ЕстьПравоРедактироватьСтраны = ПравоДоступа("Изменение", Метаданные.Справочники.Страна);
...
Процедура ОткрытьФормуЭлементаКлассификатора(ДанныеЗаполнения, ЭтоНовый = Ложь)
	...
	КонецЕсли;
	Форма = ОткрытьФорму("Справочник.Страна.ФормаОбъекта", ПараметрыФормы, Элементы.Классификатор);
...
Процедура ОповеститьОСозданииЭлементов(Ссылка)
	...
	Оповестить("Справочник.Страна.Изменение", Ссылка, ЭтотОбъект);
КонецПроцедуры
...
Функция ДанныеВыбораЭлементаКлассификатора(Знач ДанныеСтраны)
	...
	Ссылка = Справочники.Страна.НайтиПоКоду(ДанныеСтраны.Код);
	ЭтоНовый = Не ЗначениеЗаполнено(Ссылка);
	Если ЭтоНовый Тогда
		Страна = Справочники.Страна.СоздатьЭлемент();
...
Функция СостояниеКлассификатора()
	Данные = Справочники.Страна.ТаблицаКлассификатора();
	...
	Запрос = Новый Запрос("ВЫБРАТЬ Код ИЗ Справочник.Страна ГДЕ Предопределенный");
...
Функция СтраныНетВСпискеСтран(Страна)
	...
	Запрос.Текст = 
		...
		|ИЗ Справочник.Страна КАК СтраныМира
