...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "РаботаСФайламиСлужебный.УдалитьУстаревшиеОбработчикиОбновления";
	Обработчик.РежимВыполнения = "Оперативно";
...
Процедура ПередУдалениемПрисоединенногоФайлаСервер(Знач Ссылка,
	...
	Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске И НЕ Том.Пустая() Тогда
		ПолныйПуть = ПолныйПутьТома(Том) + ПутьКФайлу;
		// ++ obrv 05.09.18
		Если дт_РаботаСФайлами.ЕстьСсылкиНаФайл(Том, ПутьКФайлу, Ссылка) Тогда
			Возврат
		КонецЕсли;
		// -- obrv 05.09.18
...
Функция ОбновитьВерсиюФайла(ФайлСсылка,
	...
	Иначе
		...
		Если ВерсияСсылка = Неопределено Тогда
			...
			ТекВерсия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлСсылка, "ТекущаяВерсия");
			
			Если ТекВерсия.Пустая() Тогда
				Версия = Справочники.ВерсииФайлов.СоздатьЭлемент();
			Иначе
				Версия = ФайлСсылка.ТекущаяВерсия.ПолучитьОбъект();
			КонецЕсли;
...
Процедура УдалитьУстаревшиеОбработчикиОбновления() Экспорт
	...
	ОбновлениеИнформационнойБазы.УдалитьОтложенныйОбработчикИзОчереди("ФайловыеФункцииСлужебный.ОбновитьПутьТомаLinux");
