...
Функция СменаСтатусаЗаявкиChangeStateApp(Запрос)
	...
	Статус  	= Справочники.СтатусыWT.НайтиПоКоду(Массив.state);
	ПодСтатус   = Перечисления.ПодстатусыОбработкиЗаявок.Получить(Массив.sub_state - 1);

	Если Заявка.WTPanel <> Статус Тогда
		Заявка.WTPanel = Статус;
	КонецЕсли;

	Если (Массив.sub_state - 1) = 0 Тогда
		Заявка.ПодстатусОбработки = ПодСтатус;
	КонецЕсли;

	Если (Массив.sub_state - 1) = 1 Тогда
		ЗакрытьВремяЗК(Автор);
		ОткрытьВремяЗК(Заявка.Ссылка, Автор, Справочники.СтатусыWT.НайтиПоКоду("000000010"));
		Заявка.ПодстатусОбработки = ПодСтатус;
	КонецЕсли;

	Если (Массив.sub_state - 1) = 2 Тогда
		ЗакрытьВремяЗК(Автор);
