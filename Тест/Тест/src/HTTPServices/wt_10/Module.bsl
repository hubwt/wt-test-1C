...
//++ МазинЕС 24-07-2024	
	...
	СтруктураОтвет = ТекстЗапросаПопулярныйТоварИзЗаявок(Количество,Страница);
...
/// Комлев 7/8/24 +++
	Если Число(Запрос.ПараметрыURL["filter_stock"]) = 1 Тогда
		...
		запросТовара.УстановитьПараметр("наименование", ИндКод);
		НижнийНовгород = Справочники.Склады.НайтиПоКоду("000000002");
		запросТовара.УстановитьПараметр("Склад", НижнийНовгород);
		Выборкаобщ = запросТовара.Выполнить().Выбрать().Количество();
	ИначеЕсли Число(Запрос.ПараметрыURL["filter_stock"]) = 2 Тогда
		запросТовара.Текст =  СтрШаблон(Текстзапроса, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
			Запрос.ПараметрыURL["count"], 10000), "ЧГ="), ТекстФильтраПоСкладам); // Добавить отбор по складу
		запросТовара.УстановитьПараметр("наименование", ИндКод);
		Москва = Справочники.Склады.НайтиПоКоду("000000005");
		запросТовара.УстановитьПараметр("Склад", Москва);
		Выборкаобщ = запросТовара.Выполнить().Выбрать().Количество();
	ИначеЕсли Число(Запрос.ПараметрыURL["filter_stock"]) = 3 Тогда
		запросТовара.Текст =  СтрШаблон(Текстзапроса, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
			Запрос.ПараметрыURL["count"], 10000), "ЧГ="), ТекстФильтраПоСкладам); // Добавить отбор по складу
		запросТовара.УстановитьПараметр("наименование", ИндКод);
		Екатеринбург = Справочники.Склады.НайтиПоКоду("000000008");
		запросТовара.УстановитьПараметр("Склад", Екатеринбург);
		Выборкаобщ = запросТовара.Выполнить().Выбрать().Количество();
	Иначе
		запросТовара.Текст =  СтрШаблон(Текстзапроса, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
...
Функция ПолучитьПоследнийУчтенныйТоварGetLastCountedProducts(Запрос)
	...
	Страница = Запрос.ПараметрыURL["page"];
	Фото = ПолучениеФотоУчтенногоТовара(Страница, Количество);
	...
	Для Каждого Элемент Из Фото Цикл
		МассивИндкодов.Добавить(Строка(Элемент.indCode));
		НовСтр = ТабЗнч.Добавить();
		НовСтр.indCode = Строка(Элемент.indCode);
		Если Элемент.images.main <> "" Тогда
			НовСтр.images = Строка(Элемент.images.main);
		Иначе 
			НовСтр.images = Строка(Элемент.images.common[0]);
		КонецЕсли;
		НовСтр.dateTimeFirstPhotoAdded = Строка(Элемент.dateTimeFirstPhotoAdded);
		НовСтр.photographer = Строка(Элемент.photographer);
	КонецЦикла;
	

	
	ТабЗнч.Сортировать("dateTimeFirstPhotoAdded");
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПоступлениеЗапчастейТаблица.Партия.Наименование КАК Партия,
	|	ПоступлениеЗапчастейТаблица.Ответственный.Наименование КАК Ответственный,
	|	ПоступлениеЗапчастейТаблица.ДатаДобавления КАК Дата
	|ИЗ
	|	Документ.ПоступлениеЗапчастей.Таблица КАК ПоступлениеЗапчастейТаблица
	|ГДЕ
	|	ПоступлениеЗапчастейТаблица.Партия.Наименование В (&СписокПартий)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ";

	Запрос.УстановитьПараметр("СписокПартий", МассивИндкодов);
	...
	Для Каждого Стр Из ТЗ_Товары Цикл
		...
		НайденнаяСтрока  = ТабЗнч.Найти(Строка(Стр.Партия),"indCode");
		//Если НайденнаяСтрока <> Неопределено Тогда
			СтруктураТовара = Новый Структура;
			
			СтруктураТовара.Вставить("id", Стр.Партия);
			СтруктураТовара.Вставить("date", НайденнаяСтрока.dateTimeFirstPhotoAdded);
			
			//@skip-check query-in-loop
			СтруктураТовара.Вставить("uchetchik", Строка(ПолучитьКодСотрудника(Стр.Ответственный)));
			СтруктураТовара.Вставить("photographer", Строка(НайденнаяСтрока.photographer));
			СтруктураТовара.Вставить("photo", Строка(НайденнаяСтрока.images));
...
//	
//	ЗапросУчтенногоТовара.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1000
//								  |	ПРЕДСТАВЛЕНИЕ(ПоступлениеЗапчастейТаблица.Ответственный) КАК Ответственный,
//								  |	ПоступлениеЗапчастейТаблица.ДатаДобавления КАК Дата,
//								  |	ПРЕДСТАВЛЕНИЕ(ПоступлениеЗапчастейТаблица.Партия) КАК Партия
//								  |ИЗ
//								  |	Документ.ПоступлениеЗапчастей.Таблица КАК ПоступлениеЗапчастейТаблица
//								  |ГДЕ
//								  |	ПоступлениеЗапчастейТаблица.Ответственный <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
//								  |
//								  |УПОРЯДОЧИТЬ ПО
//								  |	Дата УБЫВ";
...
//
//	Пока Выборка.Следующий() Цикл
//		МассивИндКодов.Добавить(Выборка.Партия);
...
//	КонецЦикла;
//
	//МассивФотокВесь = ПолучениеФото(МассивИндКодов);
//	МассивФоток = Новый Массив;
//
//	Для Каждого ОбъектФото Из МассивФотокВесь Цикл
//		Если ОбъектФото.urls.Количество() <> 0 Тогда
//			ОбъектФото1 = Новый Структура;
//			ОбъектФото1.Вставить("indCode", ОбъектФото.indCode);
//			ОбъектФото1.Вставить("urls", ОбъектФото.urls[0]);
//			ОбъектФото1.Вставить("photographer", ОбъектФото.photographer);
//			МассивФоток.Добавить(ОбъектФото1);
//
//		КонецЕсли;
//	КонецЦикла;
//
//	КарточкиТовараСФото = Новый Массив;
//
//	Для Каждого ОбъектСФото Из МассивФоток Цикл
//		Для Каждого Карточка Из МассивКарточек Цикл
//			Если Карточка.id = ОбъектСФото.indCode Тогда
//				Карточка.Вставить("photo", ОбъектСФото.urls);
//				Карточка.Вставить("photographer", ОбъектСФото.photographer);
//				КарточкиТовараСФото.Добавить(Карточка);
//			КонецЕсли;
//		КонецЦикла;
//	КонецЦикла;
//
...
//	Для Каждого Элемент Из КарточкиТовараСФото Цикл
//
//		Если (Счетчик >= Нач) И (Счетчик <= Кон) Тогда
//			ВыходнойМассив.Добавить(Элемент);
//		КонецЕсли;
//		Если ВыходнойМассив.Количество() >= Число(Количество) Тогда
//			Прервать;
//		КонецЕсли;
//		Счетчик = Счетчик + 1;
//	КонецЦикла;
//	ОбщКоличество = КарточкиТовараСФото.Количество();
//	Попытка
...
//	СтруктураОтветаОбщ.Вставить("count", ОбщКоличество);
//	СтруктураОтветаОбщ.Вставить("page", КоличествоСтраниц);
//	СтруктураОтветаОбщ.Вставить("products", ВыходнойМассив);
//
...
//		КонецЦикла;
	...
	ИндКоды = ТЗ_Товары.ВыгрузитьКолонку("Коды");
	Фотки = ПолучениеФотокарточек(ИндКоды);
	Итер = 0;
...
//	СтруктураОтвета.Вставить("info", СтруктураИнфо);
	...
	Если МассивТоваров.Количество() = 0 Тогда
		...
		Пока ВыборкаАналоги.Следующий() Цикл
			...
			МассивФоток = ПолучениеФотокарточек(МассивКодов);
			...
			СтруктураАналогов.Вставить("photos", МассивФоток[0].images.common);
...
Функция	ТекстЗапросаПопулярныйТоварИзЗаявок(Количество,Страница)
//++ МазинЕС 24-07-2024/
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ЗаказКлиента.Ответственный.Наименование, ""менеджер не указан"") КАК ОтветственныйНаименование,
		|	ЗаказКлиента.Дата КАК Дата,
		|	ЕСТЬNULL(ВТ_Остатки.КолвоОстаток, 0) КАК КолвоОстаток
		|ИЗ
		|	Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
		...
		|		ПО (ЗаказКлиентаТовары.Номенклатура = ВТ_Остатки.Товар)
		|ГДЕ
		...
		|	ЕСТЬNULL(ЗаказКлиента.Ответственный.Наименование, ""менеджер не указан"")
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
...
//	КоличествоРезультат = ВыборкаДетальныеЗаписи.Количество(); 
	КоличествоФото = 0;
	Счетчик =0;
	Нач = Количество*(Страница-1) ;
	Кон = Количество*Страница -1 ;
	...
	ФоткиИндкод = ПолучениеФотоБезИндНомер(МассивИндкод); 
	Фоткикарточек = ПолучениеФотокарточек(МассивКодов);
	ФоткиОбщие = новый Массив; 
			...
			Для каждого Строка Из ФоткиИндкод Цикл 
				...
				Если Строка.indCode = ВыборкаДетальныеЗаписи.ПартияНаименование Тогда
					...
					Если Строка.urls.Количество() > 0 Тогда 
						...
						Структура.Вставить("url",МассивТест[0]); 
						Структура.Вставить("type","PRODUCT");
						// ++ 
						Структура.Вставить("count_registered",ВыборкаДетальныеЗаписи.КолвоОстаток); 
						Структура.Вставить("manager",Строка(ВыборкаДетальныеЗаписи.ОтветственныйНаименование)); 
						//-- 
				...
				Если Строка.id = ВыборкаДетальныеЗаписи.Код Тогда
					Структура = Новый Структура; 
						Если Строка.images.main <> "" тогда 
							...
							Структура.Вставить("url",МассивПром[0]);
	...
	СтруктураОтвет.Вставить("count",Счетчик);
...
Функция ПолучениеФотоУчтенногоТовара(Страница, Количество)
	//@skip-check empty-except-statement
	Попытка
		Соединение = Новый HTTPСоединение("192.168.0.245", 8085);
