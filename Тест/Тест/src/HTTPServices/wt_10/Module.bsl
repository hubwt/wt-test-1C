...
//++ МазинЕС 24-07-2024/
	...
	ТекстЗапроса =
		...
		|ИЗ
		|	РегистрНакопления.РегистрНакопления1.Остатки(,%1) КАК РегистрНакопления1Остатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
		|				ПО (ЗаказКлиента.Ответственный.Ссылка = Сотрудники.Пользователь.Ссылка)
		...
		|		ПО (ВТ_МестоРаботы.Пользователь.Ссылка = ЗаказКлиентаТовары.Ссылка.Ответственный.Ссылка)
		|ГДЕ
...
/// %1 - &Склад // %2 - &МестоРаботы
	...
	Если Город = 1 Тогда
		...
		ТекстСклад 			= "Склад.Ссылка = &Склад";
		ТекстМестоРаботы	=  "ГДЕ Сотрудники.МестоРаботы = &МестоРаботы";
		Запрос.Текст = СтрШаблон (ТекстЗапроса,ТекстСклад,ТекстМестоРаботы); 
		Запрос.УстановитьПараметр("Склад", СкладСсылка);
		Запрос.УстановитьПараметр("МестоРаботы", ГородСсылка);
		РезультатЗапроса = Запрос.Выполнить();
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать(); 
