...
//++ МазинЕС 24-07-2024	
	...
	//ФильтрПоКоличеству = Число(Запрос.ПараметрыURL["filter_count"]);
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
...
Функция ПолучитьПоследнийУчтенныйТоварGetLastCountedProducts(Запрос)
	...
	|	ПоступлениеЗапчастейТаблица.Товар.Наименование КАК ТоварНаименование
	...
	Для Каждого Стр Из ТабЗнч Цикл
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			СтруктураТовара.Вставить("name", Строка(НайденнаяСтрока.ТоварНаименование));
...
Функция ПоставитьСтатусКладовщиковНовыйВЗаявкеsetstorekeeperstatusnewinapp(Запрос)
	...
	Массив  = ПрочитатьJSON(ЧтениеJSON);
	НомерЗаявки = Строка(Массив.id);

	Заявка = Документы.ЗаказКлиента.НайтиПоНомеру(НомерЗаявки);
	
	Попытка
		Если Не ЗначениеЗаполнено(Заявка.СтатусОбработкиЗаявкиКладовщиком) Тогда
			ДокОбъект = Заявка.ПолучитьОбъект();
			ДокОбъект.СтатусОбработкиЗаявкиКладовщиком = Перечисления.СтатусыОбработкиЗаявокКладовщиком.НоваяЗаявка;
			ДокОбъект.Записать();
			
			ЗаписьJSON = Новый ЗаписьJSON;
			ЗаписьJSON.УстановитьСтроку();
			ЗаписатьJSON(ЗаписьJSON, "Статус успешно установлен!");
			СтруктураОтвета = ЗаписьJSON.Закрыть();
			Ответ = Новый HTTPСервисОтвет(200);
			Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
			Ответ.УстановитьТелоИзСтроки(СтруктураОтвета);
...
//++ МазинЕС 24-07-2024/
	...
	|	МАКСИМУМ(Сотрудники.МестоРаботы) КАК Город
	|ИЗ
	|	Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
...
/// Комлев 19/09/24 +++
// Думаю можно и покороче, но пока так, (
	ТекстСклад 			= " И РегистрНакопления1Остатки.Склад = &Склад";
	ТекстМестоРаботы	=  " И Сотрудники.МестоРаботы = &МестоРаботы";
	
	ФильтрКоличество1 =  " И РегистрНакопления1Остатки.КолвоОстаток <= 3";
	ФильтрКоличество2 =  " И РегистрНакопления1Остатки.КолвоОстаток > 3  И  РегистрНакопления1Остатки.КолвоОстаток <  10";
	ФильтрКоличество3 =  " И РегистрНакопления1Остатки.КолвоОстаток >= 10";
	
//	Если Город = 1 И ФильтрПоКоличеству = 0 Тогда
//		// Нижний Новгород
...
//	ИначеЕсли Город = 1 И ФильтрПоКоличеству = 1 Тогда 
//		// Нижний Новгород
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000002");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000003");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество1);
//	ИначеЕсли Город = 1 И ФильтрПоКоличеству = 2 Тогда 
//		// Нижний Новгород
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000002");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000003");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество2);
//	ИначеЕсли Город = 1 И ФильтрПоКоличеству = 3 Тогда 
//		// Нижний Новгород
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000002");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000003");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество3);
//	ИначеЕсли Город = 2 И ФильтрПоКоличеству = 0 Тогда
//		// Москва
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000005");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000002");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, "");
//	ИначеЕсли Город = 2 И ФильтрПоКоличеству = 1 Тогда
//		// Москва
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000005");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000002");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество1);
//	ИначеЕсли Город = 2 И ФильтрПоКоличеству = 2 Тогда
//		// Москва
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000005");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000002");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество2);
//	ИначеЕсли Город = 2 И ФильтрПоКоличеству = 3 Тогда
//		// Москва
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000005");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000002");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество3);
//	ИначеЕсли Город = 3 И ФильтрПоКоличеству = 0 Тогда
//		// Екатеренбург
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000008");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000004");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, "");
//	ИначеЕсли Город = 3 И ФильтрПоКоличеству = 1 Тогда
//		// Екатеренбург
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000008");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000004");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество1);
//	ИначеЕсли Город = 3 И ФильтрПоКоличеству = 2 Тогда
//		// Екатеренбург
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000008");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000004");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество2);
//	ИначеЕсли Город = 3 И ФильтрПоКоличеству = 3 Тогда
//		// Екатеренбург
//		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000008");
//		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000004");
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество3);
//	ИначеЕсли Город = 0 И ФильтрПоКоличеству = 1 Тогда
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", ФильтрКоличество1);
//	ИначеЕсли Город = 0 И ФильтрПоКоличеству = 2 Тогда
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", ФильтрКоличество2);
//	ИначеЕсли Город = 0 И ФильтрПоКоличеству = 3 Тогда
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", ФильтрКоличество3);
//	ИначеЕсли  Город = 0 И ФильтрПоКоличеству = 0 Тогда 
//		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", "");
//	КонецЕсли;
//	
//	Если Город > 0 И Город < 4 Тогда
//		ЗапросЗаявки.УстановитьПараметр("Склад", СкладСсылка);
//		ЗапросЗаявки.УстановитьПараметр("МестоРаботы", ГородСсылка);
//	КонецЕсли;
/// Комлев 19/09/24 ---	
	ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", ""); // Не забудь удалить Комлев
		...
		Для Каждого Строка Из ФоткиИндкод Цикл
			...
			Если Строка.indCode = ВыборкаДетальныеЗаписи.ПартияНаименование Тогда
				...
				Если Строка.urls.Количество() > 0 Тогда
					...
					Структура.Вставить("city", Строка(ВыборкаДетальныеЗаписи.Город)); 
						//-- 
...
Функция ТекстЗапросаПопулярныйТоварИзЗаявокСФильтрами(Количество, Страница, Город, ФильтрПоКоличеству)
...
/// Комлев 25/09/24 +++
	...
	Если Город = 1 И (ФильтрПоКоличеству = 0 ИЛИ ФильтрПоКоличеству > 3) Тогда
		// Нижний Новгород
		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000002");
		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000003");
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, "");
	...
	ИначеЕсли Город = 1 И ФильтрПоКоличеству = 3 Тогда 
		...
		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000003");
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество3);
	ИначеЕсли Город = 2 И (ФильтрПоКоличеству = 0 ИЛИ ФильтрПоКоличеству > 3) Тогда
		// Москва
		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000005");
		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000002");
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, "");
	...
	ИначеЕсли Город = 2 И ФильтрПоКоличеству = 3 Тогда
		...
		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000002");
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество3);
	ИначеЕсли Город = 3 И (ФильтрПоКоличеству = 0 ИЛИ ФильтрПоКоличеству > 3) Тогда
		// Екатеренбург
		СкладСсылка	= Справочники.Склады.НайтиПоКоду("000000008");
		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000004");
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, "");
	...
	ИначеЕсли Город = 3 И ФильтрПоКоличеству = 3 Тогда
		...
		ГородСсылка		= Справочники.МестоРаботы.НайтиПоКоду("000000004");
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, ТекстСклад, ТекстМестоРаботы, ФильтрКоличество3);
	ИначеЕсли (Город = 0 ИЛИ Город > 3) И ФильтрПоКоличеству = 1 Тогда
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", ФильтрКоличество1);
	ИначеЕсли (Город = 0 ИЛИ Город > 3) И ФильтрПоКоличеству = 2 Тогда
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", ФильтрКоличество2);
	ИначеЕсли (Город = 0 ИЛИ Город > 3) И ФильтрПоКоличеству = 3 Тогда
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", ФильтрКоличество3);
	ИначеЕсли  (Город = 0 ИЛИ Город > 3) И (ФильтрПоКоличеству = 0 ИЛИ ФильтрПоКоличеству > 3) Тогда 
		ЗапросЗаявки.Текст = СтрШаблон(ТекстЗапроса, "", "", "");
	КонецЕсли;
			...
			Если Строка.indCode = ВыборкаДетальныеЗаписи.ПартияНаименование Тогда
				...
				Если Строка.urls.Количество() > 0 Тогда
					Структура = Новый Структура;
						...
						//МассивТест.Добавить(Строка.urls[0]);
					Структура.Вставить("id", Строка.indCode);
					Структура.Вставить("name", Строка(Справочники.ИндКод.НайтиПоНаименованию(Строка.indCode).Владелец));
					Структура.Вставить("url", Строка.urls[0]);
					Структура.Вставить("type", "PRODUCT");
						// ++ 
					Структура.Вставить("count_registered", ВыборкаДетальныеЗаписи.КолвоОстаток);
	...
	//	Иначе
		Для Каждого Строка Из Фоткикарточек Цикл
			Если Строка.id = ВыборкаДетальныеЗаписи.Код Тогда
				...
				Если Строка.images.main <> "" Тогда
					Структура.Вставить("id", ВыборкаДетальныеЗаписи.Код);
					Структура.Вставить("name", ВыборкаДетальныеЗаписи.НаименованиеТовара);
						//МассивПром = Новый Массив;
						//МассивПром.Добавить(Строка.images.main);
					Структура.Вставить("url", Строка.images.main);
					Структура.Вставить("type", "CARD_PRODUCT"); 
				...
				Иначе
					Если Строка.images.common.Количество() > 0 Тогда
						Структура.Вставить("id", ВыборкаДетальныеЗаписи.Код);
						Структура.Вставить("name", Строка.НаименованиеТовара);
						Структура.Вставить("url", Строка.images.common[0]);
						Структура.Вставить("type", "CARD_PRODUCT");
						ФоткиОбщие.добавить(Структура);
						КоличествоФото = КоличествоФото + 1;
...
Функция ПопулярныйТоварИзЗаявокСФильтрамиGET(Запрос)
	/// Комлев 25/09/24 +++
	Количество = Запрос.ПараметрыURL["count"];
	Страница = Запрос.ПараметрыURL["page"];
	Город = Число(Запрос.ПараметрыURL["filter_stock"]);
	...
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
	СтруктураОтвет = ТекстЗапросаПопулярныйТоварИзЗаявокСФильтрами(Количество, Страница, Город, ФильтрПоКоличеству);
	ЗаписатьJSON(ЗаписьJSON, СтруктураОтвет);
	СтрокаДляОтвета = ЗаписьJSON.Закрыть();
	Ответ = Новый HTTPСервисОтвет(200);
	Ответ.Заголовки.Вставить("Content-type", "application/json;  charset=utf-8");
