...
Функция ПолучитьПоследнийУчтенныйТоварGetLastCountedProducts(Запрос)
	...
	Для Каждого Выборка Из Тз Цикл
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			СтруктураТовара.Вставить("uchetchik", Строка(ПолучитьКодСотрудника(Выборка.Ответственный)));
	...
	Для Каждого Индкод Из Индкода Цикл
		...
		//Массивкодов = ТЗ.ВыгрузитьКолонку("индкод");
			...
			Для Каждого стр Из ТЗ Цикл
				...
				СтруктураТоваров.Вставить("shelf", Строка(стр.Адрес));
				//++
				СтруктураТоваров.Вставить("yearcar", Строка(стр.машинаГод));
		...
		Иначе
			...
			СтруктураТоваров.Вставить("count_registered", Строка(Кодноменк));
