...
//	ЗапросБонусыСотрудников = Новый Запрос;
//	ЗапросБонусыСотрудников.Текст = "ВЫБРАТЬ
...
//	|	БонусыСотрудниковОтПродажи.Сотрудник";
//
//	ЗапросБонусыСотрудников.УстановитьПараметр("Датаначала", 	НачалоОтбора);
//	ЗапросБонусыСотрудников.УстановитьПараметр("ДатаОкончания", КонецОтбора);
//
//	Выборка = ЗапросБонусыСотрудников.Выполнить().Выбрать();
	МассивОтвета = Новый Массив;
	СтруктураОтвета = Новый Структура;
//	Пока Выборка.Следующий() Цикл
		
//		СтруктураОтвета.Вставить("name", 		Строка(Выборка.Сотрудник));
//		СтруктураОтвета.Вставить("money_sale", 	Строка(Выборка.Сумма));
//		СтруктураОтвета.Вставить("bonus", 		Строка(Выборка.Бонус));
		...
		|	СУММА(БонусыСотрудниковОтПродажи.Сумма) КАК Сумма,
		|	СУММА(БонусыСотрудниковОтПродажи.Бонус) КАК Бонус,
		|	БонусыСотрудниковОтПродажи.Продажа КАК Продажа
		|ИЗ
		|	РегистрСведений.БонусыСотрудниковОтПродажи КАК БонусыСотрудниковОтПродажи
		|ГДЕ
		|	БонусыСотрудниковОтПродажи.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
		...
		|СГРУППИРОВАТЬ ПО
		|	БонусыСотрудниковОтПродажи.Сотрудник,
		|	БонусыСотрудниковОтПродажи.Продажа";
	...
	Пока РезультатЗапроса.Следующий() Цикл
		СтруктураБонусовПоПродажам = Новый Структура;
		СтруктураБонусовПоПродажам.Вставить("name", 		Строка(РезультатЗапроса.Сотрудник));
		СтруктураБонусовПоПродажам.Вставить("money_sale", 	РезультатЗапроса.Сумма);
		СтруктураБонусовПоПродажам.Вставить("bonus", 		РезультатЗапроса.Бонус);
		СтруктураБонусовПоПродажам.Вставить("sale", 		Строка(РезультатЗапроса.Продажа));
		МассивБонусовПоПродажам.Добавить(СтруктураБонусовПоПродажам);
	СуммаБонусов = СуммаБонусов + РезультатЗапроса.Бонус;
	СуммаПродажи = СуммаПродажи + РезультатЗапроса.Сумма;
	КонецЦикла;
		//СтруктураОтвета.Вставить("bonuses", МассивБонусовПоПродажам);
		СтруктураОтвета.Вставить("sum_by_sales", СуммаПродажи);
		СтруктураОтвета.Вставить("bonus_by_sales", СуммаБонусов);
...
Функция ИзменитьКарточкуТовараИзменитьКаротчкуТовара(Запрос)
	...
	Если ВыборкаДетальныеЗаписи.Количество() = 0 тогда 	
		Сообщение = "Ненайдена катрочка товара";
		Ответ = Новый HTTPСервисОтвет(400);
		Ответ.Заголовки.Вставить("Content-type", "application/json;  charset=utf-8");
	...
	Ответ = Новый HTTPСервисОтвет(400);
	Ответ.Заголовки.Вставить("Content-type", "application/json;  charset=utf-8");
	Ответ.УстановитьТелоИзСтроки("Пацаны Пришли к успеху");
	Возврат Ответ;
КонецФункции
