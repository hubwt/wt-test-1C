...
Функция ПолучитьПоследнийУчтенныйТоварGetLastCountedProducts(Запрос)
	...
	Пока Выборка.Следующий() Цикл
		...
		КороткийНомерТовара = "0" + УбираемЛидНули(Выборка.ТоварКод);
		...
		СтруктураРаботник.Вставить("id", ПолучитьКодСотрудника(Выборка.Ответственный));
		СтруктураРаботник.Вставить("name", Выборка.Ответственный);
		СтруктураКарточки.Вставить("worker", СтруктураРаботник);
		СтруктураКарточки.Вставить("photo", МассивФото[0].images.main);
