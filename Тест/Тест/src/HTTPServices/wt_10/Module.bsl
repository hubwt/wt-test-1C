...
Функция ПолучитьПоследнийУчтенныйТоварGetLastCountedProducts(Запрос)
	...
	Для Каждого Стр Из ТабЗнч Цикл
		...
		НайденнаяСтрока  = ТЗ_Товары.Найти(Строка(Стр.indCode),"Партия");
		Если НайденнаяСтрока <> Неопределено Тогда
			СтруктураТовара = Новый Структура;
			
			СтруктураТовара.Вставить("id", НайденнаяСтрока.Партия);
			СтруктураТовара.Вставить("date", Стр.dateTimeFirstPhotoAdded);
			
			//@skip-check query-in-loop
			СтруктураТовара.Вставить("uchetchik", Строка(ПолучитьКодСотрудника(НайденнаяСтрока.Ответственный)));
...
//МассивОтбор.Добавить(Метаданные.РегистрыСведений.ИндНомер);
	...
	Для Каждого Результат Из СписокПоиска Цикл
		...
		МассивКодов.Добавить(Строка(Результат.Значение.Код));
		МассивФоток = ПолучениеФотокарточек(МассивКодов);
		...
		СтруктураТоваров = Новый Структура;
			...
			СтруктураТоваров.Вставить("count_registered", Число(КоличествоУчтенногоТовара));
			СтруктураТоваров.Вставить("type", "CARD_PRODUCT");
			СтруктураТоваров.Вставить("photos", МассивФоток[0].images.common);
		МассивТоваров.Добавить(СтруктураТоваров);
	КонецЦикла;
