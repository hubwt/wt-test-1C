...
//	СтруктураОтвета.Вставить("data", МассивТоваров);
	...
	Для Каждого Результат Из СписокПоиска Цикл
		...
		СтруктураТоваров = Новый Структура;
		СтруктураТоваров.Вставить("id", Строка(Результат.Значение.индкод.владелец.Код));
...
Функция ПолучитьПоследнийУчтенныйТоварGetLastCountedProducts(Запрос)
	...
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1000
				   |	ПРЕДСТАВЛЕНИЕ(ПоступлениеЗапчастейТаблица.Товар) КАК Товар,
				   |	ПРЕДСТАВЛЕНИЕ(МИНИМУМ(ПоступлениеЗапчастейТаблица.Ответственный)) КАК Ответственный,
				   |	МАКСИМУМ(ПоступлениеЗапчастейТаблица.Ссылка.Дата) КАК Дата,
				   ...
				   |СГРУППИРОВАТЬ ПО
				   |	ПРЕДСТАВЛЕНИЕ(ПоступлениеЗапчастейТаблица.Товар),
				   |	ПоступлениеЗапчастейТаблица.Товар.Код
				   |
				   |УПОРЯДОЧИТЬ ПО
				   |	Дата УБЫВ
				   |;
	...
	МассивНомеровТовара = Новый Массив;
	...
	Пока Выборка.Следующий() Цикл
		КороткийНомерТовара = "0" + СтрЗаменить(Выборка.ТоварКод, "0", "");
		МассивНомеровТовара.Добавить(КороткийНомерТовара);
		СтруктураКарточки.Вставить("id", Выборка.ТоварКод);
		СтруктураКарточки.Вставить("date", Формат(Выборка.Дата, "ДФ=dd.MM.yyyy;"));
		СтруктураКарточки.Вставить("worker", Выборка.Ответственный);
		МассивКарточек.Добавить(СтруктураКарточки);

	КонецЦикла;

	МассивФото = ПолучениеФотокарточек(МассивНомеровТовара);

	Для Индекс = 0 По МассивКарточек.ВГраница() Цикл
		МассивКарточек[Индекс].Вставить("photo", МассивФото[Индекс].images.main);
	КонецЦикла;
	...
	СтруктураОтветаОбщ.Вставить("products", МассивКарточек);
