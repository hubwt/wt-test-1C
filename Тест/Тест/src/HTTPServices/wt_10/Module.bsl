	...
	Если НомерПродажи <> "" Тогда
		...
		Попытка
				...
				Если НайденныеСтроки.количество() > 0 Тогда
					...
					//@skip-check query-in-loop
					СтрокаТЧ.Машина = ПолучитьМашину(СтрокаТЧ.Партия);
...
//	СтруктураОтвета.Вставить("info", СтруктураИнфо);
	...
	Если МассивТоваров.Количество() = 0 Тогда
		...
		ТЗ = ЗапросАналогов.Выполнить().Выбрать();
		ОбщееКоличествоАналогов = ТЗ.Количество();
		...
		Пока ВыборкаАналоги.Следующий() Цикл
			...
			МассивАналогов.Добавить(СтруктураАналогов);
		КонецЦикла;
		
		Итог = ОбщееКоличествоАналогов / ?(Число(Запрос.ПараметрыURL["count"]) = 0, Выборкаобщ, Число(Запрос.ПараметрыURL["count"]));
		Итог = ?((Итог - Цел(Итог)) > 0, Цел(Итог) + 1, Цел(Итог));
		
		СтуктураАналогов.Вставить("pages", Итог);
		СтуктураАналогов.Вставить("count", ОбщееКоличествоАналогов);
		СтуктураАналогов.Вставить("analog_cards", МассивАналогов);
	Иначе
		СтуктураАналогов.Вставить("pages", 0);
		СтуктураАналогов.Вставить("count", 0);
		СтуктураАналогов.Вставить("analog_cards", Новый Массив);
	КонецЕсли;
	...
	ЗаписатьJSON(ЗаписьJSON, СтруктураТоварИАналоги);
