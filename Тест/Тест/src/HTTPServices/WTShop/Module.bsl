...
Функция ПолучитьАвторизациюAuthorization(Запрос)
	...
	Иначе
		...
		Ответ = Новый HTTPСервисОтвет(200);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(РаботаССайтомWT.СформироватьОтветСтруктурой(Истина,
			"Пользователь успешно зарегестрирован", СтруктураОтвета, ));
		Возврат Ответ;
	КонецЕсли;
...
//	Массив = ПрочитатьJSON(ЧтениеJSON);
	...
	Иначе
		...
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(РаботаССайтомWT.СформироватьОтветСтруктурой(Ложь, "Ошибка!", ,
			РаботаССайтомWT.СформироватьСтруктуруОшибки(401, "Пользователь не найден.",
			"Данный идентификатор не обнаржен в базе или введён не корректно.")));
		Возврат Ответ;
	КонецЕсли;
...
Функция СоздатьЗаявкуcreateapplication(Запрос)
	...
	Попытка
		...
		Иначе
			...
			Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
			Ответ.УстановитьТелоИзСтроки(РаботаССайтомWT.СформироватьОтветСтруктурой(Истина,
				"Запрос частично выполнен", СтруктураОтвета, ));
		КонецЕсли;
...
Функция ПолучитьСписокТоваровGetProductsList(Запрос)
	...
	запросТовара.Текст =  текстДляНоменклатуры();
...
Функция текстДляНоменклатуры()
	Текстзапроса = "ВЫБРАТЬ
				   ...
				   |	ВТ_данныеНоменклатур.Комментарий КАК Комментарий,
				   |	ВТ_данныеНоменклатур.Артикул КАК Артикул,
				   |	ВТ_данныеНоменклатур.Подкатегория2 КАК Категория,
				   |	ВТ_данныеНоменклатур.Адрес КАК Адрес,
				   |	ВТ_данныеНоменклатур.Склад КАК Склад,
				   |	ВТ_данныеНоменклатур.НомерЗаписи КАК НомерЗаписи,
				   |	ВТ_данныеНоменклатур.Поддон КАК Поддон,
