...
Функция ПолучитьАвторизациюAuthorization(Запрос)
	...
	Если ВыборкаСотрудника.Количество() > 0 Тогда
		...
		Ответ.Заголовки.Вставить("Content-Type","application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(СформироватьОтветСтруктурой(Ложь,"Ошибка выполнения запроса",,СформироватьСтруктуруОшибки(404,"Такого пользователя нет в базе","")));
		Возврат Ответ;
		КонецЕсли;
...
Функция СформироватьОтветСтруктурой(Success,Message,Data=null,Error=Null)
