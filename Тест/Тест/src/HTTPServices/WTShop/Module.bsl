...
Функция ПолучитьАвторизациюAuthorization(Запрос)
	...
	Иначе
		...
		Ответ = Новый HTTPСервисОтвет(200);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(СформироватьОтветСтруктурой(Истина, "Пользователь успешно зарегестрирован",
			СтруктураОтвета, ));
		Возврат Ответ;
	КонецЕсли;
...
//	Массив = ПрочитатьJSON(ЧтениеJSON);
	...
	Иначе
		...
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(СформироватьОтветСтруктурой(Ложь, "Ошибка!", , СформироватьСтруктуруОшибки(401,
...
Функция ПолучитьСписокКатегорийgetCategories(Запрос)
	...
	Ответ.УстановитьТелоИзСтроки(СформироватьОтветСтруктурой(Истина, "Запрос успешно выполнен", МассивКатегорий, ));
...
Функция СоздатьЗаявкуcreateapplication(Запрос)
	...
	Попытка
		...
		Для Каждого Товар Из Массив.products Цикл
			//@skip-check query-in-loop
			ИнформацияОпартии = ПолучитьИнформациюОПартии(Товар);
...
//Иначе
		...
		Ответ = Новый HTTPСервисОтвет(200);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(СформироватьОтветСтруктурой(Истина, "Запрос частично выполнен", СтруктураОтвета, ));
//КонецЕсли;
	...
	Исключение
		...
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(СформироватьОтветСтруктурой(Ложь, "Ошибка выполнения запроса", ,
			СформироватьСтруктуруОшибки(500, "Заявку создать не удалось", Информация.Описание)));
	КонецПопытки;
...
Функция ПравильныйПоискНоменклатурыGetProductsList(Запрос)
	...
	Иначе
		...
		Если ЕстьКириллица(ПоисковаяСтрока) Тогда
			СписокПоиска.СтрокаПоиска=ПоисковаяСтрока + "#1";//+Строка(ЛимитОшибок-1);
		Иначе
			СписокПоиска.СтрокаПоиска=ПоисковаяСтрока;
		КонецЕсли;
		...
		Для Каждого Результат Из СписокПоиска Цикл
			СтруктураТоваров = Новый Структура;
			СтруктураТоваров.Вставить("name", Строка(Результат.Значение.ссылка));
			СтруктураТоваров.Вставить("article",  Строка(Результат.Значение.индкод.Владелец.Артикул));
			СтруктураТоваров.Вставить("category", Строка(Результат.Значение.индкод.Владелец.Подкатегория2));
			СтруктураТоваров.Вставить("name",     Строка(Результат.Значение.индкод.Владелец));
			МассивТоваров.Добавить(СтруктураТоваров);
...
//	запросТовара = Новый запрос;
//	запросТовара.Текст =  текстДляНоменклатурыПП();
//	запросТовара.УстановитьПараметр("наименование", "%" + ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(Строка(
//		Запрос.ПараметрыURL["text"])) + "%");
//
//
...
Функция текстДляНоменклатурыПП()
	Текстзапроса = "ВЫБРАТЬ ПЕРВЫЕ 50
	|	Номенклатура.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВТ_Номенклатура
	|ИЗ
