...
Функция ПолучитьТоварыЗаказНарядаgetproductsorder(Запрос)
	...
	|	//|		ПО ЗаказНарядТовары.Партия = РегистрНакопления1Остатки.индкод
	|ГДЕ
	...
	|	ЕстьNULL(ВТ_Пред.МеткаТовараПорядок, 13) КАК МеткаТовараПорядок,
	|	ВТ_Пред.НомерЗаписи
	|	//|	ВТ_Пред.Стеллаж КАК Стеллаж,
	|	//|	ВТ_Пред.Поддон КАК Поддон,
	|	//|	ВТ_Пред.машина КАК Машина
	|ИЗ
	|	ВТ_Пред КАК ВТ_Пред
	|ГДЕ
	|	ВТ_Пред.НомерЗаписи >= &НачинаяСЗаписи
	...
	ЗапросРабот.Текст =  СтрШаблон(ТекстЗапроса, Формат(10000, "ЧГ="), СтрокаГруппировки);
	Если Строка(Запрос.ПараметрыURL["text"]) <> "13"  Тогда
