...
Функция ПоискЗаявокSearchApplications(Запрос)
	/// Комлев АА 20/11/24 +++
	Попытка
		СтрокаПоиска = Запрос.ПараметрыURL.Получить("search");
		...
		Пока Выборка.Следующий() Цикл
			...
			СтруктураЗаявки.Вставить("name", Строка(Выборка.НаименованиеКлиента) + " " + Строка(Выборка.НомерТелефона));
...
Функция ПолучитьТекстПоискаЗаявок()
	...
	|	ИЛИ ЗаказКлиента.НомерТелефона ПОДОБНО  &СтрокаПоиска 
