...
Функция ИзменитьСтатусЗаявкиchangestatusapp(Запрос)
	...
	Попытка
		...
		Иначе
			...
			АктуализироватьНумерациюВКонцеСписка(НачальнаяПозиция, НачальныйСтатус);
		
			//Актуализировать номера конечной группы заявок
			АктуализироватьНумерациюВКонцеСписка(КонечнаяПозиция, КонечныйСтатус);
		КонецЕсли;
...
Процедура АктуализироватьНумерациюВКонцеСписка(ПозицияЗаявки, Статус)
	...
	ЗапросНачальнаяГруппаЗаявки.Текст = "ВЫБРАТЬ
										...
										|	Документ.ЗаказКлиента КАК ЗаказКлиента
										|ГДЕ
										|	ЗаказКлиента.ПорядковыйНомерПоСтатусу > &ПозицияЗаявки
										|	И ЗаказКлиента.ПорядковыйНомерПоСтатусу > 0
										|	И ЗаказКлиента.Состояние = &Статус
										|
										|УПОРЯДОЧИТЬ ПО
										|	ЗаказКлиента.ПорядковыйНомерПоСтатусу";
	...
	Если ВыборкаНачальнаяГруппаЗаявки.Количество() > 0 Тогда
		...
		Пока ВыборкаНачальнаяГруппаЗаявки.Следующий() Цикл
			ОбъектЗаявки = ВыборкаНачальнаяГруппаЗаявки.Ссылка;
			ОбъектЗаявки.ПорядковыйНомерПоСтатусу = ПозицияВСписке;
...
Процедура АктуализироватьНумерациюВНачалеИКонцеСписка(НомерЗаявки, ПозицияЗаявки, Статус)
	...
	ЗапросНачалоСписка.Текст = "ВЫБРАТЬ
										...
										|
										|УПОРЯДОЧИТЬ ПО
										|	ЗаказКлиента.ПорядковыйНомерПоСтатусу УБЫВ";
	...
	Если ВыборкаНачалоСписка.Количество() > 0 Тогда
		...
		Пока ВыборкаНачалоСписка.Следующий() Цикл
			ОбъектЗаявки = ВыборкаНачалоСписка.Ссылка;
			ОбъектЗаявки.ПорядковыйНомерПоСтатусу = ПозицияВСписке;
			ОбъектЗаявки.Записать(РежимЗаписиДокумента.Запись);
			ПозицияВСписке = ПозицияВСписке - 1;
		КонецЦикла;
	...
	Если ВыборкаКонецСписка.Количество() > 0 Тогда
		...
		Пока ВыборкаКонецСписка.Следующий() Цикл
			ОбъектЗаявки = ВыборкаКонецСписка.Ссылка;
			ОбъектЗаявки.ПорядковыйНомерПоСтатусу = ПозицияВСписке;
			ОбъектЗаявки.Записать(РежимЗаписиДокумента.Запись);
			ПозицияВСписке = ПозицияВСписке + 1;
		КонецЦикла;
