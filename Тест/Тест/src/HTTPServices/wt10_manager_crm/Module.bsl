...
Функция ОбъединитьТЗИПронумеровать(ТЗНачало, ТЗКонец, Статус, ПозицияВСписке)

Для Каждого СтрокаТЗ Из ТЗКонец Цикл
		НоваяСтрока = ТЗНачало.Добавить();
		НоваяСтрока.Заявка = СтрокаТЗ.Заявка;
		НоваяСтрока.ПорядковыйНомер = ПозицияВСписке;
...
//	ТЗКонец = ПостроительЗапроса.Результат.Выгрузить();
//	
//	ИтоговаяТЗ = ОбъединитьТЗИПронумеровать(ТЗНачало, ТЗКонец, Статус, ПозицияВСписке);
//	
