...
//	КлиентОбъект.ПаспортВыданДата = ДатаВыдачи;
	...
	СтуктураКлиента.Вставить("name_client_card", Строка(Выборка.Наименование));
	СтуктураКлиента.Вставить("phone", Строка(Выборка.Телефон));
	
	СтруктураГород = Новый Структура("name, id", Строка(Выборка.Город2), Строка(Выборка.КодГорода) );
	СтуктураКлиента.Вставить("city", СтруктураГород);
	
	СтруктураОбласть = Новый Структура("name, id", Строка(Выборка.Область), Строка(Выборка.КодОбласти) );
	СтуктураКлиента.Вставить("region", СтруктураОбласть);
	
	СтруктураСтрана = Новый Структура("name, id", Строка(Выборка.Страна2), Строка(Выборка.КодСтраны) );
	СтуктураКлиента.Вставить("country", СтруктураСтрана);
	СтуктураКлиента.Вставить("count_car", Число(Выборка.КоличествоАвтомобилей));
...
Функция ПолучитьИнформациюОЗаявкеgetinfoapp(Запрос)
	...
	Попытка
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДокПродажаЗапчастей.Ссылка,
		|			ЗНАЧЕНИЕ(Документ.ПродажаЗапчастей.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Документ.ПродажаЗапчастей.ПустаяСсылка)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		...
		Для Каждого Результат Из Выборка Цикл
			...
			СтруктураЗаявки.Вставить("sale", Булево(Результат.ЕстьПродажа));
