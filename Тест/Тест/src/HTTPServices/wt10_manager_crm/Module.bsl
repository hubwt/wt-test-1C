...
Функция ПолучитьГлавныеСтатусыЗаявкиgetmainstatusapp(Запрос)
	...
	МассивГлавныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Ожидание));
	МассивГлавныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.ВРаботе));
	МассивГлавныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.СобираетсяНаСкладе));
	МассивГлавныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Думает));
	МассивГлавныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Продажа));
	МассивГлавныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Завершенные));
...
Функция ПолучитьВторостепенныеСтатусыЗаявкиgetsecondarystatusapp(Запрос)
	...
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Консультация));
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.ОжидаетПриход));
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.НетВНаличии));
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Дорого));
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Отказ));
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.ДругаяМарка));
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Спам));
	МассивВторостепенныхСтатусов.Добавить(Строка(Перечисления.дт_СостоянияЗаказовКлиента.Перекупка));
...
Функция РедактироватьКлиентаEditClient(Запрос)
	...
	НаимнованиеКарточкиКлиента = Строка(Массив.name_client);
...
Функция ПолучитьСписокТоваровПоЗаявкеgetlistproductsbyapp(Запрос)
	...
	Запрос.ПараметрыURL["page"]) > 1 Тогда
			...
			Иначе
				ЗапросТоваров.УстановитьПараметр("НачинаяСЗаписи", 0);
			КонецЕсли;
			СписокПоиска = ЗапросТоваров.Выполнить().Выгрузить();
			МассивКодов = Новый Массив;
			МассивТоваров = Новый Массив;
			Для Каждого Результат Из СписокПоиска Цикл

				СтруктураТоваров = Новый Структура;
				СтруктураТоваров.Вставить("name", Строка(Результат.Номенклатура));
				СтруктураТоваров.Вставить("id", Строка(Результат.Код));
				СтруктураТоваров.Вставить("count", Результат.Количество);
				СтруктураТоваров.Вставить("price", Результат.Цена);
				СтруктураТоваров.Вставить("summ", Результат.Сумма);
				СтруктураТоваров.Вставить("sklad", Строка(Результат.Склад));
				МассивКодов.Добавить(Строка(Результат.Код));
				МассивТоваров.Добавить(СтруктураТоваров);
		...
		Иначе
			...
			СтруктураИнфо.Вставить("pages", 0);
			СтруктураИнфо.Вставить("count", 0);
			СтруктураИнфо.Вставить("name", "");
			СтруктураИнфо.Вставить("id", "");
			СтруктураИнфо.Вставить("count", 0);
			СтруктураИнфо.Вставить("price", 0);
			СтруктураИнфо.Вставить("summ", 0);
			СтруктураИнфо.Вставить("sklad", "");
