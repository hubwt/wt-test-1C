...
Процедура АктуализироватьНумерациюВКонцеСписка(НомерЗаявки, ПозицияЗаявки, Статус, ЭтоКонечныйСтатус)
	...
	Для Каждого СтрокаТЗ Из ТЗНачало Цикл
	...
	ТЗОтборНачало = ПостроительЗапроса.Отбор.Добавить("Заявка");
	ТЗОтборНачало.ВидСравнения = ВидСравнения.НеРавно;
	ТЗОтборНачало.Значение = Документы.ЗаказКлиента.НайтиПоНомеру(НомерЗаявки);
	ТЗОтборНачало.Использование = Истина;
