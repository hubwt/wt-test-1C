...
Функция ПолучитьЗаявкиПоСтатусуgetappbystatus(Запрос)
	...
	Попытка
		...
		СписокПоиска = ЗапросЗаявок.Выполнить().Выгрузить();
			...
			ЗапросРСПорядокЗаявок = Новый Запрос;
			ЗапросРСПорядокЗаявок.Текст = "ВЫБРАТЬ
			...
			ЗапросРСПорядокЗаявок.УстановитьПараметр("Статус", Статус);
			Выборка = ЗапросРСПорядокЗаявок.Выполнить().Выбрать();
			
			Если Выборка.Количество() > 0 Тогда
				Пока Выборка.Следующий() Цикл
					ЗаписьВРегистреСведений = РегистрыСведений.ПорядокЗаявок.СоздатьМенеджерЗаписи();
					ЗаписьВРегистреСведений.Заявка = Выборка.Заявка;
		...
		Для Каждого Результат Из СписокПоиска Цикл
			...
			//Заполняем подядок заявок в регистре сведений
			ЗаписьВРегистреСведений = РегистрыСведений.ПорядокЗаявок.СоздатьМенеджерЗаписи();
			ЗаписьВРегистреСведений.Заявка = Результат.Ссылка;
			ЗаписьВРегистреСведений.Статус = Статус;
			ЗаписьВРегистреСведений.ПорядковыйНомер = ПорядковыНомер;
			ЗаписьВРегистреСведений.Записать();
			
			ПорядковыНомер = ПорядковыНомер + 1;	
...
Функция ИзменитьСтатусЗаявкиchangestatusapp(Запрос)
	...
	Попытка
		...
		Если НачальныйСтатус = КонечныйСтатус Тогда
			АктуализироватьНумерациюВНачалеИКонцеСписка(НомерЗаявки, КонечнаяПозиция, КонечныйСтатус);
		Иначе
			//Актуализировать номера начальной группы заявок
			АктуализироватьНумерациюВКонцеСписка(НомерЗаявки, НачальнаяПозиция, НачальныйСтатус);
		
			//Актуализировать номера конечной группы заявок
			АктуализироватьНумерациюВКонцеСписка(НомерЗаявки, КонечнаяПозиция, КонечныйСтатус);
		КонецЕсли;
...
Процедура АктуализироватьНумерациюВКонцеСписка(НомерЗаявки, ПозицияЗаявки, Статус)
	...
	Если ВыборкаНачальнаяГруппаЗаявки.Количество() > 0 Тогда
		...
		Пока ВыборкаНачальнаяГруппаЗаявки.Следующий() Цикл
			ЗаписьВРегистеСведений = РегистрыСведений.ПорядокЗаявок.СоздатьМенеджерЗаписи();
			ЗаписьВРегистеСведений.Заявка = Документы.ЗаказКлиента.НайтиПоНомеру(НомерЗаявки);
			ЗаписьВРегистеСведений.ПорядковыйНомер = ПозицияВСписке;
			ЗаписьВРегистеСведений.Записать();
...
Процедура АктуализироватьНумерациюВНачалеИКонцеСписка(НомерЗаявки, ПозицияЗаявки, Статус)
	
	///+ТатарМА 31.10.2024
	//Актуализировать нумерацию в начале списка
	...
	//Актуализировать нумерацию в конце списка
	ЗапросКонецСписка = Новый Запрос;
	ЗапросКонецСписка.Текст = "ВЫБРАТЬ
							  |	ЗаказКлиента.Ссылка КАК Ссылка
							  |ИЗ
							  |Документ.ЗаказКлиента КАК ЗаказКлиента
							  |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокЗаявок КАК ПорядокЗаявок
	...
	ЗапросКонецСписка.УстановитьПараметр("Номер", 		  	НомерЗаявки);
	ЗапросКонецСписка.УстановитьПараметр("ПозицияЗаявки", 	ПозицияЗаявки);
	ЗапросКонецСписка.УстановитьПараметр("Статус", 			Статус);

	ВыборкаКонецСписка = ЗапросКонецСписка.Выполнить().Выбрать();
	Если ВыборкаКонецСписка.Количество() > 0 Тогда
		ПозицияВСписке = ПозицияЗаявки + 1;
		Пока ВыборкаКонецСписка.Следующий() Цикл
			ЗаписьВРегистреСведений = РегистрыСведений.ПорядокЗаявок.СоздатьМенеджерЗаписи();
			ЗаписьВРегистреСведений.Заявка = Документы.ЗаказКлиента.НайтиПоНомеру(НомерЗаявки);
			ЗаписьВРегистреСведений.ПорядковыйНомер = ПозицияВСписке;
			ЗаписьВРегистреСведений.Записать();
