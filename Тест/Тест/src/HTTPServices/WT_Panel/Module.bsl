...
Функция ТипыЗаявокtypes_application(Запрос)
			...
			Пока выборка.Следующий() Цикл
				Если Выборка.СтатусОбработки = "Упаковка" Или Выборка.СтатусОбработки = "Отправка в тк" Тогда
					Если Выборка.СтатусОбработки = "Отправка в тк" Тогда
						...
						ЗапросКоличество.УстановитьПараметр("СостояниеСборки", Справочники.СтатусыWT.НайтиПоНаименованию(Выборка.СтатусОбработки));
	...
	Иначе
		Пока выборка.Следующий() Цикл
			Если Выборка.СтатусОбработки = "Отправка в тк" ИЛИ Выборка.СтатусОбработки = "Отправлено клиенту" ИЛИ Выборка.СтатусОбработки = "Заказ получен" Тогда
				ЗапросКоличество = Новый Запрос;
				ЗапросКоличество.Текст =
					"ВЫБРАТЬ
					|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПродажаЗапчастей.Ссылка) КАК Количество
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		Пока РезультатЗапроса.Следующий() Цикл
			...
			Если НаборЗаписей.Количество() = 1 Тогда
				...
				ТекстЛога = "Ваша продажа № " +ДокументОбъект.номер +" для клиента "+ДокументОбъект.Клиент +" получила статус ТК " +Строка(ДокументОбъект.СтатусЗаказаВТК);
...
Функция ПолучитьПотерянныеТоварыgetlost(Запрос)
	...
	Если Запрос.ПараметрыURL["id"] <> " " Тогда
		...
		запросТовара.Текст = СтрЗаменить(запросТовара.Текст, "РегИндНомер.АвитоЧастник = &Ач) КАК Поле2}",
			...
			+ "		ИЛИ РегИндНомер.индкод.Владелец.МестоНаСкладе2.Наименование ПОДОБНО &НазваниеПоиска СПЕЦСИМВОЛ ""~"""
...
//	Иначе     
	Если ТекущаяДата() - 3600 * 24 * 60 < '20240620' Тогда
		Пока выборка.Следующий() Цикл
			...
			Если СтатусОбработки = "Отправка в тк" ИЛИ СтатусОбработки = "Отправлено клиенту" ИЛИ СтатусОбработки = "Заказ получен" Тогда
				...
				ЗапросКоличество.УстановитьПараметр("Дата", '20240620');
				ЗапросКоличество.УстановитьПараметр("СостояниеСборки", Справочники.СтатусыWT.НайтиПоНаименованию(СтатусОбработки));
...
СтруктураОтветственных.Вставить("id_executor", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",Стр.Сотрудник).Код);
...
Функция СтатистикаКонвейерИФотоgetstatisticspipelinephoto(Запрос)
	...
	ИндкодаСФото = ПолучитьИндкодаСФото(Строка(Запрос.ПараметрыURL["datestart"]), Строка(Запрос.ПараметрыURL["dateend"]));
...
Функция ПолучитьИндкодаСФото(ДатаНачала, ДатаОкончания)
	...
	Соединение = Новый HTTPСоединение("192.168.0.245", 8085);
	Запрос = Новый HTTPЗапрос("/v1/photographer/indcode?date_start=" + ДатаНачала + "&date_end=" + ДатаОкончания, Заголовки);
	//В запросе можно обратиться к нужному ресурсу и с нужными параметрами 
	Ответ = Соединение.Получить(Запрос);
...
Функция ГрафикКонвейерИФотоgetgraphpipelinephoto(Запрос)
	...
	ДатаНачала		= НачалоМесяца(Дата(Строка(Запрос.ПараметрыURL["year"]) + Строка(Запрос.ПараметрыURL["month"]) + "01"));
	ДатаОкончания 	= КонецМесяца(Дата(Строка(Запрос.ПараметрыURL["year"]) + Строка(Запрос.ПараметрыURL["month"]) + "01"));
	...
	ВыборкаПоСотрудникам = ЗапросОтчётаПоСотруднику.Выполнить().Выбрать();
		Пока ВыборкаПоСотрудникам.Следующий() Цикл
			...
			СтруктураПоСотруднику.Вставить("money", 	Число(ВыборкаПоСотрудникам.Сумма));
			СтруктураПоСотруднику.Вставить("person", 	Строка(ВыборкаПоСотрудникам.Ответственный));
			СтруктураПоСотруднику.Вставить("date", 		Строка(Формат(ВыборкаПоСотрудникам.ДатаИзмененияКонвеера, "ДФ=dd.MM.yyyy;")));
			МассивДеталиПоСотруднику.Добавить(СтруктураПоСотруднику);
			Исключение
	...
	//Статистика фотографии
	ИндкодаСФото = ПолучитьИндкодаСФото(Строка(Формат(ДатаНачала, "ДФ=yyyyMMdd;")), Строка(Формат(ДатаНачала, "ДФ=yyyyMMdd;")));
...
Функция ПолучитьУчастниковЗадачи(Задача)
	...
	Пока ВыборкаУчастника.Следующий() Цикл
		...
		СтруктураИсполнителя.Вставить("id_executor", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",ВыборкаУчастника.Ответственный).Код);
