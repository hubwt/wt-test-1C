...
Иначе
	...
	Выборкаобщ = ЗапросЗадач.Выполнить().Выбрать().Количество();
	ЗапросЗадач.Текст =  СтрШаблон(Текст, Формат(?(Число(Запрос.ПараметрыURL["count"])>0,Запрос.ПараметрыURL["count"],10000), "ЧГ="));
	Если Число(((Запрос.ПараметрыURL["count"])*(Запрос.ПараметрыURL["page"])))>0 и Число(Запрос.ПараметрыURL["page"])>1 Тогда
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи",  Число(((Запрос.ПараметрыURL["count"])*((Запрос.ПараметрыURL["page"])-1)+1)));
	Иначе 
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", 0);
...
Функция ДашбордРВРgetdashrvr(Запрос)
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураПроекта.Вставить("day_to_end",		?(Выборка.ДатаСдачи 		  = '00010101', 0, (Выборка.ДатаСдачи-текущаядата())/(3600*24)));
		СтруктураПроекта.Вставить("day_to_start",	?(Выборка.ПлановаяДатаРемонта = '00010101', 0, (Выборка.ПлановаяДатаРемонта-текущаядата())/(3600*24)));
