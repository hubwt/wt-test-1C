...
Функция ПолучитьКаталогМашинgetlisttrucks(Запрос)
	...
	//ЗапросМашин = Новый Запрос;
	ЗапросМашин.Текст =  СтрШаблон(ЗапросМашин.Текст, Формат(10000, "ЧГ="));  
	...
	///+ГомзМА 02.04.2024
	ЗапросМашин.Текст =  СтрШаблон(текстДляКарточекНоменклатуры(), Формат(?(Число(Запрос.ПараметрыURL["count"])>0,Запрос.ПараметрыURL["count"],100000), "ЧГ="));
	///-ГомзМА 02.04.2024
