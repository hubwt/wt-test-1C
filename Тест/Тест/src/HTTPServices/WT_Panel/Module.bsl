...
Функция текстДляСпискаПродаж()
	Текст = "ВЫБРАТЬ
			...
			|	ВТ_Продажи.СтатусЗаказаВТК КАК СтатусЗаказаВТК,
...
Функция текстДляНоменклатуры()
	...
	|
	|ВЫБРАТЬ
	|	НоменклатураНомераЗамен.Ссылка КАК Номенклатура
	|ИЗ
	|	Справочник.Номенклатура.НомераЗамен КАК НоменклатураНомераЗамен
	|ГДЕ
	|	НоменклатураНомераЗамен.НомерЗамены ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~""
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ_данныеНоменклатур.Состояние
	|ИЗ
...
Функция текстДляТиповТоваров2()
	Текст = "ВЫБРАТЬ
			|НоменклатураНомераЗамен.Ссылка.Наименование  КАК Замена
			|ПОМЕСТИТЬ Замены
			|ИЗ
			|Справочник.Номенклатура.НомераЗамен КАК НоменклатураНомераЗамен
			|ГДЕ
	...
	|	ИЛИ РегИндНомер.индкод.Владелец.НомераЗамен.НомерЗамены ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~""
	...
	Возврат Текст;
КонецФункции
Функция текстДолжностиСотрудников() 
	...
	Текст = "ВЫБРАТЬ
			...
			|	Документ.КадровыйПриказ КАК КадровыйПриказ
			|ГДЕ
			|	КадровыйПриказ.Сотрудник.Код В (&Код)
			|СГРУППИРОВАТЬ ПО
			|	КадровыйПриказ.Сотрудник
			|;
			|
			...
			|		И (ВТ_ДолжностиДаты.ДатаДоговора = КадровыйПриказ.Дата)
			|ГДЕ
			|	ВТ_ДолжностиДаты.Сотрудник.Код В (&Код)
			|СГРУППИРОВАТЬ ПО
			|	КадровыйПриказ.Сотрудник
			|;
			|
...
Функция текстДляТовара()
	...
	|	РегИндНомер.АвитоЧастник КАК АвитоЧастник,
	|	Номенклатурас.Размеры КАК Размеры,
	|	Номенклатурас.Вес КАК Вес,
	|	Номенклатурас.выс КАК выс,
	|	Номенклатурас.длин КАК длин,
	|	Номенклатурас.шир КАК шир,
	...
	|		ПО ИндНомер.индкод = РегистрНакопления1Остатки.индкод
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Номенклатурас
	|		ПО ИндНомер.индкод.Владелец.Ссылка = Номенклатурас.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК РегИндНомер
	|		ПО ИндНомер.индкод = РегИндНомер.индкод
	|{ГДЕ
	|	(РегИндНомер.АвитоЧастник = &Ач) КАК Поле2}
...
Функция текстДляЗадач()
	Текст = "ВЫБРАТЬ ПЕРВЫЕ 10000
			...
			|	докЗадача.РодительскаяЗадача.Номер,
			|	докЗадача.ЗадачаСПортала,
			|	докЗадача.РодительскаяЗадача.Наименование
			...
			|   Вт_Предварительно.ЗадачаСПортала КАК ЗадачаСПортала,
			|	Вт_Предварительно.РодительскаяЗадачаНомер Как номерРодителя,
			|	Вт_Предварительно.РодительскаяЗадачаНаименование КАК НаименованиеРодителя
			|ИЗ
...
Функция ОтчётПоКонвейеруpipelinereport(Запрос)
	...
	Если Число(Запрос.ПараметрыURL["city"]) = 1 Тогда //Все
		МассивРегионов.Добавить(Справочники.Подразделения.НайтиПоКоду("000000030")); //ВОРКТРАК Нижний Новгород
		МассивРегионов.Добавить(Справочники.Подразделения.НайтиПоКоду("000000060")); //Филиал Екатеринбург
		МассивРегионов.Добавить(Справочники.Подразделения.НайтиПоКоду("000000029")); //Филиал Москва
	...
	ЗапросОтчёта.Текст = "ВЫБРАТЬ
						 ...
						 |	ИндНомер.АвитоЧастник = ИСТИНА
						 |	И ИндНомер.ДатаИзмененияКонвеера МЕЖДУ &ДатаНачала И &ДатаОкончания
						 |	И ИндНомер.Ответственный.Подразделение В (&Подразделение)
						 |СГРУППИРОВАТЬ ПО
						 |	ИндНомер.ДатаИзмененияКонвеера
						 |
						 |УПОРЯДОЧИТЬ ПО
	...
	ЗапросОтчёта.УстановитьПараметр("Подразделение", МассивРегионов);
	...
	Если Выборка.Количество() > 0 Тогда
		Пока выборка.Следующий() Цикл
			...
			ЗапросОтчётаПоСотруднику.УстановитьПараметр("Подразделение", МассивРегионов);
...
Функция ПолучитьТоварGetProduct(Запрос)
	...
	Для Каждого стр Из ТЗ Цикл
		...
		СтруктураТоваров.Вставить("ind_code", Строка(стр.индкод));
...
Функция ПолучитьТоварыВПоддонеGetProductInPoddon(Запрос)
	...
	запросТовара.Текст =  СтрШаблон(текстДляТоваровВПоддонеИлиПолке(), Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
	...
	СтруктураИнфо.Вставить("count", Выборкаобщ);
	СтруктураИнфо.Вставить("place", Строка(Справочники.Поддоны.НайтиПоНаименованию(Строка(
...
Функция ЗаявкиИПродажиapplication_and_sale(Запрос)
	...
	Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Администратор")) = Неопределено Тогда
		Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Менеджер")) <> Неопределено Тогда
			ТекстЗаменыОтборОтветственныйЗаявки = "И (ЗаказКлиента.Ответственный = &Ответственный" + Символы.ПС
				+ "		ИЛИ ЗаказКлиента.ОтветственныйЗаОбработку = &Ответственный)";
			ТекстЗаменыОтборОтветственныйПродажи = "И (ПродажаЗапчастей.КтоПродал = &Ответственный" + Символы.ПС
				+ " 	ИЛИ ПродажаЗапчастей.ОтветственныйКладовщик = &Ответственный)";

			ЗапросЗиП.Текст =  СтрШаблон(текстДляЗаявокИПродаж(), Формат(10000, "ЧГ="),
				ТекстЗаменыОтборОтветственныйЗаявки, ТекстЗаменыОтборОтветственныйПродажи);
		...
		ИначеЕсли Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Водитель")) <> Неопределено Тогда
			...
			ТекстЗаменыОтборОтветственныйПродажи = "И ПродажаЗапчастей.WTpanel В (&WTpanel)" + Символы.ПС
				+ "И ПродажаЗапчастей.Водитель = &Ответственный";
			ЗапросЗиП.Текст =  СтрШаблон(текстДляЗаявокИПродаж(), Формат(10000, "ЧГ="),
				ТекстЗаменыОтборОтветственныйЗаявки, ТекстЗаменыОтборОтветственныйПродажи);
			ЗапросЗиП.УстановитьПараметр("Ответственный", Справочники.Сотрудники.НайтиПоКоду(
				Запрос.ПараметрыURL["user_id"]));
		...
		ИначеЕсли Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Кладовщик")) <> Неопределено Тогда
			...
			ТекстЗаменыОтборОтветственныйПродажи = "И ПродажаЗапчастей.WTpanel В (&WTpanel)" + Символы.ПС + "И (ПродажаЗапчастей.ОтветственныйКладовщик = &Ответственный"
				+ Символы.ПС
				+ "	ИЛИ ПродажаЗапчастей.ОтветственныйКладовщик = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))";
			ЗапросЗиП.Текст =  СтрШаблон(текстДляЗаявокИПродаж(), Формат(10000, "ЧГ="),
				ТекстЗаменыОтборОтветственныйЗаявки, ТекстЗаменыОтборОтветственныйПродажи);
			ЗапросЗиП.УстановитьПараметр("Ответственный", Ответственный);
	...
	Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Администратор")) = Неопределено Тогда
		Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Менеджер")) <> Неопределено Тогда
			...
			ЗапросЗиП.Текст =  СтрШаблон(текстДляЗаявокИПродаж(), Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
				Запрос.ПараметрыURL["count"], 10000), "ЧГ="), ТекстЗаменыОтборОтветственныйЗаявки,
				ТекстЗаменыОтборОтветственныйПродажи);
	...
	Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
		Запрос.ПараметрыURL["page"]) > 1 Тогда
		ЗапросЗиП.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["count"])
			* ((Запрос.ПараметрыURL["page"]) - 1) + 1)));
	Иначе
		ЗапросЗиП.УстановитьПараметр("НачинаяСЗаписи", 0);
	КонецЕсли;
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураОтветсвенного.Вставить("id", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.Ответственный).Код));
		СтруктураОтветсвенного.Вставить("name", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.Ответственный).Наименование));
		...
		СтруктураПортера.Вставить("id", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.ОтветственныйЗаОбработку).Код));
		СтруктураПортера.Вставить("name", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.ОтветственныйЗаОбработку).Наименование));
...
Функция ТипыЗаявокtypes_application(Запрос)
	...
	Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Администратор")) = Неопределено Тогда
		Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Менеджер")) <> Неопределено Тогда
			...
			ЗапросЗаявок.Текст =  СтрШаблон(ТекстДляТиповЗаявок(), ТекстЗаменыОтборОтветственныйЗаявки,
				ТекстЗаменыОтборОтветственныйПродажи);
	...
	ОбщееКоличество = 0;
	Если ТекущаяДата() - 3600 * 24 * 60 < '20240620' Тогда
		Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Администратор")) = Неопределено И Роли.Найти(
		Справочники.ДолжностиДляУК.НайтиПоНаименованию("Менеджер")) = Неопределено Тогда
			...
			Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Водитель")) <> Неопределено Тогда
				Пока выборка.Следующий() Цикл
					Если Выборка.СтатусОбработки = "Упаковка" Или Выборка.СтатусОбработки = "Отправка в тк" Тогда
						Если Выборка.СтатусОбработки = "Отправка в тк" Тогда
							...
							ЗапросКоличество.УстановитьПараметр("Дата", '20240620');
							ЗапросКоличество.УстановитьПараметр("СостояниеСборки",
								Справочники.СтатусыWT.НайтиПоНаименованию(Выборка.СтатусОбработки));

							РезультатЗапросаКоличество = ЗапросКоличество.Выполнить().Выбрать();
							РезультатЗапросаКоличество.Следующий();
							Попытка
								КоличествоПродаж = Число(РезультатЗапросаКоличество.Количество);
							Исключение
								КоличествоПродаж = 0;
							КонецПопытки;

							СтруктураЗаявки = Новый Структура;
							СтруктураЗаявки.Вставить("count", КоличествоПродаж);
							СтруктураЗаявки.Вставить("title", Выборка.СтатусОбработки);

							Если Выборка.СтатусОбработки = "Обращение" Или Выборка.СтатусОбработки = "Заявка"
								Или Выборка.СтатусОбработки = "Сборка" Тогда
							...
							ОбщееКоличество = ОбщееКоличество + КоличествоПродаж;
						Иначе
							СтруктураЗаявки = Новый Структура;
			...
			ИначеЕсли Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Кладовщик")) <> Неопределено Тогда
				Пока выборка.Следующий() Цикл
					...
					Если Выборка.СтатусОбработки = "Сборка" Или Выборка.СтатусОбработки = "Продажа"
						Или Выборка.СтатусОбработки = "Упаковка" Тогда
						СтруктураЗаявки = Новый Структура;
						СтруктураЗаявки.Вставить("count", Выборка.Количество);
						СтруктураЗаявки.Вставить("title", Строка(Выборка.СтатусОбработки));
						Если Выборка.СтатусОбработки = "Обращение" Или Выборка.СтатусОбработки = "Заявка"
		...
		Иначе
			Пока выборка.Следующий() Цикл
				Если Выборка.СтатусОбработки = "Отправка в тк" Или Выборка.СтатусОбработки = "Отправлено клиенту"
					Или Выборка.СтатусОбработки = "Заказ получен" Тогда
					ЗапросКоличество = Новый Запрос;
					ЗапросКоличество.Текст =
					"ВЫБРАТЬ
					|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПродажаЗапчастей.Ссылка) КАК Количество
					...
					ЗапросКоличество.УстановитьПараметр("Дата", '20240620');
					ЗапросКоличество.УстановитьПараметр("СостояниеСборки", Справочники.СтатусыWT.НайтиПоНаименованию(
...
Функция ПолучитьСписокТоваровGetProductsList(Запрос)
	...
	Для Каждого стр Из ТЗ Цикл
		...
		СтруктураТоваров.Вставить("poddon", Строка(стр.Поддон));
		СтруктураТоваров.Вставить("state", Строка(стр.Состояние));
		СтруктураТоваров.Вставить("request_text", Строка(Запрос.ПараметрыURL["id"]));
...
Функция ПолучитьВесьСписокЗаявокalllistapplication(Запрос)
	...
	Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Администратор")) = Неопределено Тогда
		Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Менеджер")) <> Неопределено Тогда
			...
			ЗапросЗаявки.Текст =  СтрШаблон(ТекстдляСпискаЗаявок(), Формат(10000, "ЧГ="),
				ТекстЗаменыОтборОтветственныйЗаявки);
		...
		ИначеЕсли Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Водитель")) <> Неопределено Тогда
			...
			ЗапросЗаявки.Текст =  СтрШаблон(ТекстдляСпискаЗаявок(), Формат(10000, "ЧГ="),
				ТекстЗаменыОтборОтветственныйЗаявки);
			ЗапросЗаявки.УстановитьПараметр("Ответственный", Справочники.Сотрудники.НайтиПоКоду(
				Запрос.ПараметрыURL["userid"]));
	...
	Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Администратор")) = Неопределено Тогда
		Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Менеджер")) <> Неопределено Тогда
			...
			ЗапросЗаявки.Текст =  СтрШаблон(ТекстдляСпискаЗаявок(), Формат(?(Число(Запрос.ПараметрыURL["Count"]) > 0,
				Запрос.ПараметрыURL["Count"], 10000), "ЧГ="), ТекстЗаменыОтборОтветственныйЗаявки);
	...
	Если Число(((Запрос.ПараметрыURL["Count"]) * (Запрос.ПараметрыURL["Page"]))) > 0 И Число(
		Запрос.ПараметрыURL["Page"]) > 1 Тогда
		запросзаявки.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["Count"])
			* (Запрос.ПараметрыURL["Page"] - 1) + 1)));
...
Функция ПолучитьВесьСписокПродажalllissale(Запрос)
	...
	Если состояниеСборки = Справочники.СтатусыWT.НайтиПоКоду("000000006") Или //Отправка в тк
	состояниеСборки = Справочники.СтатусыWT.НайтиПоКоду("000000007") Или //Отправлено клиенту
	состояниеСборки = Справочники.СтатусыWT.НайтиПоКоду("000000008") Тогда //Заказ получен
		Если ТекущаяДата() - 3600 * 24 * 60 < '20240620' Тогда
			запросзаявки.УстановитьПараметр("Дата", '20240620');
		Иначе
			запросзаявки.УстановитьПараметр("Дата", ТекущаяДата() - 3600 * 24 * 60);
		КонецЕсли;
	Иначе
	...
	Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Администратор")) = Неопределено Тогда
		Если Роли.Найти(Справочники.ДолжностиДляУК.НайтиПоНаименованию("Менеджер")) <> Неопределено Тогда
			...
			запросзаявки.Текст =  СтрШаблон(ТекстдляСпискапродаж(), Формат(?(Число(Запрос.ПараметрыURL["Count"]) > 0,
				Запрос.ПараметрыURL["Count"], 10000), "ЧГ="), ТекстЗаменыОтборОтветственныйПродажи);
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураПродажи.Вставить("tk_processing", Строка(выборка.СтатусЗаказаВТК));

		СтруктураОтветсвенного = Новый структура;
		СтруктураОтветсвенного.Вставить("id", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.ктопродал).Код));
		СтруктураОтветсвенного.Вставить("name", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.ктопродал).Наименование));
		СтруктураПродажи.Вставить("responsible", СтруктураОтветсвенного);
		СтруктураПортера= Новый структура;
		СтруктураПортера.Вставить("id", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.ОтветственныйКладовщик).Код));
		СтруктураПортера.Вставить("name", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.ОтветственныйКладовщик).Наименование));
...
Функция ПолучитьЗадачиGetTasks(Запрос)
	...
	ЗапросЗадач.Текст =  СтрШаблон(текстДляЗадач(), Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
		Запрос.ПараметрыURL["count"], 10000), "ЧГ="));
	Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
		Запрос.ПараметрыURL["page"]) > 1 Тогда
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["count"])
			* ((Запрос.ПараметрыURL["page"]) - 1) + 1)));
	Иначе
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", 0);
	КонецЕсли;
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураРодителя.Вставить("id", Строка(выборка.номерРодителя));
		СтруктураРодителя.Вставить("name", Строка(выборка.НаименованиеРодителя));
		СтруктураЗадачи.Вставить("parent_task", СтруктураРодителя);
		СтруктураЗадачи.Вставить("project", выборка.ЗадачаСПортала);
...
Функция ПолучитьЗадачиВсеGetTasksAll(Запрос)
	ЗапросЗадач = Новый Запрос;
	Статус = Новый массив;
	...
	ЗапросЗадач.Текст =  СтрШаблон(Текст, Формат(100000, "ЧГ="));
	ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", 0);
	...
	Выборкаобщ = ЗапросЗадач.Выполнить().Выбрать().Количество();
	ЗапросЗадач.Текст =  СтрШаблон(Текст, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
		Запрос.ПараметрыURL["count"], 10000), "ЧГ="));
	Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
		Запрос.ПараметрыURL["page"]) > 1 Тогда
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["count"])
	...
	Возврат Ответ;
КонецФункции
Функция ПолучитьСписокПроектовЗадачGetProjectTasks(Запрос)
	ЗапросЗадач = Новый Запрос;
	Статус = Новый массив;
	...
	Текст = "ВЫБРАТЬ ПЕРВЫЕ 10000
			...
			|	ИЛИ докЗадача.Описание ПОДОБНО &Описание)
			|	И докЗадача.Дата >= ДАТАВРЕМЯ(2024, 1, 1)
			|	И докЗадача.ЗадачаСПортала = ИСТИНА
			|
			|УПОРЯДОЧИТЬ ПО
			|	Статус,
			|	ДатаВыполнения УБЫВ,
...
Функция ПолучитьСписокПроектовЗадачВсеGetProjectTasksAll(Запрос)
	ЗапросЗадач = Новый Запрос;
	Статус = Новый массив;
...
Функция ПолучитьЗадачиОтАвтораGetTasksFromAuthor(Запрос)
	...
	Выборкаобщ = ЗапросЗадач.Выполнить().Выбрать().Количество();
	ЗапросЗадач.Текст =  СтрШаблон(текстДляЗадачОтАвтора(), Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
		Запрос.ПараметрыURL["count"], 10000), "ЧГ="));
	Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
		Запрос.ПараметрыURL["page"]) > 1 Тогда
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["count"])
...
Функция ПолучитьДашбордGetDash(Запрос)
	...
	ЗапросСотрудников.УстановитьПараметр("ОтделНов", Справочники.Подразделения.НайтиПоКоду(
		Запрос.ПараметрыURL["department_id"]));
...
Функция ПолучитьСтатистикуЗадачgettaskstatistics(Запрос)
	...
	Пока ВыборкаСотрудников.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			//@skip-check query-in-loop
			//@skip-check query-in-loop
			СрокВыполненияЗадачи = ПолучитьСрокВыполненияЗадачи(Выборка.ЗадачаСсылка, НачалоДня(Дата(Строка(
				Запрос.ПараметрыURL["datestart"]))), КонецДня(Дата(Строка(Запрос.ПараметрыURL["dateend"]))));
			Если Выборка.ЗадачаСтатус = Перечисления.СтатусыЗадач.Выполнена И Выборка.ЗадачаДатаВыполнения >= НачалоДня(
				Дата(Строка(Запрос.ПараметрыURL["datestart"]))) И Выборка.ЗадачаДатаВыполнения <= КонецДня(Дата(Строка(
				Запрос.ПараметрыURL["dateend"]))) Тогда
...
Функция ПолучитьПеремещенияgetinmove(Запрос)
	...
	Если Число(Запрос.ПараметрыURL["city"]) = 1 Тогда //Все
		...
		МассивГруппМестХранения.Добавить(Справочники.ГруппыМестХранения.НайтиПоКоду("000000005")); //E
		МассивГруппМестХранения.Добавить(Справочники.ГруппыМестХранения.НайтиПоКоду("000000006")); //F
		МассивГруппМестХранения.Добавить(Справочники.ГруппыМестХранения.НайтиПоКоду("000000007")); //G
	...
	МассивСотрудников.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Ягольник Александр Юрьевич"));
	МассивСотрудников.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Князев Андрей Владимирович"));
	///-ГомзМА 29.02.2024
...
Функция ПолучитьСчетgetcheck(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		Руководитель = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьОсновноеОтветственноеЛицоОрганизации(
			Выборка.Организация, Выборка.Дата);
		...
		СтруктураИнфо.Вставить("provider", Строка(ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике,
			"НаименованиеДляПечатныхФорм,ЮридическийАдрес,Телефоны,Email")));
		СтруктураИнфо.Вставить("buyer", Строка(ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе,
			"ПолноеНаименование,ИНН,КПП,ЮридическийАдрес,Телефоны,Email,НомерСчета,Банк,БИК,КоррСчет")));
		...
		СтруктураИнфо.Вставить("allsum", Строка(Формат(ИтогоСумма + ?(Выборка.СуммаВключаетНДС, 0, Окр(ИтогоСумма
			* СтавкаНДС / (100 + СтавкаНДС), 2)), "ЧГ=0")));
	КонецЦикла;
...
Функция НаполнитьПродажуfillingsale(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		Иначе
			...
			ТекстЛога =  " В продаже " + ОбъектПродажи.номер + " добавил партию в товаре " + Новаястрока.Товар + " "
				+ Новаястрока.Партия;
			ЛогированиеWT10(Выборка.ссылка, Автор, ТекстЛога);
			ОбъектПродажи.Записать(РежимЗаписиДокумента.Проведение);
...
Функция ВыдатьПродажуissueasale(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		ОбъектПродажи.таблица[Число(массив.pose) - 1].ОтветственныйЗаВыдачу = Справочники.Сотрудники.НайтиПоКоду(
			Массив.author).Пользователь;
		Если ОбъектПродажи.таблица[Число(массив.pose) - 1].Склад = Справочники.Склады.ПустаяСсылка()
			Или ОбъектПродажи.таблица[Число(массив.pose) - 1].Склад = Неопределено Тогда
		...
		Если выдано = 0 Тогда
			...
			//ОбъектПродажи.записать();  
		КонецЕсли;
		ТекстЛога =  " В продаже " + ОбъектПродажи.номер + "ц выдал товар " + ОбъектПродажи.таблица[Число(массив.pose)
			- 1].Товар + " " + ОбъектПродажи.таблица[Число(массив.pose) - 1].Партия;
		ЛогированиеWT10(Выборка.ссылка, Массив.author, ТекстЛога);
		ОбъектПродажи.Записать(РежимЗаписиДокумента.Проведение);
	КонецЦикла;
	Структураответа = Новый структура;
...
Функция НаполнитьЗаявкуfillingapplication(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		Если ОбъектЗаявки.Товары[Позиция - 1].Номенклатура = Справочники.ИндКод.НайтиПоНаименованию(ИндКод).Владелец Тогда
			...
			ТекстЛога =  " В заявке " + ОбъектЗаявки.номер + " установил партию в товаре " + ОбъектЗаявки.Товары[Позиция
...
Функция ДобавитьТрекНомерВПродажуaddtracknumber(Запрос)
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		Пока РезультатЗапроса.Следующий() Цикл
			...
			ИнтеграцияAPIТранспортныеКомпанииВызовСервера.УзнатьСтатусДоставки(Массив.track_number,
				РезультатЗапроса.ТранспортнаяКомпания);

			Исполнитель = ДокументОбъект.КтоПродал;
			...
			Если НаборЗаписей.Количество() = 1 Тогда
				...
				ТекстЛога = "Ваша продажа № " + ДокументОбъект.номер + " для клиента " + ДокументОбъект.Клиент
					+ " получила статус ТК " + Строка(ДокументОбъект.СтатусЗаказаВТК);
...
Функция РедактироватьПолку(индкод, Место = Неопределено, Автор)
	...
	/// Комлев 22/07/24 закомментировал. Деталь можно перемещать из корзины в корзину.	
	...
	Для Каждого индкод Из индкоды Цикл
		...
		ИначеЕсли НаборЗаписей.Количество() = 1 Тогда
			...
			Если Поддон <> Справочники.Поддоны.ПустаяСсылка() Тогда
				...
				ЛогированиеИндНомера(Строка(НовыйНомер.индкод), Автор, Лог, Поддон.Стеллаж.Наименование, Место,
					НовыйНомер.Стеллаж, НовыйНомер.поддон);
...
Функция ПолучитьЗаявкиИПродажиgetappandsale(Запрос)
	...
	запросзаявки.УстановитьПараметр("номер", "%" + ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(Строка(
		Запрос.ПараметрыURL["num"])) + "%");
...
Функция СменаРеквизитовТовараEditList(Запрос)
	...
	Ответ = РедактироватьТовар(Массив.id, Массив.coment, число(Массив.cost), Состояние, Массив.name, Массив.author);
	// КонецЦикла;

	Возврат Ответ;
КонецФункции

Функция РедактироватьТовар(индкод, комментарий = Неопределено, цена = Неопределено, Состояние = Неопределено, Наименование = Неопределено, Автор = Неопределено)
	Запрос = Новый запрос;
	Запрос.Текст = "ВЫБРАТЬ
				   |	ИндНомер.индкод КАК индкод,
				   |	ИндНомер.Код КАК Код,
...
/// Комлев +++ 23/07/24 +++
		...
		Если Наименование <> Неопределено И СтароеНазвание <> Наименование Тогда
			...
			НоменклатураОбъект.Лог = НоменклатураОбъект.Лог + Символы.ПС + Формат(ТекущаяДата(), "ДЛФ=DD;") + " "
				+ Автор + " изменил наименование с " + """" + СтароеНазвание + """" + ", " + "на " + """" + Наименование + """" +  "."
				+ Символы.ПС
				+ "--------------------------------------------------------------------------------------------------------------------------";
		КонецЕсли;
...
Функция ПолучитьПотерянныеТоварыgetlost(Запрос)
	...
	Выборкаобщ = Пред.Количество();
	запросТовара.Текст =  СтрШаблон(текстДляТовара(), Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
		Запрос.ПараметрыURL["count"], 10000), "ЧГ="));
	///+ГомзМА 19.02.2024
	Если Запрос.ПараметрыURL["id"] <> " " Тогда
		запросТовара.Текст = СтрЗаменить(запросТовара.Текст, "{ГДЕ", "ГДЕ");
	...
	Если выборка.Количество() > 0 Тогда
		Пока выборка.Следующий() Цикл
			...
			Если Не ОбъектПродажи.Таблица[Позиция - 1].Отменено Тогда
				ТекстЛога =  " В заявке " + ОбъектПродажи.номер + " не найден товар " + ОбъектПродажи.Таблица[Позиция
					- 1].Товар + " " + ОбъектПродажи.Таблица[Позиция - 1].Партия;
			Иначе
				ТекстЛога =  " В заявке " + ОбъектПродажи.номер + " вернул товар " + ОбъектПродажи.Таблица[Позиция
					- 1].Товар + " " + ОбъектПродажи.Таблица[Позиция - 1].Партия;
			КонецЕсли;
			ЛогированиеWT10(Выборка.ссылка, Автор, ТекстЛога);
...
Функция СоздатьЗадачуcreatework(Запрос)
	...
	///+ГомзМА 26.02.2024
	Если число(Массив.level) = 1 Тогда
		ПоставитьПриоритетЗадачи(Документы.Задача.НайтиПоНомеру(Новаязадача.номер),
			Справочники.Подразделения.НайтиПоКоду(Массив.direction));
	ИначеЕсли число(Массив.level) = 3 Тогда
		ПоставитьПриоритетЗадачиСрочно(Документы.Задача.НайтиПоНомеру(Новаязадача.номер),
			Справочники.Подразделения.НайтиПоКоду(Массив.direction));
	КонецЕсли;
	///-ГомзМА 26.02.2024
	Попытка
		Для Каждого стр Из Справочники.Подразделения.НайтиПоКоду(Массив.direction).Участники Цикл
			...
			НаборЗаписей.Прочитать();
			Если НаборЗаписей.Количество() = 1 Тогда
				ТекстЛога = " Для вашего направления создана задача - " + Новаязадача.номер + " "
					+ Новаязадача.Наименование + символы.ПС + "Автор: " + Строка(Ответственный) + символы.ПС
					+ "Ссылка: " + "https://10.wt10.ru/task/" + Строка(Новаязадача.номер);
				Телега = НаборЗаписей[0].ID_Пользователя;
		...
		КонецЦикла;
		ТекстЛога = "Для " + Строка(Новаязадача.Отдел) + " создана задача - " + Новаязадача.номер + " "
			+ Новаязадача.Наименование + символы.ПС + "Автор: " + Строка(Ответственный) + символы.ПС + "Ссылка: "
			+ "https://10.wt10.ru/task/" + Строка(Новаязадача.номер);
		Если число(Массив.level) = 3 Тогда
			ТекстЛога = "❗️❗️СРОЧНО❗️❗️ " + ТекстЛога;
		КонецЕсли;
		...
		НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() = 1 Тогда
			ТекстЛога = " получил от вас задачу - " + Новаязадача.номер + " " + Новаязадача.Наименование + символы.ПС
				+ "Ссылка: " + "https://10.wt10.ru/task/" + Строка(Новаязадача.номер);
			Телега = НаборЗаписей[0].ID_Пользователя;
...
Функция ПолучитьСписокПеремещенийgetlistmove(Запрос)
	...
	запросНарядов.Текст =  СтрШаблон(текстДляСпискаПеремещений(), Формат(?(Число(Запрос.ПараметрыURL["Count"]) > 0,
		Запрос.ПараметрыURL["Count"], 10000), "ЧГ="));
	...
	СтруктураИнфо= Новый Структура;
	СтруктураИнфо.Вставить("pages", Цел(Выборкаобщ / ?(Число(Запрос.ПараметрыURL["Count"]) = 0, Выборкаобщ, Число(
		Запрос.ПараметрыURL["Count"]) + 1)));
	СтруктураИнфо.Вставить("count", Выборкаобщ);
...
Функция СменаCтатусаТоваровВПеремещенииchangestatemove(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		Если Массив.type = "Нет в наличии" Тогда
			...
			ТекстЛога =  " В перемещении " + ОбъектПерем.номер + " установил нет в наличии в товаре " + СтрокаТЧ.Товар
				+ " " + СтрокаТЧ.Партия;
			ЛогированиеWT10(Выборка.ссылка, Автор, ТекстЛога);
		...
		ИначеЕсли Массив.type = "Собрано" Или Массив.type = "Принят" Тогда
			...
			Если НайденныеСтроки.количество() > 0 Тогда 
				...
				Если Массив.type = "Собрано" Тогда
					...
					Если ЗаписьВРегистреСведений.Выбран() Тогда
						...
						ЛогированиеИндНомера(ЗаписьВРегистреСведений.индкод, Автор, Лог, Стеллаж, ,
							ЗаписьВРегистреСведений.Стеллаж);
				...
				ТекстЛога =  " В продаже " + ОбъектПерем.номер + " установил партию в товаре " + СтрокаТЧ.Товар + " "
					+ СтрокаТЧ.Партия;
				ЛогированиеWT10(Выборка.ссылка, Автор, ТекстЛога);
				ОбъектПерем.Записать(РежимЗаписиДокумента.Проведение);
			...
			Иначе
				...
				Если НайденныеСтроки.количество() > 0 Тогда
					...
					ТекстЛога =  " В продаже " + ОбъектПерем.номер + " добавил партию в товаре " + СтрокаТЧ.Товар + " "
						+ СтрокаТЧ.Партия;
					ЛогированиеWT10(Выборка.ссылка, Автор, ТекстЛога);
					ОбъектПерем.Записать(РежимЗаписиДокумента.Проведение);
				...
				Иначе
					...
					ТекстЛога =  " В продаже " + ОбъектПерем.номер + " добавил партию в товаре " + Новаястрока.Товар
						+ " " + Новаястрока.Партия;
					ЛогированиеWT10(Выборка.ссылка, Автор, ТекстЛога);
					ОбъектПерем.Записать(РежимЗаписиДокумента.Проведение);
...
Функция СменаПриоритетаЗадачиChangePriorityTask(Запрос)
	...
	ЗапросКоличесвоЗадач.УстановитьПараметр("Направление", Справочники.Подразделения.НайтиПоКоду(
		Запрос.ПараметрыURL["direction"]));
	...
	Если ЗаписьРегистраСведений.Выбран() И ЗаписьРегистраСведений.Задача <> Документы.Задача.ПустаяСсылка() Тогда
		Если ЗаписьРегистраСведений.Приоритет > Приоритет Тогда
			СдвинутьПорядокЗадачПлюсОдин(ЗаписьРегистраСведений.Приоритет, Приоритет,
				ЗаписьРегистраСведений.Направление);
		Иначе
...
Функция СменаОтветственногоЗадачиChangeExecutorTask(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		Попытка
			...
			НаборЗаписей.Прочитать();
			Если НаборЗаписей.Количество() = 1 Тогда
				ТекстЛога = " На вас переведена задача - " + ОбъектЗадачи.номер + " " + ОбъектЗадачи.Наименование
					+ символы.ПС + "Автор: " + Строка(ОбъектЗадачи.Ответственный);
				Телега = НаборЗаписей[0].ID_Пользователя;
				ТекстЛога = Строка(Исполнитель) + ТекстЛога;
				Telegram_Сервер.ОтправитьСообщениеПользователю(Телега, ТекстЛога);
			КонецЕсли;
...
Функция СменаРодителяЗадачиChangeParentTask(Запрос)
	ЗапросЗадачи = Новый Запрос;
	ЗапросЗадачи.Текст = текстДляЗадачи();
	ЗапросЗадачи.УстановитьПараметр("Номер", Запрос.ПараметрыURL["id"]);
	Выборка = ЗапросЗадачи.Выполнить().Выбрать();
	...
	Мама = Документы.Задача.НайтиПоНомеру(Массив.id_mama);
	...
	Пока выборка.Следующий() Цикл
		...
		Попытка
			//@skip-check query-in-loop
			ЗакрытьВремяЗадачи(Выборка.ссылка, ОбъектЗадачи.исполнитель);
			ОбъектЗадачи.РодительскаяЗадача = Мама;
			ОбъектЗадачи.Исполнитель = Мама.Исполнитель;
			ОбъектЗадачи.Статус = Перечисления.СтатусыЗадач.Отложена;
			ОбъектЗадачи.Записать();
			...
			Ответ.УстановитьТелоИзСтроки("Успех");
			НаборЗаписей = РегистрыСведений.Telegram_Пользователи.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Пользователь.Установить(ОбъектЗадачи.Исполнитель);
			НаборЗаписей.Прочитать();
			Если НаборЗаписей.Количество() = 1 Тогда
				ТекстЛога = " На вас переведена задача - " + ОбъектЗадачи.номер + " " + ОбъектЗадачи.Наименование
					+ символы.ПС + "Автор: " + Строка(ОбъектЗадачи.Ответственный);
	...
	Возврат Ответ;
КонецФункции
Функция СменаАвтораЗадачиChangeAuthorTask(Запрос)
	ЗапросЗадачи = Новый Запрос;
	ЗапросЗадачи.Текст = текстДляЗадачи();
	ЗапросЗадачи.УстановитьПараметр("Номер", Запрос.ПараметрыURL["id"]);
	Выборка = ЗапросЗадачи.Выполнить().Выбрать();
	...
	Пока выборка.Следующий() Цикл
		...
		Попытка
			...
			ЗакрытьВремяЗадачи(Выборка.ссылка, ОбъектЗадачи.исполнитель);
			//ОбъектЗадачи.РодительскаяЗадача = Мама;
			ОбъектЗадачи.Ответственный = Юзер;
			//ОбъектЗадачи.Статус = Перечисления.СтатусыЗадач.Отложена;
			ОбъектЗадачи.Записать();
...
Функция ОтправитьСообщениеsendmessage(Запрос)
	Тело = Запрос.ПолучитьТелоКакстроку();
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(Тело);
	...
	Получатель = Справочники.Сотрудники.НайтиПоКоду(Массив.id).Пользователь;

	НаборЗаписей = РегистрыСведений.Telegram_Пользователи.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Пользователь.Установить(Получатель);
	НаборЗаписей.Прочитать();
	Если НаборЗаписей.Количество() = 1 Тогда
		ТекстЛога = Массив.text;
		Телега = НаборЗаписей[0].ID_Пользователя;
		Telegram_Сервер.ОтправитьСообщениеПользователю(Телега, ТекстЛога);

		Ответ = Новый HTTPСервисОтвет(200);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки("Сообщение отправлено");
		Возврат Ответ;
	КонецЕсли;

	Ответ = Новый HTTPСервисОтвет(200);
	Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
	Ответ.УстановитьТелоИзСтроки("Сообщение не отправлено");
	Возврат Ответ;
КонецФункции

Функция ОтправитьСообщениеВГруппуsendmessagegroup(Запрос)
	Тело = Запрос.ПолучитьТелоКакстроку();
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(Тело);
...
Функция ПолучитьСписокКарточекТоваровGetProductsList(Запрос)
	...
	запросТовара.Текст =  СтрШаблон(текстДляКарточекНоменклатуры(), Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
...
Функция ПолучитьЗадачуGetTask(Запрос)
	...
	ЗапросЗадач.Текст = "ВЫБРАТЬ
						...
						|	Задача.Направление,
						|	Задача.ИдентификаторРаботыЗаказНарядаНаВосстановление,
						|	Задача.РодительскаяЗадача.номер КАК номерРодителя,
						|	Задача.РодительскаяЗадача.Наименование КАК НаименованиеРодителя,
						|	Задача.Авторы.(
						|		Ссылка,
						|		НомерСтроки,
						|		Автор) КАК Авторы
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураИсполнителя.Вставить("id", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.Исполнитель).Код);
		...
		СтруктураАвтора.Вставить("id", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.Ответственный).Код);
		...
		Для Каждого Стр Из Исполнители Цикл
			...
			СтруктураИсполнителей.Вставить("id", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
				Стр.Исполнитель).Код);
		...
		Авторы = Выборка.Авторы.Выгрузить();
		Для Каждого Стр Из Авторы Цикл
			СтруктураАвторов = Новый Структура;
			СтруктураАвторов.Вставить("name", Строка(Стр.Автор));
			СтруктураАвторов.Вставить("id", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь", Стр.Автор).Код);
			МассивАвторов.Добавить(СтруктураАвторов);
		КонецЦикла;
		СтруктураЗадачи.Вставить("sub_authors", МассивАвторов);
		СтруктураЗадачи.Вставить("project", выборка.ЗадачаСПортала);
...
Функция ПолучитьЗадачиОтделаGetTasksDepartment(Запрос)
	...
	Выборкаобщ = ЗапросЗадач.Выполнить().Выбрать().Количество();
	ЗапросЗадач.Текст =  СтрШаблон(текстДляЗадачОтдела(), Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
		Запрос.ПараметрыURL["count"], 10000), "ЧГ="));
	Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
		Запрос.ПараметрыURL["page"]) > 1 Тогда
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["count"])
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураЗадачи.Вставить("number", Строка(выборка.Номер));
		СтруктураЗадачи.Вставить("executor", Строка(выборка.Исполнитель));
		СтруктураЗадачи.Вставить("id_executor", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			выборка.Исполнитель, , ).код));
		СтруктураЗадачи.Вставить("author", Строка(выборка.Ответственный));
		СтруктураЗадачи.Вставить("state", Строка(выборка.Статус));
		СтруктураЗадачи.Вставить("srochno", Срочно);
	...
	Возврат Ответ;
КонецФункции
Функция ПолучитьПроектыЗадачОтделаGetProjectsTasksDepartment(Запрос)
...
Функция ПолучитьСтатистикуПродажgetsalesstatistics(Запрос)
	...
	ОбщаяКонверсия = Формат(?(ОбщееКоличествоЗаявок <> 0, ОбщееКоличествоПродаж / ОбщееКоличествоЗаявок * 100, 0),
		"ЧДЦ=2");
...
Функция ПолучитьСписокНаправленийgetlistdirection(Запрос)
	...
	ЗапросНаправлений.УстановитьПараметр("Отдел", Справочники.Подразделения.НайтиПоКоду(
		Запрос.ПараметрыURL["department_id"]));
...
Функция СменаНаправленияЗадачиChangeDirectionTask(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		Попытка
			...
			ОбъектЗадачи.Отдел = Справочники.Подразделения.НайтиПоКоду(Массив.direction).Родитель;
			ОбъектЗадачи.Записать();
...
Функция ПолучитьСписокФотографовУчетчиковGetUchetchikPhotographerList(Запрос)
	...
	запросСотрудников.УстановитьПараметр("Сотрудник", Справочники.Сотрудники.НайтиПоНаименованию(
		"Барчев Дмитрий Олегович"));
...
Функция ПолучитьОргСтруктуруgetorgstructure(Запрос)
	...
	ЗапросОргСтруктуры.Текст = "ВЫБРАТЬ
							   ...
							   |ПО
							   |	Ссылка Только ИЕРАРХИЯ";
	ЗапросДолжностей = Новый запрос;
	ЗапросДолжностей.текст = "ВЫБРАТЬ
							 |	КадровыйПриказ.Сотрудник КАК Сотрудник,
							 |	МАКСИМУМ(КадровыйПриказ.Дата) КАК ДатаДоговора
							 |ПОМЕСТИТЬ ВТ_ДолжностиДаты
							 |ИЗ
							 ...
							 |
							 |	ПодразделенияУчастники.Сотрудник КАК Сотрудник,
							 |	ВТ_ДолжностиНовые.Должность КАК Должность
							 |ИЗ
							 |	Справочник.Подразделения.Участники КАК ПодразделенияУчастники
							 |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДолжностиНовые КАК ВТ_ДолжностиНовые
							 |		ПО ПодразделенияУчастники.Сотрудник = ВТ_ДолжностиНовые.Сотрудник.Пользователь
							 |Где
							 |	ПодразделенияУчастники.Сотрудник = &Сотрудник";
			...
			Для Каждого стрТретьегоУровня Из СтрВторогоУровня.Строки Цикл
				Если стрТретьегоУровня.Ссылка <> СтрВторогоУровня.Ссылка Тогда
					...
					Для Каждого Стр Из ТУ Цикл
						СтруктураУчастников = Новый Структура;
						СтруктураУчастников.Вставить("id", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
							Стр.сотрудник).Код);
						ЗапросДолжностей.УстановитьПараметр("Сотрудник", Стр.сотрудник);
						Выборкадол = ЗапросДолжностей.Выполнить().Выбрать();
						Выборкадол.Следующий();
						СтруктураУчастников.Вставить("post", Строка(Выборкадол.Должность));
...
Функция ПолучитьКаталогМашинgetlisttrucks(Запрос)
	...
	///+ГомзМА 02.04.2024
	ЗапросМашин.Текст =  СтрШаблон(ТекстЗапроса, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
		Запрос.ПараметрыURL["count"], 100000), "ЧГ="));
	///-ГомзМА 02.04.2024

	Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
		Запрос.ПараметрыURL["page"]) > 1 Тогда
		ЗапросМашин.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["count"])
			* ((Запрос.ПараметрыURL["page"]) - 1) + 1)));
	Иначе
		ЗапросМашин.УстановитьПараметр("НачинаяСЗаписи", 0);
	КонецЕсли;
...
Функция ПолучитьФинансовуюСтатистикуgetfinancialstatistics(Запрос)
	...
	ЗапросВыручка.УстановитьПараметр("Ответственный", Справочники.Пользователи.НайтиПоНаименованию(
		"Вадяева Дарья Викторовна"));
	...
	Пока РезультатЗапросаВыручка.Следующий() Цикл
		Сумма = ?(РезультатЗапросаВыручка.СуммаПриход = Null, 0, Число(Формат(РезультатЗапросаВыручка.СуммаПриход,
			"ЧДЦ=0; ЧН=0;")));
	...
	Пока РезультатЗапросаРасходПоставщики.Следующий() Цикл
		Сумма = ?(РезультатЗапросаРасходПоставщики.Сумма = Null, 0, Число(Формат(
			РезультатЗапросаРасходПоставщики.Сумма, "ЧДЦ=0; ЧН=0;")));
...
Функция РедактироватьКомментарийКМашинеedittruckcomment(Запрос)
	...
	Комментарий = символы.ПС + "//////////" + символы.ПС + Строка(Юзер) + " " + Строка(ТекущаяДата()) + ": "
		+ массив.comment;
...
Функция ПолучитьГодовуюФинансовуюСтатистикуgetyearfinancialstatistics(Запрос)
	...
	//@skip-check query-in-loop
		...
		Выручка = ?(РезультатЗапроса.СуммаПриход = Null, 0, Число(Формат(РезультатЗапроса.СуммаПриход, "ЧДЦ=0; ЧН=0;")));
		Расход 	= ?(РезультатЗапроса.СуммаРасход = Null, 0, Число(Формат(РезультатЗапроса.СуммаРасход,
			"ЧДЦ=0; ЧН=0;")));
		Прибыль = Выручка - Расход;
		Рентабельность = Число(Формат(?(Выручка = 0, 0, Прибыль / Выручка * 100), "ЧДЦ=0; ЧН=0"));
...
Функция ПолучитьЧекЛистgetchecklist(Запрос)
	...
	Пока ВыборкаЧек.Следующий() Цикл
		...
		Если ПроверкаСтатусаРодительскойЗадачи(ВыборкаЧек.ссылка) = 1 Тогда
			СтруктураЧек.Вставить("state", Строка(Перечисления.СтатусыЗадач.ВРаботе));
		Иначе
		...
		СтруктураАвтора.Вставить("id", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			ВыборкаЧек.Ответственный).Код));
...
Функция ПроверкаСтатусаРодительскойЗадачи(Задача)
	Запрос = Новый Запрос;
	Запрос.текст = "ВЫБРАТЬ
				   |	Задача.Ссылка
				   |ИЗ
				   ...
				   |	ИЛИ Задача.РодительскаяЗадача.РодительскаяЗадача = &РодительскаяЗадача
				   |	ИЛИ Задача.РодительскаяЗадача.РодительскаяЗадача.РодительскаяЗадача = &РодительскаяЗадача
				   |	ИЛИ Задача.РодительскаяЗадача.РодительскаяЗадача.РодительскаяЗадача.РодительскаяЗадача = &РодительскаяЗадача)
				   |	И Задача.Статус = &Статус";
	Запрос.УстановитьПараметр("РодительскаяЗадача", Задача);
	Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыЗадач.ВРаботе);
	Задачки = Запрос.Выполнить().Выбрать().Количество();

	Если Задачки > 0 Тогда
		Возврат 1;
	Иначе
		Возврат 0;
	КонецЕсли;
...
//Исполнитель = стр.Сотрудник;
		...
		НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() = 1 Тогда
			ТекстЛога = " Для вас создана задача - " + Новаязадача.номер + " " + Новаязадача.Наименование + символы.ПС
				+ "Автор: " + Строка(Ответственный) + символы.ПС + "Ссылка: " + "https://10.wt10.ru/task/" + Строка(
				Новаязадача.номер);
			Телега = НаборЗаписей[0].ID_Пользователя;
			ТекстЛога = Строка(Исполнитель) + ТекстЛога;
...
//КонецЦикла;
		ТекстЛога = "Для " + Строка(Исполнитель) + " создана задача - " + Новаязадача.номер + " "
			+ Новаязадача.Наименование + символы.ПС + "Автор: " + Строка(Ответственный) + символы.ПС + "Ссылка: "
			+ "https://10.wt10.ru/task/" + Строка(Новаязадача.номер);
...
Функция ПолучитьСтатистикуВосстановленияМашинgetrestoringmachines(Запрос)
	...
	Пока РезультатЗапросаПроект.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Результат.СуммаДоходПотенциальный = Результат.СуммаДоходПотенциальный
				+ ВыборкаДетальныеЗаписи.СуммаДоходПотенциальный;
...
Функция ПолучитьФотографов(МассивИндкодов)
	...
	Структура.Вставить("id", МассивИндкодов);
...
Функция СоздатьЗадачуИзЗаказНарядаВосстановленияcreateworkorderrecovery(Запрос)
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		Пока РезультатЗапроса.Следующий() Цикл
			...
			Попытка
				Для Каждого стр Из Справочники.Подразделения.НайтиПоНаименованию(
					РезультатЗапросаОтдел.Направление).Участники Цикл
					Исполнитель = стр.Сотрудник;
					НаборЗаписей = РегистрыСведений.Telegram_Пользователи.СоздатьНаборЗаписей();
					НаборЗаписей.Отбор.Пользователь.Установить(Исполнитель);
					НаборЗаписей.Прочитать();
				...
				НаборЗаписей.Прочитать();
				Если НаборЗаписей.Количество() = 1 Тогда
					ТекстЛога = " получил от вас задачу - " + Новаязадача.номер + " " + Новаязадача.Наименование
						+ символы.ПС + "Ссылка: " + "https://10.wt10.ru/task/" + Строка(Новаязадача.номер);
					Телега = НаборЗаписей[0].ID_Пользователя;
					ТекстЛога = Строка(Новаязадача.Отдел) + ТекстЛога;
					Telegram_Сервер.ОтправитьСообщениеПользователю(Телега, ТекстЛога);
				Иначе 
...
Функция ПолучитьСписокНарядовВосстановленияgetlistordersrecovery(Запрос)
	...
	запросНарядов.Текст =  СтрШаблон(текстДляСпискаНарядовВосстановления(), Формат(?(Число(Запрос.ПараметрыURL["Count"])
		> 0, Запрос.ПараметрыURL["Count"], 10000), "ЧГ="));
...
Функция ДобавитьАвтораВЗадачуaddauthor(Запрос)
	...
	Для Каждого aut Из Массив.author Цикл
		Автор = Справочники.Сотрудники.НайтиПоКоду(aut).Пользователь;
		Отбор = Новый Структура;
		Отбор.Вставить("Автор", Автор);
		НайденноеЗначение = Задача.Авторы.НайтиСтроки(Отбор);

		Если НайденноеЗначение.Количество() = 0 Тогда
			НовыйАвтор = Задача.Авторы.Добавить();
			НовыйАвтор.автор = Автор;
			Задача.Записать();
		Иначе
			Ответ = Новый HTTPСервисОтвет(200);
			Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
...
Функция УдалитьОтветственногоВЗадачуdeleteexecutor(Запрос)
	...
	Возврат Ответ;
КонецФункции
Функция УдалитьАвтораВЗадачуdeleteexecutor(Запрос)
...
Функция ПолучитьСписокНарядовДляРаботgetlistordersforworks(Запрос)
	...
	ЗапросНарядов.Текст =  СтрШаблон(Текст, Формат(?(Число(Запрос.ПараметрыURL["Count"]) > 0,
		Запрос.ПараметрыURL["Count"], 10000), "ЧГ="));
...
Функция ИзменитьСтатусЗаявкиeditstatusapplication(Запрос)
	...
	Статус = Справочники.СтатусыWT.НайтиПоКоду(Массив.status);
	...
	Исключение
		Ответ = Новый HTTPСервисОтвет(200);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки("Не удалось изменить статус заявки");
	КонецПопытки;
...
Функция ДашбордРВРgetdashrvr(Запрос)
	ДатаНачала    = НачалоДня(Дата(Запрос.ПараметрыURL["date_start"]));
	ДатаОкончания = КонецДня(Дата(Запрос.ПараметрыURL["date_end"]));
	...
	ЗапросЗН.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросЗН.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	...
	ЗапросПроекта.Текст = "ВЫБРАТЬ
						  ...
						  |	СУММА(ЗаказНарядТовары.Количество) КАК Количество,
						  |	СУММА(ВЫБОР
						  |		КОГДА ИндНомер.Состояние = ЗНАЧЕНИЕ(Перечисление.Состояние.Новый)
						  |			ТОГДА ЗаказНарядТовары.СуммаВсего
						  |		ИНАЧЕ 0
						  |	КОНЕЦ) КАК СуммаКаширин
						  |ПОМЕСТИТЬ ВТ_Товары
						  |ИЗ
						  |	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
						  |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК ИндНомер
						  |		ПО ЗаказНарядТовары.Партия = ИндНомер.индкод
						  |СГРУППИРОВАТЬ ПО
						  |	ЗаказНарядТовары.Ссылка
						  |;
						  |
						  ...
						  |	ЗаказНаряд.Ссылка КАК Ссылка,
						  |	ЗаказНаряд.ТипНаценки КАК ТипНаценки
						  |ПОМЕСТИТЬ ВТ_ТипНаценки
						  |ИЗ
						  |	Документ.ЗаказНаряд КАК ЗаказНаряд
						  |СГРУППИРОВАТЬ ПО
						  |	ЗаказНаряд.Ссылка,
						  |	ЗаказНаряд.ТипНаценки
						  |;
						  |
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ докЗаказНаряд.Номер) КАК Номер,
						  ...
						  |	СУММА(ЕСТЬNULL(ВТ_Товары.СуммаКаширин, 0)) КАК СуммаКаширин,
						  |	ВТ_ТипНаценки.ТипНаценки КАК ТипНаценки,
						  ...
						  |ИЗ
						  |	Справочник.ПроектыРазработки КАК ПроектыРазработки
						  |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаряд КАК докЗаказНаряд
						  |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Приход КАК ВТ_Приход
						  |			ПО докЗаказНаряд.Ссылка = ВТ_Приход.Сделка
						  |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Товары КАК ВТ_Товары
						  |			ПО докЗаказНаряд.Ссылка = ВТ_Товары.Ссылка
						  |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Работы КАК ВТ_Работы
						  |			ПО докЗаказНаряд.Ссылка = ВТ_Работы.Ссылка
						  |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Расходники КАК ВТ_Расходники
						  |			ПО докЗаказНаряд.Ссылка = ВТ_Расходники.Ссылка
						  |			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТипНаценки КАК ВТ_ТипНаценки
						  |			ПО докЗаказНаряд.Ссылка = ВТ_ТипНаценки.Ссылка
						  |		ПО ПроектыРазработки.Ссылка = докЗаказНаряд.Проект
						  |ГДЕ
						  |	докЗаказНаряд.Ответственный = &Фильтр
						  |	И докЗаказНаряд.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
						  |СГРУППИРОВАТЬ ПО
						  |	ПроектыРазработки.Ссылка,
						  |	ПроектыРазработки.ДатаПриема,
						  |	ПроектыРазработки.ДатаСдачи,
						  ...
						  |	ВТ_ТипНаценки.ТипНаценки
						  |
						  |УПОРЯДОЧИТЬ ПО
						  |	Ссылка";
	ЗапросПроекта.УстановитьПараметр("Фильтр", Справочники.Сотрудники.НайтиПоКоду("000000040").Пользователь);
	ЗапросПроекта.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросПроекта.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураПроекта.Вставить("day_to_end", ?(Выборка.ДатаСдачи = '00010101', 0, (Выборка.ДатаСдачи - текущаядата())
			/ (3600 * 24)));
		СтруктураПроекта.Вставить("day_to_start", ?(Выборка.ПлановаяДатаРемонта = '00010101', 0,
			(Выборка.ПлановаяДатаРемонта - текущаядата()) / (3600 * 24)));
		СтруктураПроекта.Вставить("cost_works", Выборка.СуммаРабот);
		СтруктураПроекта.Вставить("cost_details", Выборка.СуммаДеталей);
		СтруктураПроекта.Вставить("cost_kashirin", Выборка.СуммаКаширин);
		СтруктураПроекта.Вставить("stonks", Выборка.СуммаДокумента);
		СтруктураПроекта.Вставить("pay_type", Строка(Выборка.ТипНаценки));
	...
	МассивЧеловеков.Добавить("000000063"); //Бодягин Игорь Юрьевич

	ЗапросЗП.УстановитьПараметр("Работник", МассивЧеловеков);
	ЗапросЗП.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросЗП.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	...
	///+ГомзМА 07.05.2024
	ЗапросВыручка = Новый запрос;
	ЗапросВыручка.Текст = "ВЫБРАТЬ
						  |	СУММА(ВЫБОР
						  |		КОГДА ЗаказНаряд.ТипНаценки.Код = ""000000002""
						  |			ТОГДА ЗаказНаряд.СуммаДокумента
						  |		ИНАЧЕ 0
						  |	КОНЕЦ) КАК СНДС,
						  |	СУММА(ВЫБОР
						  |		КОГДА ЗаказНаряд.ТипНаценки.Код <> ""000000002""
						  |		ИЛИ ЗаказНаряд.ТипНаценки = ЗНАЧЕНИЕ(Справочник.ТипыНаценок.ПустаяСсылка)
						  |			ТОГДА ЗаказНаряд.СуммаДокумента
						  |		ИНАЧЕ 0
						  |	КОНЕЦ) КАК БезНДС
						  |ПОМЕСТИТЬ ВТ_Суммы
						  ...
						  |	И ЗаказНаряд.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
						  |	И ЗаказНаряд.Ответственный = &Ответственный
						  |;
						  |
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	СУММА(ЗаказНаряд.СуммаДокумента) КАК СуммаТехнопарк
						  |ПОМЕСТИТЬ ВТ_СуммыТехнопарк
						  |ИЗ
						  |	Документ.ЗаказНаряд КАК ЗаказНаряд
						  |ГДЕ
						  |	ЗаказНаряд.Организация = &Организация
						  ...
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	ВТ_Суммы.СНДС КАК СНДС,
						  |	ВТ_Суммы.БезНДС КАК БезНДС,
						  |	ВТ_СуммыТехнопарк.СуммаТехнопарк КАК СуммаТехнопарк
						  |ИЗ
						  |	ВТ_Суммы КАК ВТ_Суммы,
						  |	ВТ_СуммыТехнопарк КАК ВТ_СуммыТехнопарк";

	ЗапросВыручка.УстановитьПараметр("Организация", Справочники.Организация.НайтиПоКоду("000000008"));
	ЗапросВыручка.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросВыручка.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	ЗапросВыручка.УстановитьПараметр("Ответственный", Справочники.Сотрудники.НайтиПоКоду("000000040").Пользователь); //Грязев Илья Константинович

	ВыборкаВыручка = ЗапросВыручка.Выполнить().Выбрать();
	МассивВыручка = Новый Массив;
	ВыборкаВыручка.Следующий();
	СтруктураВыручка = Новый Структура;

	Попытка
		СтруктураВыручка.Вставить("with_nds", Число(ВыборкаВыручка.СНДС));
	Исключение
		СтруктураВыручка.Вставить("with_nds", 0);
	КонецПопытки;

	Попытка
		СтруктураВыручка.Вставить("without_nds", Число(ВыборкаВыручка.БезНДС));
	Исключение
		СтруктураВыручка.Вставить("without_nds", 0);
	КонецПопытки;

	Попытка
		СтруктураВыручка.Вставить("texnopark", Число(ВыборкаВыручка.СуммаТехнопарк));
	Исключение
		СтруктураВыручка.Вставить("texnopark", 0);
	КонецПопытки;

	МассивВыручка.Добавить(СтруктураВыручка);
	///-ГомзМА 07.05.2024
	...
	СтруктураОтвета.Вставить("revenue", МассивВыручка);
...
Функция ДашбордМаляркиgetdashpainter(Запрос)
	ДатаНачала = Дата(Запрос.ПараметрыURL["date_start"]);
	ДатаОкончания = Дата(Запрос.ПараметрыURL["date_end"]);
	...
	ЗапросВыручка.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросВыручка.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	ЗапросВыручка.УстановитьПараметр("Ответственный", Справочники.Сотрудники.НайтиПоКоду("000000039").Пользователь); //Емелин Константин Валерьевич
...
Функция ДашбордВосстановленияgetdashrecovery(Запрос)
	...
	ЗапросВыручка = Новый запрос;
	ЗапросВыручка.Текст = "ВЫБРАТЬ
						  |	СУММА(ИндНомер.Цена) КАК СуммаТовары
						  |ПОМЕСТИТЬ ВТ_Товары
						  |ИЗ
						  |	Документ.Рас_Комплектовка.НоменклатураСписок КАК Рас_КомплектовкаНоменклатураСписок
						  |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК ИндНомер
						  ...
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	СУММА(Рас_КомплектовкаРаботыСписок.СуммаВсего) КАК СуммаРаботы
						  |ПОМЕСТИТЬ ВТ_Работы
						  |ИЗ
						  |	Документ.Рас_Комплектовка.РаботыСписок КАК Рас_КомплектовкаРаботыСписок
						  |ГДЕ
						  ...
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	ВТ_Работы.СуммаРаботы + ВТ_Товары.СуммаТовары КАК Выручка
						  |ИЗ
	...
	Попытка
		СтруктураВыручка.Вставить("sum", Число(ВыборкаВыручка.Выручка));
	Исключение
		СтруктураВыручка.Вставить("sum", 0);
	КонецПопытки;
...
//	Иначе     
	Если ТекущаяДата() - 3600 * 24 * 60 < '20240620' Тогда
		Пока выборка.Следующий() Цикл
			СтатусОбработки = Строка(Выборка.СтатусОбработки);
			Если СтатусОбработки = "Отправка в тк" Или СтатусОбработки = "Отправлено клиенту" Или СтатусОбработки
				= "Заказ получен" Тогда
...
//Сергеев Ф.В. ++ Дата: 13.05.2024
Функция ИзменитьПроектностьЗадачиdeleteauthor(Запрос)
	Тело = Запрос.ПолучитьТелоКакстроку();
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(Тело);
	...
	Массив = ПрочитатьJSON(ЧтениеJSON);
	Задача = Документы.Задача.НайтиПоНомеру(Массив.id).ПолучитьОбъект();
	Задача.ЗадачаСПортала = Не Задача.ЗадачаСПортала;
	Задача.Записать();
...
КонецФункции
//Сергеев Ф.В. -- Дата: 13.05.2024
Функция ПолучитьДолжностиСотрудниковGetWorkersPositions(Запрос)
	...
	ЗапросДолжности.Текст =  текстДолжностиСотрудников();

	ЗапросДолжности.УстановитьПараметр("Код", Массив.code);
	...
	Пока ВыборкаДолжности.Следующий() Цикл
		СтруктураСотрдников = Новый Структура;
		СтруктураСотрдников.Вставить("name", Строка(ВыборкаДолжности.Наименование));
		СтруктураСотрдников.Вставить("code", Строка(ВыборкаДолжности.Код));
		СтруктураСотрдников.Вставить("post", Строка(ВыборкаДолжности.ДолжностьНов));
...
Функция ПолучитьНеликвидgetliquid(Запрос)
	ЗапросТоваров = Новый Запрос;
	ЗапросТоваров.Текст = "ВЫБРАТЬ
						  |	ЗаказКлиентаТовары.Номенклатура КАК Номенклатура,
						  |	СУММА(ЗаказКлиентаТовары.Количество) КАК Количество,
						  |	НАЧАЛОПЕРИОДА(ЗаказКлиентаТовары.Ссылка.Дата, ГОД) КАК Дата
						  |ПОМЕСТИТЬ ВТ_ТоварыПоГодам
						  |ИЗ
						  |	Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
						  |СГРУППИРОВАТЬ ПО
						  |	ЗаказКлиентаТовары.Номенклатура,
						  |	НАЧАЛОПЕРИОДА(ЗаказКлиентаТовары.Ссылка.Дата, ГОД)
						  |;
						  |
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	ВТ_ТоварыПоГодам.Номенклатура КАК Номенклатура,
						  |	СРЕДНЕЕ(ВТ_ТоварыПоГодам.Количество) КАК Количество
						  |ПОМЕСТИТЬ ВТ_СреднемВГод
						  |ИЗ
						  |	ВТ_ТоварыПоГодам КАК ВТ_ТоварыПоГодам
						  |СГРУППИРОВАТЬ ПО
						  |	ВТ_ТоварыПоГодам.Номенклатура
						  |;
						  |
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	ИндНомер.индкод.Владелец КАК индкодВладелец,
						  |	СРЕДНЕЕ(ИндНомер.Цена) КАК Цена
						  |ПОМЕСТИТЬ ВТ_Ценник
						  |ИЗ
						  |	РегистрСведений.ИндНомер КАК ИндНомер
						  |СГРУППИРОВАТЬ ПО
						  ...
						  |	СУММА(ВЫБОР
						  |		КОГДА РегИндНомер.Цена > 0
						  |			ТОГДА РегИндНомер.Цена * РегистрНакопления1Остатки.КолвоОстаток
						  |		ИНАЧЕ ИндНомер.Номенклатура.РекомендованаяЦена * РегистрНакопления1Остатки.КолвоОстаток
						  |	КОНЕЦ) КАК Цена,
						  |	ВЫБОР
						  |		КОГДА ИндНомер.Количество >= 10
						  |			ТОГДА ""Ликвид""
						  |		ИНАЧЕ ""Неликвид""
						  |	КОНЕЦ КАК Наименование
						  |ИЗ
						  |	РегистрНакопления.РегистрНакопления1.Остатки КАК РегистрНакопления1Остатки
						  |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СреднемВГод КАК ИндНомер
						  |		ПО (ИндНомер.Номенклатура = РегистрНакопления1Остатки.Товар)
						  |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Ценник КАК РегИндНомер
						  |		ПО (РегистрНакопления1Остатки.Товар = РегИндНомер.индкодВладелец)
						  |СГРУППИРОВАТЬ ПО
						  |	ВЫБОР
						  |		КОГДА ИндНомер.Количество >= 10
						  |			ТОГДА ""Ликвид""
	...
	Пока Выборка.Следующий() Цикл
		СтруктураСотрдников = Новый Структура;
		СтруктураСотрдников.Вставить("name", Строка(Выборка.Наименование));
		СтруктураСотрдников.Вставить("count", Строка(Выборка.КолвоОстаток));
		СтруктураСотрдников.Вставить("cost", Строка(Выборка.Цена));

		МассивТоваров.Добавить(СтруктураСотрдников);
	КонецЦикла;
...
Функция ПолучитьНеликвидВладимирскийgetliquidVlad(Запрос)

	ДатаОкончания = КонецДня(ТекущаяДатаСеанса());
	ДатаНачала = НачалоДня(ДатаОкончания - 86400 * 365);

	ЗапросТоваров = Новый Запрос;
	ЗапросТоваров.Текст = "ВЫБРАТЬ
						  |	ЗаказКлиентаТовары.Номенклатура КАК Товар,
						  |	СУММА(ЗаказКлиентаТовары.Количество) КАК Количество
						  |ПОМЕСТИТЬ ВТ_ТоварыЗаПериод
						  |ИЗ
						  |	Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
						  |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК ИндНомер
						  ...
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  |	ВТ_ТоварыЗаПериод.Товар КАК Товар,
						  |	ВТ_ТоварыЗаПериод.Количество КАК Количество,
						  |	ВЫРАЗИТЬ(ВТ_ТоварыЗаПериод.Количество / РАЗНОСТЬДАТ(&ДатаНачала, &ДатаОкончания, ДЕНЬ) КАК ЧИСЛО(15, 3)) КАК
						  |		ПроданоВДень,
						  |	ВЫРАЗИТЬ(ВТ_ТоварыЗаПериод.Количество / РАЗНОСТЬДАТ(&ДатаНачала, &ДатаОкончания, ДЕНЬ) * 14 КАК ЧИСЛО(5, 3)) КАК
						  |		НаДвеНедели,
						  |	ВЫРАЗИТЬ(ВТ_ТоварыЗаПериод.Количество / РАЗНОСТЬДАТ(&ДатаНачала, &ДатаОкончания, ДЕНЬ) * 7 КАК ЧИСЛО(5, 3)) КАК
						  |		НаНеделю,
						  |	ЕСТЬNULL(РегистрНакопления1Остатки.КолвоОстаток, 0) КАК КолвоОстаток,
						  |	ВЫБОР
						  |		КОГДА ЕСТЬNULL(РегистрНакопления1Остатки.КолвоОстаток, 0) >= ВТ_ТоварыЗаПериод.Количество / РАЗНОСТЬДАТ(&ДатаНачала,
						  |			&ДатаОкончания, ДЕНЬ)
						  |		И ЕСТЬNULL(РегистрНакопления1Остатки.КолвоОстаток, 0) <= ВТ_ТоварыЗаПериод.Количество / РАЗНОСТЬДАТ(&ДатаНачала,
						  |			&ДатаОкончания, ДЕНЬ) * 7
						  |			ТОГДА ИСТИНА
						  |		ИНАЧЕ ЛОЖЬ
						  |	КОНЕЦ КАК Желтый,
						  |	ВЫБОР
						  ...
						  |		ИНАЧЕ ЛОЖЬ
						  |	КОНЕЦ КАК Красный,
						  |	ВТ_ТоварыЗаПериод.Товар.Артикул КАК ТоварАртикул,
						  |	ВТ_ТоварыЗаПериод.Товар.Код КАК Код
						  |ИЗ
						  |	ВТ_ТоварыЗаПериод КАК ВТ_ТоварыЗаПериод
						  |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РегистрНакопления1.Остатки КАК РегистрНакопления1Остатки
						  |		ПО ВТ_ТоварыЗаПериод.Товар = РегистрНакопления1Остатки.Товар
						  |ГДЕ
						  |	ЕСТЬNULL(РегистрНакопления1Остатки.КолвоОстаток, 0) <= ВТ_ТоварыЗаПериод.Количество / РАЗНОСТЬДАТ(&ДатаНачала,
						  |		&ДатаОкончания, ДЕНЬ) * 14
						  |
						  |УПОРЯДОЧИТЬ ПО
						  |	ПроданоВДень УБЫВ";

	ЗапросТоваров.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросТоваров.УстановитьПараметр("ДатаОкончания", ДатаОкончания);

	Выборка = ЗапросТоваров.Выполнить().Выгрузить();
	МассивТоваров = Новый массив;

	Выборка.Колонки.Добавить("колФото", Новый ОписаниеТипов("Число"));
	Выборка.Колонки.Добавить("Коды", Новый ОписаниеТипов("Строка"));
	МассивТоваров = Новый Массив;
...
//		Стр.Коды = "00" + код;
//	КонецЦикла;
	ИндКоды = Выборка.ВыгрузитьКолонку("Коды");
	Фотки = ПолучениеФотокарточек(ИндКоды);
	Итер = 0;
	...
	Для Каждого стр Из Выборка Цикл
		...
		СтруктураТоваров = Новый Структура;
		СтруктураТоваров.Вставить("date_sale", Строка(ПолучитьПоследнююПродажу(Стр.Товар)));
		СтруктураТоваров.Вставить("name", Строка(Стр.Товар));
		СтруктураТоваров.Вставить("article", Строка(Стр.ТоварАртикул));
		СтруктураТоваров.Вставить("id", Строка(Стр.Код));
		СтруктураТоваров.Вставить("count", Строка(Стр.КолвоОстаток));
		СтруктураТоваров.Вставить("in_day", Строка(Стр.ПроданоВДень));
		СтруктураТоваров.Вставить("in_week", Строка(Стр.НаНеделю));
		СтруктураТоваров.Вставить("in_2_week", Строка(Стр.НаДвеНедели));
		СтруктураТоваров.Вставить("is_not_liquid_products", ПроверитьНеЛиквид(стр.Товар));
		//Рейтинг
		КоличествоТоваров = Выборка.Количество();
		Шаг = КоличествоТоваров / 5;
		ОднаЗвезда 		= КоличествоТоваров;
		ДвеЗвезды 		= ОднаЗвезда - Шаг;
		ТриЗвезды 		= ДвеЗвезды - Шаг;
		ЧетыреЗвезды 	= ТриЗвезды - Шаг;
		ПятьЗвезд 		= ЧетыреЗвезды - Шаг;

		Если Итер <= ПятьЗвезд Тогда
			СтруктураТоваров.Вставить("rating", 5);
		ИначеЕсли Итер >= ПятьЗвезд И Итер <= ЧетыреЗвезды Тогда
			СтруктураТоваров.Вставить("rating", 4);
		ИначеЕсли Итер >= ЧетыреЗвезды И Итер <= ТриЗвезды Тогда
			СтруктураТоваров.Вставить("rating", 3);
		ИначеЕсли Итер >= ТриЗвезды И Итер <= ДвеЗвезды Тогда
			СтруктураТоваров.Вставить("rating", 2);
		ИначеЕсли Итер >= ДвеЗвезды И Итер <= ОднаЗвезда Тогда
			СтруктураТоваров.Вставить("rating", 1);
		КонецЕсли;

		Если Стр.красный Тогда
			СтруктураТоваров.Вставить("color", "Красный");
		ИначеЕсли Стр.Желтый Тогда
			СтруктураТоваров.Вставить("color", "Желтый");
		Иначе
			СтруктураТоваров.Вставить("color", "Зелёный");
		КонецЕсли;
		МассивТоваров.Добавить(СтруктураТоваров);
...
Функция ПроверитьНеЛиквид(Товар)
	Запрос2 = Новый Запрос;
	Запрос2.Текст = 
	"ВЫБРАТЬ
	|
	|	НоменклатураПредложения.НомерСтроки КАК НомерСтроки
	|ИЗ
	|	Справочник.Номенклатура.Предложения КАК НоменклатураПредложения
	|ГДЕ
	|	НоменклатураПредложения.Ссылка = &Ссылка";

	Запрос2.УстановитьПараметр("Ссылка", Товар);
...
КонецФункции
Функция ПолучитьВремяЗН(Исполнитель, ЗаказНаряд)
	Запрос = Новый запрос;
	Запрос.Текст = "ВЫБРАТЬ
				   |	СУММА(ВремяВыполненияЗН.Срок) КАК Срок,
				   |	ВремяВыполненияЗН.Ответственный КАК Ответственный
				   |ИЗ
				   |	РегистрСведений.ВремяВыполненияЗН КАК ВремяВыполненияЗН
				   |ГДЕ
				   |	ВремяВыполненияЗН.Ответственный = &Ответственный
				   |	И ВремяВыполненияЗН.ЗаказНаряд = &ЗаказНаряд
				   |
				   |СГРУППИРОВАТЬ ПО
	...
	Запрос = Новый запрос;
	Запрос.Текст = "ВЫБРАТЬ
				   |	ВремяВыполненияЗН.ЗаказНаряд КАК Задача,
	...
	СтруктураОтвета.Вставить("state", Статус);
	Возврат СтруктураОтвета;

КонецФункции

Процедура ЗакрытьВремяЗН(Ответственный)
	Запрос = Новый запрос;
	Запрос.Текст = "ВЫБРАТЬ
				   |	ВремяВыполненияЗН.ЗаказНаряд КАК Задача,
				   |	ВремяВыполненияЗН.Период КАК ДатаСреза
				   |ИЗ
				   |	РегистрСведений.ВремяВыполненияЗН КАК ВремяВыполненияЗН
				   |ГДЕ
				   |	 ВремяВыполненияЗН.КонецЗамера < ДАТАВРЕМЯ(2000, 1, 1)
				   |И ВремяВыполненияЗН.Ответственный = &Ответственный
				   |УПОРЯДОЧИТЬ ПО
				   |	ВремяВыполненияЗН.Период УБЫВ";
	Запрос.УстановитьПараметр("Ответственный", Ответственный);
	Выборка = Запрос.Выполнить().Выбрать();
	Если выборка.Количество() > 0 Тогда
		Выборка.Следующий();
		НаборЗаписей = РегистрыСведений.ВремяВыполненияЗН.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ЗаказНаряд.Установить(Выборка.Задача);
...
Процедура ОткрытьВремяЗН(ЗН, Ответственный)

	НаборЗаписей = РегистрыСведений.ВремяВыполненияЗН.СоздатьМенеджерЗаписи();

	НаборЗаписей.ЗаказНаряд = ЗН;
	НаборЗаписей.Период = ТекущаяДата();
	НаборЗаписей.НачалоЗамера = ТекущаяДата();
	НаборЗаписей.Ответственный = Ответственный;
	НаборЗаписей.Записать();
...
Функция ПолучитьСписокНарядовСоВременемgetlistorderstime(Запрос)
	ЗапросНарядов = Новый Запрос;

	Текст = "ВЫБРАТЬ
			|	Сотрудники.Код КАК Код,
			|	ПроектыРазработки.Код КАК КодПроекта,
			|	ПроектыРазработки.Ссылка КАК Ссылка,
			|	ПроектыРазработки.Клиент КАК Клиент,
			|	ПроектыРазработки.Ответственный КАК Ответственный,
			|	ПроектыРазработки.Состояние КАК Состояние,
			|	ПроектыРазработки.Ответственные.(
			|		Ссылка,
			|		НомерСтроки,
			|		Сотрудник,
			|		Роль),
			|	ПроектыРазработки.ДатаСдачи КАК ДатаСдачи,
			|	ПроектыРазработки.Наименование
			|ИЗ
			|	Справочник.ПроектыРазработки КАК ПроектыРазработки
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
			|		ПО ПроектыРазработки.Ответственный = Сотрудники.Ссылка
			|Где
			|	ПроектыРазработки.Состояние = Значение(перечисление.СостоянияЗаказНаряда.ВРаботе)";
	ЗапросНарядов.Текст =  Текст;
	...
	Пока выборка.Следующий() Цикл
		СтруктураИнфо = Новый Структура;
		СтруктураИнфо.Вставить("num", Строка(выборка.КодПроекта));
		...
		//СтруктураИнфо.Вставить("comment", Строка(выборка.Комментарий));
		СтруктураИнфо.Вставить("project", Строка(выборка.Наименование));
		СтруктураИнфо.Вставить("date_end", Строка(выборка.ДатаСдачи));
		Ответственные = Выборка.Ответственные.Выгрузить();
		МассивОтветсвенных = Новый Массив;
		ОбщВремя = 0;
		Статус = 0;
		Для Каждого Стр Из Ответственные Цикл
			СтруктураОтветственных = Новый Структура;
			СтруктураОтветственных.Вставить("name", Строка(Стр.Сотрудник));
			//@skip-check query-in-loop
			Струк = ПолучитьВремяЗН(Стр.Сотрудник, выборка.ссылка);
			Время = Струк.time;
			Статус = Статус + Струк.state;
			Если Струк.state > 0 Тогда
				СтруктураОтветственных.Вставить("state", Истина);
			Иначе
				СтруктураОтветственных.Вставить("state", Ложь);
			КонецЕсли;
			СтруктураОтветственных.Вставить("time", Время);
			СтруктураОтветственных.Вставить("id_executor", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
				Стр.Сотрудник).Код);
			ОбщВремя = Время + ОбщВремя;
			МассивОтветсвенных.Добавить(СтруктураОтветственных);
		КонецЦикла;
		Если Статус > 0 Тогда
			СтруктураИнфо.Вставить("state", Истина);
		Иначе
			СтруктураИнфо.Вставить("state", Ложь);
		КонецЕсли;
		СтруктураИнфо.Вставить("date_end", Строка(выборка.ДатаСдачи));

		СтруктураИнфо.Вставить("all_time", ОбщВремя);
		СтруктураИнфо.Вставить("workers", МассивОтветсвенных);
		МассивНарядов.Добавить(СтруктураИнфо);
	...
	ЗаписатьJSON(ЗаписьJSON, МассивНарядов);
...
Функция ВзятьВРаботуЗНstartworkinorder(Запрос)
	ЗапросНарядов = Новый Запрос;
	...
	Текст = "ВЫБРАТЬ
			|	Сотрудники.Код КАК Код,
			|	ПроектыРазработки.Код КАК Номер,
			|	ПроектыРазработки.Ссылка КАК Ссылка,
			|	ПроектыРазработки.Клиент КАК Клиент
			|ИЗ
			|	Справочник.ПроектыРазработки КАК ПроектыРазработки
			...
			|		ПО ПроектыРазработки.Ответственный = Сотрудники.Ссылка
			|Где
			|	ПроектыРазработки.Код = &Номер";
	ЗапросНарядов.Текст =  Текст;
	ЗапросНарядов.УстановитьПараметр("Номер", Массив.id);
	Выборка = запросНарядов.Выполнить().Выбрать();
	Ответственный = Справочники.Сотрудники.НайтиПоКоду(Массив.id_worker).Пользователь;

	выборка.Следующий();
	Попытка
		ЗакрытьВремяЗН(Ответственный);
		ОткрытьВремяЗН(выборка.ссылка, Ответственный);

		Ответ = Новый HTTPСервисОтвет(200);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки("Заказ-наряд взят в работу");
	Исключение
		Ответ = Новый HTTPСервисОтвет(401);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки("Что-то поломалось");
	КонецПопытки;
	Возврат Ответ;

КонецФункции

Функция ДобавитьВНарядОтветственногоaddworkerinorder(Запрос)
	Тело = Запрос.ПолучитьТелоКакстроку();
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(Тело);

	Массив = ПрочитатьJSON(ЧтениеJSON);
	Задача = Справочники.ПроектыРазработки.НайтиПоКоду(Массив.id).ПолучитьОбъект();

	Для Каждого aut Из Массив.author Цикл
		Автор = Справочники.Сотрудники.НайтиПоКоду(aut).Пользователь;
		Отбор = Новый Структура;
		Отбор.Вставить("Сотрудник", Автор);
		НайденноеЗначение = Задача.Ответственные.НайтиСтроки(Отбор);

		Если НайденноеЗначение.Количество() = 0 Тогда
			НовыйАвтор = Задача.Ответственные.Добавить();
			НовыйАвтор.Сотрудник = Автор;
			Задача.Записать();
		Иначе
...
Функция ПолучитьСписокДеталейНаРемонтgetlistproductsrepair(Запрос)
	ЗапросТовара = Новый Запрос;
	...
	Текст = "ВЫБРАТЬ
			...
			|	РегистрНакопления.РегистрНакопления1.Остатки КАК РегистрНакопления1Остатки
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК РегИндНомер
			|		ПО РегистрНакопления1Остатки.индкод = РегИндНомер.индкод
			|ГДЕ
			|	РегистрНакопления1Остатки.КолвоОстаток > 0
			|	И РегИндНомер.Состояние = Значение(Перечисление.Состояние.ТребуетРемонта)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ Первые %1
			...
			|УПОРЯДОЧИТЬ ПО
			|	НомерЗаписи";
	запросТовара.Текст =   СтрШаблон(Текст, Формат( 10000, "ЧГ="));
	запросТовара.УстановитьПараметр("НачинаяСЗаписи", 0);

	Выборкаобщ = запросТовара.Выполнить().Выбрать().Количество();
	запросТовара.Текст =  СтрШаблон(Текст, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
		Запрос.ПараметрыURL["count"], 10000), "ЧГ="));
	Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
		Запрос.ПараметрыURL["page"]) > 1 Тогда
		запросТовара.УстановитьПараметр("НачинаяСЗаписи", Число(((Запрос.ПараметрыURL["count"])
...
КонецФункции
Процедура ОткрытьФорму(ИмяПользователя, ИмяСправочника, ИмяФормы)
    
    // Получаем список активных сеансов
	Сеансы = ПолучитьСоединенияИнформационнойБазы();

    // Перебираем сеансы и ищем нужного пользователя
	Для Каждого Сеанс Из Сеансы Цикл
		Если Сеанс.ИмяПользователя = ИмяПользователя Тогда
            
            // Формируем строку подключения к сеансу пользователя
			СтрокаСоединения = "Srvr=""{ИнформационнаяБаза.Сервер}"";Ref=""{ИнформационнаяБаза.Имя}"";Usr=""{Сеанс.ИмяПользователя}"";Pwd="";SId=""{Сеанс.Идентификатор}"";";
            
            // Создаем объект внешнего соединения
			ВнешнееСоединение = Новый COMОбъект("V83.COMConnector");
            
            // Подключаемся к сеансу пользователя
			Попытка
				ВнешнееСоединение.Connect(СтрокаСоединения);
			Исключение
				Сообщить("Не удалось подключиться к сеансу пользователя " + ИмяПользователя);
				Возврат;
			КонецПопытки;
            
            // Открываем форму справочника
			Попытка
				ВнешнееСоединение.УправляемоеПриложение.ОткрытьФорму(ИмяСправочника, , , , , , ИмяФормы);
			Исключение
				Сообщить("Не удалось открыть форму справочника.");
			КонецПопытки;
            
            // Отключаемся от сеанса пользователя
			ВнешнееСоединение.Disconnect();
	...
	Сообщить("Пользователь с именем " + ИмяПользователя + " не найден.");

КонецПроцедуры
Функция ПолучитьСписокВсехСотрудниковИГородовGetAllWorkersCity(Запрос)
	...
	ЗапросСотрудников.Текст = "ВЫБРАТЬ
							  ...
							  |	Сотрудники.Пользователь.Подразделение КАК Подразделение,
							  |	ВЫБОР
							  |		КОГДА Сотрудники.Пользователь.Подразделение.Код = ""000000030""
							  |			ТОГДА 2
							  |		КОГДА Сотрудники.Пользователь.Подразделение.Код = ""000000060""
							  |			ТОГДА 3
							  |		КОГДА Сотрудники.Пользователь.Подразделение.Код = ""000000029""
							  |			ТОГДА 4
							  |		ИНАЧЕ 0
							  |	КОНЕЦ КАК НомерГорода
							  |ИЗ
							  |	Справочник.Сотрудники КАК Сотрудники
							  |ГДЕ
							  |	НЕ Сотрудники.Пользователь.Недействителен";
	...
	Пока ВыборкаСотрудников.Следующий() Цикл
		...
		СтруктураСотрдников.Вставить("name", Строка(ВыборкаСотрудников.ФИО));
		СтруктураСотрдников.Вставить("code", Строка(ВыборкаСотрудников.Код));
		СтруктураСотрдников.Вставить("city", Строка(ВыборкаСотрудников.Подразделение));
		СтруктураСотрдников.Вставить("number_city", Строка(ВыборкаСотрудников.НомерГорода));
...
Функция СтатистикаКонвейерИФотоgetstatisticspipelinephoto(Запрос)
	...
	ДатаНачала		= НачалоДня(Дата(Строка(Запрос.ПараметрыURL["datestart"])));
	ДатаОкончания 	= КонецДня(Дата(Строка(Запрос.ПараметрыURL["dateend"])));

	МассивСотрудников = Новый Массив;

	СоставСотрудников = Число(Запрос.ПараметрыURL["people"]);

	Если СоставСотрудников = 0 Тогда //Все

		ТекстЗамены = "";

	ИначеЕсли СоставСотрудников = 1 Тогда //Учетчики
		...
		|ГДЕ
		|	(ДолжностиСотрудников.Должность.Наименование ПОДОБНО ""%учетчик%""
		|	ИЛИ ДолжностиСотрудников.Должность.Наименование ПОДОБНО ""%учётчик%""
		|	ИЛИ ДолжностиСотрудников.Должность.Наименование ПОДОБНО ""%Фото%"")
		|	И ВТ_ДатаПоследнейЗаписи.Пользователь.Подразделение В (&Подразделение)
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ДатаПоследнейЗаписи.Сотрудник,
		|	ДолжностиСотрудников.Должность,
		|	ВТ_ДатаПоследнейЗаписи.Пользователь";

		ЗапросСотрудники.УстановитьПараметр("Подразделение", МассивРегионов);

		РезультатЗапроса = ЗапросСотрудники.Выполнить().Выбрать();

		Если РезультатЗапроса.Количество() > 0 Тогда

			Пока РезультатЗапроса.Следующий() Цикл
				МассивСотрудников.Добавить(РезультатЗапроса.Пользователь);
			КонецЦикла;
		КонецЕсли;

		ТекстЗамены = "И ИндНомер.Ответственный В (&Ответственный)";
	...
	ЗапросОтчёта.Текст = "ВЫБРАТЬ
						 ...
						 |	&Фильтр
						 |	И ИндНомер.Ответственный.Подразделение В (&Подразделение)";
	ЗапросОтчёта.Текст = СтрЗаменить(ЗапросОтчёта.Текст, "&Фильтр", ТекстЗамены);

	Если СоставСотрудников = 1 Тогда //Учетчики
		ЗапросОтчёта.УстановитьПараметр("Ответственный", МассивСотрудников);
	КонецЕсли;
	...
	МассивДеталиПоСотруднику = Новый Массив;
	СтруктураОтветаДетали = Новый Структура;

	Попытка
		СтруктураОтветаДетали.Вставить("count", Число(выборка.Количество));
		СтруктураОтветаДетали.Вставить("money", Число(выборка.Сумма));
	Исключение
		СтруктураОтветаДетали.Вставить("count", 0);
		СтруктураОтветаДетали.Вставить("money", 0);
	КонецПопытки;
	...
	ЗапросОтчётаПоСотруднику.Текст = СтрЗаменить(ЗапросОтчётаПоСотруднику.Текст, "&Фильтр", ТекстЗамены);

	Если СоставСотрудников = 1 Тогда //Учетчики
		ЗапросОтчётаПоСотруднику.УстановитьПараметр("Ответственный", МассивСотрудников);
	КонецЕсли;

	ЗапросОтчётаПоСотруднику.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросОтчётаПоСотруднику.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	...
	Пока ВыборкаПоСотрудникам.Следующий() Цикл
		Попытка
			СтруктураПоСотруднику = Новый Структура;
			СтруктураПоСотруднику.Вставить("count", Число(ВыборкаПоСотрудникам.Количество));
			СтруктураПоСотруднику.Вставить("money", Число(ВыборкаПоСотрудникам.Сумма));
			СтруктураПоСотруднику.Вставить("person", Строка(ВыборкаПоСотрудникам.Ответственный));
			МассивДеталиПоСотруднику.Добавить(СтруктураПоСотруднику);
		Исключение
		...
		КонецПопытки;
	КонецЦикла;
	СтруктураОтветаДетали.Вставить("persons", МассивДеталиПоСотруднику);

	СтруктураОтвета.Вставить("detals", СтруктураОтветаДетали);

	ЗапросОтчётаМесяц = Новый запрос;
	ЗапросОтчётаМесяц.Текст = "ВЫБРАТЬ
							  |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИндНомер.индкод) КАК Количество,
							  |	СУММА(ИндНомер.Цена) КАК Сумма
							  |ИЗ
							  |	РегистрСведений.ИндНомер КАК ИндНомер
	...
	ЗапросОтчётаМесяц.УстановитьПараметр("ДатаНачала", НачалоМесяца(ТекущаяДатаСеанса()));
	ЗапросОтчётаМесяц.УстановитьПараметр("ДатаОкончания", КонецМесяца(ТекущаяДатаСеанса()));

	ЗапросОтчётаМесяц.УстановитьПараметр("Подразделение", МассивРегионов);

	ВыборкаМесяц = ЗапросОтчётаМесяц.Выполнить().Выбрать();

	ВыборкаМесяц.Следующий();

	Попытка
		СтруктураОтветаДетали.Вставить("count_mounth", Число(выборкаМесяц.Количество));
		СтруктураОтветаДетали.Вставить("money_mounth", Число(выборкаМесяц.Сумма));
	Исключение
		СтруктураОтветаДетали.Вставить("count_mounth", 0);
		СтруктураОтветаДетали.Вставить("money_mounth", 0);
	КонецПопытки;	
	
	//Статистика фотографии
	ИндкодаСФото = ПолучитьИндкодаСФото(Строка(Запрос.ПараметрыURL["datestart"]), Строка(
		Запрос.ПараметрыURL["dateend"]));

	МассивФотоПоСотруднику = Новый Массив;

	ОбщееКоличество = 0;
	ОбщаяСумма = 0;

	Для Каждого ИндкодаПоСотруднику Из ИндкодаСФото Цикл
		Сотрудник = Справочники.Сотрудники.НайтиПоКоду(ИндкодаПоСотруднику.tabnum);
		МассивИндкодов = ИндкодаПоСотруднику.ind_code;
		...
		ЗапросОтчёта.Текст = "ВЫБРАТЬ
							 ...
							 |	ИндНомер.АвитоЧастник = ИСТИНА
							 |	И ИндНомер.Ответственный.Подразделение В (&Подразделение)
							 |	И ИндНомер.индкод.Наименование В (&МассивИндкодов)";

		ЗапросОтчёта.УстановитьПараметр("Подразделение", МассивРегионов);
		ЗапросОтчёта.УстановитьПараметр("МассивИндкодов", МассивИндкодов);

		Выборка = ЗапросОтчёта.Выполнить().Выбрать();

		Выборка.Следующий();
		СтруктураФотоПоСотруднику = Новый Структура;

		Попытка
			СтруктураФотоПоСотруднику.Вставить("tabnum", Строка(ИндкодаПоСотруднику.tabnum));
			СтруктураФотоПоСотруднику.Вставить("name", Строка(Сотрудник.Наименование));
			СтруктураФотоПоСотруднику.Вставить("count", Число(Выборка.Количество));
			СтруктураФотоПоСотруднику.Вставить("money", Число(Выборка.Сумма));

			МассивФотоПоСотруднику.Добавить(СтруктураФотоПоСотруднику);

			ОбщееКоличество = ОбщееКоличество + Выборка.Количество;
			ОбщаяСумма = ОбщаяСумма + Выборка.Сумма;
		Исключение
//			СтруктураФотоПоСотруднику.Вставить("count", 0);
//			СтруктураФотоПоСотруднику.Вставить("money", 0);
		КонецПопытки;
	КонецЦикла;

	СтруктураОтветаФотографии = Новый Структура;
	СтруктураОтветаФотографии.Вставить("count", ОбщееКоличество);
	СтруктураОтветаФотографии.Вставить("money", ОбщаяСумма);
	СтруктураОтветаФотографии.Вставить("persons", МассивФотоПоСотруднику);

	//Получение данных за текущий месяц
	НачалоТекущегоМесяца = НачалоМесяца(ТекущаяДатаСеанса());
	НачалоТекущегоМесяца = Строка(Формат(НачалоТекущегоМесяца, "ДФ=yyyy-MM-dd;"));
	НачалоТекущегоМесяца = СтрЗаменить(НачалоТекущегоМесяца, "-", "");

	КонецТекущегоМесяца = КонецМесяца(ТекущаяДатаСеанса());
	КонецТекущегоМесяца = Строка(Формат(КонецТекущегоМесяца, "ДФ=yyyy-MM-dd;"));
	КонецТекущегоМесяца = СтрЗаменить(КонецТекущегоМесяца, "-", "");

	ИндкодаСФотоМесяц = ПолучитьИндкодаСФото(НачалоТекущегоМесяца, КонецТекущегоМесяца);

	ОбщееКоличествоМесяц = 0;
	ОбщаяСуммаМесяц = 0;

	Для Каждого ИндкодаПоСотрудникуМесяц Из ИндкодаСФотоМесяц Цикл
		МассивИндкодовМесяц = ИндкодаПоСотрудникуМесяц.ind_code;
		...
		ЗапросОтчётаМесяц.УстановитьПараметр("Подразделение", МассивРегионов);
		ЗапросОтчётаМесяц.УстановитьПараметр("МассивИндкодов", МассивИндкодовМесяц);
		...
		Попытка
			ОбщееКоличествоМесяц = ОбщееКоличествоМесяц + ВыборкаМесяц.Количество;
			ОбщаяСуммаМесяц = ОбщаяСуммаМесяц + ВыборкаМесяц.Сумма;
		Исключение

		КонецПопытки;
	КонецЦикла;

	СтруктураОтветаФотографии.Вставить("count_mounth", ОбщееКоличествоМесяц);
	СтруктураОтветаФотографии.Вставить("money_month", ОбщаяСуммаМесяц);

	СтруктураОтвета.Вставить("photo", СтруктураОтветаФотографии);
...
КонецФункции
Функция ПолучитьИндкодаСФото(ДатаНачала, ДатаОкончания)
	...
	Соединение = Новый HTTPСоединение("192.168.0.245", 8085);
	Запрос = Новый HTTPЗапрос("/v1/photographer/indcode?date_start=" + ДатаНачала + "&date_end=" + ДатаОкончания,
		Заголовки);
	//В запросе можно обратиться к нужному ресурсу и с нужными параметрами 
	Ответ = Соединение.Получить(Запрос);
...
Функция ГрафикКонвейерИФотоgetgraphpipelinephoto(Запрос)
	...
	ДатаНачала		= НачалоМесяца(Дата(Строка(Запрос.ПараметрыURL["year"]) + Строка(Запрос.ПараметрыURL["month"])
		+ "01"));
	ДатаОкончания 	= КонецМесяца(Дата(Строка(Запрос.ПараметрыURL["year"]) + Строка(Запрос.ПараметрыURL["month"])
		+ "01"));
	...
	Пока ВыборкаПоСотрудникам.Следующий() Цикл
		Попытка
			...
			СтруктураПоСотруднику.Вставить("money", Число(ВыборкаПоСотрудникам.Сумма));
			СтруктураПоСотруднику.Вставить("person", Строка(ВыборкаПоСотрудникам.Ответственный));
			СтруктураПоСотруднику.Вставить("date", Строка(Формат(ВыборкаПоСотрудникам.ДатаИзмененияКонвеера,
				"ДФ=dd.MM.yyyy;")));
			МассивДеталиПоСотруднику.Добавить(СтруктураПоСотруднику);
		Исключение
	...
	//Статистика фотографии
	ИндкодаСФото = ПолучитьИндкодаСФото(Строка(Формат(ДатаНачала, "ДФ=yyyyMMdd;")), Строка(Формат(ДатаНачала,
		"ДФ=yyyyMMdd;")));
	...
	Для Каждого ИндкодаПоСотруднику Из ИндкодаСФото Цикл
		...
		ЗапросОтчёта.Текст = "ВЫБРАТЬ
							 |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИндНомер.индкод) КАК Количество,
							 |	СУММА(ИндНомер.Цена) КАК Сумма,
							 |	ИндНомер.ДатаИзмененияКонвеера КАК ДатаИзмененияКонвейера
							 |ИЗ
							 |	РегистрСведений.ИндНомер КАК ИндНомер
							 |ГДЕ
							 |	ИндНомер.АвитоЧастник = ИСТИНА
		...
		Попытка
			...
			СтруктураФотоПоСотруднику.Вставить("count", Число(Выборка.Количество));
			СтруктураФотоПоСотруднику.Вставить("money", Число(Выборка.Сумма));
			СтруктураФотоПоСотруднику.Вставить("date", Строка(Выборка.ДатаИзмененияКонвейера));
...
Функция ПолучитьПоследнююПродажу(Товар)
	...
	|	И ПродажаЗапчастейТаблица.СтатусТовара
	|СГРУППИРОВАТЬ ПО
	|	ПродажаЗапчастейТаблица.Ссылка.Дата
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПродажаЗапчастейТаблица.Ссылка.Дата УБЫВ";
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		РезультатЗапроса.Следующий();
		Результат = Формат(РезультатЗапроса.Дата, "ДФ=dd.MM.yyyy;");
...
Функция ПолучитьПроектыЗадачДашбордGetProjectsTasksDepartmentDash(Запрос)
	...
	ВыборкаНаправлений = ЗапросНаправлений.Выполнить().Выбрать();
	МассивНаправлений = Новый Массив;
	Пока ВыборкаНаправлений.Следующий() Цикл
		СтруктураНаправления = Новый Структура;
		СтруктураНаправления.Вставить("name", ВыборкаНаправлений.Наименование);
		СтруктураНаправления.Вставить("id", ВыборкаНаправлений.Код);
		Текст = "ВЫБРАТЬ ПЕРВЫЕ 10000
				|	докЗадача.Номер КАК Номер,
				|	докЗадача.Дата КАК Дата,
				|	докЗадача.ДатаОкончанияФакт КАК ДатаВыполнения,
				...
				|ГДЕ
				|	
				|	 докЗадача.ЗадачаСПортала = ИСТИНА
				|	И докЗадача.родительскаяЗадача = Значение(Документ.Задача.ПустаяСсылка)
				|   И докЗадача.Направление.Родитель = &Направление
				|УПОРЯДОЧИТЬ ПО
				|	ДатаВыполнения УБЫВ,
				|	Приоритет,
				|	Дата УБЫВ
	...
	//Направление = Справочники.Подразделения.НайтиПоКоду(Запрос.ПараметрыURL["direction"]);
		...
		ЗапросЗадач.Текст =  СтрШаблон(Текст, Формат(100000, "ЧГ="));
		ЗапросЗадач.УстановитьПараметр("НачинаяСЗаписи", 0);
		ЗапросЗадач.УстановитьПараметр("Направление", ВыборкаНаправлений.ссылка);
	//ЗапросЗадач.УстановитьПараметр("Статус", Статус);
		ТекстПоиска = Строка(Запрос.ПараметрыURL["text"]);
//	Если ТекстПоиска <> "0" Тогда
//		ЗапросЗадач.УстановитьПараметр("Описание", "%" + ТекстПоиска + "%");
...
//	КонецЕсли;
		...
		Пока выборка.Следующий() Цикл
			...
			СтруктураЗадачи.Вставить("name", Строка(выборка.Наименование));
			СтруктураЗадачи.Вставить("date", Строка(выборка.Дата));
			СтруктураЗадачи.Вставить("date_end", Строка(?(выборка.ДатаВыполнения = '00010101', "",
				выборка.ДатаВыполнения)));
		//СтруктураЗадачи.Вставить("description", Строка(выборка.Описание));
			СтруктураЗадачи.Вставить("number", Строка(выборка.Номер));
			СтруктураЗадачи.Вставить("executor", Строка(выборка.Исполнитель));
			СтруктураЗадачи.Вставить("id_executor", Строка(Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
		...
		//СтруктураЗадачи.Вставить("id_order", Строка(выборка.ИдентификаторРаботыЗаказНарядаНаВосстановление));
		//@skip-check query-in-loop
			Струк = ПолучитьУчастниковЗадачи(выборка.ссылка);
			Статус = Струк.Статус;

			Если Статус > 0 Тогда
				СтруктураЗадачи.Вставить("statedash", Истина);
			Иначе
				СтруктураЗадачи.Вставить("statedash", Ложь);
			КонецЕсли;
			СтруктураЗадачи.Вставить("all_time", Струк.ОбщВремя);
			СтруктураЗадачи.Вставить("workers", Струк.Сотрудники);
		//СтруктураТоваров.Вставить("photos", МассивФото);
			МассивЗадач.Добавить(СтруктураЗадачи);
		КонецЦикла;
		СтруктураНаправления.Вставить("projects", МассивЗадач);
		МассивНаправлений.Добавить(СтруктураНаправления);
	КонецЦикла;
	...
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("info", СтруктураИнфо);
	СтруктураОтвета.Вставить("data", МассивНаправлений);
//	СтруктураОтвета.Вставить("participants", МассивУчастников);
//	СтруктураОтвета.Вставить("number_of_persons", Участники);
...
Функция ПолучитьУчастниковЗадачи(Задача)
	ЗапросУчастника = Новый Запрос;
	ЗапросУчастника.Текст = "ВЫБРАТЬ
							|	ВремяВыполненияЗадач.Ответственный,
							|	СУММА(ВремяВыполненияЗадач.Срок) КАК Срок,
							|	Сотрудники.Код
							|ИЗ
							|	РегистрСведений.ВремяВыполненияЗадач КАК ВремяВыполненияЗадач
							|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
							|		ПО ВремяВыполненияЗадач.Ответственный = Сотрудники.Пользователь
							|ГДЕ
							|	ВремяВыполненияЗадач.Задача = &Задача
							|	ИЛИ ВремяВыполненияЗадач.Задача.РодительскаяЗадача = &Задача
							|	ИЛИ ВремяВыполненияЗадач.Задача.РодительскаяЗадача.РодительскаяЗадача = &Задача
							|	ИЛИ ВремяВыполненияЗадач.Задача.РодительскаяЗадача.РодительскаяЗадача.РодительскаяЗадача = &Задача
							|СГРУППИРОВАТЬ ПО
							|	ВремяВыполненияЗадач.Ответственный,
							|	Сотрудники.Код";
	ЗапросУчастника.УстановитьПараметр("Задача", Задача);

	ВыборкаУчастника = ЗапросУчастника.Выполнить().Выбрать();
	МассивИсполнителей = Новый Массив;
	СтатусОбщ = 0;
	ВремяОбщ = 0;
	Пока ВыборкаУчастника.Следующий() Цикл
		СтруктураИсполнителя = Новый Структура;
		СтруктураИсполнителя.Вставить("id", ВыборкаУчастника.Код);
		СтруктураИсполнителя.Вставить("executor", Строка(ВыборкаУчастника.Ответственный));
		СтруктураИсполнителя.Вставить("id_executor", Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",
			ВыборкаУчастника.Ответственный).Код);
		//СтруктураИсполнителя.Вставить("time_in_work", Выборка.Срок);
		Время = ВыборкаУчастника.Срок;
		Запрос = Новый запрос;
		Запрос.Текст = "ВЫБРАТЬ
					   |	ВремяВыполненияЗН.Задача КАК Задача,
					   |	ВремяВыполненияЗН.НачалоЗамера КАК НачалоЗамера
					   |ИЗ
					   |	РегистрСведений.ВремяВыполненияЗадач КАК ВремяВыполненияЗН
					   |ГДЕ
					   |	ВремяВыполненияЗН.КонецЗамера < ДАТАВРЕМЯ(2000, 1, 1)
					   |	И ВремяВыполненияЗН.Ответственный = &Ответственный
					   |	И (ВремяВыполненияЗН.Задача = &Задача
					   |   ИЛИ ВремяВыполненияЗН.Задача.РодительскаяЗадача = &Задача
					   |	ИЛИ ВремяВыполненияЗН.Задача.РодительскаяЗадача.РодительскаяЗадача = &Задача
					   |	ИЛИ ВремяВыполненияЗН.Задача.РодительскаяЗадача.РодительскаяЗадача.РодительскаяЗадача = &Задача)";
		Запрос.УстановитьПараметр("Ответственный", ВыборкаУчастника.Ответственный);
		Запрос.УстановитьПараметр("Задача", Задача);
		//@skip-check query-in-loop
		Выборка = Запрос.Выполнить().Выбрать();
		Статус = Ложь;
		Если Выборка.Количество() > 0 Тогда
			Выборка.Следующий();
			СтатусОбщ = СтатусОбщ + 1;
			Время = Время + (ТекущаяДата() - Выборка.НачалоЗамера);
			Статус = Истина;
		КонецЕсли;
		ВремяОбщ = ВремяОбщ + Время;
		СтруктураИсполнителя.Вставить("state", Статус);
		СтруктураИсполнителя.Вставить("time_in_work", Время);
		МассивИсполнителей.Добавить(СтруктураИсполнителя);

	КонецЦикла;
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("ОбщВремя", ВремяОбщ);
	СтруктураОтвета.Вставить("Сотрудники", МассивИсполнителей);
	СтруктураОтвета.Вставить("Статус", СтатусОбщ);
	Возврат СтруктураОтвета;
КонецФункции

Функция ДопИнфаЗвонкиМенеджерыadddatamanagercalls(Запрос)
	
	///+ГомзМА 13.06.2024
	МассивОтвета = Новый Массив;

	ЗапросОжидается = Новый Запрос;
	ЗапросОжидается.Текст =
	"ВЫБРАТЬ
	|	ЗаказКлиента.Номер КАК Номер,
	|	ЗаказКлиента.Клиент КАК Клиент,
	...
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
	|	ЗаказКлиента.WTPanel.Код = ""000000014""";

	РезультатЗапросаОжидается = ЗапросОжидается.Выполнить().Выбрать();

	МассивНомеровОжидается = Новый Массив;
	Если РезультатЗапросаОжидается.Количество() > 0 Тогда
		Пока РезультатЗапросаОжидается.Следующий() Цикл
			СтруктураОжидаетсяВн = Новый Структура;
			СтруктураОжидаетсяВн.Вставить("number", Строка(РезультатЗапросаОжидается.Номер));
			СтруктураОжидаетсяВн.Вставить("client", Строка(РезультатЗапросаОжидается.Клиент));
			СтруктураОжидаетсяВн.Вставить("manager", Строка(РезультатЗапросаОжидается.Менеджер));

			МассивНомеровОжидается.Добавить(СтруктураОжидаетсяВн);
		КонецЦикла;
	КонецЕсли;

	СтруктураОжидается = Новый Структура;
	СтруктураОжидается.Вставить("title", "Ожидается");
	СтруктураОжидается.Вставить("list", МассивНомеровОжидается);

	МассивОтвета.Добавить(СтруктураОжидается);

	ЗапросСобирается = Новый Запрос;
	ЗапросСобирается.Текст =
	"ВЫБРАТЬ
	|	ЗаказКлиента.Номер КАК Номер,
	|	ЗаказКлиента.Клиент КАК Клиент,
	|	ЗаказКлиента.Ответственный КАК Менеджер
	...
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
	|	ЗаказКлиента.WTPanel.Код = ""000000015""
	|	И ЗаказКлиента.ПодстатусОбработки = ЗНАЧЕНИЕ(Перечисление.ПодстатусыОбработкиЗаявок.Выполняется)";

	РезультатЗапросаСобирается = ЗапросСобирается.Выполнить().Выбрать();

	МассивНомеровСобирается = Новый Массив;
	Если РезультатЗапросаСобирается.Количество() > 0 Тогда
		Пока РезультатЗапросаСобирается.Следующий() Цикл
			СтруктураСобираетсяВн = Новый Структура;
			СтруктураСобираетсяВн.Вставить("number", Строка(РезультатЗапросаСобирается.Номер));
			СтруктураСобираетсяВн.Вставить("client", Строка(РезультатЗапросаСобирается.Клиент));
			СтруктураСобираетсяВн.Вставить("manager", Строка(РезультатЗапросаСобирается.Менеджер));

			МассивНомеровСобирается.Добавить(СтруктураСобираетсяВн);
		КонецЦикла;
	КонецЕсли;

	СтруктураСобирается = Новый Структура;
	СтруктураСобирается.Вставить("title", "Собирается");
	СтруктураСобирается.Вставить("list", МассивНомеровСобирается);

	МассивОтвета.Добавить(СтруктураСобирается);
	...
	|ГДЕ
	|	ЗаказКлиента.WTPanel.Код = ""000000015""
	|	И ЗаказКлиента.ПодстатусОбработки = ЗНАЧЕНИЕ(Перечисление.ПодстатусыОбработкиЗаявок.Готов)";

	РезультатЗапросаСобрано = ЗапросСобрано.Выполнить().Выбрать();

	МассивНомеровСобрано = Новый Массив;
	Если РезультатЗапросаСобрано.Количество() > 0 Тогда
		Пока РезультатЗапросаСобрано.Следующий() Цикл
			СтруктураСобраноВн = Новый Структура;
			СтруктураСобраноВн.Вставить("number", Строка(РезультатЗапросаСобрано.Номер));
			СтруктураСобраноВн.Вставить("client", Строка(РезультатЗапросаСобрано.Клиент));
			СтруктураСобраноВн.Вставить("manager", Строка(РезультатЗапросаСобрано.Менеджер));

			МассивНомеровСобрано.Добавить(СтруктураСобраноВн);
		КонецЦикла;
	КонецЕсли;

	СтруктураСобрано = Новый Структура;
	СтруктураСобрано.Вставить("title", "Собрано");
	СтруктураСобрано.Вставить("list", МассивНомеровСобрано);

	МассивОтвета.Добавить(СтруктураСобрано);

	ЗапросУпаковано = Новый Запрос;
	ЗапросУпаковано.Текст =
	"ВЫБРАТЬ
	|	ЗаказКлиента.Номер КАК Номер,
	|	ЗаказКлиента.Клиент КАК Клиент,
	|	ЗаказКлиента.Ответственный КАК Менеджер
	...
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
	|	ЗаказКлиента.WTPanel.Код = ""000000016""
	|	И ЗаказКлиента.ПодстатусОбработки = ЗНАЧЕНИЕ(Перечисление.ПодстатусыОбработкиЗаявок.Готов)";

	РезультатЗапросаУпаковано = ЗапросУпаковано.Выполнить().Выбрать();

	МассивНомеровУпаковано = Новый Массив;
	Если РезультатЗапросаУпаковано.Количество() > 0 Тогда
		Пока РезультатЗапросаУпаковано.Следующий() Цикл
			СтруктураУпакованоВн = Новый Структура;
			СтруктураУпакованоВн.Вставить("number", Строка(РезультатЗапросаУпаковано.Номер));
			СтруктураУпакованоВн.Вставить("client", Строка(РезультатЗапросаУпаковано.Клиент));
			СтруктураУпакованоВн.Вставить("manager", Строка(РезультатЗапросаУпаковано.Менеджер));

			МассивНомеровУпаковано.Добавить(СтруктураУпакованоВн);
		КонецЦикла;
	КонецЕсли;

	СтруктураУпаковано = Новый Структура;
	СтруктураУпаковано.Вставить("title", "Упаковано");
	СтруктураУпаковано.Вставить("list", МассивНомеровУпаковано);

	МассивОтвета.Добавить(СтруктураУпаковано);
...
КонецФункции
Функция ПолучитьТаблоМенеджеровgetmanagerstableau(Запрос)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ПриходДенегНаСчет.СуммаДокумента, 0) КАК СуммаПоступлений,
	|	ЕСТЬNULL(КлиентыМенеджера.ОбщееКоличествоКлиентов, 0) КАК КоличествоКлиентов,
	...
	|	ЕСТЬNULL(ПриходДенегНаСчет.СуммаДокумента, 0),
	|	ЕСТЬNULL(КлиентыМенеджера.ОбщееКоличествоКлиентов, 0)";

	ЗапросИнфо.УстановитьПараметр("ДатаНачала", ДатаНачала);
	ЗапросИнфо.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	ЗапросИнфо.УстановитьПараметр("ВидРасхода", Справочники.ВидыРасходов.НайтиПоКоду("000000041")); //Возврат Денежных средств
	ЗапросИнфо.УстановитьПараметр("ТекДата", ТекущаяДатаСеанса());

	РезультатЗапроса = ЗапросИнфо.Выполнить().Выбрать();

	Если РезультатЗапроса.Количество() > 0 Тогда
		Пока РезультатЗапроса.Следующий() Цикл
			СтруктураМенеджеров = Новый Структура;
			СтруктураМенеджеров.Вставить("manager", Строка(РезультатЗапроса.Менеджер));
			СтруктураМенеджеров.Вставить("count_client", Число(РезультатЗапроса.КоличествоКлиентов));
			СтруктураМенеджеров.Вставить("money", Число(РезультатЗапроса.СуммаПоступлений));

			МассивМенеджеров.Добавить(СтруктураМенеджеров);
		КонецЦикла;
	КонецЕсли;

	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("managers", МассивМенеджеров);
...
Функция ПолучитьВсехСотрудниковgetListEmployees(Запрос)
	Ответ = Новый HTTPСервисОтвет(200);
	Возврат Ответ;
КонецФункции
