...
Функция текстДляЗаявки()
	...
	|	ЗаказКлиента.Комментарий КАК Комментарий,
	|	ЗаказКлиента.ПодстатусОбработки КАК ПодстатусОбработки,
	|	ЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком.Ссылка КАК СтатусОбработки,
	|	ЕстьNULL(ЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком.Порядок, 0) КАК СтатусОбработкиПорядок
	|ИЗ
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
...
Функция ПолучитьЗаявкуgetapplication(Запрос)
	...
	СтруктураСтатуса.Вставить("state_build_name", Строка(выборка.СтатусОбработки) );
