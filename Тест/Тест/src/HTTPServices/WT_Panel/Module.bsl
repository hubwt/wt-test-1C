...
Функция СменаРеквизитовТовараEditList(Запрос)
	...
	Ответ = РедактироватьТовар(Массив.id, Массив.coment, число(Массив.cost), Состояние, Массив.name);
	// КонецЦикла;

	Возврат Ответ;
КонецФункции

Функция РедактироватьТовар(индкод, комментарий = Неопределено, цена = Неопределено, Состояние = Неопределено, Наименование = Неопределено)
	Запрос = Новый запрос;
	Запрос.Текст = "ВЫБРАТЬ
				   |	ИндНомер.индкод КАК индкод,
				   |	ИндНомер.Код КАК Код,
