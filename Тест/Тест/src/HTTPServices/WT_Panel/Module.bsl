...
Функция ПолучитьПринятыеgetreceived(Запрос)
	...
	|	ЛогИндНомера.дата МЕЖДУ &Датаначала И &ДатаОкончания
	|	И ЛогИндНомера.Стеллаж.ГруппаМестХранения В(&МассивГруппМестХранения)
	|	И (ЛогИндНомера.ПредыдущийСтеллаж = &Группанач
	...
	ЗапросПерещений.УстановитьПараметр("Группанач",		Справочники.Стеллаж.НайтиПоКоду("000001072"));
	...
	ЗапросКоличествоПоСотруднику.УстановитьПараметр("Группанач",				Справочники.Стеллаж.НайтиПоКоду("000001072"));
...
Функция СменаРеквизитовТовараEditList(Запрос) 
	...
	Ответ = РедактироватьТовар(Массив.id,Массив.coment, число(Массив.cost));
	// КонецЦикла;
...
Функция СоздатьЗадачуcreatework(Запрос)
	...
	//Новаязадача.Проект   				= Справочники.ПроектыРазвития.НайтиПоКоду(Массив.direction);
	Новаязадача.Направление   			= Справочники.Подразделения.НайтиПоКоду(Массив.direction);
	Новаязадача.Описание   				= Массив.text; 
	Новаязадача.ОценкаЗадач   			= оценка;
	Новаязадача.ДатаНачалаПлан  		= ТекущаяДата();
	//Новаязадача.Отдел  					= Справочники.ПроектыРазвития.НайтиПоКоду(Массив.direction).Отдел;
...
функция УчестьДетальНаСервере(Машина,товар,Автор)
	...
	об.Состояние = Перечисления.Состояние.БУ;
	об.АвитоЧастник = Истина;
	об.Записать();
	ЛогированиеИндНомера(НоваяСтрока.Партия,Автор,Строка(Ответственный)+" Учёл товар",об.Стеллаж);
	ЗначениеКлюча = Новый Структура;
...
Функция ПолучитьЗадачуGetTask(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураЗадачи.Вставить("state_author",Строка(выборка.СтатусАвтора));
		СтруктураЗадачи.Вставить("deadline",	Строка(выборка.ПлановыйСрокВыполнения));
		СтруктураЗадачи.Вставить("id_author",	Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",выборка.Ответственный).Код);
		СтруктураЗадачи.Вставить("id_executor",	Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь",выборка.Исполнитель).Код);
		//СтруктураЗадачи.Вставить("id",    	  	   Строка(выборка.Код));
...
Функция ПолучитьСтатистикуПродажgetsalesstatistics(Запрос)
	...
	ЗапросСотрудников = Новый Запрос;
	//ЗапросСотрудников.Текст = СтрЗаменить(текстДляСпискаСотрудников(), "ВТ_ДолжностиНовые.Отдел = &ОтделНов", "Сотрудники.Пользователь.Подразделение В (&Филиалы)");
	ЗапросСотрудников.Текст =  "ВЫБРАТЬ
	|	Сотрудники.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.Сотрудники КАК Сотрудники
	...
	Пока Сотрудники.Следующий() Цикл
		...
		ЗапросМенеджеров.Текст =  "ВЫБРАТЬ ПЕРВЫЕ 1
		                          ...
		                          |	И ДолжностиСотрудниковСрезПоследних.Должность.Наименование = &Должность
		                          |
		                          |УПОРЯДОЧИТЬ ПО
		                          |	ДолжностиСотрудниковСрезПоследних.Период УБЫВ";
		ЗапросМенеджеров.УстановитьПараметр("Сотрудник", Сотрудники.Ссылка);
		ЗапросМенеджеров.УстановитьПараметр("Должность", "Менеджер по продажам");
...
Функция ПолучитьСписокФотографовУчетчиковGetUchetchikPhotographerList(Запрос)
	...
	запросСотрудников.УстановитьПараметр("Должность2", "%Кладовщик - Учетчик%");
...
Функция ПолучитьСтатистикуПоСкладуgetwarehousestatistics(Запрос) 
	...
	ЗапросПерещений.Текст = "ВЫБРАТЬ
	                        ...
	                        |ГДЕ
	                        |	ЛогИндНомера.дата МЕЖДУ &Датаначала И &ДатаОкончания
	                        |	И (ЛогИндНомера.Стеллаж.ГруппаМестХранения = &ГруппаМестХранения
	                        |			ИЛИ ЛогИндНомера.Стеллаж.ГруппаМестХранения = &ГруппаМестХранения2)
	                        |	И (ЛогИндНомера.ПредыдущийСтеллаж.ГруппаМестХранения = &ГруппаМестХранения
	                        |			ИЛИ ЛогИндНомера.ПредыдущийСтеллаж.ГруппаМестХранения = &ГруппаМестХранения2)
							|	И НЕ (ЛогИндНомера.Поддон = &Потеряшки
	                        |			ИЛИ ЛогИндНомера.ПредыдущийПоддон = &Потеряшки)
							|   И (ЛогИндНомера.Стеллаж.Владелец В (&Склад)
							|			ИЛИ ЛогИндНомера.ПредыдущийСтеллаж.Владелец В (&Склад))";
	
	ЗапросПерещений.УстановитьПараметр("ГруппаМестХранения",Справочники.ГруппыМестХранения.НайтиПоКоду("000000004"));
	ЗапросПерещений.УстановитьПараметр("ГруппаМестХранения2",Справочники.ГруппыМестХранения.НайтиПоКоду("000000008"));
	...
	ЗапросПерещений.Текст = "ВЫБРАТЬ
	                        ...
	                        |ГДЕ
	                        |	ЛогИндНомера.дата МЕЖДУ &Датаначала И &ДатаОкончания
	                        |	И (ЛогИндНомера.Стеллаж.ГруппаМестХранения = &Группакон
	                        |			ИЛИ ЛогИндНомера.Стеллаж.ГруппаМестХранения = &Группакон2)
	                        |	И (ЛогИндНомера.ПредыдущийСтеллаж = &Группанач
	                        |			ИЛИ ЛогИндНомера.ПредыдущийСтеллаж.Наименование ПОДОБНО &Группанач2)
							|   И (ЛогИндНомера.Стеллаж.Владелец В (&Склад)
							|			ИЛИ ЛогИндНомера.ПредыдущийСтеллаж.Владелец В (&Склад))";
	
	ЗапросПерещений.УстановитьПараметр("Группакон",Справочники.ГруппыМестХранения.НайтиПоКоду("000000004"));
	ЗапросПерещений.УстановитьПараметр("Группакон2",Справочники.ГруппыМестХранения.НайтиПоКоду("000000008"));
	...
	ЗапросКоличествоПоСотруднику.УстановитьПараметр("Группакон",Справочники.ГруппыМестХранения.НайтиПоКоду("000000004"));
	ЗапросКоличествоПоСотруднику.УстановитьПараметр("Группакон2",Справочники.ГруппыМестХранения.НайтиПоКоду("000000008"));
	...
	ЗапросПерещений.Текст = "ВЫБРАТЬ
	                        ...
	                        |	ЛогИндНомера.дата МЕЖДУ &Датаначала И &ДатаОкончания
	                        |	И ЛогИндНомера.Стеллаж.Наименование ПОДОБНО &Группакон
	                        |	И (ЛогИндНомера.ПредыдущийСтеллаж.ГруппаМестХранения = &Группанач
	                        |			ИЛИ ЛогИндНомера.ПредыдущийСтеллаж.ГруппаМестХранения = &Группанач2)
							|   И (ЛогИндНомера.Стеллаж.Владелец В (&Склад)
							|			ИЛИ ЛогИндНомера.ПредыдущийСтеллаж.Владелец В (&Склад))";
	
	ЗапросПерещений.УстановитьПараметр("Группанач",Справочники.ГруппыМестХранения.НайтиПоКоду("000000004"));
	ЗапросПерещений.УстановитьПараметр("Группанач2",Справочники.ГруппыМестХранения.НайтиПоКоду("000000008"));
	...
	ЗапросТоваровВПродажах.Текст = "ВЫБРАТЬ
	                               ...
	                               |		ПО ПродажаЗапчастейТаблица.Партия = ИндНомер.индкод
	                               |ГДЕ
	                               |	(ИндНомер.Стеллаж.ГруппаМестХранения = &Группанач
	                               |			ИЛИ ИндНомер.Стеллаж.ГруппаМестХранения = &Группанач2)
	                               |	И ПродажаЗапчастейТаблица.ДатаВыдачи МЕЖДУ &ДатаНачала И &ДатаОкончания
	                               |	И ПродажаЗапчастейТаблица.СтатусТовара
								   |   	И ИндНомер.Стеллаж.Владелец В (&Склад)";
	
	ЗапросТоваровВПродажах.УстановитьПараметр("Группанач",Справочники.ГруппыМестХранения.НайтиПоКоду("000000004"));
	ЗапросТоваровВПродажах.УстановитьПараметр("Группанач2",Справочники.ГруппыМестХранения.НайтиПоКоду("000000008"));	
	...
	ЗапросКоличествоПоСотруднику.УстановитьПараметр("Группанач",	Справочники.ГруппыМестХранения.НайтиПоКоду("000000004"));
	ЗапросКоличествоПоСотруднику.УстановитьПараметр("Группанач2",	Справочники.ГруппыМестХранения.НайтиПоКоду("000000008"));
...
Функция ПолучитьФинансовуюСтатистикуgetfinancialstatistics(Запрос)
	...
	|	ВЫБОР
	|		КОГДА &ЭтоНН = 1
	|			ТОГДА СУММА(ВТ_Расходы.СуммаРасход)
	|		ИНАЧЕ 0
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Доход.СуммаПриход КАК СуммаПриход,
	...
	Выручка = ?(РезультатЗапроса.СуммаПриход = Null, 0, Формат(РезультатЗапроса.СуммаПриход, "ЧДЦ=0;"));
	Расход 	= ?(РезультатЗапроса.СуммаРасход = Null, 0, Формат(РезультатЗапроса.СуммаРасход, "ЧДЦ=0;"));
	Прибыль = Выручка - Расход;
	Рентабельность = Формат(?(Выручка = 0, 0, Прибыль / Выручка * 100), "ЧДЦ=0;");
	
	ОтветИнфо = Новый Структура;
	ОтветИнфо.Вставить("revenue", Выручка);
	...
	Пока РезультатЗапросаВыручка.Следующий() Цикл
		Если РезультатЗапросаВыручка.Выручка = "Перекупка" Тогда
			ОтветИнфо.Вставить("revenue_buying", РезультатЗапросаВыручка.СуммаПриход);
		ИначеЕсли РезультатЗапросаВыручка.Выручка = "Продажи" Тогда	
			ОтветИнфо.Вставить("revenue_sale", РезультатЗапросаВыручка.СуммаПриход);
		ИначеЕсли РезультатЗапросаВыручка.Выручка = "Восстановления" Тогда
			ОтветИнфо.Вставить("revenue_recovery", РезультатЗапросаВыручка.СуммаПриход);
		КонецЕсли;	
	КонецЦикла;
		
	ОтветИнфо.Вставить("expenditure", Расход);
	...
	ЗапросРасход.Текст =
		...
		|	ВЫБОР
		|		КОГДА &ЭтоНН = 1
		|			ТОГДА СУММА(Расходы.Сумма) + СУММА(ЛичныйВывод.Сумма)
		|		ИНАЧЕ СУММА(Расходы.Сумма)
		|	КОНЕЦ
		|ИЗ
		|	Документ.Расходы КАК Расходы,
		|	Документ.ЛичныйВывод КАК ЛичныйВывод
	...
	Пока РезультатЗапросаРасход.Следующий() Цикл
		Сумма = ?(РезультатЗапросаРасход.Сумма = Null, 0, Формат(РезультатЗапросаРасход.Сумма, "ЧДЦ=0;"));
		Если РезультатЗапросаРасход.Расход = "ЗП" Тогда
			ОтветИнфо.Вставить("expenditure_salary", Сумма);
		ИначеЕсли РезультатЗапросаРасход.Расход = "Оплата поставщикам" Тогда	
			ОтветИнфо.Вставить("expenditure_suppliers", Сумма);
		ИначеЕсли РезультатЗапросаРасход.Расход = "Текущие расходы" Тогда
			ОтветИнфо.Вставить("expenditure_current_consumption", Сумма);
		ИначеЕсли РезультатЗапросаРасход.Расход = "Капитальные затраты" Тогда	
			ОтветИнфо.Вставить("expenditure_capital_expenditures", Сумма);
		ИначеЕсли РезультатЗапросаРасход.Расход = "Возврат заемных средств" Тогда
			ОтветИнфо.Вставить("expenditure_repayment_borrowed_funds", Сумма);
		ИначеЕсли РезультатЗапросаРасход.Расход = "Прочие" Тогда	
			ОтветИнфо.Вставить("expenditure_others", Сумма);
		КонецЕсли;	
	КонецЦикла;
	
	ОтветИнфо.Вставить("profit", 			Прибыль);
	ОтветИнфо.Вставить("profitability", 	Рентабельность);
...
Функция ПолучитьМашинуgettruck(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		Структурамашины.Вставить("id", 			Строка(выборка.Код));
		Структурамашины.Вставить("name",		Строка(выборка.Наименование));
		Структурамашины.Вставить("code",		Строка(выборка.Код));
		Структурамашины.Вставить("vin_code",	Строка(выборка.ВинКод));
		Структурамашины.Вставить("model",		Строка(выборка.Модель));
		Структурамашины.Вставить("model_motor",	Строка(выборка.МодельДвигателя));
		Структурамашины.Вставить("serial",		Строка(выборка.Серия));
		Структурамашины.Вставить("type_cabin",	Строка(выборка.ТипКабины));
		Структурамашины.Вставить("kpp",			Строка(выборка.КПП));
		Структурамашины.Вставить("reductor",	Строка(выборка.Редуктор));
		Структурамашины.Вставить("year",		Строка(выборка.Год));
		Структурамашины.Вставить("power",		Строка(выборка.Мощность));
		Структурамашины.Вставить("comment",		Строка(выборка.Комментарий));
