...
Функция ПолучитьРоектыЗадачиGetProjectTasks(Запрос)
ЗапросЗадач = новый Запрос;
	Статус = новый массив;
...
Функция СменаРодителяЗадачиChangeParentTask(Запрос)
	...
	Мама = Документы.Задача.НайтиПоНомеру(Массив.id_mama).Пользователь; 
...
Функция ДашбордРВРgetdashrvr(Запрос)
	...
	ЗапросКаширин = новый запрос;
	ЗапросКаширин.Текст = "ВЫБРАТЬ
	|	СУММА(ЗаказНарядТовары.СуммаВсего) КАК СуммаВсего
	|ИЗ
	|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК ИндНомер
	...
	|	И ЗаказНарядТовары.Ссылка.Ответственный = &Ответственный
	|	И ИндНомер.Состояние = ЗНАЧЕНИЕ(Перечисление.Состояние.Новый)";
	
	ЗапросКаширин.УстановитьПараметр("ДатаНачала", 		ДатаНачала);
	ЗапросКаширин.УстановитьПараметр("ДатаОкончания", 	ДатаОкончания);
	ЗапросКаширин.УстановитьПараметр("Ответственный",	Справочники.Сотрудники.НайтиПоКоду("000000040").Пользователь); //Грязев Илья Константинович
	
	ВыборкаКаширин = ЗапросКаширин.Выполнить().Выбрать();
	МассивКаширин = Новый Массив;
	ВыборкаКаширин.Следующий();
	СтруктураКаширин = Новый Структура;
		
	Попытка
		СтруктураКаширин.Вставить("sum_kashirin",	Число(ВыборкаКаширин.СуммаВсего));		
	Исключение
		СтруктураКаширин.Вставить("sum_kashirin",	0);	
	КонецПопытки;
	
	МассивКаширин.Добавить(СтруктураКаширин);
	///-ГомзМА 07.05.2024
	...
	СтруктураОтвета.Вставить("kashirin", 		МассивКаширин);
...
Функция ДашбордМаляркиgetdashpainter(Запрос)
	...
	ЗапросКаширин.УстановитьПараметр("ДатаНачала", 		ДатаНачала);
	ЗапросКаширин.УстановитьПараметр("ДатаОкончания", 	ДатаОкончания);
	ЗапросКаширин.УстановитьПараметр("Ответственный",	Справочники.Сотрудники.НайтиПоКоду("000000039").Пользователь); //Емелин Константин Валерьевич
