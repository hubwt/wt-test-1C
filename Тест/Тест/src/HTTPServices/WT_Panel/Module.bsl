...
Функция ТекстДляПлохихЗаявок()
	Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|			ИЛИ ЗаказКлиента.Состояние = ЗНАЧЕНИЕ(перечисление.дт_Сост6оянияЗаказовКлиента.НетВНаличии)
...
Функция текстДляЗадач()
	...
	|	ИЛИ докЗадача.Номер ПОДОБНО &Описание
	|	ИЛИ докЗадача.Ответственный.Наименование ПОДОБНО &Описание)
...
Функция ПолучитьСписокПроектовЗадачВсеGetProjectTasksAll(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		//@skip-check query-in-loop
		//
		//СтруктураЗадачи.Вставить("workers", ПолучитьУчастниковЗадачи(выборка.ссылка));
		//СтруктураЗадачи.Вставить("id",    	  	   Строка(выборка.Код));
...
Функция СменаСтатусаЗадачиChangeStateTask(Запрос)
	...
	ИначеЕсли статус = 5 Тогда //Возврат из отмененных и выполненных
		...
		///-ГомзМА 27.02.2024
		ИначеЕсли статус = 6 Тогда //Пауза
		///+ГомзМА 19.03.2024
		Состояние = Перечисления.СтатусыЗадач.Создана;
		ДокОбъект = Документы.Задача.НайтиПоНомеру(Запрос.ПараметрыURL["id"]).ПолучитьОбъект();
		//ДокОбъект.Статус = Перечисления.СтатусыЗадач.Отложена;
		//ЗакрытьВремяЗадачи(ДокОбъект.ссылка, ДокОбъект.Исполнитель);
		ДокОбъект.Записать();
		
		ИначеЕсли статус = 7 Тогда //Пауза
		///+ГомзМА 19.03.2024
		Состояние = Перечисления.СтатусыЗадач.НаСогласовании;
		ДокОбъект = Документы.Задача.НайтиПоНомеру(Запрос.ПараметрыURL["id"]).ПолучитьОбъект();
		//ДокОбъект.Статус = Перечисления.СтатусыЗадач.Отложена;
		//ЗакрытьВремяЗадачи(ДокОбъект.ссылка, ДокОбъект.Исполнитель);
		ДокОбъект.Записать();
...
Функция ПолучитьЗадачуGetTask(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураЗадачи.Вставить("id_sub_department", выборка.Отдел.Код);
		СтруктураЗадачи.Вставить("sub_department", Строка(выборка.Отдел));
...
Функция СоздатьЧекЛистcreatecheck(Запрос)
	...
	Если Ответственный <> ЗадачаРодитель.Ответственный и ЗадачаРодитель.ЗадачаСПортала Тогда
		Новаязадача.Статус   	    		= Перечисления.СтатусыЗадач.НаСогласовании;
	Иначе
		Новаязадача.Статус   	    		= Перечисления.СтатусыЗадач.Создана;
	КонецЕсли;
	Новаязадача.Ответственный   		= Ответственный;  
