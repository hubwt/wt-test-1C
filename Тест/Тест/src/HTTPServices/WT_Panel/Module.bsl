...
Функция СтатистикаКонвейерИФотоgetstatisticspipelinephoto(Запрос)
	...
	Если Число(Запрос.ПараметрыURL["people"]) = 1 Тогда //Все
		
	ИначеЕсли Число(Запрос.ПараметрыURL["people"]) = 2 Тогда //Учетчики
		МассивСотрудников = Новый Массив;
	...
	Попытка
		СтруктураОтветаДетали.Вставить("count", выборка.Количество);
		СтруктураОтветаДетали.Вставить("money", выборка.Сумма);
	Исключение
	...
	Попытка
		СтруктураОтветаДетали.Вставить("count_mounth", выборкаМесяц.Количество);
		СтруктураОтветаДетали.Вставить("money_mounth", выборкаМесяц.Сумма);
	Исключение
