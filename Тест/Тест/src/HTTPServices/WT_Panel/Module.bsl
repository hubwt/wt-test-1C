...
Функция ПолучитьФинансовуюСтатистикуgetfinancialstatistics(Запрос)
	...
	Прибыль = РезультатЗапроса.БалансПриход - РезультатЗапроса.БалансРасход;
	Рентабельность = ?(РезультатЗапроса.БалансПриход = 0, 0, Прибыль / РезультатЗапроса.БалансПриход * 100);
	ОтветИнфо = Новый Структура;
	ОтветИнфо.Вставить("revenue", 			РезультатЗапроса.БалансПриход);
	ОтветИнфо.Вставить("expenditure", 		РезультатЗапроса.БалансРасход);
