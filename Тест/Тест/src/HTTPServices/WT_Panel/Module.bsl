...
Функция ПолучитьНеликвидВладимирскийgetliquidVlad(Запрос)
	...
	ЗапросТоваров.Текст = "ВЫБРАТЬ
						  ...
						  |		Иначе Ложь
						  |	Конец как Красный,
						  |	ВТ_ТоварыЗаПериод.Товар.Артикул,
						  |	РегистрНакопления1Остатки.Товар.Код КАК Код
						  |ИЗ
	...
	Для Каждого стр Из Выборка Цикл
		...
		ИначеЕсли Итер >= 7 И Итер <= 12 Тогда
			СтруктураТоваров.Вставить("rating", 4);
		ИначеЕсли Итер >= 13 И Итер <= 18 Тогда
			СтруктураТоваров.Вставить("rating", 3);
		ИначеЕсли Итер >= 19 И Итер <= 24 Тогда
			СтруктураТоваров.Вставить("rating", 2);
		ИначеЕсли Итер >= 25 И Итер <= 30 Тогда
