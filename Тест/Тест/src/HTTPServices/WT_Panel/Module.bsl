...
Функция ПолучитьВсехСотрудниковgetListEmployees(Запрос)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			СтруктураСотрудник = Новый Структура;
			СтруктураСотрудник.Вставить("name", Строка(Выборка.Наименование));
			СтруктураСотрудник.Вставить("tabnum", Строка(Выборка.Код));
			СтруктураСотрудник.Вставить("town", Строка(Выборка.МестоРаботы));
			МассивСотрудников.Добавить(СтруктураСотрудник);
		КонецЦикла;
	КонецЕсли;

	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("workers", МассивСотрудников);
