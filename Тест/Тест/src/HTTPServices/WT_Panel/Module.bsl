...
Функция ПолучитьФинансовуюСтатистикуgetfinancialstatistics(Запрос)
	...
	Пока РезультатЗапросаВыручка.Следующий() Цикл
		Сумма = ?(РезультатЗапросаВыручка.СуммаПриход = Null, 0, Формат(РезультатЗапросаВыручка.СуммаПриход, "ЧДЦ=0;"));
	...
	Пока РезультатЗапросаРасходПоставщики.Следующий() Цикл
		Сумма = ?(РезультатЗапросаРасходПоставщики.Сумма = Null, 0, Формат(РезультатЗапросаРасходПоставщики.Сумма, "ЧДЦ=0;"));
...
Функция ПолучитьЧекЛистgetchecklist(Запрос)
	...
	|ГДЕ
	|	ЧекЛисты.Задача.Номер = &Задача";
	
	
	ЗапросЧек.Текст = ЗапросЧек.Текст + Символы.ПС + "упорядочить по"+ Символы.ПС +"Порядок убыв";
	
	ЗапросЧек.УстановитьПараметр("Задача", Запрос.ПараметрыURL["id_task"]);
	...
	Пока ВыборкаЧек.Следующий() Цикл
		...
		СтруктураЧек.Вставить("done", Строка(ВыборкаЧек.Выполнено));
		СтруктураЧек.Вставить("executor", Строка(ВыборкаЧек.Ответственный));
		СтруктураЧек.Вставить("date", Строка(ВыборкаЧек.Дата));
		СтруктураЧек.Вставить("ordinal", Строка(ВыборкаЧек.Порядок));
		СтруктураЧек.Вставить("date_done", Строка(ВыборкаЧек.ДатаВыполнения));
...
Функция СоздатьЧекЛистcreatecheck(Запрос)
			...
			ЗаписьРегистраСведений.Дата 		 	= ДатаСоздания;
			ЗаписьРегистраСведений.Ответственный    = Ответственный;
			ЗаписьРегистраСведений.Выполнено
			ЗаписьРегистраСведений.Описание
			ЗаписьРегистраСведений.Порядок
КонецФункции
