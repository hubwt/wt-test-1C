...
Функция ПолучитьФинансовуюСтатистикуgetfinancialstatistics(Запрос)
	...
	Запрос = Новый Запрос;
	//Запрос.Текст =  "ВЫБРАТЬ
	//|	БалансОстаткиИОбороты.ПериодМесяц КАК ПериодМесяц,
	//|	МАКСИМУМ(БалансОстаткиИОбороты.БалансНачальныйОстаток) КАК БалансНачальныйОстаток,
	//|	СУММА(БалансОстаткиИОбороты.БалансПриход) КАК БалансПриход,
	//|	СУММА(БалансОстаткиИОбороты.БалансРасход) КАК БалансРасход,
	//|	СУММА(БалансОстаткиИОбороты.БалансОборот) КАК БалансОборот,
	//|	МАКСИМУМ(БалансОстаткиИОбороты.БалансКонечныйОстаток) КАК БалансКонечныйОстаток
	//|ИЗ
	//|	РегистрНакопления.Баланс.ОстаткиИОбороты(&ДатаНачала, &ДатаОкончания, Авто,,) КАК БалансОстаткиИОбороты
	//|ГДЕ
	//|	((ТИПЗНАЧЕНИЯ(БалансОстаткиИОбороты.Регистратор) = ТИП(Документ.ПриходДенегНаСчет)
	//|	ИЛИ ТИПЗНАЧЕНИЯ(БалансОстаткиИОбороты.Регистратор) = ТИП(Документ.Расходы)
	//|	ИЛИ ТИПЗНАЧЕНИЯ(БалансОстаткиИОбороты.Регистратор) = ТИП(Документ.ЛичныйВывод)))
	//|	И БалансОстаткиИОбороты.Измерение1.Подразделение В (&Подразделение)
	//|СГРУППИРОВАТЬ ПО
	//|	БалансОстаткиИОбороты.ПериодМесяц";	
	...
	Прибыль = Формат(РезультатЗапроса.СуммаПриход - РезультатЗапроса.СуммаРасход, "ЧДЦ=0;");
	Рентабельность = Формат(?(РезультатЗапроса.СуммаПриход = 0, 0, Прибыль / РезультатЗапроса.СуммаПриход * 100), "ЧДЦ=0;");
	ОтветИнфо = Новый Структура;
