...
Функция ПолучитьФинансовуюСтатистикуgetfinancialstatistics(Запрос)
	...
	Прибыль = РезультатЗапроса.СуммаПриход - РезультатЗапроса.СуммаРасход;
	Рентабельность = ?(РезультатЗапроса.СуммаПриход = 0, 0, Прибыль / РезультатЗапроса.СуммаПриход * 100);
	ОтветИнфо = Новый Структура;
	ОтветИнфо.Вставить("revenue", 			РезультатЗапроса.СуммаПриход);
	ОтветИнфо.Вставить("expenditure", 		РезультатЗапроса.СуммаРасход);
