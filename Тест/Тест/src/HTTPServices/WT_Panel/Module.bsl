...
Функция ДашбордРВРgetdashrvr(Запрос)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказНарядс.Номер) КАК Номер,
	...
	|ИЗ
	|	Справочник.ПроектыРазработки КАК ПроектыРазработки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаряд КАК ЗаказНарядс
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Приход КАК ВТ_Приход
	|			ПО (ЗаказНарядс.Ссылка = ВТ_Приход.Сделка)
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Товары КАК ВТ_Товары
	|			ПО (ЗаказНарядс.Ссылка = ВТ_Товары.Ссылка)
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Работы КАК ВТ_Работы
	|			ПО (ЗаказНарядс.Ссылка = ВТ_Работы.Ссылка)
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Расходники КАК ВТ_Расходники
	|			ПО (ЗаказНарядс.Ссылка = ВТ_Расходники.Ссылка)
	|		ПО (ПроектыРазработки.Ссылка = ЗаказНарядс.Проект)
	|ГДЕ
	|	ЗаказНарядс.Ответственный = &Фильтр
	|	И ЗаказНарядс.Дата МЕЖДУ &ДатаНачала и &ДатаОкончания
	|СГРУППИРОВАТЬ ПО
	|	ПроектыРазработки.Ссылка,
	|	ПроектыРазработки.ДатаПриема,
	|	ПроектыРазработки.ДатаСдачи,
...
Функция ДашбордМаляркиgetdashpainter(Запрос)
	...
	|			ПО ЗаказНаряд.Ссылка = ВТ_ТипНаценки.Ссылка
