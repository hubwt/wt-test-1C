...
Функция НаполнитьЗаявкуfillingapplication(Запрос)
	...
	Попытка
		...
		Если СтрНайти(Строка(Покупка), "_") > 0 Тогда
			...
			СтруктураОтбора.Вставить("Партия", Справочники.ИндКод.ПустаяСсылка());
			...
			Если НайденныеСтроки.Количество() <> 0 Тогда
				...
				Ответ = Новый HTTPСервисОтвет(200);
				Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
				Ответ.УстановитьТелоИзСтроки("Успех. Партия установлена для " + Строка(
...
|	ЗаказКлиента.WTPanel КАК WTPanel,
|	АвтономерЗаписи() КАК НомерЗаписи,
|ЗаказКлиента.СтатусыДействия КАК СтатусыДействия,
|ЗаказКлиента.ФинансовыеСтатусы КАК ФинансовыеСтатусы
|ПОМЕСТИТЬ ВТ_ДанныеЗаявки
|ИЗ
|	Документ.ЗаказКлиента КАК ЗаказКлиента
...
//	КонецЕсли;
	...
	Если Число(Запрос.ПараметрыURL["fin_status"]) = 1 И Число(Запрос.ПараметрыURL["action_status"]) = 0 Тогда
		...
		ЗапросЗаявок.УстановитьПараметр("ФинансовыеСтатусы", Перечисления.ФинансовыеСтатусыДляЗаявкиКлиента.ЖдемДенег);
		
	ИначеЕсли Число(Запрос.ПараметрыURL["fin_status"]) = 2 И Число(Запрос.ПараметрыURL["action_status"]) = 0 Тогда 
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), ТекстЗаменыФинСтатусы);
		ЗапросЗаявок.УстановитьПараметр("ФинансовыеСтатусы", Перечисления.ФинансовыеСтатусыДляЗаявкиКлиента.ЧастичноОплачено);
		
	ИначеЕсли Число(Запрос.ПараметрыURL["fin_status"]) = 3 И Число(Запрос.ПараметрыURL["action_status"]) = 0 Тогда 
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), ТекстЗаменыФинСтатусы);
		ЗапросЗаявок.УстановитьПараметр("ФинансовыеСтатусы", Перечисления.ФинансовыеСтатусыДляЗаявкиКлиента.Оплачено);
		
	ИначеЕсли Число(Запрос.ПараметрыURL["fin_status"]) = 4 И Число(Запрос.ПараметрыURL["action_status"]) = 0 Тогда 
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), ТекстЗаменыФинСтатусы);
		ЗапросЗаявок.УстановитьПараметр("ФинансовыеСтатусы", Перечисления.ФинансовыеСтатусыДляЗаявкиКлиента.НеОплачено);
		
	ИначеЕсли Число(Запрос.ПараметрыURL["fin_status"]) = 5 И Число(Запрос.ПараметрыURL["action_status"]) = 0 Тогда 
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), ТекстЗаменыФинСтатусы);
		ЗапросЗаявок.УстановитьПараметр("ФинансовыеСтатусы", Перечисления.ФинансовыеСтатусыДляЗаявкиКлиента.НеБудетОплачено);
	...
	Для Каждого Результат Из СписокПоиска Цикл
			...
			СтруктураЗаявок.Вставить("action_status", Строка(Результат.СтатусыДействия));
			СтруктураЗаявок.Вставить("fin_state", Строка(Результат.ФинансовыеСтатусы));
