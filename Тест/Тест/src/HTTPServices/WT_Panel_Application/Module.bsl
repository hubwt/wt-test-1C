...
Функция ВыйтиИзЗаявкиgetoutapp(Запрос)
	...
	Если ТипВыхода = "Менеджер" Тогда
		ОбъектЗаявки.Ответственный = Справочники.Пользователи.ПустаяСсылка();
	ИначеЕсли ТипВыхода = "Кладовщик" Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("Сотрудник", Ответственный);
		НайденныеСтроки = ОбъектЗаявки.Ответственные.НайтиСтроки(Отбор);
		Если НайденныеСтроки.Количество() <> 0 Тогда
...
Функция ПолучитьЗаявкуgetapplication(Запрос)
	...
	Попытка
		...
		ЗаписьJSON.УстановитьСтроку();
		Информация = ИнформацияОбОшибке();
		ЗаписатьJSON(ЗаписьJSON, СформироватьСтруктуруОшибки( , "Ошибка выполнения запроса", "Данная заявка не обнаружена"));
...
Функция ПечатьНарядаНаДоставкуPrintingDelivery(Запрос)
	...
	Исключение
		HTTPОтвет = Новый HTTPСервисОтвет(404); // 400 - Bad Request
