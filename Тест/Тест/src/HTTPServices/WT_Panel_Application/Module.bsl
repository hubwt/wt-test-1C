	...
	Попытка
		...
		Если СтрНайти(Строка(Покупка), "_") > 0 Тогда
			...
			Если НайденныеСтроки.Количество() <> 0 Тогда
				...
				НайденныеСтроки[0].Партия = Партия;
				//@skip-check reading-attribute-from-database
					
				// ++ Это условие чтобы не изменялась цена в заявке при добавлении сборщиком партии 
				Если Цена = Неопределено ИЛИ Цена = 0 Тогда 
					НайденныеСтроки[0].Цена = ?(ИнфоПартия.Цена <> 0, ИнфоПартия.Цена, Номенклатура.РекомендованаяЦена);
					НайденныеСтроки[0].ЦенаСоСкидкойНаценкой = НайденныеСтроки[0].Цена; 
					НайденныеСтроки[0].Сумма = НайденныеСтроки[0].Цена;  
				Иначе 
				...
				Разница = Цена - ИнфоПартия.Цена; 
				
				Если Разница < 0 тогда 
					Структура = Новый Структура;
					Структура.Вставить("priceproduct","Цена товара меньше на "+ -Разница); 
				ИначеЕсли Разница > 0 тогда 
					Структура = Новый Структура;
					Структура.Вставить("priceproduct","Цена товара больше на "+ Разница); 
				Иначе 
					Структура = Новый Структура;
					Структура.Вставить("priceproduct","equal" ); 
				КонецЕсли; 
				...
				ЗаписатьJSON(НоваяЗапись,Структура);
				СтрокаДляОтвета = НоваяЗапись.Закрыть(); 
				Ответ = Новый HTTPСервисОтвет(200);
				Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
				Ответ.УстановитьТелоИзСтроки(СтрокаДляОтвета);
				Возврат Ответ;
...
Попытка
	...
	Если Статус = 3 И Не (Заявка.Номер = "000054204" Или Заявка.Номер = "000054206" Или Заявка.Номер = "000054207"
		Или Заявка.Номер = "000054208" Или Заявка.Номер = "000054195" Или Заявка.Номер = "000054321" Или Заявка.Номер = "000054334"
		Или Заявка.Номер = "000054332" Или Заявка.Номер = "000054331" Или Заявка.Номер = "000054333" Или Заявка.Номер
		= "000054354") Тогда
...
|ЗаказКлиента.СтатусыДействия КАК СтатусыДействияИмя,
|ЗаказКлиента.ФинансовыеСтатусы КАК ФинансовыеСтатусыИмя
|ПОМЕСТИТЬ ВТ_ДанныеЗаявки
|ИЗ
|	Документ.ЗаказКлиента КАК ЗаказКлиента
...
|ВТ_ДанныеЗаявки.ФинансовыеСтатусы КАК ФинансовыеСтатусы,
|ВТ_ДанныеЗаявки.СтатусыДействия КАК СтатусыДействия, 
|ВТ_ДанныеЗаявки.ФинансовыеСтатусыИмя КАК ФинансовыеСтатусыИмя,
|ВТ_ДанныеЗаявки.СтатусыДействияИмя КАК СтатусыДействияИмя
|ИЗ
|	ВТ_ДанныеЗаявки КАК ВТ_ДанныеЗаявки
|ГДЕ
|	ВТ_ДанныеЗаявки.НомерЗаписи >= &НачинаяСЗаписи
...
//	КонецЕсли;
	...
	/// Комлев ++++23/07/24 +++
	ТекстЗаменыФинСтатусы = СтрокаПоиска + " И ЗаказКлиента.ФинансовыеСтатусы = &ФинансовыеСтатусы";
	ТекстЗаменыСтатусыДействия = СтрокаПоиска + " И ЗаказКлиента.СтатусыДействия = &СтатусыДействия";
	/// Статусы действия
	
	Если Число(Запрос.ПараметрыURL["action_status"]) = 1 И Число(Запрос.ПараметрыURL["fin_status"]) = 0 Тогда
			ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), ТекстЗаменыСтатусыДействия);
	...
	/// Финансовые 
		...
		Если Число(Запрос.ПараметрыURL["fin_status"]) = 1 И Число(Запрос.ПараметрыURL["action_status"]) = 0 Тогда
			ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), ТекстЗаменыФинСтатусы);
	...
	Для Каждого Результат Из СписокПоиска Цикл
			...
			СтруктураЗаявок.Вставить("action_status", Строка(Результат.СтатусыДействияИмя));
			СтруктураЗаявок.Вставить("fin_state", Строка(Результат.ФинансовыеСтатусыИмя));
...
СтрокаПоиска = "	Где (ЗаказКлиента.Номер ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~"""+ Символы.ПС+
" ИЛИ ЗаказКлиента.КлиентНаименование ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~"""+ Символы.ПС+
" ИЛИ ЗаказКлиента.Клиент.ПолноеНаименование ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~"""+ Символы.ПС+
" ИЛИ ЗаказКлиента.НомерТелефона ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~"")";			
ЗапросЗаявок = Новый Запрос;
...
Функция ПолучитьЗаявкиДляКладовщикаgetappstorekeeper(Запрос)
...
Процедура ОткрытьВремяЗК(Заявка, Ответственный, Статус)

	НаборЗаписей = РегистрыСведений.ВремяВыполненияЗаявок.СоздатьМенеджерЗаписи();

	НаборЗаписей.ЗаявкаПродажа = Заявка;
	//@skip-check use-non-recommended-method
...
//            "main": "",
//            "common": [
//                "http://192.168.0.245:8085/card/image/05811/1-20181213_114723.jpg",
//                "http://192.168.0.245:8085/card/image/05811/20181213_114650.jpg",
//                "http://192.168.0.245:8085/card/image/05811/20181213_114709.jpg",
//                "http://192.168.0.245:8085/card/image/05811/20181213_114925.jpg",
//                "http://192.168.0.245:8085/card/image/05811/20181213_115014.jpg"
//            ]
//        }
МассивОтвета = новый массив;
			СтруктураОтвета = новый структура;
