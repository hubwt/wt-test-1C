...
//КонецЕсли;
	...
	СтруктураКлиента.Вставить("name",ИмяКлиентаПерем );
...
//	КонецЕсли;
	...
	//Менеджер = Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь", выборка.Ответственный).Код;
		...
		//@skip-check query-in-loop
			//СтруктураВремени =  Новый Структура; //ПолучитьВремяЗК(Результат.Ссылка);
			СтруктураЗаявок.Вставить("time_work",  0);//СтруктураВремени.time_work);
			СтруктураЗаявок.Вставить("time_wait", 0); //СтруктураВремени.time);
			СтруктураЗаявок.Вставить("workers", 0); //СтруктураВремени.workers);
			СтруктураЗаявок.Вставить("in_work", 0); //СтруктураВремени.in_work);
...
//	СтруктураИнфо.Вставить("comment", Строка(выборка.Комментарий));
	...
	ТЗ_Товары = Запросзаявки.Выполнить().Выгрузить();
