...
Функция ПолучитьЗаявкуgetapplication(Запрос)
	...
	//Пока выборка.Следующий() Цикл 
		...
		СтруктураКлиента.Вставить("id", выборка.Клиент.Код);
		СтруктураКлиента.Вставить("phone", выборка.Клиент.Телефон);
	...
	Попытка
		...
		Пока выборка.Следующий() Цикл
			...
			//СтрокаТЧ.Артикул = Номенклатура.Артикул; 
				ТекстЛога =  " В продаже " + ОбъектЗаявки.номер + " установил партию в товаре " + СтрокаТЧ.Товар + " "
					+ СтрокаТЧ.Партия;
			...
			Иначе
				...
				//СтрокаТЧ.СтатусТовара = Истина;
					...
					ТекстЛога =  " В продаже " + ОбъектЗаявки.номер + " установил выдал партию в товаре "
...
Процедура ПересчётСуммыЗаявки(СсылкаНаЗаявку)
	ОбъектЗаявки = Документы.ЗаказКлиента.НайтиПоНомеру().ПолучитьОбъект();//СсылкаНаЗаявку.ПолучитьОбъект();
...
Функция УдалитьТоварИзЗаявкиdeleteproductapplication(Запрос)
	...
	//ИндКод  = Массив.IndCode;
		...
		Если Юзер = Справочники.Пользователи.ПустаяСсылка() Тогда
			...
			Информация = ИнформацияОбОшибке();
			Ответ.УстановитьТелоИзСтроки( СформироватьСтруктуруОшибки( , "Ошибка выполнения запроса",
				Информация.Описание));
			Возврат Ответ;
...
//	КонецЕсли;
//	
		ТекстЗаменыОтборОтветственныйЗаявки = " ЗаказКлиента.Ответственный = &Ответственный";
	
	/// Комлев 22/07/24 +++
		ТекстЗаменыОтборПоФинансовомуСтатусу = ТекстЗаменыОтборОтветственныйЗаявки
			+ " И  ЗаказКлиента.ФинансовыеСтатусы = &ФинансовыеСтатусы";
		ТекстЗаменыОтборПоСтатусуДействия = ТекстЗаменыОтборОтветственныйЗаявки
			+ " И ЗаказКлиента.СтатусыДействия = &СтатусыДействия";	
	/// Комлев 22/07/24 ---
		...
		Текст = "ВЫБРАТЬ
				...
				|	ВТ_ДанныеЗаявки.WTPanel КАК WTPanel,
				|	ВТ_ДанныеЗаявки.ФинансовыеСтатусы КАК ФинансовыйСтатус,
				|	ВТ_ДанныеЗаявки.СтатусыДействия КАК СтатусДействия
				|ИЗ
				|	ВТ_ДанныеЗаявки КАК ВТ_ДанныеЗаявки
				|ГДЕ
				|	ВТ_ДанныеЗаявки.НомерЗаписи >= &НачинаяСЗаписи";
...
//	КонецЕсли;
	...
	/// Отбор по статусу действия
		...
		ИначеЕсли Число(Запрос.ПараметрыURL["action_status"]) = 0 И Число(Запрос.ПараметрыURL["fin_status"]) = 0 Тогда
			ЗапросЗаявок.УстановитьПараметр("Ответственный", Справочники.Сотрудники.НайтиПоКоду(
		Запрос.ПараметрыURL["saler"]).пользователь);
		КонецЕсли;
	 ...
	 /// Отбор по статусу финансовому
		...
		ИначеЕсли Число(Запрос.ПараметрыURL["fin_status"]) = 0 И Число(Запрос.ПараметрыURL["action_status"]) = 0 Тогда
			ЗапросЗаявок.УстановитьПараметр("Ответственный", Справочники.Сотрудники.НайтиПоКоду(
		Запрос.ПараметрыURL["saler"]).пользователь);
		КонецЕсли;
	 ...
	 /// Комлев 22/07/24 ---
		...
		Для Каждого Результат Из СписокПоиска Цикл
			...
			СтруктураТоваров = Новый Структура;
			СтруктураТоваров.Вставить("id", Строка(Результат.Номер));
			СтруктураТоваров.Вставить("date", Строка(Результат.Дата));
			СтруктураТоваров.Вставить("state", Строка(Результат.WTPanel));
			...
			СтруктураКлиента.Вставить("id", Результат.Клиент.Код);
			СтруктураКлиента.Вставить("phone", Результат.Клиент.Телефон);
			СтруктураТоваров.Вставить("client", СтруктураКлиента);
	...
	//Менеджер = Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь", выборка.Ответственный).Код;
			...
			СтруктураТоваров.Вставить("manager", СтруктураМенеджера);
		...
		СтруктураОтвета = Новый Структура;
		СтруктураОтвета.Вставить("info", СтруктураИнфо);
		СтруктураОтвета.Вставить("applications", МассивТоваров);
...
Функция текстДляЗаявки()
	Текст = "ВЫБРАТЬ
			...
			|	И {ЗаказКлиента.Состояние = &Состояние}";
	Возврат Текст;
КонецФункции
