...
Функция ПолучитьКарточкуТовараgetcardproduct(Запрос)
	...
	Пока выборка.Следующий() Цикл
		...
		СтруктураИнфо.Вставить("count_verified_NN", Строка(Выборка.ОстатокСовпадает));
		СтруктураИнфо.Вставить("count_verified_MSK", Строка(Выборка.ОстатокСовпадаетМСК));
		СтруктураИнфо.Вставить("count_verified_EKB", Строка(Выборка.ОстатокСовпадаетЕКБ));
//		Пока Лев(Код, 1) = "0" Цикл
//			Код = Прав(Код, СтрДлина(Код) - 1);
//		КонецЦикла;
//		Код = "0" + код;
