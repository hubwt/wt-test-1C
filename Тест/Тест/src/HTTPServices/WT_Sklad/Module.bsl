...
//		КонецЕсли;
		...
		Если ЗначениеФильтра <> 0 Тогда
			...
			ИначеЕсли ЗначениеФильтра = 3 Тогда
					...
					Запрос1.Текст =
						...
						|ГДЕ
						|	ЗаказКлиентаТовары.Ссылка.ЭтапыПродажи = ЗНАЧЕНИЕ(Перечисление.ЭтапыПродажи.Отказ)
						|	И ЗаказКлиентаТовары.Ссылка.Дата > &ДатаОтсчёта
						|	И ИндНомер.Стеллаж.Наименование ПОДОБНО &Наименование1
						|;
						|
						|////////////////////////////////////////////////////////////////////////////////
						|ВЫБРАТЬ РАЗЛИЧНЫЕ
					...
					Запрос1.УстановитьПараметр("ДатаОтсчёта", ТекущаяДата() - (3600 * 240 * 30));
					Корзина = "Korzina_"; 
					Запрос1.УстановитьПараметр("Наименование1", "%" + Корзина + "%");
					Массив = Новый Массив; 
					РезультатЗапроса = Запрос1.Выполнить();
						...
						КонецЦикла;
						СтрокаФильтраСтатус = " И ДокЗаказКлиента.Ссылка В (&Массив)";
			КонецЕсли;
		КонецЕсли;
		...
		Если  ЗначениеФильтра = 3 Тогда
			ЗапросЗаявок.УстановитьПараметр("Массив", Массив);
		КонецЕсли;  
		
		т =    Запрос.ПараметрыURL["text"]; 
		//==============================================================================================================
