...
Функция ПроверитьДетальНаНелеквидПоНомеруПроизводителяcontol(Запрос)
	...
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("name", "Номенклатуры с таким номером производителя нет.");
	СтруктураОтвета.Вставить("permission", Ложь);
	СтруктураОтвета.Вставить("count", 0);
	СтруктураОтвета.Вставить("required", 0);

	Если РезультатЗапроса.Пустой() Тогда
		СтруктураОтвета.permission = Истина;
		ЗаписьJSON = Новый ЗаписьJSON;
		ЗаписьJSON.УстановитьСтроку();
		ЗаписатьJSON(ЗаписьJSON, СтруктураОтвета);
		СтрокаДляОтвета = ЗаписьJSON.Закрыть();
	...
	Выборка.Следующий();
	СтруктураОтвета.name = Выборка.Деталь;
	СтруктураОтвета.count = Выборка.КолвоОстаток;
	СтруктураОтвета.required = Выборка.НеобходимыйОстаток;
	...
	Иначе
		Если Выборка.КолвоОстаток > Выборка.НеобходимыйОстаток И Выборка.НеобходимыйОстаток <> 0 Тогда
			СтруктураОтвета.permission  = Ложь;
