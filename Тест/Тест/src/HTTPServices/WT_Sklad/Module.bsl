...
Функция ПолучитьВсеЗаявкиGetAllApp(Запрос)
	...
	Попытка
		...
		СтрокаФильтрПоКладовщику = "";
		КладовщикПарам = Запрос.ПараметрыURL.Получить("filter_stockman");
		Если КладовщикПарам <> "00" Тогда
			ПользовательКладовщик = Справочники.Сотрудники.НайтиПоКоду(КладовщикПарам).Пользователь;
			СтрокаФильтрПоКладовщику = "И ЗаказКлиента.Ответственные.Сотрудник = &СотрудникПользователь";
		КонецЕсли;
		// Комлев АА 26/12/24 ---
		...
		Текст = "ВЫБРАТЬ
				...
				|	УПОРЯДОЧИТЬ ПО
				|	Дата УБЫВ";
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), СтрокаПоиска, СтрокаФильтрПоКладовщику);
		...
		ЗапросЗаявок.УстановитьПараметр("СотрудникПользователь", ПользовательКладовщик);
		...
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
			Запрос.ПараметрыURL["count"], 10000), "ЧГ="), СтрокаПоиска, СтрокаФильтрПоКладовщику);
	...
	//Менеджер = Справочники.Сотрудники.НайтиПоРеквизиту("Пользователь", выборка.Ответственный).Код;
		...
		//@skip-check query-in-loop
			...
			// Комлев АА 26/12/24 ++++
			МассивКладовщиков  = ПолучитьКладовщиковЗаявки(Строка(Результат.Номер));
			СтруктураЗаявок.Вставить("stock_man_list", МассивКладовщиков);
			// Комлев АА 26/12/24 ----
			//@skip-check query-in-loop
...
Функция ПолучитьКладовщиковЗаявки(НомерЗаявки)
	// Комлев АА 26/12/24 ++++
	МассивКладовщиков = Новый Массив;
	ЗапросКладовщиков = Новый Запрос;
	ЗапросКладовщиков.Текст =
	"ВЫБРАТЬ
	...
	|		ПО Сотрудники.Пользователь = ЗаказКлиентаОтветственные.Сотрудник
	|ГДЕ
	|	ЗаказКлиентаОтветственные.Ссылка.Номер = &НомерЗаявки";

	ЗапросКладовщиков.УстановитьПараметр("НомерЗаявки", НомерЗаявки);

	РезультатЗапроса = ЗапросКладовщиков.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		Возврат МассивКладовщиков;
	КонецЕсли;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтруктураКладовщик.Вставить("name", Строка(ВыборкаДетальныеЗаписи.Наименование));
		СтруктураКладовщик.Вставить("id", Строка(ВыборкаДетальныеЗаписи.Код));
		МассивКладовщиков.Добавить(СтруктураКладовщик);
	КонецЦикла;
	Возврат МассивКладовщиков;
	// Комлев АА 26/12/24 ---
КонецФункции
...
Функция ПолучитьВсеЗаявкиПоСтатусамИПоискомGET(Запрос)
	//{count}/{page}/{text}/{filter_status}/{filter_stockman}	- 1С
	//{count}/{page}/00/{state}/{worker}						- Web
	//count - количество
	//page - страница
	//state - состояние. 1 - новая, 2 - в работе, 3 - заверешенные
	//worker - 00 - все работники
...
//		Если Запрос.ПараметрыURL["text"] <> "00" Тогда
//			СтрокаПоиска = "	И (ДокЗаказКлиента.Номер ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~""" + Символы.ПС
//				+ " ИЛИ ДокЗаказКлиента.КлиентНаименование ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~""" + Символы.ПС
//				+ " ИЛИ ДокЗаказКлиента.Клиент.ПолноеНаименование ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~""" + Символы.ПС
//				+ " ИЛИ ДокЗаказКлиента.НомерТелефона ПОДОБНО &Наименование СПЕЦСИМВОЛ ""~"")";
//		КонецЕсли;
		...
		Если КладовщикПарам <> "00" Тогда
			ПользовательКладовщик = Справочники.Сотрудники.НайтиПоКоду(КладовщикПарам).Пользователь;
			СтрокаФильтрПоКладовщику = " И ДокЗаказКлиента.Ответственные.Сотрудник = &СотрудникПользователь";
		КонецЕсли;

		СтрокаФильтраСтатус = "";
		ЗначениеФильтра = Число(Запрос.ПараметрыURL["filter_status"]);
//		Если ЗначениеФильтра <> 0 Тогда
//			Если ЗначениеФильтра = 3 Тогда
//				СтрокаФильтраСтатус = " И  ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.Завершено)";
//			ИначеЕсли ЗначениеФильтра = 1 Тогда
//				СтрокаФильтраСтатус = " И (ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.НоваяЗаявка) ИЛИ ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.ПустаяСсылка))";
//			ИначеЕсли ЗначениеФильтра = 2 Тогда
//				СтрокаФильтраСтатус = " И ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.ВРаботе) ";
//			ИначеЕсли ЗначениеФильтра = 4 Тогда
//				СтрокаФильтраСтатус = " И ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.Отменено)";
//			КонецЕсли;
//		КонецЕсли;

		Если ЗначениеФильтра <> 0 Тогда
			Если ЗначениеФильтра = 1 Тогда
				СтрокаФильтраСтатус = " И  ДокЗаказКлиента.ЭтапыПродажи.Ссылка = ЗНАЧЕНИЕ(Перечисление.ЭтапыПродажи.Собрать)";
			ИначеЕсли ЗначениеФильтра = 2 Тогда
				СтрокаФильтраСтатус = " И ДокЗаказКлиента.ЭтапыПродажи.Ссылка = ЗНАЧЕНИЕ(Перечисление.ЭтапыПродажи.Выдать)";
			КонецЕсли;
		КонецЕсли;
		...
		Текст = "ВЫБРАТЬ
				...
				|	ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком КАК СтатусОбработкиЗаявкиКладовщиком,
				...
				|	ДокЗаказКлиента.WTPanel КАК WTPanel,
				|	АвтономерЗаписи() КАК НомерЗаписи,
				|	ДокЗаказКлиента.СтатусыДействия.Порядок КАК СтатусыДействия,
				|	ДокЗаказКлиента.ФинансовыеСтатусы.Порядок КАК ФинансовыеСтатусы,
				|	ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком.Ссылка КАК СтатусОбработкиЗаявкиКладовщикомСсылка,
				|	ДокЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком.Порядок КАК СтатусОбработкиЗаявкиКладовщикомПорядок,
				|	ЕСТЬNULL(ПродажаЗапчастей.Номер, """") КАК НомерПродажи,
				|	ДокЗаказКлиента.ЭтапыПродажи.Ссылка КАК ЭтапыПродажи
				|ПОМЕСТИТЬ ВТ_ДанныеЗаявки
				|ИЗ
				|	Документ.ЗаказКлиента КАК ДокЗаказКлиента
				|		ЛЕВОЕ СОЕДИНЕНИЕ ТелефоныСлужебные КАК ТелефоныСлужебные
				|		ПО ДокЗаказКлиента.Ответственный = ТелефоныСлужебные.ссылка.пользователь
				|		ЛЕВОЕ СОЕДИНЕНИЕ ПродажаЗапчастей КАК ПродажаЗапчастей
				|		ПО ПродажаЗапчастей.ЗаказКлиента = ДокЗаказКлиента.Ссылка
				|Где
				|	ДокЗаказКлиента.Дата > &ДатаОтсчёта
				|	И ДокЗаказКлиента.СуммаДокумента <> 0
				|%2
				|%3
				|%4
				|УПОРЯДОЧИТЬ ПО
				...
				|	ВТ_ДанныеЗаявки.КлиентКод КАК КлиентКод,
				|	ВТ_ДанныеЗаявки.Телефон КАК Телефон,
				|	ВТ_ДанныеЗаявки.НомерПродажи,
				|	ВТ_ДанныеЗаявки.ЭтапыПродажи
				|ИЗ
				|	ВТ_ДанныеЗаявки КАК ВТ_ДанныеЗаявки
				|ГДЕ
				|	ВТ_ДанныеЗаявки.НомерЗаписи >= &НачинаяСЗаписи
		...
		ЗапросЗаявок = Новый Запрос;
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(10000, "ЧГ="), СтрокаПоиска, СтрокаФильтраСтатус,
			СтрокаФильтрПоКладовщику);
		ЗапросЗаявок.УстановитьПараметр("Наименование", "%" + ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(
				Строка(
		Запрос.ПараметрыURL["text"])) + "%");

		ДатаОтсчёта = ТекущаяДата() - (3600 * 24 * 30);
		ЗапросЗаявок.УстановитьПараметр("ДатаОтсчёта", ДатаОтсчёта);
		ЗапросЗаявок.УстановитьПараметр("НачинаяСЗаписи", 0);
		ЗапросЗаявок.УстановитьПараметр("СотрудникПользователь", ПользовательКладовщик);
		ОбщееКолво = ЗапросЗаявок.Выполнить().Выбрать().Количество();
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
			Запрос.ПараметрыURL["count"], 10000), "ЧГ="), СтрокаПоиска, СтрокаФильтраСтатус, СтрокаФильтрПоКладовщику);
		ЗапросЗаявок.УстановитьПараметр("Наименование", "%" + ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(
				Строка(
		Запрос.ПараметрыURL["text"])) + "%");
		Если Число(((Запрос.ПараметрыURL["count"]) * (Запрос.ПараметрыURL["page"]))) > 0 И Число(
...
//			СтруктураСтатуса.Вставить("state_number", Число(Результат.СтатусОбработкиЗаявкиКладовщикомПорядок) + 1);
			...
			Если Результат.ЭтапыПродажи = Перечисления.ЭтапыПродажи.Собрать Тогда
				СтруктураСтатуса.Вставить("state_name", Строка(Результат.ЭтапыПродажи));
				СтруктураСтатуса.Вставить("state_number", 1);
			ИначеЕсли Результат.ЭтапыПродажи = Перечисления.ЭтапыПродажи.Выдать Тогда
				СтруктураСтатуса.Вставить("state_name", Строка(Результат.ЭтапыПродажи));
				СтруктураСтатуса.Вставить("state_number", 2);
			КонецЕсли;
			...
			МассивКладовщиков  = ПолучитьКладовщиковЗаявки(Строка(Результат.Номер));
			СтруктураЗаявок.Вставить("stock_man_list", МассивКладовщиков);
			СтруктураЗаявок.Вставить("sale", Строка(Результат.НомерПродажи));
			Если Строка(Результат.НомерПродажи) <> "" Тогда
				РезультатВыданВесьТоварДаНет = ПроверкаВыдачиТовараВПродаже(Результат.НомерПродажи);
				СтруктураЗаявок.Вставить("all_product_issued", РезультатВыданВесьТоварДаНет);
			Иначе
				РезультатВыданВесьТоварДаНет = ПроверкаЕстьЗаменаВЗаявке(Результат.Ссылка);
				СтруктураЗаявок.Вставить("all_product_issued", РезультатВыданВесьТоварДаНет);
...
Функция ПолучитьЗаявкуGetApp(Запрос)
	...
	Попытка
		...
		//СтруктураСтатуса.Вставить("state_build_number", Число(?(ТипЗнч(выборка.СтатусОбработкиПорядок) = Тип("NULL"), 0,  выборка.СтатусОбработкиПорядок + 1)));

		СтруктураСтатуса.Вставить("state_build_name", Строка(выборка.ЭтапыПродажи));
		СтруктураСтатуса.Вставить("state_build_number", Число(?(ТипЗнч(выборка.СтатусОбработкиПорядок) = Тип("NULL"),
			0, выборка.СтатусОбработкиПорядок + 1)));
		...
		ЕстьТоварЗаменить = Ложь;
		ЕстьТоварОтменить = Ложь;
...
//					НайденаяСтрока = ТЗ_Товары.Найти(стр.Партия, "Партия");
...
// 				Комлев 27/08/24 ---
				...
				Если стр.Выдан = Истина И стр.Заменить = Ложь Тогда
					СтруктураТоваров.Вставить("issued_status", "Выдан");
				ИначеЕсли стр.Выдан = Истина И стр.Заменить = Истина Тогда
					СтруктураТоваров.Вставить("issued_status", "Заменить");
					ЕстьТоварЗаменить = Истина;
				ИначеЕсли стр.Выдан = Ложь И стр.Заменить = Истина Тогда
					СтруктураТоваров.Вставить("issued_status", "Заменить");
					ЕстьТоварЗаменить = Истина;
				ИначеЕсли стр.Выдан = Ложь И стр.Заменить = Ложь Тогда
					СтруктураТоваров.Вставить("issued_status", "");
				КонецЕсли;
			...
			Иначе
				...
				ЗапросПолки.УстановитьПараметр("индкодВладелецКод", стр.Код);

				РезультатЗапроса = ЗапросПолки.Выполнить();

				ВыборкаПолки = РезультатЗапроса.Выбрать();

				Пока ВыборкаПолки.Следующий() Цикл
					СтруктураПолки =  Новый Структура;
					СтруктураПолки.Вставить("place", Строка(ВыборкаПолки.Стеллаж));
					СтруктураПолки.Вставить("count", ВыборкаПолки.Количество);
		...
		Если ЭтоДеталь Тогда
			...
			Если СтрНайти(Строка(Покупка), "_") > 0 Тогда
				...
				Если Не ПартияУжеЕсть Тогда
					...
					НайденныяСтрока = ОбъектЗаявки.Товары.Найти(Справочники.ИндКод.НайтиПоНаименованию(
						Покупка).Владелец, "Номенклатура");
...
Функция СменатовараЗаявкиreplaceproductapplication(Запрос)
	...
	ТекстЗапроса = "ВЫБРАТЬ
				   |	ЗаказКлиента.Ссылка КАК Ссылка,
				   |	ЗаказКлиента.Продажа КАК Продажа
				   |ИЗ
				   |	Документ.ЗаказКлиента КАК ЗаказКлиента
				   |ГДЕ
				   |	ЗаказКлиента.Номер = &Номер";
	...
	Попытка
		...
		ОбъектЗаявки = СсылкаЗаявка.ПолучитьОбъект();
		...
		Если СтрНайти(Строка(Замена), "_") > 0 Тогда
			...
			Если Новаястрока.Заменить = Истина Тогда
				Новаястрока.Заменить = Ложь;
			КонецЕсли;
			...
			СсылкаЗаявка = ОбъектЗаявки.Ссылка;
			СсылкаПродажа = ОбъектЗаявки.Продажа;
			ЗаявкаНомер = ОбъектЗаявки.Номер;
			ТЧЗаявка = ОбъектЗаявки.Товары.Выгрузить(); 
				// ----
			ОбъектЗаявки.Записать(РежимЗаписиДокумента.Запись);
		...
		// ++++ ОбновитьПродажу 
		ЭтапДо = ВсеСинхронизацииПродажиИЗаявки.ПолучитьТекущийЭтапЗаявки(ЗаявкаНомер);
		Если ЗначениеЗаполнено(СсылкаПродажа) Тогда
			ВсеСинхронизацииПродажиИЗаявки.ОбновитьПродажуТЧ(ТЧЗаявка, СсылкаПродажа);
		КонецЕсли;
		ЭтапПосле = ВсеСинхронизацииПродажиИЗаявки.ПолучитьТекущийЭтапЗаявки(ЗаявкаНомер);

		Если ЭтапДо <> ЭтапПосле Тогда
			ОбъектЗаявки = СсылкаЗаявка.ПолучитьОбъект();
			ОбъектЗаявки.Записать(РежимЗаписиДокумента.Запись);
		КонецЕсли; 
		// ----
...
Функция ПолучитьТоварыКарточкиproductscard(Запрос)
	...
	Текстзапроса = "ВЫБРАТЬ
				   ...
				   |	РегИндНомер.ДатаПроверкиЕстьВНаличии КАК ДатаИнвент,
				   |	РегИндНомер.ОтветственныйЕстьВНаличии КАК ОтветственныйИнвент,
				   |	ЕСТЬNULL(ВТ_ГодМашины.машинаГод, 0) КАК машинаГод
				   ...
				   |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ГодМашины КАК ВТ_ГодМашины
				   |		ПО (ИндНомер.Ссылка = ВТ_ГодМашины.индкод)
				   |ГДЕ
				   |	РегистрНакопления1Остатки.КолвоОстаток > 0
				   ...
				   |	ВТ_данныеНоменклатур.ДатаИнвент КАК ДатаИнвент,
				   |	ВТ_данныеНоменклатур.ОтветственныйИнвент КАК ОтветственныйИнвент
				   |ИЗ
...
/// Комлев 7/8/24 +++
	...
	Для Каждого стр Из ТЗ_Товары Цикл
		...
		Если стр.ДатаИнвент <> Дата(1, 1, 1) Тогда
			СтруктураТоваров.Вставить("date_inventory", Формат(стр.ДатаИнвент, "ДФ=dd.MM.yyyy;"));
		Иначе
			СтруктураТоваров.Вставить("date_inventory", "");
		КонецЕсли;
		Если стр.ОтветственныйИнвент <> Справочники.Пользователи.ПустаяСсылка() Тогда
			Сотрудник  = Справочники.Сотрудники.НайтиПоНаименованию(стр.ОтветственныйИнвент);
			СтруктураТоваров.Вставить("worker_inventory", Строка(Сотрудник.Код));
		Иначе
			СтруктураТоваров.Вставить("worker_inventory", "");
		КонецЕсли;
		...
		Попытка
		  ...
		  //ОбъектПродажи =Документы.ПродажаЗапчастей.НайтиПоНомеру().ПолучитьОбъект();
			...
			Если НайденныеСтроки.количество() > 0 Тогда
				...
				СсылкаЗаказКлиента	= ОбъектПродажи.ЗаказКлиента;
				НомерПродажа	= ОбъектПродажи.номер;
				ОбъектПродажи.Записать(РежимЗаписиДокумента.Проведение);
				
				// +++ Этопирование заявки------------------------------------------------------------------------------
				Если ЗначениеЗаполнено(СсылкаЗаказКлиента) Тогда
					ВсеСинхронизацииПродажиИЗаявки.СинхронизоватьВыданСЗаказомКлиента(СсылкаЗаказКлиента, НомерПродажа);
				КонецЕсли; 
				// -----------------------------------------------------------------------------------------------------
...
Функция СменаCтатусаТоваровВЗНchangestateorder(Запрос)
	...
	Для Каждого элемент Из Массив.pose Цикл
		...
		Попытка
			...
			КонецЕсли;  
			//--Если установили статус: "Установлено" тогда запиписываю время
			ТекстЛога =  " В заказ-наряде " + ОбъектНаряда.номер + " сменил статус в товаре " + СтрокаТЧ.Номенклатура
				+ " " + СтрокаТЧ.Партия;
...
//			Возврат Ответ;
			...
			ЗаписатьJSON(ЗаписьJSON, РаботаССайтомWT.СформироватьСтруктуруОшибки( ,
				"Не удалось сменить статус товара!", Информация.Описание));
			СтрокаДляОтвета = ЗаписьJSON.Закрыть();
...
Функция ПолучитьТоварыЗаказНарядаgetproductsorder(Запрос)
	...
	Если Строка(Запрос.ПараметрыURL["text"]) <> "13" Тогда
		ЗапросРабот.УстановитьПараметр("СостояниеТовара", Перечисления.СтатусыТовараВЗаказНаряде.Получить(Число(
			Запрос.ПараметрыURL["text"])));
	КонецЕсли;
	/// Комлев 11/9/24 +++
	Если Строка(Запрос.ПараметрыURL["text_search"]) <> "00" Тогда
...
Функция ПолучитьСписокНарядовgetlistorders(Запрос)
	...
	Если Строка(Запрос.ПараметрыURL["text"]) <> "13" Тогда
		ЗапросНарядов.УстановитьПараметр("Состояние", Перечисления.СостоянияЗаказНаряда.Получить(Число(
			Запрос.ПараметрыURL["text"])));
	КонецЕсли;
...
Функция ПолучитьТоварGetProduct(Запрос)
	...
	ЗапросТовара = Новый Запрос;
	ЗапросТовара.Текст =  СтрШаблон(текстДляТовараСДопВт(), Формат(10000, "ЧГ="));
	...
	Если ТЗ_Товары.Количество() > 0 Тогда
		...
		//@skip-check query-in-loop
			...
			СтруктураТоваров.Вставить(" yearcar", Число(стр.машинаГод));
			//--МазинЕС 05-08-2024
			СтруктураТоваров.Вставить("rating", стр.Спрос);
			СтруктураТоваров.Вставить("name", Строка(стр.Наименование));
			СтруктураТоваров.Вставить("article", Строка(стр.Артикул));
			СтруктураТоваров.Вставить("price", стр.Цена);
			СтруктураТоваров.Вставить("comment", Строка(стр.Комментарий));
...
Функция ПолучитьЗаявкиКладовщикаGetAppStorekeeper(Запрос)
	...
	// Комлев АА 09/01/25 +++
		...
		Если ЗначениеФильтра <> 0 Тогда
			Если ЗначениеФильтра = 3 Тогда
				СтрокаФильтраСтатус = " И ЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.Завершено)";
			ИначеЕсли ЗначениеФильтра = 1 Тогда
				СтрокаФильтраСтатус = " И (ЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.НоваяЗаявка) ИЛИ ЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.ПустаяСсылка))";
			ИначеЕсли ЗначениеФильтра = 2 Тогда
				СтрокаФильтраСтатус = " И ЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.ВРаботе) ";
			ИначеЕсли ЗначениеФильтра = 4 Тогда
				СтрокаФильтраСтатус = " И ЗаказКлиента.СтатусОбработкиЗаявкиКладовщиком = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиЗаявокКладовщиком.Отменено)";
			КонецЕсли;
		КонецЕсли;
	// Комлев АА 09/01/25 ---
		...
		ЗапросЗаявок.Текст =  СтрШаблон(Текст, Формат(?(Число(Запрос.ПараметрыURL["count"]) > 0,
			Запрос.ПараметрыURL["count"], 10000), "ЧГ="), СтрокаПоиска, СтрокаФильтраСтатус);
		ЗапросЗаявок.УстановитьПараметр("Наименование", "%" + ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(
				Строка(
		Запрос.ПараметрыURL["text"])) + "%");
...
Функция ПолучитьКарточкиИзЗаявокGetProductCardsFromApplications(Запрос)
	...
	Попытка
		ТекстЗапросаКарточек = "ВЫБРАТЬ
							   ...
							   |ГДЕ
							   |	ВТ_ДанныеИнвентОстаток.Товар <> ЗНАЧЕНИЕ(Справочник.Номенклатура.пустаяСсылка)
							   |
							   |УПОРЯДОЧИТЬ ПО
							   |	Дата Убыв
							   |;
		...
		ЗапросТовара.УстановитьПараметр("НачинаяСЗаписи", 0);
		ДатаНачала = НачалоДня(ТекущаяДата() - (24 * 60 * 60) * 30);
		...
		Пока Выборка.Следующий() Цикл
			...
			СтруктураКарточки.Вставить("rating", Выборка.Спрос);
...
Функция ПроверитьНаличиеТовараCheckTheProduct(Запрос)
	/// МазинЕС-06-12-20-24	
	...
	Если МестоХранения = 0 Тогда
		...
		Исключение
			ЗаписьJSON = Новый ЗаписьJSON;
			ЗаписьJSON.УстановитьСтроку();
			ЗаписатьJSON(ЗаписьJSON, СформироватьСтруктуруОшибкиGetProduct(
				"Не удалось проставить отметку ""Товар найден""", ""));
			СтрокаДляОтвета = ЗаписьJSON.Закрыть();
			Ответ = Новый HTTPСервисОтвет(500);
			Ответ.Заголовки.Вставить("Content-type", "application/json;  charset=utf-8");
			Ответ.УстановитьТелоИзСтроки( СтрокаДляОтвета);
...
Функция ПолучитьСписокКладовщиковGetListStockman(Запрос)
	//Комлев АА 26/12/24 ++++
	Попытка
		Запрос = Новый Запрос;
		Запрос.Текст =
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
		|		ПО (Сотрудники.Пользователь.Ссылка = ПодразделенияУчастники.Сотрудник.Ссылка)
		|ГДЕ
		|	ПодразделенияУчастники.Ссылка.Код В (""000000096"", ""000000087"", ""000000133"")";
		...
		Пока Выборка.Следующий() Цикл
			СтруктураКладовщик = Новый Структура;
			СтруктураКладовщик.Вставить("name", Строка(Выборка.Наименование));
			СтруктураКладовщик.Вставить("id", Строка(Выборка.Код));
			МассивКладовщиков.Добавить(СтруктураКладовщик);
		КонецЦикла;
		ЗаписьJSON = Новый ЗаписьJSON;
		ЗаписьJSON.УстановитьСтроку();
		ЗаписатьJSON(ЗаписьJSON, МассивКладовщиков);
		СтрокаДляОтвета = ЗаписьJSON.Закрыть();
		Ответ = Новый HTTPСервисОтвет(200);
		Ответ.Заголовки.Вставить("Content-type", "application/json;  charset=utf-8");
		Ответ.УстановитьТелоИзСтроки(СтрокаДляОтвета, КодировкаТекста.UTF8, ИспользованиеByteOrderMark.НеИспользовать);
...
Функция ПроверитьДетальНаНелеквидПоНомеруПроизводителяcontol(Запрос)
	НомерПроизводителя = Запрос.ПараметрыURL["number"];
	...
	ЗапросДетали.УстановитьПараметр("НомерПроизводителя", НомерПроизводителя);
	ЗапросДетали.УстановитьПараметр("ДатаНачала", НачалоДня(ТекущаяДата() - 31536000));
	ЗапросДетали.УстановитьПараметр("ДатаОкончания", КонецДня(ТекущаяДата()));
	РезультатЗапроса = ЗапросДетали.Выполнить();
	СтруктураОтвета = Новый Структура("message", "");
	Если РезультатЗапроса.Пустой() Тогда
		Сообщение =  "Можно учесть. Детали с таким номером производителя нет.";
	...
	Если (ТекущаяДата() - Выборка.ДатаСоздания) <= (31536000 * 2) Тогда
		Сообщение = Выборка.Деталь + " Можно учесть.";
	Иначе
		Если Выборка.КолвоОстаток > Выборка.НеобходимыйОстаток И Выборка.НеобходимыйОстаток <> 0 Тогда
			Сообщение = Выборка.Деталь + ". Нельзя учесть. Количество на складе превышает необходимый запас.";

		ИначеЕсли Выборка.НеобходимыйОстаток = 0 И Выборка.КолвоОстаток < 10 Тогда
			Сообщение = Выборка.Деталь + ". Можно учесть. Необходимый запас 0, но нужно минимум 10 шт.";

		ИначеЕсли Выборка.КолвоОстаток < Выборка.НеобходимыйОстаток И Выборка.НеобходимыйОстаток <> 0 Тогда
			Сообщение = Выборка.Деталь + ". Можно учесть. Остаток на складе меньше необходимого запаса.";

		Иначе
			Сообщение = Выборка.Деталь + ". Нельзя учесть. Необходимый запас 0. А количество на складе - "
...
Функция ПолучитьСтеллажиСКоличествомПоСкладам(СсылкаНаНоменклатуру)
	...
	Иначе
		...
		СкладМСК = Новый Структура("name, places", "СкладМСК", Новый Массив);
		СкладЕКБ = Новый Структура("name, places", "СкладЕКБ", Новый Массив);
		Пока Выборка.Следующий() Цикл
			Стеллаж = Новый Структура;
			Стеллаж.Вставить("id", Строка(Выборка.Стеллаж));
			Стеллаж.Вставить("count", Выборка.КоличествоДеталей);
			Стеллаж.Вставить("count_inventory", Выборка.КоличествоИнвент);
			Если Строка(Выборка.Склад) = "" Тогда
				Продолжить;
			ИначеЕсли СокрЛП(Выборка.Склад) = "Нижний Новгород WorkTruck" Тогда
...
Функция текстДляТовараСДопВт()
	Текстзапроса = "ВЫБРАТЬ
				   ...
				   |ВЫБРАТЬ
				   |	ИндНомер.индкод КАК индкод,
				   |	ИндНомер.ДатаПроверкиЕстьВНаличии КАК ДатаПроверкиЕстьВНаличии,
				   |	ИндНомер.ОтветственныйЕстьВНаличии КАК ОтветственныйЕстьВНаличии
				   |ПОМЕСТИТЬ ВТ_предКоды
				   |ИЗ
				   |	РегистрСведений.ИндНомер КАК ИндНомер
				   |ГДЕ
				   ...
				   |		ПО ИндНомер.индкод = РегИндНомер.индкод
				   |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Спрос КАК ВТ_Спрос
				   |		ПО ИндНомер.индкод.Владелец = ВТ_Спрос.Номенклатура
				   |{ГДЕ
				   |	(РегИндНомер.АвитоЧастник = &Ач) КАК Поле2}
				   |;
				   |
				   ...
				   |	ВТ_данныеНоменклатур.ОтветственныйЕстьВНаличии КАК ОтветственныйЕстьВНаличии,
				   |	ВТ_данныеНоменклатур.Спрос КАК Спрос
				   |ИЗ
				   |	ВТ_данныеНоменклатур КАК ВТ_данныеНоменклатур
				   |ГДЕ
				   |	ВТ_данныеНоменклатур.НомерЗаписи >= &НачинаяСЗаписи
...
Функция текстДляЗаявки()
	Текст = "ВЫБРАТЬ
			...
			|		Номенклатура.Код КАК Код,
			|		Выдан КАК Выдан,
			|		Заменить КАК Заменить) КАК Товары,
			...
			|	ЗаказКлиентас.ЭтапыПродажи КАК ЭтапыПродажи
			|ИЗ
...
Функция ПроверкаВыдачиТовараВПродаже(НомерПродажи)

	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПродажаЗапчастейТаблица.СтатусТовара КАК Выдан,
	|	ПродажаЗапчастейТаблица.Заменить КАК Заменить
	|ИЗ
	|	Документ.ПродажаЗапчастей.Таблица КАК ПродажаЗапчастейТаблица
	|ГДЕ
	|	ПродажаЗапчастейТаблица.Ссылка.Номер = &НомерПродажи";

	Запрос.УстановитьПараметр("НомерПродажи", НомерПродажи);
	...
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	КолТоваровНеВыданы	 = 0;
	КолТоварыЗаменить	 = 0;

	Результат = "";
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл

		Если Булево(ВыборкаДетальныеЗаписи.Выдан) = Ложь Тогда
			КолТоваровНеВыданы = КолТоваровНеВыданы + 1;
		КонецЕсли;

		Если Булево(ВыборкаДетальныеЗаписи.Заменить) = Истина Тогда
			КолТоварыЗаменить = КолТоварыЗаменить + 1;
		КонецЕсли;

	КонецЦикла;

	Если КолТоваровНеВыданы = 0 И КолТоварыЗаменить = 0 Тогда
		Результат  = "Выдан";
	ИначеЕсли КолТоварыЗаменить > 0 Тогда
		Результат  = "Есть замена";
	Иначе
		Результат = "";
	КонецЕсли;
...
Функция ПроверкаЕстьЗаменаВЗаявке(Ссылка)

	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗаказКлиентаТовары.Заменить КАК Заменить
	|ИЗ
	|	Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
	|ГДЕ
	|	ЗаказКлиентаТовары.Ссылка = &Ссылка";
	...
	Если КолТоварыЗаменить > 0 Тогда
		Результат  = "Есть замена";
	Иначе
		Результат = "";
	КонецЕсли;
