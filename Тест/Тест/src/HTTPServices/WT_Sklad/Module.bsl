		...
		Если ЭтоДеталь Тогда
			...
			Если СтрНайти(Строка(Покупка), "_") > 0 Тогда
				...
				Если Не ПартияУжеЕсть Тогда
					...
					НайденныяСтрока = ОбъектЗаявки.Товары.Найти("Партия", Справочники.ИндКод.НайтиПоНаименованию(Покупка)); ;
					
					Если НайденныяСтрока > 0 Тогда
						ОбъектЗаявки.Товары[НайденныяСтрока].Партия = Справочники.ИндКод.НайтиПоНаименованию(Покупка);
						ТекстЛога =  " В заявке " + ОбъектЗаявки.номер + " установил партию в товаре "
							+ ОбъектЗаявки.Товары[НайденныяСтрока].Номенклатура + " " + ОбъектЗаявки.Товары[НайденныяСтрока].Партия;
