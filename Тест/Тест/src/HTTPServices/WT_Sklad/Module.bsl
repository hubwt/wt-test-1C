			...
			Если СтрНайти(Строка(Покупка), "_") > 0 Тогда
				...
				Если Не ПартияУжеЕсть Тогда
					...
					Если НайденныяСтрока <> Неопределено Тогда
						...
						// +++ Этопирование заявки
						Этап = ЭтапированиеЗаявки.ЭтапированиеЗаявки(ОбъектЗаявки); 
						Если ЗначениеЗаполнено(Этап) Тогда 
							ОбъектЗаявки.ЭтапыПродажи = Этап; 
						КонецЕСли;
						// -------------------------------------------------------------------------------------------------
	...
	Попытка
		...
		Если СтрНайти(Строка(Замена), "_") > 0 Тогда
			...
			Номенклатура = Партия.Владелец;
				...
				ЛогированиеWT10(Выборка.ссылка, Автор, ТекстЛога);
					// +++ Этопирование заявки--------------------------------------------------------------------------
					Если ЗначениеЗаполнено(ОбъектЗаявки.Продажа) тогда 
						СинхронизацияТЧЗаявкиИПродажиSklad.ОбновитьПродажу(ОбъектЗаявки);
					КонецЕсли; 
					Этап = ЭтапированиеЗаявки.ЭтапированиеЗаявки(ОбъектЗаявки); 
					Если ЗначениеЗаполнено(Этап) Тогда 
						ОбъектЗаявки.ЭтапыПродажи = Этап; 
		...
		Попытка
		  ...
		  //ОбъектПродажи =Документы.ПродажаЗапчастей.НайтиПоНомеру().ПолучитьОбъект();
			...
			Если НайденныеСтроки.количество() > 0 Тогда
				...
				// +++ Этопирование заявки------------------------------------------------------------------------------
					Если ЗначениеЗаполнено (ОбъектПродажи.ЗаказКлиента) тогда 
						ОбъектЗаявки = ОбъектПродажи.ЗаказКлиента.ПолучитьОбъект(); 
						СинхронизацияТЧЗаявкиИПродажиSklad.ОбновитьЗаявку(ОбъектЗаявки);
						Этап = ЭтапированиеЗаявки.ЭтапированиеЗаявки(ОбъектЗаявки); 
						Если ЗначениеЗаполнено(Этап) Тогда 
							ОбъектЗаявки.ЭтапыПродажи = Этап; 
							ОбъектЗаявки.Записать(РежимЗаписиДокумента.Запись); 
			...
			Иначе
				...
				//@skip-check query-in-loop
					...
					ОбъектПродажи.Записать(РежимЗаписиДокумента.Проведение);
					// +++ Этопирование заявки--------------------------------------------------------------------------
						СинхронизацияТЧЗаявкиИПродажиSklad.ОбновитьПродажу(Заказ);
						Этап = ЭтапированиеЗаявки.ЭтапированиеЗаявки(Заказ); 
						Если ЗначениеЗаполнено(Этап) Тогда 
							Заказ.ЭтапыПродажи = Этап; 
						КонецЕСли;
					// -------------------------------------------------------------------------------------------------
