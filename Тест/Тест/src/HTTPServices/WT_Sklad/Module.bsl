...
Функция ПроверитьДетальНаНелеквидПоНомеруПроизводителяcontol(Запрос)
	...
	Попытка
		...
		ЗапросДетали.Текст = дт_Склад.ТекстЗапросаПоискНоменклатурыПоНомеруПроизводителяИлиНомеруЗамены();
		...
		Выборка.Следующий();
		СтруктураОтвета.name = Строка(Выборка.Деталь);
		СтруктураОтвета.id = Выборка.Код;
	...
	Исключение 
		...
		ЗаписьJSON = Новый ЗаписьJSON;
		ЗаписьJSON.УстановитьСтроку();
		ЗаписатьJSON(ЗаписьJSON, СтрокаОшибки);
		СтрокаДляОтвета = ЗаписьJSON.Закрыть();
		Ответ = Новый HTTPСервисОтвет(500);
		Ответ.Заголовки.Вставить("Content-type", "application/json;  charset=utf-8");
		Ответ.УстановитьТелоИзСтроки( СтрокаДляОтвета);
