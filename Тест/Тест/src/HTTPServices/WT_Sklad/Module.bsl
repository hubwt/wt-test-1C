...
//ПодСостояниеСборки = Новый Массив;
	...
	Попытка
		...
		Текст = "ВЫБРАТЬ
				...
				|ВТ_ДанныеЗаявки.СтатусОбработкиЗаявкиКладовщикомПорядок КАК СтатусОбработкиЗаявкиКладовщикомПорядок
				|ИЗ
				|	ВТ_ДанныеЗаявки КАК ВТ_ДанныеЗаявки
				|ГДЕ
				|	ВТ_ДанныеЗаявки.НомерЗаписи >= &НачинаяСЗаписи
...
//	КонецЕсли;
	...
	//ЗапросЗаявок.УстановитьПараметр("ПодстатусОбработки", ПодСостояниеСборки);
		...
		Для Каждого Результат Из СписокПоиска Цикл
			...
			СтрукткраСтатуса.Вставить("state_number", Число(Результат.СтатусОбработкиЗаявкиКладовщикомПорядок));
