...
Функция ПереместитьПоддонНаПолкуМетод(Запрос)
	...
	Автор = массив.author;
	Поддон = Справочники.Поддоны.НайтиПоКоду(Массив.pallet);
	Стеллаж = Справочники.Стеллаж.НайтиПоКоду(Массив.shelf);
	...
	Если Справочники.Поддоны.НайтиПоКоду(Массив.pallet) <> Справочники.Поддоны.ПустаяСсылка() Тогда
		Запрос.УстановитьПараметр("Поддон", Справочники.Поддоны.НайтиПоКоду(Массив.pallet));
	Иначе
		Ответ = Новый HTTPСервисОтвет(415);
		Ответ.Заголовки.Вставить("Content-Type", "application/json; charset=utf-8");
		Ответ.УстановитьТелоИзСтроки("Поддон не корректен!");
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		Пока РезультатЗапроса.Следующий() Цикл
			...
			Если ЗаписьВРегистреСведений.Выбран() Тогда
				...
				ЛогированиеИндНомера(Строка(РезультатЗапроса.индкод), Автор, Лог, Стеллаж, ,
...
Функция ПолучитьСписокТоваровНаПоддонеGetListProductsOnPallet(Запрос)
	...
	|	И ЛогИндНомера.Поддон.Код = &Код
	|	И ЛогИндНомера.Ответственный <> &Ответственный
	|СГРУППИРОВАТЬ ПО
	|	ЛогИндНомера.Ответственный";

	ЗапросСотрудники.УстановитьПараметр("Код", Строка(Запрос.ПараметрыURL["id"]));
...
Функция ПолучитьСписокТоваровНаСтеллажеGetListProductsOnShelf(Запрос)
	...
	|ГДЕ
	|	РегистрНакопления1Остатки.КолвоОстаток > 0
	|	И ЛогИндНомера.Стеллаж.Код = &Код
	|	И ЛогИндНомера.Ответственный <> &Ответственный
	|СГРУППИРОВАТЬ ПО
	|	ЛогИндНомера.Ответственный";
