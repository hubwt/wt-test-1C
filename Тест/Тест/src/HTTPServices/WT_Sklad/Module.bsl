...
Функция ПолучитьКарточкиИзЗаявокGetProductCardsFromApplications(Запрос)
	...
	Попытка
		...
		|ГДЕ
		|	РегистрНакопления1Остатки.КолвоОстаток > 0
		|	И ИндНомер.индкод.Владелец В
		|		(ВЫБРАТЬ
		|			ДаныеИзЗаявки.Номенклатура
		|		ИЗ
		|			ДаныеИзЗаявки КАК ДаныеИзЗаявки)
		|
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДаныеИзЗаявки.Номенклатура КАК Номенклатура,
		|	МАКСИМУМ(ДаныеИзЗаявки.НоменклатураКод) КАК Код,
		|	МАКСИМУМ(ДаныеИзЗаявки.Номер) КАК НомерЗаявки,
		|	МАКСИМУМ(ДаныеИзЗаявки.Дата) КАК ДатаЗаявки,
		|	СУММА(ВЫБОР
		|		КОГДА ДанныеИзРегистров.ДатаПроверкиЕстьВНаличии <> ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ) КАК КоличесствоИнвент,
		|	СУММА(ВЫБОР
		|		КОГДА ДанныеИзРегистров.ДатаПроверкиЕстьВНаличии = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ) КАК КоличесствоНеИнвент,
		|	КОЛИЧЕСТВО(ДанныеИзРегистров.индкод) КАК ОбщееКоличество,
		|	АВТОНОМЕРЗАПИСИ() КАК НомерЗаписи
		|ПОМЕСТИТЬ ТоварыСКолчествоИнвент
		|ИЗ
		|	ДаныеИзЗаявки КАК ДаныеИзЗаявки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеИзРегистров КАК ДанныеИзРегистров
		|		ПО (ДанныеИзРегистров.Товар = ДаныеИзЗаявки.Номенклатура)
		|СГРУППИРОВАТЬ ПО
		|	ДаныеИзЗаявки.Номенклатура
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ %1
		|	ТоварыСКолчествоИнвент.Номенклатура КАК Номенклатура,
		|	ТоварыСКолчествоИнвент.Код КАК Код,
		|	ТоварыСКолчествоИнвент.НомерЗаявки КАК НомерЗаявки,
		|	ТоварыСКолчествоИнвент.ДатаЗаявки КАК ДатаЗаявки,
		|	ТоварыСКолчествоИнвент.КоличесствоИнвент КАК КоличесствоИнвент,
		|	ТоварыСКолчествоИнвент.КоличесствоНеИнвент КАК КоличесствоНеИнвент,
		|	ТоварыСКолчествоИнвент.ОбщееКоличество КАК ОбщееКоличество,
		|	ТоварыСКолчествоИнвент.НомерЗаписи КАК НомерЗаписи
		|ИЗ
		|	ТоварыСКолчествоИнвент КАК ТоварыСКолчествоИнвент
		|ГДЕ
		|	ТоварыСКолчествоИнвент.НомерЗаписи >= &НачинаяСЗаписи
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерЗаписи";
		...
		ЗапросТовара.УстановитьПараметр("НачинаяСЗаписи", 0);
		ДатаНачала = ТекущаяДата() - (24 * 60 * 60) * 10;
		...
		Пока Выборка.Следующий() Цикл
			...
			СтруктураКарточки.Вставить("sklads", ПолучитьМассивСкладов(Выборка.Номенклатура));
			СтруктураКарточки.Вставить("places", ПолучитьСтеллажиСКоличеством(Выборка.Номенклатура));
			СтруктураКарточки.Вставить("all_count", Выборка.ОбщееКоличество);
			СтруктураКарточки.Вставить("accounting_count", Выборка.КоличесствоИнвент);
...
Функция ПолучитьСтеллажиСКоличеством(СсылкаНаНоменклатуру)
	// Комлев АА ++++
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			СртуктураСтеллажа = Новый Структура;
			СртуктураСтеллажа.Вставить("id", Строка(Выборка.Стеллаж));
			СртуктураСтеллажа.Вставить("count", Выборка.КоличествоДеталей);
			МассивСтеллажей.Добавить(СртуктураСтеллажа);
		КонецЦикла;
		Возврат МассивСтеллажей;
	КонецЕсли;
	// Комлев АА --- 
КонецФункции
...
Функция ПолучитьМассивСкладов(СсылкаНаНоменклатуру)
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			МассивСкладов.Добавить(Строка(Выборка.Склад));
		КонецЦикла;
