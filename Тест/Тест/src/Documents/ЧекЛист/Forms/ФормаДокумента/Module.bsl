...
Процедура ЗаблокироватьЭлементы()
	/// Комлев АА 28/11/24 +++
	Если Объект.ЗаблокированоДляИзменения = Истина Тогда
		...
		КонецЦикла;
	ИначеЕсли Объект.ЗаблокированоДляИзменения = Ложь Тогда
		Для Каждого Элемент Из Элементы Цикл
			Если ТипЗнч(Элемент) <> Тип("ДекорацияФормы") Тогда
...
&НаКлиенте
Процедура ЗаблокированоДляИзмененияПриИзменении(Элемент)
	ЗаблокироватьЭлементы();
	ПоказатьКнопкуБлокировки();
КонецПроцедуры

Процедура ПоказатьКнопкуБлокировки()
	Если ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Трофимов Василий Юрьевич"
		Или ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(
		Метаданные.Роли.Найти("дт_ПолныеПрава")) Тогда
		Элементы.ЗаблокированоДляИзменения.Доступность = Истина;
	КонецЕсли;
КонецПроцедуры
