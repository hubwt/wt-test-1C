...
Процедура СписокНоменклатураПартияПриИзмененииСервер(НомерСтроки)
	...
	Если ДанныеПоИнкоду <> Неопределено 
		И  ДанныеПоИнкоду.Машина <> Справочники.Машины.НайтиПоКоду("000000008") 
		И ДанныеПоИнкоду.Машина <> Справочники.Машины.НайтиПоКоду("000000204") 
		И  ДанныеПоИнкоду.Машина <> Справочники.Машины.НайтиПоКоду("000000200") Тогда
		...
		НашаСтрока.Партия);
		ЦенаДетали = ПолучитьЦенуДетали(НашаСтрока.партия);
		НашаСтрока.ЦенаДетали 		= ?(ЦенаДетали <> 0, ЦенаДетали, ДанныеПоИнкоду.ТоварРекомендованаяЦена);
	Иначе
...
Функция ПолучитьДанныеИзПоступленияЗапчастей(Партия)
	...
	|	ПоступлениеЗапчастейТаблица.Ссылка.Склад КАК Склад
	|ИЗ
