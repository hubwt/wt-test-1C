...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("ПолныеПрава")) Тогда
		Элементы.НоменклатураКод.Видимость = Истина;
		Элементы.Машина.Видимость = Истина;
	КонецЕсли;
...
Процедура СписокНоменклатураПриИзменении(Элемент)
	Если Объект.НоменклатураСписок.Количество() <> 0 Тогда
		ТекСтрока = Элементы.СписокНоменклатура.ТекущиеДанные;
		...
		КонецЕсли;
		Элементы.СписокНоменклатура.ТекущиеДанные.Место = ПолучитьМестоХранения(
			Элементы.СписокНоменклатура.ТекущиеДанные.Номенклатураспис, Элементы.СписокНоменклатура.ТекущиеДанные.Склад);
	КонецЕсли;
...
//	ЦенаИндкода = ПолучитьЦенуДетали(ТекСтрока.Партия);
//	ТекСтрока.ЦенаДетали = ЦенаИндкода;
	
КонецПроцедуры

&НаСервере
Процедура СписокНоменклатураПартияПриИзмененииСервер(НомерСтроки)
   
	НашаСтрока = объект.НоменклатураСписок[НомерСтроки-1];
	ДанныеПоИнкоду 		   = ПолучитьДанныеПоИнкоду(НашаСтрока.партия);
	// Комлев АА 18/12/24 +++
	// На новые детали брать цену из поступления запчастей. Цену закупки
	Если ДанныеПоИнкоду <> Неопределено И  ДанныеПоИнкоду.Машина <> Справочники.Машины.НайтиПоКоду("000000008") И 
	ДанныеПоИнкоду.Машина <> Справочники.Машины.НайтиПоКоду("000000204") И  ДанныеПоИнкоду.Машина <> Справочники.Машины.НайтиПоКоду("000000200") Тогда
		НашаСтрока.Номенклатураспис = ДанныеПоИнкоду.Товар;
		НашаСтрока.Количество = 1;
		НашаСтрока.Автомобиль       = ДанныеПоИнкоду.машина;
		НашаСтрока.Склад		    = ДанныеПоИнкоду.Склад;
		НашаСтрока.Комментарий      = ПолучитьКомментарийИзРегистра(НашаСтрока.Автомобиль, НашаСтрока.Номенклатураспис,
			НашаСтрока.Партия);
		НашаСтрока.ЦенаДетали 		= ?(ПолучитьЦенуДетали(НашаСтрока.партия) <> 0, ПолучитьЦенуДетали(
			НашаСтрока.партия), ДанныеПоИнкоду.РекомендованнаяЦена);
	Иначе
		ДанныеПоИнКоду = ПолучитьДанныеИзПоступленияЗапчастей(НашаСтрока.партия);
		НашаСтрока.Номенклатураспис = ДанныеПоИнкоду.Товар;
		НашаСтрока.Количество = 1;
		НашаСтрока.Автомобиль       = ДанныеПоИнкоду.Машина;
		НашаСтрока.Склад		    = ДанныеПоИнкоду.Склад;
		НашаСтрока.Комментарий      = ПолучитьКомментарийИзРегистра(НашаСтрока.Автомобиль, НашаСтрока.Номенклатураспис,
			НашаСтрока.Партия);
		НашаСтрока.ЦенаДетали 		= ДанныеПоИнкоду.Цена;
	КонецЕсли;
	// Комлев АА 18/12/24 ---
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьДанныеИзПоступленияЗапчастей(Партия)
	// Комлев АА 18/12/24 ---
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПоступлениеЗапчастейТаблица.Товар КАК Товар,
	|	ПоступлениеЗапчастейТаблица.ЦенаПоступления КАК Цена,
	|	ПоступлениеЗапчастейТаблица.Автомобиль КАК Машина,
	...
	|		ПО (ИндНомер.индкод = ПоступлениеЗапчастейТаблица.Партия)
	|ГДЕ
	|	ПоступлениеЗапчастейТаблица.Партия = &Партия";
...
&НаСервереБезКонтекста
Функция ПолучитьЦенуДетали(Индкод)
...
Функция ПолучитьДанныеПоИнкоду(Индкод)
	...
	Запрос.Текст = 
		...
		|	РегистрНакопления1Остатки.Товар.РекомендованаяЦена
		|ИЗ
...
Процедура РаботаПриИзменении(ТекущаяСтрока, ЗаполнитьНормуВремени = Ложь) Экспорт
	...
	// Комлев АА 08/03/25 +++
	СтруктураСтроки.Цена = ПолучитьПочасовуюСтавкуНаРаботы(Объект.Дата, 240);
	// Комлев АА 08/03/25 ---
...
&НаСервереБезКонтекста
Функция ПолучитьПочасовуюСтавкуНаРаботы(Дата, ЗначениеПоУмолчанию) 
	Срез = РегистрыСведений.ЧастоваяСтавкаРаботВостановлениеДетали.ПолучитьПоследнее(Дата);
	Если Срез.Цена <> 0 Тогда
		Возврат Срез.Цена; 
	Иначе 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//КонецЕсли;
	///-ГомзМА 15.08.2023
	//СсылкаНаВостановлениеНаСервере();
	
	УстановитьПризнакКлиентскойДетали();
	
	
КонецПроцедуры


Процедура УстановитьПризнакКлиентскойДетали() 
	
	///+ТатарМА 23.09.2024
	Если Объект.Принадлежность = Перечисления.ПринадлежностьДетали.ДетальКлиента Тогда
		ЗаписьВРегистреСведений 		= РегистрыСведений.ИндНомер.СоздатьМенеджерЗаписи();
		ЗаписьВРегистреСведений.индкод 	= Объект.Инкод;
		ЗаписьВРегистреСведений.Прочитать();
		Если ЗаписьВРегистреСведений.Выбран() Тогда
			ЗаписьВРегистреСведений.ДетальКлиента = Истина;
...
&НаСервере
Процедура СсылкаНаВостановлениеНаСервере()
	
	
	Если Объект.Проект.Пустая() Тогда 
Иначе
	
		Ссылка1 = Объект.Проект;
		СсылкаЗаказНаряд = Ссылка1.ПолучитьОбъект();	
		СсылкаЗаказНаряд.ЗаказНаряд = Объект.Ссылка;
		СсылкаЗаказНаряд.Записать();
		
	 	//СсылкаНаЗаказНаряд = Объект.Проект.ПолучитьОбъект(); 
		//СсылкаНаЗаказНаряд.ЗаказНарядСсылка = Объект.Ссылка;
		//СсылкаНаЗаказНаряд.Записать();
	КонецЕсли;
