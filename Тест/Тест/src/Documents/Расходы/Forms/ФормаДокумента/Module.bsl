...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СсылкаТекПользователя.Код = "000000018"  Тогда 
		Элементы.ДляКонструированияРасходов.Видимость = Истина; 
	Иначе Элементы.ДляКонструированияРасходов.Видимость = Ложь;
	КонецЕсли; 
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	//Волков ИО 16.01.24 ++ 
	Элементы.Партия.Видимость = Ложь;
	ВидРасходаЗакупкаЕвропаИРоссия();
	//Волков ИО 16.01.24 -- 
	// Конец СтандартныеПодсистемы.ПодключаемыеКоманды
	...
	КонецЕсли; 
	Если НЕ ЗначениеЗаполнено(Объект.ДатаРасхода) Тогда 
			Объект.ДатаРасхода = объект.Дата; 
	КонецЕсли; 
КонецПроцедуры
...
Процедура ВидРасходаЗакупкаЕвропаИРоссия()
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	///+ТатарМА 02.10.2024
	Если Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.Зарплата Тогда
		Объект.Зарплата = Истина;
	КонецЕсли;
	///-ТатарМА 02.10.2024
КонецПроцедуры
