		...
		//+++ МазинЕС 20-11-24
			...
			Если ЗначениеЗаполнено(ЗаявкаНаРасход) Тогда 
				...
				Запрос.Текст =
					...
					|ГДЕ
					|	ПоступлениеТМЦВорктрак.РаспоряжениенаРасход = &РаспоряжениенаРасход";
				
				Запрос.УстановитьПараметр("РаспоряжениенаРасход", ЗаявкаНаРасход);
				...
				Если 	НЕ ЗначениеЗаполнено(ПостублениеСсылка) Тогда 
			
					
							Запрос = Новый Запрос;
							Запрос.Текст =
								"ВЫБРАТЬ
								|	БюджетТМЦ.ИнвентарныйНомер КАК ИНвентарныйТМЦ
								|ИЗ
							...
							Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
								МассивИнкодовТМЦ.Добавить(ВыборкаДетальныеЗаписи.ИНвентарныйТМЦ);
							КонецЦикла;
