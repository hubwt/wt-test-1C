...
//МАЗИН----------------------------КОНЕЦ
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ПоступлениеЗапчастей.СуммаНакладной КАК СуммаНакладнойПоступление,
		|	ПоступлениеЗапчастей.Ссылка КАК СсылкаНакладнойПоступление
		|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Соответствие.Вставить("СуммаНакладнойПоступление",ВыборкаДетальныеЗаписи.СуммаНакладнойПоступление);
		Соответствие.Вставить("СсылкаНакладнойПоступление",ВыборкаДетальныеЗаписи.СсылкаНакладнойПоступление);
	КонецЦикла;

	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СУММА(Расходы.Сумма) КАК СуммаРасходы
		|ИЗ
		|	Документ.Расходы КАК Расходы
		|ГДЕ
		|	Расходы.Основание = &Основание";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Соответствие.Вставить("СуммаРасходы",ВыборкаДетальныеЗаписи.СуммаРасходы);
	КонецЦикла;

	Попытка
     СсылкаПост = Соответствие.СсылкаНакладнойПоступление; 
     ОбъектПост = СсылкаПост.ПолучитьОбъект(); 
     
     Если Соответствие.СуммаРасходы >= Соответствие.СуммаНакладнойПоступление Тогда    
     ОбъектПост.ЦветСписка = ложь; 
     ОбъектПост.Записать(); 
     Иначе 
     	   ОбъектПост.ЦветСписка = истина; 
     	ОбъектПост.Записать(); 
     КонецЕсли; 
	Исключение 
