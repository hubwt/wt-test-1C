		...
		// Комлев АА 02/12/24 +++
		 Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗакупкаТовара") Тогда
		 	...
		 	Для каждого СтрокаТЧ Из ДанныеЗаполнения.ЗакупленныйТовар Цикл
		 		...
		 		НоваяСтрокаТЧ.НомерПроизводителя = СтрокаТЧ.Артикул;
		 		НоваяСтрокаТЧ.Количество = СтрокаТЧ.Количество;
		 		НоваяСтрокаТЧ.Цена = СтрокаТЧ.СуммаЗакупки / НоваяСтрокаТЧ.Количество;
		 		НоваяСтрокаТЧ.Сумма = СтрокаТЧ.СуммаЗакупки;
		 	КонецЦикла;
		 КонецЕсли;
		// Комлев АА  02/12/24 ---	
КонецПроцедуры
