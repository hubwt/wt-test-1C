&НаКлиенте
Процедура ТМЦТоварПриИзменении(Элемент)
	ТекСтрока = Элементы.ТМЦ.ТекущиеДанные;
	Если НЕ ЗначениеЗаполнено(ТекСтрока.Количество) Тогда
		ТекСтрока.Количество = 1;
	КонецЕсли;

//	Код = РаботаСДокументамиТМЦВызовСервера.ПолучениеКодаТМЦ(ТекСтрока.Товар);
//	ТекСтрока.Код = Код;
//	ТекСтрока.Код = ТекСтрока.Товар.Код; 
	
	ОбработкаИзмененияСтроки("ТМЦ", "Цена");
	
	ТекСтрока.ТипУчетаТМЦ 	= СписокТМЦТМЦПриИзмененииНаСервере();
	ТекСтрока.Склад			= ТекСтрокаТМЦСклад();
КонецПроцедуры


&НаКлиенте
Процедура ИнвентарныеНомераТМЦВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
		///+ГомзМА 22.05.2023
	ТекСтрока = Элементы.ИнвентарныеНомераТМЦ.ТекущиеДанные;
	Если Поле = Элементы.ИнвентарныеНомераТМЦСсылка Тогда
...
&НаСервере
Функция ТекСтрокаТМЦСклад()

	Возврат Справочники.МестаХраненияТМЦ.НайтиПоКоду("000000001");
...
&НаСервере
Функция СписокТМЦТМЦПриИзмененииНаСервере()

	Возврат Перечисления.ТипУчетаТМЦ.УчестьКомплектом;
...
Процедура ПриОткрытии(Отказ)
	...
	ИнвентарныеНомераТМЦ.Параметры.УстановитьЗначениеПараметра("ДокументПоступления", Объект.Ссылка);
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Для каждого СтрокаТЧ из Объект.Таблица Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.Ответственный) Тогда
			СтрокаТЧ.Ответственный = Объект.Ответственный;
		КонецЕсли;
	КонецЦикла;
	/// Комлев 12/08/24 --- 
КонецПроцедуры
...
Процедура ОткрытьНаСайте(Команда)
	ОбщДействия.ПоискКодаНаСтаромСайте(Элементы.Таблица.ТекущиеДанные.Код);
КонецПроцедуры
...
//
&НаКлиенте
Процедура УстановитьВидимостьИДоступность() ЭКСПОРТ
	
	УчетПоСкладам = дт_ОбщегоНазначенияКлиентСервер.УчетПоСкладам(Объект.Дата);
	ИспользоватьРазборку = ЭтотОбъект.ИспользоватьРазборку; 
	ЭтоНовыеЗапчасти = (Объект.Новые Или объект.Перекуп); //ЗначениеЗаполнено(Объект.Поставшик);
	ИспользуетсяСтараяСхемаУчета = ЭтотОбъект.ИспользуетсяСтараяСхемаУчета;
	
	// Шапка
	Элементы.ПолеНовые.Видимость = ИспользоватьРазборку; // ИспользоватьРазборку
	...
	Элементы.Поставшик.Видимость = ЭтоНовыеЗапчасти;
	
	Элементы.ТаблицаЗаполнитьПартии.Видимость = НЕ ИспользуетсяСтараяСхемаУчета;
	
	// Таблица
	Элементы.ТаблицаМестоХранения.Видимость = ЭтоНовыеЗапчасти; 
	...
	Элементы.ТаблицаАвтомобиль.Видимость = (НЕ ИспользуетсяСтараяСхемаУчета);
...
Функция ПроверкаНаДублиВТаблицеТМЦ()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПоступлениеЗапчастейТМЦ.Товар КАК Товар,
		|	КОЛИЧЕСТВО(ПоступлениеЗапчастейТМЦ.НомерСтроки) КАК НомерСтроки
		|ИЗ
		|	Документ.ПоступлениеЗапчастей.ТМЦ КАК ПоступлениеЗапчастейТМЦ
		|ГДЕ
		|	ПоступлениеЗапчастейТМЦ.Ссылка = &Ссылка1
		|
		|СГРУППИРОВАТЬ ПО
		|	ПоступлениеЗапчастейТМЦ.Товар
		|
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО(ПоступлениеЗапчастейТМЦ.НомерСтроки) > 1";
	
	Запрос.УстановитьПараметр("Ссылка1", Объект.Ссылка);
	РезультатЗапроса = Запрос.Выполнить();
...
КонецФункции // ()
&НаКлиенте
Процедура ЗаполнитьПартииТМЦ(Команда)
		
		ИнвентарныеНомераТМЦ.Параметры.УстановитьЗначениеПараметра("ДокументПоступления", Объект.Ссылка);
		ЗаполнитьПартииТМЦНаСервере (); 
		ИнвентарныеНомераТМЦ.Параметры.УстановитьЗначениеПараметра("ДокументПоступления", Объект.Ссылка);
...
&НаСервере
Процедура ЗаполнитьПартииТМЦНаСервере ()
	
	
	
	Если ЭтаФорма.Модифицированность ИЛИ НЕ ЗначениеЗаполнено (Объект.Ссылка) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Получать инвентарные номера можно только ПОСЛЕ сохранения документа!!!";
		Сообщение.Сообщить(); 
		Возврат; 
	КонецЕсли;
	...
	Если ПроверкаНаДублиВТаблицеТМЦ() Тогда
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.текст = " В таблице Запчасти ТМЦ есть дубли ";
		Сообщение.Сообщить();
		Возврат; 
	...
	Если Объект.Проведен тогда 
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Получать инвентарные номера можно только у непроведенного документа!!!";
		Сообщение.Сообщить(); 
	КонецЕсли; 
	
	Если НЕ Объект.Проведен тогда  		
		
		УдолитьИнвентарныеНомераПередзаписью();
		
		Для каждого СтрокаСписокТМЦ Из объект.ТМЦ Цикл 
			ЗаполнитьИнвентарныеНомераНаСервере(Ложь, СтрокаСписокТМЦ);
		КонецЦикла; 
...
Функция УдолитьИнвентарныеНомераПередзаписью()
...
//	КоличествоИнвНомеровПоТМЦ = ПолучитьКоличествоИнвентарныхНомеровПоТМЦ(СтрокаСписокТМЦ);
	...
	Если СтрокаСписокТМЦ.Количество = 0 Тогда
		Если СтрокаСписокТМЦ.ТипУчетаТМЦ = Перечисления.ТипУчетаТМЦ.УчестьПоштучно Тогда
				ДанныеЗаполнения.Вставить("ТМЦ",  			СтрокаСписокТМЦ.Товар);
				ДанныеЗаполнения.Вставить("Цена", 			СтрокаСписокТМЦ.Цена);
				ДанныеЗаполнения.Вставить("ТМЦПоНакладной", СтрокаСписокТМЦ.Товар);
				НовыйКод = Справочники.ИнвентарныеНомера.СоздатьНовыйЭлемент(ДанныеЗаполнения, Отказ); 
		ИначеЕСЛИ СтрокаСписокТМЦ.ТипУчетаТМЦ = Перечисления.ТипУчетаТМЦ.УчестьКомплектом Тогда 
			ДанныеЗаполнения.Вставить("ТМЦ",  			СтрокаСписокТМЦ.Товар);
			ДанныеЗаполнения.Вставить("Цена", 			СтрокаСписокТМЦ.Цена);
			ДанныеЗаполнения.Вставить("ТМЦПоНакладной", СтрокаСписокТМЦ.Товар);
			НовыйКод = Справочники.ИнвентарныеНомера.СоздатьНовыйЭлемент(ДанныеЗаполнения, Отказ); 
	...
	Если СтрокаСписокТМЦ.Количество > 0  Тогда
		...
		Если СтрокаСписокТМЦ.ТипУчетаТМЦ = Перечисления.ТипУчетаТМЦ.УчестьКомплектом Тогда
			ДанныеЗаполнения.Вставить("ТМЦ",  			СтрокаСписокТМЦ.Товар);
			ДанныеЗаполнения.Вставить("Цена", 			СтрокаСписокТМЦ.Цена);
			ДанныеЗаполнения.Вставить("ТМЦПоНакладной", СтрокаСписокТМЦ.Товар);
			НовыйКод = Справочники.ИнвентарныеНомера.СоздатьНовыйЭлемент(ДанныеЗаполнения, Отказ); 
...
Функция ПолучитьКоличествоИнвентарныхНомеровПоТМЦ(СтрокаСписокТМЦ)
	...
	Запрос.УстановитьПараметр("ДокументПоступления", Объект.Ссылка);
	Запрос.УстановитьПараметр("Владелец", 			 СтрокаСписокТМЦ.Товар);
...
Процедура УстановитьОбщуюСумму()
	...
	//++МазинЕС 
	Объект.СуммаНакладной = Объект.СуммаНакладной +  Объект.ТМЦ.Итог("Сумма"); 
	//--МазинЕС
КонецПроцедуры
...
&НаКлиенте
Процедура ПолеНовыеПриИзменении(Элемент) 
		ПроверкаПоставщика();	
	///+ГомзМА 27.12.2022 (Задача №000002209 от 10.11.2022)
//	Если Объект.Перекуп тогда
//		Объект.Новые = Ложь;
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаЗаказНаряды Тогда
		СписокЗаказНарядов.Параметры.УстановитьЗначениеПараметра("Ссылка", Объект.ссылка);
		элементы.СписокЗаказНарядов.Обновить();
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция ЗаполнитьМашинуВТЧТаблица()
	
	Таблица = Объект.Таблица;
	Для каждого Строка Из Таблица Цикл
		
		Состояние = ПроставитьМашинуВТЧТаблица(Строка.Товар);
	
		Если Состояние = ПредопределенноеЗначение("Перечисление.Состояние.Новый") Тогда
			Строка.Автомобиль = УстановитьАвтомобиль("000000008");
		ИначеЕсли Состояние = ПредопределенноеЗначение("Перечисление.Состояние.БУ")  Тогда
			Строка.Автомобиль = УстановитьАвтомобиль("000000204");
		КонецЕсли;
	КонецЦикла;
...
Функция ПроставитьМашинуВТЧТаблица(Товар)
	...
	Иначе 
		Возврат Перечисления.Состояние.ПустаяСсылка(); 
	КонецЕсли; 
