...
/// Комлев АА 25/11/24 +++
	ПоменятьСтатусыПриходаТовараВДокументеЗакупкаТовара();
...
КонецПроцедуры
	...
	Процедура ПоменятьСтатусыПриходаТовараВДокументеЗакупкаТовара();
	/// Комлев АА 25/11/24 +++
	Для каждого СтрокаТЧ Из Таблица Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ДокументЗакупкаТовара) Тогда
			Продолжить;
		КонецЕсли;
		ДокументЗакупка = СтрокаТЧ.ДокументЗакупкаТовара.ПолучитьОбъект();
		Для каждого ЗакупленныйТовар Из ДокументЗакупка.ЗакупленныйТовар Цикл
			Если СтрокаТЧ.Товар = ЗакупленныйТовар.Товар Тогда 
				ЗакупленныйТовар.СтатусПоступления = Перечисления.СтатусЗакупкиТовара.Поступил;
			КонецЕсли;
		КонецЦикла;
		ДокументЗакупка.Записать();
	КонецЦикла;
	/// Комлев АА 25/11/24 ---
КонецПроцедуры
