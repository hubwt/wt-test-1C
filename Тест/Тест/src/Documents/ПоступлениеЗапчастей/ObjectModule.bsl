...
Процедура АвтаматизацияСтатусовЗакупки ()
	...
	КонецЕсли; 
		...
		ТаблицаЗакупки.Загрузить(Запрос.Выполнить().Выгрузить());
		// -- получаем список закуаемого товара
		
		// ++ Получаем список поступившего товара  
		ТаблицаПоступившегоТовара = Новый ТаблицаЗначений;
		ТаблицаПоступившегоТовара.Загрузить(Таблица.Выгрузить(,"Товар"));
		...
		ТаблицаПоступившегоТовара.Загрузить(Запрос1.Выполнить().Выгрузить());

		// -- Получаем список поступившего товара  
		
		// ++ Проверка, все ли поступило из того что заказывали
		
		Для	Каждого Строка ИЗ ТаблицаПоступившегоТовара Цикл 
			
			Если ТаблицаПоступившегоТовара.Найти(Строка) = Неопределено Тогда
            	Возврат;  
        	КонецЕсли;
	...
	Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗакупкаТовара") Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				СтрокаТаблицы.СуммаПоступления 	= Выборка.ЦенаПоступления * Выборка.Количество;
				СтрокаТаблицы.Цена 				= Выборка.Цена; // ЦенаПродажи
