...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.СпросЗаГод = 0 Тогда
			Отказ = Истина;
			Сообщить("Поступление товара  " + Выборка.Товар  + " запрещено. Не пользуется спросом. Строка № " + Выборка.НомерСтроки );
			Возврат;
		КонецЕсли;
		Если Выборка.КолвоОстаток > (Выборка.СпросЗаГод * 2) Тогда
			Отказ = Истина;
			Сообщить("Поступление товара  " + Выборка.Товар  + " запрещено. Остаток на складе " + Склад + " превышает спрос. Строка № " + Выборка.НомерСтроки);
			Возврат;
		КонецЕсли;
		Если (Выборка.Колво + Выборка.КолвоОстаток) > (Выборка.СпросЗаГод * 2) Тогда
			Отказ = Истина;
			Сообщить("Поступление товара  " + Выборка.Товар  + " запрещено. Остаток на складе " + Склад + ", с учетом поступления превышает спрос. Строка № " + Выборка.НомерСтроки + " Можно оприходовать " + ((Выборка.СпросЗаГод * 2) - Выборка.КолвоОстаток));
			Возврат;
		КонецЕсли;
	КонецЦикла;
