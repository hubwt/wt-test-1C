...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Не Перекуп И Не Новые И Дата > '20241204170000' И Константы.УчестьОбязательно.Получить() = Ложь Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
					   |	ПоступлениеЗапчастейТаблица.Товар КАК Товар,
					   |	ПоступлениеЗапчастейТаблица.Колво КАК Колво,
					   |	ПоступлениеЗапчастейТаблица.НомерСтроки КАК НомерСтроки,
					   |	ПоступлениеЗапчастейТаблица.Автомобиль.Серия
					   ...
					   |ИЗ
					   |	РегистрНакопления.РегистрНакопления1.Остатки(&ТекущаяДата, Товар В
					   |		(ВЫБРАТЬ
					   |			ВТ_ТЧ_Товары.Товар
					   |		ИЗ
		...
		ДатаПолгодаНазад = ДобавитьМесяц(ТекущаяДатаСеанса(), -6);
		Пока Выборка.Следующий() Цикл
			Если Выборка.АвтомобильСерия <> Справочники.Серии.НайтиПоКоду("000000001") Тогда
				Если Выборка.Товар.ДатаСоздания <= ДатаПолгодаНазад Тогда
	...
	Для Каждого ТекСтрокаТаблица Из ТаблицаТовары Цикл
		...
		Если (Не об.Выбран()) И ((Не ПустаяСтрока(ТекСтрокаТаблица.Код)) Или ЗначениеЗаполнено(
			...
			об.дн = ДатаНакладной;
			об.п =  Поставшик;
			Если (ЗначениеЗаполнено(Перекуп) Или ЗначениеЗаполнено(Новые)) И ЗначениеЗаполнено(
				ТекСтрокаТаблица.МестоХраненияЗапчасти) Тогда
				об.Стеллаж = ТекСтрокаТаблица.МестоХраненияЗапчасти;
			КонецЕсли;
			об.Записать();
		Иначе
			об.наценка = ТекСтрокаТаблица.наценка;
			...
			// ++ Заполнить стелаж, если это новая или перекупная Запчасть
			Если (ЗначениеЗаполнено(Перекуп) Или ЗначениеЗаполнено(Новые)) И ЗначениеЗаполнено(
				ТекСтрокаТаблица.МестоХраненияЗапчасти) Тогда
				об.Стеллаж = ТекСтрокаТаблица.МестоХраненияЗапчасти;
			КонецЕсли; 
	...
	Если ТаблицаТовары.Количество() <> 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.Товар = ТекСтрокаТаблица.Товар Тогда
				...
				Для Каждого Строка Из ДокЗакупка.ЗакупленныйТовар Цикл

					Если Строка.Товар = Выборка.Товар Тогда
						Строка.СтатусПоступления = Перечисления.СтатусЗакупкиТовара.Поступил;
					КонецЕсли;
...
// -- Движение по регистру ДвижениеТМЦВорктрак (регистр ДвижениеТМЦВорктрак Приход)	
	...
	ОбновитьСтатусытоваровВЗакупкеЗапчастей(Таблица);

КонецПроцедуры

Функция ОбновитьСтатусытоваровВЗакупкеЗапчастей(Таблица)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Запрос.УстановитьПараметр("Ссылка", Строка.ДокументЗакупкаТовара);

		РезультатЗапроса = Запрос.Выполнить();

		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.Товар = Строка.Товар Тогда
				СсылкаДокЗакупки = Строка.ДокументЗакупкаТовара;
				ДокЗакупка = СсылкаДокЗакупки.ПолучитьОбъект();
				
//				ДокЗакупка.ЗакупленныйТовар[Число(Выборка.НомерСтроки)
//					- 1].СтатусПоступления = Перечисления.СтатусЗакупкиТовара.Поступил;
//				ДокЗакупка.Записать();

				Для Каждого Строка1 Из ДокЗакупка.ЗакупленныйТовар Цикл

					Если Строка1.Товар = Выборка.Товар Тогда
						Строка1.СтатусПоступления = Перечисления.СтатусЗакупкиТовара.Поступил;
					КонецЕсли;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон("Не удалось записать %1. %2", Спр, ОписаниеОшибки()), , , , Отказ);
		КонецПопытки;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаявкаНаРасход") Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Организация);
			//Волков 29.11.2023 --
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗакупкаТовара") Тогда
			// Комлев АА 16/01/25 +++
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗакупкаТовараЗакупленныйТовар.Товар КАК Товар,
		|	ЗакупкаТовараЗакупленныйТовар.Количество КАК Количество,
		|	ЗакупкаТовараЗакупленныйТовар.ЦенаБЕЗНДС КАК ЦенаБЕЗНДС,
		|	ЗакупкаТовараЗакупленныйТовар.Сумма КАК Сумма,
		|	ЗакупкаТовараЗакупленныйТовар.СтоимостьДоставки КАК СтоимостьДоставки,
		|	ЗакупкаТовараЗакупленныйТовар.ЦенаПродажи КАК ЦенаПродажи,
		|	ЗакупкаТовараЗакупленныйТовар.НДС КАК НДС,
		|	ЗакупкаТовараЗакупленныйТовар.НазначениеДетали КАК НазначениеДетали,
		|	ЗакупкаТовараЗакупленныйТовар.Ссылка КАК ДокументЗакупкаТовара,
		|	ЗакупкаТовараЗакупленныйТовар.Ссылка.Основание КАК Заявка
		|ИЗ
		|	Документ.ЗакупкаТовара.ЗакупленныйТовар КАК ЗакупкаТовараЗакупленныйТовар
		|ГДЕ
		|	ЗакупкаТовараЗакупленныйТовар.Ссылка = &Ссылка
		|	И (ЗакупкаТовараЗакупленныйТовар.СтатусПоступления = ЗНАЧЕНИЕ(Перечисление.СтатусЗакупкиТовара.ОжидаемПоступление)
		|	ИЛИ ЗакупкаТовараЗакупленныйТовар.СтатусПоступления = ЗНАЧЕНИЕ(Перечисление.СтатусЗакупкиТовара.ПустаяСсылка))";

		Запрос.УстановитьПараметр("Ссылка", ДанныеЗаполнения);
		Поставшик = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "Поставщик");
		...
		Пока Выборка.Следующий() Цикл
			...
			Пока Счетчик > 0 Цикл
				...
				СтрокаТЧ.Колво = 1;
				Если ТипЗнч(Выборка.НазначениеДетали) = Тип("ДокументСсылка.ПродажаЗапчастей") Тогда
					СтрокаТЧ.Продажа = Выборка.НазначениеДетали;
				ИначеЕсли ТипЗнч(Выборка.НазначениеДетали) = Тип("СправочникСсылка.ПроектыРазработки") Тогда
					СтрокаТЧ.Проект = Выборка.НазначениеДетали;
				КонецЕсли;

				СтрокаТЧ.Цена 				= Выборка.ЦенаПродажи / Выборка.Количество;
				СтрокаТЧ.ЦенаПоступления 	= Выборка.ЦенаБЕЗНДС;
				СтрокаТЧ.НДС				= Выборка.НДС;
				СтрокаТЧ.СуммаПоступления	= Выборка.Сумма / Выборка.Количество;
				СтрокаТЧ.СтоимостьДоставки 	= Выборка.СтоимостьДоставки / Выборка.Количество;
				СтрокаТЧ.ДатаДобавления 	= ТекущаяДатаСеанса();
				СтрокаТЧ.ДокументЗакупкаТовара = Выборка.ДокументЗакупкаТовара;

				СтрокаТЧ.Заявка				= Выборка.Заявка;
				СтрокаТЧ.Ответственный 		= дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьПользователяПоИмени(
					ПолноеИмя);
				Счетчик = Счетчик - 1;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ИспользоватьНаименованияПоставщиков Или Не ЗначениеЗаполнено(Поставшик)
...
Процедура ПроверитьРазрешенияПриСтаройСхемеУчета(Отказ)
	...
	Если Таблица.Количество() > КоличествоСтрок Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон("Запрещено добавление строк в старых документах (до %1) без заявки на сборку", Формат(
			дт_Склад.ДатаНачалаИспользованияЗаявокНаСборку(), "ДЛФ=D;")), , , , Отказ);
