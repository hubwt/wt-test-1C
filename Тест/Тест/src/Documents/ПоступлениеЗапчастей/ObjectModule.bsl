...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Для Каждого ТекСтрокаТаблица Из ТаблицаТовары Цикл
		...
		Иначе
			...
			Если (ЗначениеЗаполнено(Перекуп) Или ЗначениеЗаполнено(Новые)) И  ЗначениеЗаполнено (ТекСтрокаТаблица.МестоХраненияЗапчасти)Тогда
...
// -- Движение по регистру ДвижениеТМЦВорктрак (регистр ДвижениеТМЦВорктрак Приход)	
	...
	АвтаматизацияСтатусовЗакупки ();
	
КонецПроцедуры


Процедура АвтаматизацияСтатусовЗакупки ()
	
	Если НЕ ТИПЗНЧ(Основание) = Тип("ДокументСсылка.ЗакупкаТовара") тогда  
		Возврат; 
	КонецЕсли; 
		
		// ++ получаем список закуаемого товара
		...
		Запрос.Текст =
			...
			|ГДЕ
			|	ЗакупкаТовараЗакупленныйТовар.Ссылка = &Ссылка
			|	И ЗакупкаТовараЗакупленныйТовар.Ссылка.Проведен = ИСТИНА";
		
		Запрос.УстановитьПараметр("Ссылка", Основание);
		ТаблицаЗакупки = Новый ТаблицаЗначений;
		ТаблицаЗакупки = Запрос.Выполнить().Выгрузить();
		
		// -- получаем список закуаемого товара
		
		// ++ Получаем список поступившего товара  
		ТаблицаПоступившегоТовара = Новый ТаблицаЗначений;
		ТаблицаПоступившегоТовара = (Таблица.Выгрузить(,"Товар"));
		...
		Запрос1.Текст =
			...
			|	Документ.ПоступлениеЗапчастей.Таблица КАК ПоступлениеЗапчастейТаблица
			|ГДЕ
			|	ПоступлениеЗапчастейТаблица.Ссылка.Основание = &Основание
			|	И ПоступлениеЗапчастейТаблица.Ссылка.Проведен = ИСТИНА";
		
		Запрос1.УстановитьПараметр("Основание", Основание);
		ТаблицаПоступившегоТовара = Запрос1.Выполнить().Выгрузить();

		// -- Получаем список поступившего товара  
		
		// ++ Проверка, все ли поступило из того что заказывали
		
		
		Для	Каждого Строка ИЗ ТаблицаЗакупки Цикл 
			
			Если ТаблицаПоступившегоТовара.Найти(Строка.Товар) = Неопределено Тогда
            	Возврат;  
        	КонецЕсли;
		...
		// ++ Если дошли до этого момента В заявку и в закупке устанавливаем статусы "Выполнено"

		ОбъектЗакупка	 			= Основание.ПолучитьОбъект();
		ОбъектЗакупка.СтатусЗакупки = Перечисления.СтатусыЗаявкаЗакупкаТоваров.Отработанно;
		ОбъектЗакупка.Записать(); 
		СсылкаЗаявка	 			= ОбъектЗакупка.Основание; 
		Если ЗначениеЗаполнено (СсылкаЗаявка)Тогда 
	...
	Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗакупкаТовара") Тогда
			...
			Запрос.Текст =
				...
				|	ЗакупкаТовараЗакупленныйТовар.ЦенаБЕЗНДС КАК ЦенаПоступления,
				|	ЗакупкаТовараЗакупленныйТовар.Сумма КАК СуммаПоступления,
				|	ЗакупкаТовараЗакупленныйТовар.ЦенаПродажи КАК Цена,
			...
			Пока Выборка.Следующий() Цикл
				...
				СтрокаТаблицы.Колво 			= Выборка.Количество;
				СтрокаТаблицы.ЦенаПоступления 	= Выборка.ЦенаПоступления;
				СтрокаТаблицы.СтоимостьДоставки = Выборка.СтоимостьДоставки / Выборка.Количество; 
				СтрокаТаблицы.СуммаПоступления 	= (Выборка.ЦенаПоступления * Выборка.Количество) + СтрокаТаблицы.СтоимостьДоставки;
				СтрокаТаблицы.Цена 				= Выборка.ЦенаПродажи;// ЦенаПродажи
				СтрокаТаблицы.ДатаДобавления = ТекущаяДатаСеанса();
				
				ПолноеИмя = ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя;
				СтрокаТаблицы.Ответственный = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьПользователяПоИмени(ПолноеИмя);
