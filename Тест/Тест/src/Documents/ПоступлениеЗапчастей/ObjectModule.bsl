...
Если ТМЦ.Количество() > 0 Тогда 
	...
	КонецЕсли; 
// -- Движение по регистру ДвижениеТМЦВорктрак (регистр ДвижениеТМЦВорктрак Приход)	
	Для Каждого СтрокаТовар Из Таблица Цикл
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("Ссылка", СтрокаТовар.ДокументЗакупкаТовара);
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.Товар = СтрокаТовар.Товар Тогда
				ДокЗакупка = СтрокаТовар.ДокументЗакупкаТовара.ПолучитьОбъект();
				ДокЗакупка.ЗакупленныйТовар[Число(Выборка.НомерСтроки) - 1].СтатусПоступления = Перечисления.СтатусЗакупкиТовара.Поступил;
				ДокЗакупка.Записать();
			КонецЕсли;
		КонецЦикла;
