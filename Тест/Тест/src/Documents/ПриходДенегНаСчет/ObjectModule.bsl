...
Если ОбменДанными.Загрузка Тогда
	...
	Если ЗначениеЗаполнено(Сделка) И ТипЗнч(Сделка) = Тип("ДокументСсылка.ПродажаЗапчастей") Тогда
		Если Дата > '20240701' Тогда //Дата начала действия бонусов	
			...
			НаборЗаписей.Отбор.Продажа.Установить(Сделка);
			НаборЗаписей.Прочитать();

//+++ Раскидываем доли бонусов по складам
		Стуктура = БонусыКладовщикиСПродажи(Сделка);
		
		ДоляКладовщикаНН 	=  СуммаДокумента 	* Стуктура.ДоляКладовщиковНН;  
		ДоляКладовщикаМСК 	=  СуммаДокумента 	* Стуктура.ДоляКладовщиковМСК;  
		ДоляКладовщикаЕКБ 	=  СуммаДокумента 	* Стуктура.ДоляКладовщиковЕКБ;  
...
//-- Раскидываем доли бонусов по складам
			...
			Если НаборЗаписей.Количество() = 0 Тогда
				...
				Для Каждого СтрокаТЧ Из СписокУчастниковРаботы Цикл 
					...
					Если СтрокаТЧ.МестоРаботыКод = МестоРаботыННКод Тогда
						...
						ЗаписьВРегистреСведений.Бонус 		= СуммаДокумента * ДоляКладовщикаНН;
						ЗаписьВРегистреСведений.Записать();
						Итер = Итер +1;
					КонецЕсли;
					Если СтрокаТЧ.МестоРаботыКод = МестоРаботыМСККод Тогда
						ЗаписьВРегистреСведений = РегистрыСведений.БонусыСотрудниковОтПродажи.СоздатьМенеджерЗаписи();
						ЗаписьВРегистреСведений.Период 		= Дата + Итер;
						ЗаписьВРегистреСведений.Сотрудник 	= СтрокаТЧ.Сотрудник;
						ЗаписьВРегистреСведений.Сумма 		= СуммаДокумента;
						...
						ЗаписьВРегистреСведений.Продажа 	= Сделка;
						ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
						ЗаписьВРегистреСведений.Бонус 		= СуммаДокумента * ДоляКладовщикаМСК;
						ЗаписьВРегистреСведений.Записать();
						Итер = Итер +1;	
					КонецЕсли;
					
					Если СтрокаТЧ.МестоРаботыКод = МестоРаботыЕКБКод Тогда
						ЗаписьВРегистреСведений = РегистрыСведений.БонусыСотрудниковОтПродажи.СоздатьМенеджерЗаписи();
						ЗаписьВРегистреСведений.Период 		= Дата + Итер;
						ЗаписьВРегистреСведений.Сотрудник 	= СтрокаТЧ.Сотрудник;
						ЗаписьВРегистреСведений.Сумма 		= СуммаДокумента;
						...
						ЗаписьВРегистреСведений.Продажа 	= Сделка;
						ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
						ЗаписьВРегистреСведений.Бонус 		= СуммаДокумента * ДоляКладовщикаЕКБ;
						ЗаписьВРегистреСведений.Записать();
						Итер = Итер +1;	
					КонецЕсли;
...
Функция БонусыКладовщикиСПродажи(Сделка)
	...
	Запрос.Текст =
		...
		|		ПО ПродажаЗапчастейОтветственные.Ссылка = ВТ_Сотрудник.Пользователь.Ссылка
		|ГДЕ
	...
	Для Каждого Строка ИЗ ТаблицаОтветственные Цикл 
		...
		СуммаДокументПродажа = Строка.СуммаДокументПродажа; 
	КонецЦикла; 
	...
	Структура.Вставить("ДоляКладовщикаНН",	СуммаСтрокиТовараНН 	/ СуммаДокументПродажа / КоличествоКладовщиковНН  * 0.005);
	Структура.Вставить("ДоляКладовщикаМСК",	СуммаСтрокиТовараМСК 	/ СуммаДокументПродажа / КоличествоКладовщиковМСК * 0.005);
	Структура.Вставить("ДоляКладовщикаЕКБ",	СуммаСтрокиТовараЕКБ 	/ СуммаСтрокиТовараЕКБ / КоличествоКладовщиковЕКБ * 0.005);
