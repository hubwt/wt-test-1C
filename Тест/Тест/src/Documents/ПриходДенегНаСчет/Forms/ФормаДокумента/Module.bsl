...
&НаКлиенте
Процедура ПерезаписатьПриходДенегНаСчет(Команда)
	ПерезаписатьПриходДенегНаСчетНаСервере(); 
КонецПроцедуры

&НаСервере
Процедура ПерезаписатьПриходДенегНаСчетНаСервере()
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		СсылкаВызывДок= Объект.Ссылка;
		дт_Зарплата.ПерезаписатьПриходДенегНаСчет(СсылкаВызывДок);
	Иначе дт_Зарплата.ПерезаписатьПриходДенегНаСчет(); 
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("ПомощникБухгалтера"))
 	Тогда 
 		Элементы.ПерезаписатьПриходДенегНаСчет.Видимость = истина; 
 	иначе 
 	 	Элементы.ПерезаписатьПриходДенегНаСчет.Видимость = ложь;
	КонецЕсли;  
