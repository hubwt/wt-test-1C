...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		Объект.СтатусЗакупки = Перечисления.СтатусыЗаявкаЗакупкаТоваров.ВРаботе;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьЛог(Команда)
	Если Элементы.Логи.Видимость = Ложь Тогда 
		Элементы.Логи.Видимость = Истина; 
	Иначе Элементы.Логи.Видимость = Ложь; 
	КонецЕсли; 
КонецПроцедуры
...
Функция СтоимостьДоставкиЗаЕдиницуТовара() 
	...
	Если ЗначениеЗаполнено(Объект.ТранспортнаяКомпания) И ЗначениеЗаполнено(Объект.СтоимостьДоставки) Тогда 
		...
		Для Каждого Строка ИЗ Объект.ЗакупленныйТовар Цикл
			Если Строка.СтатусПоступления <> Перечисления.СтатусЗакупкиТовара.Отказ Тогда 
...
&НаКлиенте
Процедура ЗакупленныйТоварСтатусПоступленияПриИзменении(Элемент)
	ПересчитьСтоимостьДоставкиТоваров(); 
КонецПроцедуры
...
Функция ПересчитьСтоимостьДоставкиТоваров()
		...
		ПосчитатьНДСДоставки(); 
		Для Каждого СтрокаТЧ ИЗ Объект.ЗакупленныйТовар Цикл 
			Если СтрокаТЧ.СтатусПоступления <> Перечисления.СтатусЗакупкиТовара.Отказ Тогда  
				СтрокаТЧ.СтоимостьДоставки = СтоимостьДоставкиЗаЕдиницуТовара() * СтрокаТЧ.Количество; 
				СтрокаТЧ.Сумма 	= (СтрокаТЧ.ЦенаБЕЗНДС + СтрокаТЧ.НДС )	* СтрокаТЧ.Количество + СтрокаТЧ.СтоимостьДоставки; 
			КонецЕсли; 
		КонецЦикла; 
		
		Если Объект.ЗакупленныйТовар <> 0 Тогда 
...
&НаКлиенте
Процедура ЗакупленныйТоварПослеУдаления(Элемент)
	ПересчитьСтоимостьДоставкиТоваров(); 
КонецПроцедуры
