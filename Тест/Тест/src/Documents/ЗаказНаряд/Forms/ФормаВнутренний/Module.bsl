...
//Мазин --------------Начало
&НаСервере
Процедура ПересчетТаблицТоварыИРаботаСкидкаНаценка (Отказ = Неопределено)
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр, СтрокаТаблицы)
	...
	ТекДанные.Состояние = СтатусТовара();
	//СтатусТовара();	
КонецПроцедуры
...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	Если ТекСтрока.ЦенаСоСкидкойНаценкой = Неопределено ИЛИ ТекСтрока.ЦенаСоСкидкойНаценкой = 0  Тогда  
		ТекСтрока.Сумма = ТекСтрока.Цена * ТекСтрока.Количество;
		ТекСтрока.ЦенаСоСкидкойНаценкой = ТекСтрока.Цена; 
	КонецЕсли;
	Элементы.ДляБланкаОтчёта.Заголовок = "Для бланка отчёта "+ "("+КоличествоДеталей+")";
...
//	ОбновитьПоказателиТоваров();
	...
	Если Объект.ТипНаценки = ПроверкаБезНДС() Тогда 
		ТекСтрока.СуммаНалог 				= ТекСтрока.СуммаВсего * 0.06 / 1.06;
			...
			КонецЕсли; 
		ТекСтрока.ЦенаСоСкидкойНаценкой		= ТекСтрока.СуммаВсего / (ТекСтрока.Количество * 1.06);
		ТекСтрока.Цена						= ТекСтрока.ЦенаСоСкидкойНаценкой; 
	...
	ИначеЕсли Объект.ТипНаценки = ПроверкаНДС()  Тогда
		ТекСтрока.СуммаНалог 				= ТекСтрока.СуммаВсего * 0.2 / 1.2;
			Если ТекСтрока.Количество = 0 ИЛИ ТекСтрока.Количество = Неопределено Тогда 
				ТекСтрока.Количество = 1; 
			КонецЕсли; 
		ТекСтрока.ЦенаСоСкидкойНаценкой		= ТекСтрока.СуммаВсего / (ТекСтрока.Количество * 1.2);
		ТекСтрока.Цена						= ТекСтрока.ЦенаСоСкидкойНаценкой; 
	иначе 
...
Процедура РаботыСуммаВсегоПриИзменении(Элемент)
	...
	иначе 
		ТекСтрока.СуммаНалог 				= 0;	
		ТекСтрока.ЦенаСоСкидкойНаценкой		= ТекСтрока.СуммаВсего / ТекСтрока.Количество;
	КонецЕсли;
	///+ГомзМА 22.12.2023
	ТекДанные = Элементы.Работы.ТекущиеДанные;
	Если НЕ Объект.Ссылка.Пустая() Тогда 
