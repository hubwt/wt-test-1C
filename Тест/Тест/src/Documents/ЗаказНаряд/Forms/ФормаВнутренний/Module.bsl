...
//	СтруктураСтроки.Вставить("Дата", Объект.Дата);
	...
	ТекДанные.Состояние 	= ПолучитьСтатусОжидание(); // для WT10 "Ожидание" для прил склад "Планово"  
	
КонецПроцедуры

&НаСервере 
Функция ПолучитьСтатусОжидание()
	
	Статус = Перечисления.СтатусыТовараВЗаказНаряде.Ожидание; 
