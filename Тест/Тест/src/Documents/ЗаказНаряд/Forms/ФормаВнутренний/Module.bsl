...
Функция ЗаполнитьТаблицуСуммаИПроцентПоИсполнителям()
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказНарядИсполнители.Исполнитель) КАК КоличествоИсполнителей,
	|	МАКСИМУМ(ЗаказНарядРаботы.СуммаВсего) КАК СуммаВсего,
	|	МАКСИМУМ(ЗаказНарядРаботы.Ссылка.СуммаРаботы) КАК СуммаРаботы
	...
	|		ПО (ЗаказНарядРаботы.ИдентификаторСтроки = ЗаказНарядИсполнители.ИдентификаторСтрокиРодитель)
	|ГДЕ
	...
	|	СУММА(ВЫБОР
	|		КОГДА ВТ_КоличествоИсполнителейИСуммаРаботы.КоличествоИсполнителей <> 0
	|			ТОГДА ВТ_КоличествоИсполнителейИСуммаРаботы.СуммаВсего /
	|				ВТ_КоличествоИсполнителейИСуммаРаботы.КоличествоИсполнителей
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК СуммаПоИсполнителю,
	|	МАКСИМУМ(ВТ_КоличествоИсполнителейИСуммаРаботы.СуммаРаботы) КАК ОбщаяСуммаРабот
	|ПОМЕСТИТЬ ВТ_СуммаПоИсполнителю
	|ИЗ
	...
	|		ПО (ВТ_Исполнители.Работа = ВТ_КоличествоИсполнителейИСуммаРаботы.Работа)
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_СуммаПоИсполнителю.Исполнитель КАК Исполнитель,
	|	ВТ_СуммаПоИсполнителю.СуммаПоИсполнителю КАК СуммаПоИсполнителю,
	|	ВЫБОР
	|		КОГДА ВТ_СуммаПоИсполнителю.ОбщаяСуммаРабот <> 0
	|			ТОГДА ВЫРАЗИТЬ(ВТ_СуммаПоИсполнителю.СуммаПоИсполнителю / ВТ_СуммаПоИсполнителю.ОбщаяСуммаРабот * 100 КАК ЧИСЛО(15,
	|				2))
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Процент
	|ИЗ
	|	ВТ_СуммаПоИсполнителю КАК ВТ_СуммаПоИсполнителю
	|ГДЕ
	|	ВТ_СуммаПоИсполнителю.Исполнитель <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)";
