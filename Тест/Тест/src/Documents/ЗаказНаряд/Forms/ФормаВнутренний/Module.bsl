...
Процедура ОбработкаИзмененияСтроки(ИмяТабличнойЧасти, Поле = Неопределено, ДанныеСтроки = Неопределено)
	...
	Если Объект.Дата > Дата("20240110") и ИмяТабличнойЧасти = "Товары" И ТекДанные <> Неопределено Тогда
		Попытка
			//Мазин
		...
		Иначе
			Если (Объект.СкидкаМенеджер = 0 И Объект.НаценкаМенеджер = 0) Или (Объект.НаценкаМенеджер = Неопределено
				И Объект.СкидкаМенеджер = Неопределено) И ТекДанные <> Неопределено Тогда
				дт_АвтосервисКлиентСервер.ОбработкаИзмененияСтроки(Объект, ИмяТабличнойЧасти, ТекДанные, Поле);
			КонецЕсли; 
...
&НаКлиенте
Процедура ТоварыКлиентскаяДетальПриИзменении(Элемент)
	
	ТекДанные = Элементы.Товары.ТекущиеДанные;
	ТоварыКлиентскаяДетальПриИзмененииНаСервере(ТекДанные.КлиентскаяДеталь, ТекДанные.Партия);
КонецПроцедуры

&НаСервере
Процедура ТоварыКлиентскаяДетальПриИзмененииНаСервере(КлиентскаяДеталь, Инкод)
	...
	Если ЗаписьВРегистреСведений.Выбран() Тогда
		ЗаписьВРегистреСведений.ДетальКлиента = КлиентскаяДеталь;
		ЗаписьВРегистреСведений.Записать();
	КонецЕсли;
