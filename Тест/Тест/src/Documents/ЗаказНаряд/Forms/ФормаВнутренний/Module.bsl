...
&НаКлиенте
Процедура ДефектовочныйЛистКомментарийПриИзменении(Элемент)
	/// +++  Синхронизировать таблицу товары и таблицу дефектовочный лист в части комментария
	
		КомментарийИзмен = Элементы.ДефектовочныйЛист.ТекущиеДанные.Комментарий; 
		Если ЗначениеЗаполнено(Элементы.ДефектовочныйЛист.ТекущиеДанные.Партия) ТОгда 
			Партия =  Элементы.ДефектовочныйЛист.ТекущиеДанные.Партия ;
			Для каждого Строка ИЗ Объект.Товары Цикл 
				Если Строка(Строка.Партия) = Партия Тогда 
					Строка.Комментарий = КомментарийИзмен; 
				КонецЕсли; 
			КонецЦикла; 
		Иначе 
			Номенклатура  = Элементы.ДефектовочныйЛист.ТекущиеДанные.Номенклатура;
			Для каждого Строка ИЗ Объект.Товары Цикл 
...
&НаСервере
Функция СформироватьДефектоочныйЛист()
// +++ МазинЕС Сформировывает Дефектовочный лист 
	...
	Запрос.Текст =
		...
		|	ЗаказНарядТовары.Комментарий КАК Комментарий,
		|	ЗаказНарядТовары.Цена КАК Цена
		|ПОМЕСТИТЬ ВТ_ЗАКНАР_Партии
		|ИЗ
		|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
		|ГДЕ
		|	(ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Срочно)
		|	ИЛИ ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Планово))
		|	И ЗаказНарядТовары.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЗаказНарядТовары.Количество КАК Количество,
		|	""Есть на складе"" КАК Остаток,
		|	ЗаказНарядТовары.Цена КАК Цена,
		|	ЗаказНарядТовары.Комментарий КАК Комментарий
		|ИЗ
		|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
		|ГДЕ
		|	ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Установлена)
		|	И ЗаказНарядТовары.Ссылка = &Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
		|ГДЕ
		|	ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Собрано)
		|	И ЗаказНарядТовары.Ссылка = &Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	РегистрНакопления1Остатки.Товар.Артикул,
		|	РегистрНакопления1Остатки.Товар.Код,
		|	РегистрНакопления1Остатки.Товар,
		|	""Срочно/Планово"",
		|	""Партия не добавлена"",
		|	ВТ_ЗАКНАР_Партии.Количество,
		|	""Есть на складе"",
		|	ВТ_ЗАКНАР_Партии.Цена КАК Цена,
		|	ВТ_ЗАКНАР_Партии.Комментарий КАК Комментарий
		|ИЗ
		|	РегистрНакопления.РегистрНакопления1.Остатки(&Дата2, Товар В
		|		(ВЫБРАТЬ
		|			ВТ_ЗАКНАР_Партии.Номенклатура
		|		ИЗ
		|			ВТ_ЗАКНАР_Партии КАК ВТ_ЗАКНАР_Партии)) КАК РегистрНакопления1Остатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗАКНАР_Партии КАК ВТ_ЗАКНАР_Партии
		|		ПО (ВТ_ЗАКНАР_Партии.Номенклатура = РегистрНакопления1Остатки.Товар)
		|ГДЕ
		...
		|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
		|ГДЕ
		|	ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Заказано)
		|	И ЗаказНарядТовары.Ссылка = &Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
		|ГДЕ
		|	ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.НетВНаличии)
		|	И ЗаказНарядТовары.Ссылка = &Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	ИЛИ ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Планово))
		|	И ЗаказНарядТовары.Ссылка = &Ссылка
		|	И НЕ ЗаказНарядТовары.Номенклатура В
		|		(ВЫБРАТЬ
		|			РегистрНакопления1Остатки.Товар КАК Товар
		|		ИЗ
		|			РегистрНакопления.РегистрНакопления1.Остатки(,) КАК РегистрНакопления1Остатки)";
	...
	Запрос.УстановитьПараметр("Дата2", ТекущаяДатаСеанса());

	Результат = Запрос.Выполнить();
	СпособОбхода = ОбходРезультатаЗапроса.Прямой;
	ТабЗнач = Результат.Выгрузить(СпособОбхода);

	Объект.ДефектовочныйЛист.Очистить();
	Объект.ДефектовочныйЛист.Загрузить(ТабЗнач);	

// --- МазинЕС Сформировывает Дефектовочный лист
КонецФункции

&НаКлиенте
Процедура ОБновитьДефектовочныйЛист(Команда)
	
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопроса",ЭтотОбъект);	
 
    ПоказатьВопрос(Оповещение,
        "Для обновления деффектовочного листа нужно сохранить документ, сохранить?",
        РежимДиалогаВопрос.ДаНетОтмена,10, // таймаут в секундах
        КодВозвратаДиалога.Да, // (необ.) кнопка по умолчанию
        "Обновить" // (необ.) заголовок
    	);    
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		СписокЗаказаныйТовар.Параметры.УстановитьЗначениеПараметра("Основание",Объект.Ссылка);  
		
		ЗПИсполнителиРабот.Параметры.УстановитьЗначениеПараметра("Ссылка", объект.Ссылка);
		ПодвалЗПИсполнителиРабот(); 
	КонецЕсли;
	
	//  +++ МазинЕС СформироватьДефектоочныйЛист
	СформироватьДефектоочныйЛист(); 
	//  +++ МазинЕС СформироватьДефектоочныйЛист
	...
	КонецЕсли;  
	////
	Если   ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("дт_ИзменениеЦенНаРаботы")) Тогда
		Элементы.ГруппаШапка.ТолькоПросмотр = Ложь;
		Элементы.Работы.ТолькоПросмотр = Ложь;
	КонецЕсли; 
	///-ГомзМА 25.01.2023
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) ТОгда 
		СсылкаНаВТ10 = "https://sklad.wt10-crm.ru/zakaz-naryad/" + Объект.Номер; 
	КонецЕсли;
	
	/// +++ Расчитать сбестоимость всех товаров в таблеце // МазинЕС
		СуммаТовара = РасчетСебестоимости(); 
		СуммаСебестоимость 	= СуммаТовара * 0.47;
		СуммаВаловаяПрибыль = СуммаТовара - СуммаСебестоимость;
		Если ЗначениеЗаполнено(СуммаСебестоимость) Тогда 
			Рентабельность 		= СуммаВаловаяПрибыль / СуммаСебестоимость * 100; 
		КонецЕсли; 	
	// --- Расчитать сбестоимость всех товаров в таблеце // МазинЕС
	...
	//ЗаполнитьТаблицуСуммаИПроцентПоИсполнителям();
КонецПроцедуры
...
&НаСервере
Функция ЗаполнитьТаблицуСуммаИПроцентПоИсполнителям()
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗаказНарядИсполнители.Исполнитель КАК Исполнитель,
	|	ЗаказНарядИсполнители.ИдентификаторСтрокиРодитель КАК ИдентификаторСтрокиРодитель
	|ПОМЕСТИТЬ ВТ_ИсполнителиРеал
	|ИЗ
	...
	|	ЗаказНарядИсполнители.ИдентификаторСтрокиРодитель
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаказНарядРаботы.Работа КАК Работа,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_ИсполнителиРеал.Исполнитель) КАК ИсполнительКол,
	|	ЗаказНарядРаботы.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ПОМЕСТИТЬ ВТ_КоличествоИсполнителей
	|ИЗ
	|	ВТ_ИсполнителиРеал КАК ВТ_ИсполнителиРеал
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаряд.Работы КАК ЗаказНарядРаботы
	|		ПО ВТ_ИсполнителиРеал.ИдентификаторСтрокиРодитель = ЗаказНарядРаботы.ИдентификаторСтроки
	|ГДЕ
	...
	|	ВТ_ИсполнителиРеал.Исполнитель КАК Исполнитель,
	|	ЗаказНарядРаботы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ВТ_КоличествоИсполнителей.ИсполнительКол КАК ИсполнительКол,
	|	ЗаказНарядРаботы.СуммаВсего / ВТ_КоличествоИсполнителей.ИсполнительКол КАК СуммаИсполнитель
	|ПОМЕСТИТЬ ВТ_КоличествоИсполнителейИСуммаРаботы
	|ИЗ
	|	ВТ_ИсполнителиРеал КАК ВТ_ИсполнителиРеал
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаряд.Работы КАК ЗаказНарядРаботы
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_КоличествоИсполнителей КАК ВТ_КоличествоИсполнителей
	|			ПО ЗаказНарядРаботы.ИдентификаторСтроки = ВТ_КоличествоИсполнителей.ИдентификаторСтроки
	|		ПО ВТ_ИсполнителиРеал.ИдентификаторСтрокиРодитель = ЗаказНарядРаботы.ИдентификаторСтроки
	|ГДЕ
	|	ЗаказНарядРаботы.Ссылка = &Ссылка
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_КоличествоИсполнителейИСуммаРаботы.Работа КАК Работа,
	|	ВТ_КоличествоИсполнителейИСуммаРаботы.СуммаВсего КАК СуммаВсего
	|ПОМЕСТИТЬ ВТ_СписокРабот
	|ИЗ
	|	ВТ_КоличествоИсполнителейИСуммаРаботы КАК ВТ_КоличествоИсполнителейИСуммаРаботы
	|СГРУППИРОВАТЬ ПО
	|	ВТ_КоличествоИсполнителейИСуммаРаботы.Работа,
	|	ВТ_КоличествоИсполнителейИСуммаРаботы.СуммаВсего
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ВТ_СписокРабот.СуммаВсего) КАК СуммаВсехРабот
	|ПОМЕСТИТЬ ВТ_СуммаРабот
	|ИЗ
	|	ВТ_СписокРабот КАК ВТ_СписокРабот
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_КоличествоИсполнителейИСуммаРаботы.Исполнитель КАК Исполнитель,
	|	СУММА(ВТ_КоличествоИсполнителейИСуммаРаботы.СуммаИсполнитель) КАК СуммаИсполнитель,
	|	ВТ_СуммаРабот.СуммаВсехРабот КАК СуммаВсехРабот,
	|	СУММА(ВТ_КоличествоИсполнителейИСуммаРаботы.СуммаИсполнитель) / ВТ_СуммаРабот.СуммаВсехРабот * 100 КАК
	|		ПроцентОТОбщейСуммыРабот
	|ПОМЕСТИТЬ ВТ_Итоговая
	|ИЗ
	|	ВТ_КоличествоИсполнителейИСуммаРаботы КАК ВТ_КоличествоИсполнителейИСуммаРаботы,
	|	ВТ_СуммаРабот КАК ВТ_СуммаРабот
	|СГРУППИРОВАТЬ ПО
	|	ВТ_КоличествоИсполнителейИСуммаРаботы.Исполнитель,
	|	ВТ_СуммаРабот.СуммаВсехРабот
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Итоговая.Исполнитель КАК Исполнитель,
	|	ВЫРАЗИТЬ(ВТ_Итоговая.СуммаИсполнитель КАК ЧИСЛО(11, 0)) КАК СуммаПоИсполнителю,
	|	ВЫРАЗИТЬ(ВТ_Итоговая.ПроцентОТОбщейСуммыРабот КАК ЧИСЛО(11, 0)) КАК Процент
	|ИЗ
	|	ВТ_Итоговая КАК ВТ_Итоговая";

	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);

	СуммаИПроцентПоИсполнителям.Загрузить(Запрос.Выполнить().Выгрузить());
КонецФункции

Процедура ПодвалЗПИсполнителиРабот()
	
	Схема = Элементы.ЗПИсполнителиРабот.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	Настройки = Элементы.ЗПИсполнителиРабот.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	...
	ЭтотОбъект.ЭтаФорма.Элементы.ЗПИсполнителиРаботСуммаПоИсполнителю.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.ЗПИсполнителиРаботПроцент.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	
	ЭтотОбъект.ЭтаФорма.Элементы.ЗПИсполнителиРаботСуммаПоИсполнителю.ТекстПодвала = Формат(Результат.Итог("СуммаПоИсполнителю"),"ЧДЦ=0; ЧН=-");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗПИсполнителиРаботПроцент.ТекстПодвала =  Формат(Результат.Итог("Процент"),"ЧДЦ=0; ЧН=-");
...
&НаСервере
Функция РасчетСебестоимости()
	СумаВсего 		= 0;
	Для Каждого  Строка Из объект.Товары Цикл 
		СумаВсего = СумаВсего + Строка.Сумма; 
	КонецЦикла; 
	Возврат СумаВсего; 	
КонецФункции
...
Процедура ТоварыПартияПриИзменении(Элемент)
	...
	// ++ Изменяем место хранения Партии на место "Ячека РВР"
	Если ЗначениеЗаполнено(Элементы.Товары.ТекущиеДанные.Партия)  ТОгда 
		Элементы.Товары.ТекущиеДанные.Состояние = ПредопределенноеЗначение("Перечисление.СтатусыТовараВЗаказНаряде.Собрано");
	//	СторокаТекст = "Переместить партию на стеллаж: " + Элементы.ЯчейкаРВР.ТекстРедактирования +"."; 
	//	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопроса2",ЭтотОбъект);	
	//    ПоказатьВопрос(Оповещение,СторокаТекст,
	//        РежимДиалогаВопрос.ДаНетОтмена,20, // таймаут в секундах
	//        КодВозвратаДиалога.Да, // (необ.) кнопка по умолчанию
	//        "Перемещение партии" // (необ.) заголовок
	//    	);    
	КонецЕсли; 
	КонецПроцедуры

//&НаКлиенте
//Процедура ПослеЗакрытияВопроса2(Результат, Параметры) Экспорт
// 
//    Если Результат = КодВозвратаДиалога.Да Тогда
//		ЭтотОбъект.Записать(); 
//		Партия = Элементы.Товары.ТекущиеДанные.Партия; 
//		ИзменяемМестоХраненияПартии(Партия,Объект.ЯчейкаРВР); 
//    КонецЕсли;	
// 
//КонецПроцедуры

//&НаСервереБезКонтекста
//Функция ИзменяемМестоХраненияПартии(Партия,Место)
//	Если Партия <> Справочники.ИндКод.ПустаяСсылка() тогда 
//		Запись = РегистрыСведений.ИндНомер.СоздатьМенеджерЗаписи();
//		Запись.индкод = Партия;
//		Запись.Прочитать();
//		Если Запись.Выбран() Тогда
//			Запись.Стеллаж = Место;
//			Запись.Записать();
//		КонецЕсли;
//	КонецЕсли; 
//КонецФункции  
...
&НаСервереБезКонтекста
Функция СменаМестХраненияПартий(НомерЗН) 
	...
	Запрос.Текст = 
		...
		|	ЗаказНарядТовары.Ссылка.ЯчейкаРВР КАК ЯчейкаРВР
		|ИЗ
		|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИндНомер КАК ИндНомер
		|		ПО ЗаказНарядТовары.Партия = ИндНомер.индкод
		|ГДЕ
		|	ЗаказНарядТовары.Партия <> ЗНАЧЕНИЕ(Справочник.индкод.ПустаяСсылка)
		|	И (ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Собрано)
		|			ИЛИ ЗаказНарядТовары.Состояние = ЗНАЧЕНИЕ(Перечисление.СтатусыТовараВЗаказНаряде.Установлена))
		|	И ЗаказНарядТовары.Ссылка.Номер = &НомерЗН";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Партия <> Справочники.ИндКод.ПустаяСсылка() тогда 
			Запись = РегистрыСведений.ИндНомер.СоздатьМенеджерЗаписи();
			Запись.индкод = Выборка.Партия;
			Запись.Прочитать();
		Если Запись.Выбран() Тогда
			
			Запись.Стеллаж = Выборка.ЯчейкаРВР;
			Запись.Записать();
		КонецЕсли;
	КонецЕсли; 
...
Функция ВыдачаТовара(Партия)
	...
	Если ЗначениеЗаполнено(Партия) Тогда
		Результат = Перечисления.СтатусыТовараВЗаказНаряде.Собрано;
	Иначе
...
Процедура ОбновитьПоказателиТоваров(ОбновитьСебестоимость = Ложь)
	...
	Если ОбновитьСебестоимость Тогда
		...
		Если ВыборкаВсего.Следующий() Тогда
			СуммаСебестоимость1 = ВыборкаВсего.Себестоимость;
	...
	СуммаВаловаяПрибыль1 = Объект.Товары.Итог("СуммаВсего") - СуммаСебестоимость;
	Рентабельность1 = ?(СуммаСебестоимость = 0, 0, Окр(СуммаВаловаяПрибыль * 100 / СуммаСебестоимость, 2));
...
Процедура ТоварыСостояниеПриИзменении(Элемент)
	...
	Если ТекДанные.Состояние = ПредопределенноеЗначение("Перечисление.СтатусыТовараВЗаказНаряде.Установлена") Тогда
		ТекДанные.ДатаУстановки = ТекущаяДата(); 
	КонецЕсли;
...
Если ЗначениеЗаполнено(Объект.ЯчейкаРВР) И Объект.ЯчейкаРВР <> Справочники.Стеллаж.ПустаяСсылка()  Тогда 	
	СменаМестХраненияПартий(оБЪЕКТ.Номер);
КонецЕсли; 
КонецПроцедуры; 
