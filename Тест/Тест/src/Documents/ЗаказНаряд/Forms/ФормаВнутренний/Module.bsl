...
&НаСервере
Процедура ПересчетТаблицТоварыИРаботаСкидкаНаценкаСервер ()
...
&НаСервере
Процедура ПересчетТаблицТоварыИРаботаСкидкаНаценка ()
...
Если Объект.НаценкаМенеджер = 0  И Объект.СкидкаМенеджер > 0 Тогда 
		Если Объект.Товары.Количество() <> 0 Тогда 
				...
				СтрокаТЧ.ЦенаСоСкидкойНаценкой = СтрокаТЧ.Цена - СтрокаТЧ.Цена* Объект.СкидкаМенеджер/100; 
...
Если Объект.НаценкаМенеджер > 0  И Объект.СкидкаМенеджер = 0 Тогда 
		Если Объект.Товары.Количество() <> 0 Тогда 
			Для Каждого СтрокаТЧ ИЗ Объект.Товары Цикл 
			СтрокаТЧ.ЦенаСоСкидкойНаценкой = СтрокаТЧ.Цена + СтрокаТЧ.Цена* Объект.НаценкаМенеджер/100;  
			ОбработкаИзмененияСтрокиНаСервере("Товары",, СтрокаТЧ);
			КонецЦикла;
		КонецЕсли;
		Если Объект.Работы.Количество() <>  0 Тогда
...
Процедура ОбработкаИзмененияСтрокиНаСервере(ИмяТабличнойЧасти, Поле = Неопределено, ДанныеСтроки = Неопределено)
	...
	КонецЕсли;
//МАЗИН_Е_С_НАЧАЛО	
	//Если ИмяТабличнойЧасти = "Работы"  И  Объект.СкидкаМенеджер > 0 Тогда 
		//ТекДанные.СуммаВсего = ТекДанные.количество * ТекДанные.ЦенаСоСкидкойНаценкой;
	//КонецЕсли;
	//Если ИмяТабличнойЧасти = "Работы"  И Объект.НаценкаМенеджер > 0  Тогда 
		//ТекДанные.СуммаВсего = ТекДанные.количество * ТекДанные.ЦенаСоСкидкойНаценкой;
	//КонецЕсли;
//МАЗИН_Е_С_КОНЕЦ	
	Если Объект.Дата > Дата("20240110") и ИмяТабличнойЧасти = "Товары" Тогда
		ТекДанные.Сумма = ТекДанные.количество * ТекДанные.ЦенаСоСкидкойНаценкой;  
		...
		Если Объект.ТипНаценки = ПроверкаБезНДС() Тогда 	
			...
			ТекДанные.СуммаВсего = ТекДанные.Сумма + ТекДанные.СуммаНалог;
		КонецЕсли;
		Если Объект.ТипНаценки = Наличка() Тогда 	
			ТекДанные.СуммаНалог = 0;
			ТекДанные.СуммаВсего = ТекДанные.Сумма + ТекДанные.СуммаНалог;
		КонецЕсли;
		//--Мазин
		// ++Мазин 13_05_24 Добавил "ЕСЛИ"
	Иначе Если  (Объект.СкидкаМенеджер = 0  И Объект.НаценкаМенеджер = 0) Или (Объект.НаценкаМенеджер = Неопределено 
...
Если Объект.НаценкаМенеджер = 0 Тогда 
...
Если Объект.СкидкаМенеджер = 0 Тогда 
ПересчетТаблицТоварыИРаботаСкидкаНаценка ();
КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Запрос.Текст =
		//"ВЫБРАТЬ
		//|	Сотрудники.Наименование КАК Вадяева
		//|ИЗ
	...
	//Запрос.УстановитьПараметр("Наименование", "Вадяева Дарья Викторовна");
	...
	//Пока Выборка.Следующий()Цикл
		//Вадяева = Выборка.Вадяева;
	//КонецЦикла;
	
	
	
	//Если НЕ ТекущийПользователь.ПолноеИмя = Вадяева Тогда
 		//Элементы.СостояниеРасчетов.Доступность = Ложь;
	//КонецЕсли;
...
Процедура ПроверитьПользователя()
	...
	Если Пользователь = Справочники.Пользователи.НайтиПоНаименованию("Емелин Константин Валерьевич")  
		...
		// ++ МазинЕС 06-06-24
		Элементы.ЗаблокироватьТЧТовары.ТолькоПросмотр  = Ложь;
		// -- МазинЕС 06-06-24
	КонецЕсли;
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	ЕстьДоступКУдалению = ЕстьДоступКУдалению(ТекДанные.Состояние); 
	
	Если НЕ ЕстьДоступКУдалению Тогда
		Отказ = Истина;
		Сообщить("Нельзя удалить строку, запчасть установлена!");
	Иначе	
		Если НЕ Объект.Ссылка.Пустая() Тогда 
			ЗаписьЛога("удалил строку товара №" + ТекДанные.НомерСтроки + ", " + ТекДанные.Номенклатура + ", " + ТекДанные.Партия + ", " + ТекДанные.Автомобиль + ", "
			+ ТекДанные.Количество + ", " + ТекДанные.Цена + ", " + ТекДанные.Состояние + ", " + ТекДанные.Сумма + ", " + ТекДанные.СуммаНалог + ", " + ТекДанные.СуммаВсего
			+ ", " + ТекДанные.Склад);
		КонецЕсли;
	КонецЕсли;
	///-ГомзМА 22.12.2023
	...
	Если ВРаботе И Не ГрязевИлиСоболев() Тогда
		Отказ = Истина;
	КонецЕсли;
	///-ГомзМА 06.05.2024
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр, СтрокаТаблицы)
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда 
		Если ТекДанные = Неопределено Тогда
			ЗаписьЛога("Добавил строку товара №1");
		Иначе
			ЗаписьЛога("Добавил строку товара №" + Число(ТекДанные.НомерСтроки + 1));
		КонецЕсли;
	КонецЕсли;
	///-ГомзМА 22.12.2023
...
Процедура СостояниеПриИзмененииНаСервере()
	...
	//Если НЕ ТекущийПользователь.ПолноеИмя = Вадяева И Объект.Состояние = Перечисления.СостоянияЗаказНаряда.Выполнен Тогда
 		//Сообщить("Изменить статус на ""Выполнен"" нельзя");
...
//МАЗИН_Е_С_КОНЕЦ	
	Если Объект.Дата > Дата("20240110") и ИмяТабличнойЧасти = "Товары" Тогда
		Попытка
			//Мазин
			// изменил строку:  ТекДанные.Сумма = ТекДанные.количество * ТекДанные.Цена; 
			ТекДанные.Сумма = ТекДанные.количество * ТекДанные.ЦенаСоСкидкойНаценкой;  
		Исключение
		КонецПопытки;
		Если Объект.ТипНаценки = ПроверкаНДС() Тогда 	
...
Процедура РаботыПередУдалением(Элемент, Отказ)
	...
	ТекДанные = Элементы.Работы.ТекущиеДанные;
	Если НЕ Объект.Ссылка.Пустая() Тогда 
		ЗаписьЛога("удалил строку товара №" + ТекДанные.НомерСтроки + ", " + ТекДанные.Работа + ", " + ТекДанные.Содержание + ", " + ТекДанные.Цена + ", "
		+ ТекДанные.Количество + ", " + ТекДанные.СуммаВсего);
	КонецЕсли;
	///-ГомзМА 22.12.2023
...
Функция ГрязевИлиСоболев()
	ТекущийПользователь = ТекущийПользовательБазы();
		Если ТекущийПользователь = "Грязев Илья Константинович"  ИЛИ ТекущийПользователь = "Соболев Дмитрий Львович" Тогда
			Возврат Истина;
		Иначе 
			Возврат Ложь;
		КонецЕсли;
