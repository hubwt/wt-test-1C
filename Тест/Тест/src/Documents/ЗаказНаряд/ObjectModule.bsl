#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Запрос.Текст =
		...
		|	ЗаказНарядТовары.Партия КАК индкод,
		|	СУММА(ЕСТЬNULL(ЗаказНарядТовары.Количество, 0)) КАК Количество
		|ПОМЕСТИТЬ ВТ_ТЧЗаказНаряд
		|ИЗ
		|	Документ.ЗаказНаряд.Товары КАК ЗаказНарядТовары
		|ГДЕ
		|	ЗаказНарядТовары.Ссылка = &Ссылка
		|	И ЗаказНарядТовары.Партия <> ЗНАЧЕНИЕ(Справочник.ИндКод.ПустаяСсылка)
		...
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(РегистрНакопления1Остатки.Товар) КАК ПредставлениеТовар,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(РегистрНакопления1Остатки.индкод) КАК ПредставлениеИндкод,
		|	ЕСТЬNULL(РегистрНакопления1Остатки.КолвоОстаток, 0) КАК Остаток,
		|	ЕСТЬNULL(ВТ_ТЧЗаказНаряд.Количество, 0) КАК Количество,
		|	РегистрНакопления1Остатки.Склад КАК Склад,
		|	РегистрНакопления1Остатки.машина КАК Машина
		|ИЗ
		|	ВТ_ТЧЗаказНаряд КАК ВТ_ТЧЗаказНаряд
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РегистрНакопления1.Остатки(&МоментВремени, (Товар, индкод) В
		|			(ВЫБРАТЬ
		|				Т.Товар,
		|				Т.индкод
		|			ИЗ
		|				ВТ_ТЧЗаказНаряд КАК Т)) КАК РегистрНакопления1Остатки
		|		ПО ВТ_ТЧЗаказНаряд.Товар = РегистрНакопления1Остатки.Товар";
...
        //  Выполнение движений в регистры
			...
			Движение			 = Движения.РегистрНакопления1.ДобавитьРасход();
...
//		ЕСЛИ ЗначениеЗаполнено (ТекСтрокаТовары.Партия) ТОгда 	
