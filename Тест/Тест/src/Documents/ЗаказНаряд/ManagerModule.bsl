...
Функция ПолучитьДанныеПечатиДефектовочныйЛистТаб(МассивОбъектов) Экспорт
	...
	Запрос.Текст = 
		...
		|	И НЕ ИндНомер.Стеллаж.Наименование ПОДОБНО ""%"" + ""ANGAR%"" + ""%""
		|	И НЕ ИндНомер.Стеллаж.Наименование ПОДОБНО ""%"" + ""Korzina%"" + ""%""
		|	И РегистрНакопления1Остатки.Склад.Код <> ""000000008""
		|	И РегистрНакопления1Остатки.Склад.Код <> ""000000005""
		|СГРУППИРОВАТЬ ПО
		|	ВТ_1.Ссылка,
		|	ВТ_1.НомерСтроки,
		|	ВТ_1.Наименование,
