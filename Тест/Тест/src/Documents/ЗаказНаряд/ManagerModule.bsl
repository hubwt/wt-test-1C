...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//+++ МазинЕС 
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ДефектовочныйЛистДляКлиента";
	КомандаПечати.Представление = НСтр("ru = 'Дефектовочный лист для клиента'");
	КомандаПечати.Порядок       = 890;
	//--- МазинЕС 
КонецПроцедуры
...
//+++ МзинЕС
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ДефектовочныйЛистДляКлиента") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ДефектовочныйЛистДляКлиента", НСтр("ru = 'Дефектовочный лист для клиента'"), 
		ПечатьДефектовочныйЛистДляКлиента(МассивОбъектов, ОбъектыПечати) ,,"Документ.ЗаказНаряд.ПФ_MXL_ДефектовочныйЛистДляКлиента");
	КонецЕсли;
//+++ МзинЕС	
...
Функция ПечатьДефектовочныйЛистДляКлиента(МассивОбъектов, ОбъектыПечати)

//+++ МазинЕС печать дефектовочного листа для клиента	

	ДанныеПечатиШапки  = ПолучитьДанныеПечатиДефектовочныйЛистШапка(МассивОбъектов);// Переиспользую
	ДанныеПечатиТаб    = ПолучитьДанныеПечатиДефектовочныйЛистТабДляКлиента(МассивОбъектов);
	ДанныеПечатиПодвал = ПолучитьДанныеПечатиДефектовочныйЛистПодвал(МассивОбъектов);// Переиспользую

	Шапка 		 = ДанныеПечатиШапки;

	ТабДок = Новый ТабличныйДокумент;
	Макет  = ПолучитьМакет("ПФ_MXL_ДефектовочныйЛистДляКлиента");
	...
	//ОбластьТаблица.Параметры.Заполнить(ДанныеПечатиТаб); 
	//ТабДок.Область(НомерСтроки, ).ЦветФона = Новый Цвет(251, 140,112);	
	//ТабДок.Вывести(ОбластьТаблица);
			
	//ОбластьТаблица.Параметры.Заполнить(ДанныеПечатиТаб);   // ТабличныйДокумент.Область
	//ОбластьОформления = ТабДок.Вывести(ОбластьТаблица);
	////Строка = Строка.Низ;  
	//Строка = 1;
	//ТабДок.Область(Строка, 1,Строка ,10 ).ЦветФона = WebЦвета.Красный; 
	//Строка = Строка + 1; 	
	
	//ОбластьТаблица.Параметры.Заполнить(ДанныеПечатиТаб);
	//Обл = ТабДок.Присоединить(ОбластьТаблица);  
	//
	//Если ОбластьТаблица.Параметры.Остаток = "Нет на складе" Тогда
	//	Обл.Область(1,1,1,10).ЦветФона = Новый Цвет(251,140,112);
	//КонецЕсли;    
	
		ОбластьТаблица.Параметры.Заполнить(ДанныеПечатиТаб);
		
		Если ОбластьТаблица.Параметры.Остаток = "Нет на складе" Тогда
			ОбластьТаблица.Область(1,1,1,10).ЦветФона = Новый Цвет(251, 140, 112);
		Иначе
			ОбластьТаблица.Область(1,1,1,10).ЦветФона = Новый Цвет(255, 255, 255);    
		КонецЕсли;
...
Функция ПечатьЗаказНаряд(МассивОбъектов, ОбъектыПечати, ИмяМакета = "ЗаказНаряд", ВыводитьТоварыНаКомиссии = Истина) Экспорт
	...
	ВыборкаТоварыКлиента = ДанныеПечати.ТоварыКлиента;
	...
	Пока Шапка.Следующий() Цикл
		...
		ЕстьТоварыКлиента = ВыборкаТоварыКлиента.НайтиСледующий(СтруктураПоиска) И НЕ ВыводитьТолькоУслуги;
		...
		ПредставлениеКлиента = ?(СтрДлина(ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКлиенте, "ПолноеНаименование")) > 15, ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКлиенте, "ПолноеНаименование"), Шапка.Клиент);
		ДанныеПечатиШапкаДоп.Вставить("ЗаказчикПредставление","" +  ПредставлениеКлиента + " " +  ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКлиенте, "ИНН,ФактическийАдрес,Телефоны"));
		...
		Если ЕстьТовары Тогда
			...
			Пока СтрокаВыборки.Следующий() Цикл
				...
				//ОбластьМакета.Параметры.Заполнить(СтрокаВыборки);
				ЗаполнитьПараметрыОбласти(ОбластьМакета, СтрокаВыборки);
			     ОбластьМакета[0].Параметры.НомерСтроки = НомерСтроки; 
				НомерСтроки = НомерСтроки + 1;
				//ТабличныйДокумент.Вывести(ОбластьМакета);
				ВывестиОбластиВТабличныйДокумент(ТабличныйДокумент, ОбластьМакета);
		...
		// ТоварыКлиента  
		//Макет = Документы.ЗаказНаряд.ПолучитьМакет("ПФ_MXL_ЗаказНаряд");
		Если ЕстьТоварыКлиента Тогда
			ОбластьМакета = ПолучитьОбластьЗаказНаряда(Макет, "ДеталиКлиента", ЕстьСкидка, Истина);
			ДанныеПечатиШапкаДоп.Вставить("Заголовок", "Детали Клиента");
			ЗаполнитьПараметрыОбласти(ОбластьМакета, Шапка);
			ЗаполнитьПараметрыОбласти(ОбластьМакета, ДанныеПечатиШапкаДоп);
			...
			ОбластьМакета = ПолучитьОбластьЗаказНаряда(Макет, "СтрокаДеталейКлиента", ЕстьСкидка, Истина);
			СтрокаВыборки = ВыборкаТоварыКлиента.Выбрать();
			СуммаВсего = 0;
			СкидкаСумма = 0;
			Количество = 0;
			НДС = 0;
			...
			ОбластьМакета = ПолучитьОбластьЗаказНаряда(Макет, "ПодвалДеталейКлиента", ЕстьСкидка, Истина);
			//ОбластьМакета.Параметры.Заполнить(Шапка);
			//ОбластьМакета.Параметры.Заполнить(ДанныеПечатиДоп);
			ЗаполнитьПараметрыОбласти(ОбластьМакета, Шапка);
			ЗаполнитьПараметрыОбласти(ОбластьМакета, ДанныеПечатиДоп);
...
Функция ПолучитьДанныеПечатиДефектовочныйЛистТаб(МассивОбъектов) Экспорт
	...
	Запрос.Текст = 
		...
		|		ПО (ВТ_1.Наименование = ИндНомер.индкод.Владелец)
		|СГРУППИРОВАТЬ ПО
		|	ВТ_1.Ссылка,
		|	ВТ_1.НомерСтроки,
		|	ВТ_1.Наименование,
	...
	СостоянияТоваров.Добавить(Перечисления.СтатусыТовараВЗаказНаряде.Ожидание);
...
КонецФункции 
Функция ПолучитьДанныеПечатиДефектовочныйЛистТабДляКлиента(МассивОбъектов) Экспорт
/// +++ МазинЕС Функции для заполнения Шапки и подвала беру те которые используются для печади дифектов листа РВР
	...
	Запрос.Текст =
		...
		|	И ЗаказНарядТовары.Ссылка В (&МассивОбъектов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ПолучитьДанныеПечатиЗаказНаряда(МассивОбъектов) Экспорт
		...
		|	ТЧТовары.Партия.Представление КАК Партия,
		...
		|ГДЕ
		|	ТЧТовары.КлиентскаяДеталь = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ТЧТовары.Состояние КАК Статус,
		|	ТЧТовары.Автомобиль КАК Автомобиль
		|ПОМЕСТИТЬ ВТ_ТоварыКлиента
		|ИЗ
		|	ВТ_Шапка КАК Док
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаряд.Товары КАК ТЧТовары
		|		ПО (ТЧТовары.Ссылка = Док.Ссылка)
		|ГДЕ
		|	ТЧТовары.КлиентскаяДеталь = ИСТИНА
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ТоварыКлиента.НомерСтроки КАК НомерСтроки,
		|	ВТ_ТоварыКлиента.Номенклатура КАК Номенклатура,
		|	ВТ_ТоварыКлиента.Наименование КАК Наименование,
		|	ВТ_ТоварыКлиента.Артикул КАК Артикул,
		|	ВТ_ТоварыКлиента.НомерПроизводителя КАК НомерПроизводителя,
		|	ВТ_ТоварыКлиента.Код КАК Код,
		|	ВТ_ТоварыКлиента.Количество КАК Количество,
		|	ВТ_ТоварыКлиента.Цена КАК Цена,
		|	ВТ_ТоварыКлиента.Ссылка КАК Ссылка,
		|	ВТ_ТоварыКлиента.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	ВТ_ТоварыКлиента.Партия КАК Партия,
		|	ВТ_ТоварыКлиента.Сумма КАК Сумма,
		|	ВТ_ТоварыКлиента.СкидкаСумма КАК СкидкаСумма,
		|	ВТ_ТоварыКлиента.СуммаВсего КАК СуммаВсего,
		|	ВТ_ТоварыКлиента.НДС КАК НДС,
		|	ВТ_ТоварыКлиента.Склад КАК Склад,
		|	ВТ_ТоварыКлиента.Автомобиль КАК Автомобиль,
		|	ВТ_ТоварыКлиента.Статус КАК Статус,
		|	МестаХранения.МестоХранения КАК МестоХранения
		|ИЗ
		|	ВТ_ТоварыКлиента КАК ВТ_ТоварыКлиента
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.дт_МестаХраненияНоменклатуры.СрезПоследних КАК МестаХранения
		|		ПО ВТ_ТоварыКлиента.Номенклатура = МестаХранения.Номенклатура
		|		И ВТ_ТоварыКлиента.Склад = МестаХранения.Склад
		|ИТОГИ
		|ПО
		|	Ссылка";
		...
		Результат.Вставить("ТоварыКлиента", РезультатЗапроса[11].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам));
		Возврат Результат;
