...
#Область До_10000				
					Если КатегорияЦены < 10000 И Состояние = Продажа Тогда
						Дней = 86400 * 2;
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 5, Дней );
						
					ИначеЕсли КатегорияЦены < 10000 И Состояние = ОтказДорого Тогда
						Дней = 86400 * 2;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 5, 35, Дней );
...
#Область От_10000_До_50000
					Если (КатегорияЦены >= 10000 И КатегорияЦены < 50000) И Состояние = Продажа Тогда
						Дней = 86400 * 2;
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 4, Дней);
						
					ИначеЕсли (КатегорияЦены >= 10000 И КатегорияЦены < 50000) И Состояние = ОтказДорого Тогда
						Дней = 86400 * 2;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 4, 30, Дней );
