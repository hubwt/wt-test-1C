...
Если ЗначениеЗаполнено(Продажа) Тогда /// Если Есть продажа то строим запросы к ней
	...
	КолТоваров	= ОбъектПродажа.Таблица.Количество(); 
		...
		Если КолТоваров >  0 Тогда 
			...
			ПроверкаСобрано = Ложь; // На всякий случай
			ПроверкаВыдано = ложь; 	// На всякий случай
			Запрос = Новый Запрос;
			Запрос.Текст =
					"ВЫБРАТЬ
			...
			Если Флаг Тогда 
				...
				Если РезультатЗапроса.Пустой() Тогда 
					...
					ПроверкаСобрано = Перечисления.ЭтапыПродажи.Собрано;
					Флаг2 = Истина;    
				КонецЕсли; 	
			КонецЕсли; 
			...
			Запрос = Новый Запрос;
					Запрос.Текст =
						...
						Если РезультатЗапроса.Пустой() Тогда  
							...
							ПроверкаВыдано = Перечисления.ЭтапыПродажи.Выдано;
						КонецЕсли; 
			...
			Если НЕ РезультатЗапроса.Пустой() ТОгда 	
				...
				Пока Выборка.Следующий() Цикл 
					...
					КонецЕсли; 
					// Оформлнено 	
					Если Выборка.РаспечатанНаряднаДоставку = Истина И ООО = Ложь Тогда 
						Этап = Перечисления.ЭтапыПродажи.Оформлено; 
					КонецЕсли ; 
					// Оформлнено 		
					Если ЗначениеЗаполнено(Выборка.НомерУПД) И Выборка.РаспечатанНаряднаДоставку = Истина  Тогда 
						Этап = Перечисления.ЭтапыПродажи.Оформлено; 
					КонецЕсли ; 
					// Отправлено		
					Если ЗначениеЗаполнено(Выборка.НомерУПД) И Выборка.Самовывоз = Истина Тогда 
						Этап = Перечисления.ЭтапыПродажи.Отправлено; 
					КонецЕсли ; 
					// Отправлено	
					Если  Выборка.Самовывоз = Истина И ООО = Ложь Тогда 
						Этап = Перечисления.ЭтапыПродажи.Отправлено; 
					КонецЕсли ; 
					// Отправлено	
