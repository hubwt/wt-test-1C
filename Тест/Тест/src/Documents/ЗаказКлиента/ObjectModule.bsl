		...
		Если Товары.Количество() <> 0 Тогда 
			...
			Для каждого СтрокаТЧ Из Товары Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					КатегорияЦены = ?(Выборка.ЦенаПослеИзменения = 0, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения );
#Область До_10000				
					Если КатегорияЦены < 10000 И Состояние = Продажа Тогда
						
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 5, 86400, Дата );
						
					ИначеЕсли КатегорияЦены < 10000 И Состояние = ОтказДорого Тогда
						
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 5, 35, 86400, Дата );
...
#Область От_10000_До_50000
					Если (КатегорияЦены >= 10000 И КатегорияЦены < 50000) И Состояние = Продажа Тогда
						...
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 4, 86400, Дата);
						
					ИначеЕсли (КатегорияЦены >= 10000 И КатегорияЦены < 50000) И Состояние = ОтказДорого Тогда
						
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 4, 30, 86400, Дата );
...
#Область От_50000_До_100000	
					Если (КатегорияЦены >= 50000 И КатегорияЦены < 100000) И Состояние = Продажа Тогда
						Дней = 86400 * 3;
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 3, Дней, Дата);
						
					ИначеЕсли (КатегорияЦены > 50000 И КатегорияЦены <= 100000) И Состояние = ОтказДорого Тогда
						Дней = 86400 * 3;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 3, 25, Дней, Дата );
...
#Область От_100000_До_200000
					Если (КатегорияЦены >= 100000 И КатегорияЦены < 200000) И Состояние = Продажа Тогда
						Дней = 86400 * 7;
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 2, Дней, Дата);
						
					ИначеЕсли (КатегорияЦены >= 100000 И КатегорияЦены < 200000) И Состояние = ОтказДорого Тогда
						Дней = 86400 * 7;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 2, 20, Дней, Дата );
...
#Область От_200000_До_500000
					...
					ИначеЕсли (КатегорияЦены >= 200000 И КатегорияЦены < 500000) И Состояние = ОтказДорого Тогда
						Дней = 86400 * 7;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 2, 15, Дней, Дата );
...
Процедура УвеличитьЦену(Номенклатура, ЦенаДоИзменения, ЦенаПослеИзменения, ПроцентЧисло, ДеньВСек, Дата)
	/// Комлев 04/06/24 +++
	...
	ДатаУстановки = НачалоДня((Дата + ДеньВСек));
	Ценообразование(Номенклатура, ДатаУстановки, НоваяЦена, Истина);
	
	/// Комлев 04/06/24 ---
КонецПроцедуры

Процедура СнизитьЦену(Номенклатура, ЦенаДоИзменения, ЦенаПослеИзменения, ПроцентЧисло, МаксПроцентЧисло, ДеньВСек, Дата)
	/// Комлев 04/06/24 +++
	Процент  = ЦенаДоИзменения / 100 * ПроцентЧисло;
