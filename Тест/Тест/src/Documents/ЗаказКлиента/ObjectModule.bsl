...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
		...
		//	Склад = Справочники;
		//КонецЕсли;
		WTPanel = Справочники.СтатусыWT.НайтиПоКоду("000000009");
...
#Область До_10000				
					Если КатегорияЦены < 10000 И Состояние = Продажа Тогда
						...
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 5, 86400 );
						
					ИначеЕсли КатегорияЦены < 10000 И Состояние = ОтказДорого Тогда
						
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 5, 35, 86400 );
...
#Область От_10000_До_50000
					Если (КатегорияЦены >= 10000 И КатегорияЦены < 50000) И Состояние = Продажа Тогда
						...
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 4, 86400);
						
					ИначеЕсли (КатегорияЦены >= 10000 И КатегорияЦены < 50000) И Состояние = ОтказДорого Тогда
						
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 4, 30, 86400 );
...
#Область От_50000_До_100000	
					Если (КатегорияЦены >= 50000 И КатегорияЦены < 100000) И Состояние = Продажа Тогда
						Дней = 86400 * 3;
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 3, Дней );
						
					ИначеЕсли (КатегорияЦены > 50000 И КатегорияЦены <= 100000) И Состояние = ОтказДорого Тогда
						Дней = 86400 * 3;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 3, 25, Дней );
...
#Область От_100000_До_200000
					Если (КатегорияЦены >= 100000 И КатегорияЦены < 200000) И Состояние = Продажа Тогда
						Дней = 86400 * 7;
						УвеличитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 2, Дней );
						
					ИначеЕсли (КатегорияЦены >= 100000 И КатегорияЦены < 200000) И Состояние = ОтказДорого Тогда
						Дней = 86400 * 7;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 2, 20, Дней );
...
#Область От_200000_До_500000
					...
					ИначеЕсли (КатегорияЦены >= 200000 И КатегорияЦены < 500000) И Состояние = ОтказДорого Тогда
						Дней = 86400 * 7;
						СнизитьЦену(Выборка.Номенклатура, Выборка.ЦенаДоИзменения, Выборка.ЦенаПослеИзменения, 2, 15, Дней );
