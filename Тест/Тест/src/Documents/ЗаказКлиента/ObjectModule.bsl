		...
		Если НЕ	ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("Руководитель")) Тогда 
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Пока РезультатЗапроса.Следующий() Цикл
					/// Комлев 23/05/24 +++
					Разница = ТекущаяДата() - РезультатЗапроса.Дата;
					/// Комлев 23/05/24 ---
					Если (Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Ожидание ИЛИ
						Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Думает И Разница < 172800 )  И /// И Заявка висит меньше 2 дней /// Комлев 23/05/24
