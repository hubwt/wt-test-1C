...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если  ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("Контролёр")) Тогда   
	
		Элементы.Прослушано.Видимость = Истина;
		Элементы.Контроль.Видимость = Истина;
		Элементы.Закрыта.Видимость = Истина; 
		//Элементы.Контролёр1.Видимость = Истина;
...
Процедура ПерейтиВЗаявкуWT10(Команда)
	...
	Товары = ""; 
	Для каждого Строка Из Объект.Товары Цикл 
		Товары = Товары + Строка.Номенклатура.Наименование + Символы.ПС; 
	КонецЦикла; 
	
	Текст = ""; 
	Текст =Текст + "Номер заявки: " +  объект.Номер; 
	Текст =Текст + "Ответственный: " + объект.Ответственный + Символы.ПС; 
	Текст =Текст + "Товары: " + Товары +  Символы.ПС; 
	Текст =Текст + "Сумма документа: " + объект.СуммаДокумента; 
