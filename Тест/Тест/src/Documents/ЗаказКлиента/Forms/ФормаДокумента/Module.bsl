...
Функция ПолучитьЗначенияПеречисления()
	...
	Результат = Новый Массив;
   	...
   	Если Объект.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Думает Тогда
		 ...
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.МониторингЦен);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ДанныеЗаполнения)
	...
	ДанныеЗаполнения.Цена = Документы.ЗаказКлиента.ПолучитьЦенуПоРасспродажеИлиРекЦену(ДанныеЗаполнения.Номенклатура, ДанныеЗаполнения.Дата);
