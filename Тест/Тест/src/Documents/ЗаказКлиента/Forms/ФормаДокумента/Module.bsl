...
Процедура СостояниеПриИзменении(Элемент)
	...
	ДопСостоянияНастроитьВидимость();
КонецПроцедуры
...
//	Объект.ДопСостояние = Неопределено;
	
КонецПроцедуры

&НаСервере
Процедура ДопСостоянияНастроитьВидимость()

	///+ТатарМА 19.12.2024
	Если Объект.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Думает ИЛИ 
	 Объект.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Отказ Тогда
...
&НаКлиенте
Процедура ДопСостояниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	///+ТатарМА 19.12.2024
	 Элемент.СписокВыбора.ЗагрузитьЗначения(ПолучитьЗначенияПеречисления());
     ///-ТатарМА 19.12.2024
        
КонецПроцедуры

&НаСервере
Функция ПолучитьЗначенияПеречисления()
	
	///+ТатарМА 19.12.2024
	Результат = Новый Массив;
    
   	Если Объект.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Думает Тогда
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ВысокаяЦена);	
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.СкоростьДоставки);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ЦенаДоставки);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.НужноСогласовать);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ХочетКупитьВсеВОдномМесте);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.КонкурентыПредложилиЛучше);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ИзменилисьПланы);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.МониторитЦены);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ЖдетСтраховую);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ЕстьНесколькоВариантов);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.СтоитПередВыбором);
		 Результат.Добавить(Перечисления.ДопСостоянияЗаявок.НеСпросил);
	ИначеЕсли Объект.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Отказ Тогда
		Результат.Добавить(Перечисления.ДопСостоянияЗаявок.НизкоеКачество);
		Результат.Добавить(Перечисления.ДопСостоянияЗаявок.УКлиентаНетДенег);
		Результат.Добавить(Перечисления.ДопСостоянияЗаявок.КонкурентыПредложилиЛучше);
		Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ИзменилисьПланы);
		Результат.Добавить(Перечисления.ДопСостоянияЗаявок.ДолгоДавалиОтвет);
		Результат.Добавить(Перечисления.ДопСостоянияЗаявок.НеСпросил);
	КонецЕсли;
...
&НаСервере
Процедура ПересчетТаблициТовары()

// Изменяем цену в ТЧ Товары с учетом скидки или наценки

	Если Объект.Наценка = 0 И Объект.Скидка > 0 Тогда
		Если Объект.Товары.Количество() <> 0 Тогда
			Для Каждого СтрокаТЧ Из Объект.Товары Цикл
				СтрокаТЧ.ЦенаСоСкидкойНаценкой = СтрокаТЧ.Цена - СтрокаТЧ.Цена * Объект.Скидка / 100;
				СтрокаТЧ.Сумма = СтрокаТЧ.ЦенаСоСкидкойНаценкой;
			КонецЦикла;
			Объект.ВыгодаКлиента =  Объект.Товары.Итог("Цена") - Объект.Товары.Итог("ЦенаСоСкидкойНаценкой");
			Объект.ВыгодаКомпании = 0;
			Объект.СуммаДокумента = Объект.Товары.Итог("ЦенаСоСкидкойНаценкой");
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект.Наценка > 0 И Объект.Скидка = 0 Тогда
		Если Объект.Товары.Количество() <> 0 Тогда
			Для Каждого СтрокаТЧ Из Объект.Товары Цикл
				СтрокаТЧ.ЦенаСоСкидкойНаценкой = СтрокаТЧ.Цена + СтрокаТЧ.Цена * Объект.Наценка / 100;
				СтрокаТЧ.Сумма = СтрокаТЧ.ЦенаСоСкидкойНаценкой;
			КонецЦикла;
			Объект.ВыгодаКомпании =  Объект.Товары.Итог("ЦенаСоСкидкойНаценкой") - Объект.Товары.Итог("Цена");
			Объект.ВыгодаКлиента = 0;
			Объект.СуммаДокумента = Объект.Товары.Итог("ЦенаСоСкидкойНаценкой");
		КонецЕсли;
	КонецЕсли;

	Если Объект.Скидка = 0 И Объект.Наценка = 0 Тогда
		Если Объект.Товары.Количество() <> 0 Тогда
			Для Каждого СтрокаТЧ Из Объект.Товары Цикл
				СтрокаТЧ.ЦенаСоСкидкойНаценкой = СтрокаТЧ.Цена;
				СтрокаТЧ.Сумма = СтрокаТЧ.ЦенаСоСкидкойНаценкой;
			КонецЦикла;
			Объект.ВыгодаКлиента = 0;
			Объект.ВыгодаКомпании = 0;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если пользователи.РолиДоступны("дт_ПолныеПрава") Тогда
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Объект.ФинансовыеСтатусы = Перечисления.ФинансовыеСтатусыДляЗаявкиКлиента.НеОплачено;
		Объект.СтатусыДействия = Перечисления.дт_СостоянияЗаказовКлиента.Ожидание;
	КонецЕсли;
	/// Комлев 22/07/24 ---
	...
	/// Комлев 22/08/24 +++
	Если Объект.ЗаявкаССайта = Истина Тогда
		Элементы.ЗаявкаЧерезСайт.Видимость = Истина;
	Иначе 
		Элементы.ЗаявкаЧерезСайт.Видимость = Ложь;
	КонецЕсли;
	/// Комлев 22/08/24 ---
	
	ДопСостоянияНастроитьВидимость();
	//Элементы.ДопСостояние.СписокВыбора.ЗагрузитьЗначения(ПолучитьЗначенияПеречисления());
	
КонецПроцедуры

&НаКлиенте
Процедура РазделСкидкиНаценкиПриИзменении(Элемент)
	//++ МазинЕС 14-06-24
	
	Если РазделСкидкиНаценки = Истина Тогда 
		Элементы.СкидкаНаценка.Видимость = истина;  
	Иначе Элементы.СкидкаНаценка.Видимость = ложь;
	Конецесли; 
...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.Скидка > 0 Тогда 
		Элементы.ВыгодаКлиента.Видимость = Истина; 
		Элементы.ВыгодаКомпании.Видимость = Ложь; 
	ИначеЕсли Объект.Наценка > 0  Тогда
		Элементы.ВыгодаКлиента.Видимость = Ложь; 
		Элементы.ВыгодаКомпании.Видимость = Истина; 
	Иначе 
		Элементы.ВыгодаКлиента.Видимость = Ложь; 
		Элементы.ВыгодаКомпании.Видимость = Ложь; 
	КонецЕсли;
	...
	Если НЕ дт_ОбщегоНазначенияВызовСервераПовтИсп.ЭтоПолноправныйПользователь() 
		...
		Иначе
			Если ЗначениеЗаполнено(ТекДанные.Номенклатура) Тогда
				Цена = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьРекомендованнуюЦену(ТекДанные.Номенклатура, Объект.Дата);
			КонецЕсли;
		КонецЕсли;
		...
		Если Цена <> Неопределено Тогда
			...
			Если ТекДанные.Сумма < МинимальнаяСумма Тогда
				...
				ТекстСообщения = "Минимальная сумма для " + ТекДанные.Номенклатура + " - " + МинимальнаяСумма;
...
Процедура ТоварыПриИзменении(Элемент)
	ТекущиеДанные = Элементы.товары.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
			ТекущиеДанные.МестоХранения = ПолучитьМестоХранения(ТекущиеДанные.Номенклатура, ТекущиеДанные.Склад);

		Если НЕ Объект.Ссылка.Пустая() Тогда 
			ЗаписьЛога("изменил/добавил " + ТекущиеДанные.НомерСтроки + " " + ТекущиеДанные.Номенклатура + " "+ ТекущиеДанные.Цена);
		КонецЕсли;
	КонецЕсли;
	заполнитьСумму();
КонецПроцедуры
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(объект.Канал) Тогда
		//@skip-check use-non-recommended-method
		Сообщить("Канал необходимо заполнить!");
		Отказ = Истина;	
	КонецЕсли;
	
	Если (Объект.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Думает ИЛИ Объект.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Отказ)  И
	Не ЗначениеЗаполнено(Объект.ДопСостояние) Тогда
		Сообщить("Необходимо заполнить Доп.состояние!");
		Отказ = Истина;
	КонецЕсли;
...
Процедура КраткоеОписание(Команда)
	...
	+ Товары  + Ссылка +  Символы.ПС + объект.СуммаДокумента + " - "+ объект.Состояние ; 
