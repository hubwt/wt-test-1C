...
Функция ПолучениеОстатковПоСкладам(Товар)
	...
	ЗапросОстатков.Текст =
		...
		|ИЗ
		|	РегистрНакопления.РегистрНакопления1.Остатки(, Товар = &Товар
		|	И Склад = &Склад) КАК РегистрНакопления1Остатки";
		
	НижН = Справочники.Склады.НайтиПоКоду("000000002");
	МСК = Справочники.Склады.НайтиПоКоду("000000005");
	ЕКБ = Справочники.Склады.НайтиПоКоду("000000008");
	
	
	ЗапросОстатков.УстановитьПараметр("Товар", Товар);
	ОстаткиСтруктура = Новый Структура();
	
	ЗапросОстатков.УстановитьПараметр("Склад", НижН);
	РезультатЗапроса = ЗапросОстатков.Выполнить().Выбрать();
	РезультатЗапроса.Следующий();
	ОстаткиСтруктура.Вставить("НН", РезультатЗапроса.КолвоОстаток);
	
	ЗапросОстатков.УстановитьПараметр("Склад", МСК);
	РезультатЗапроса = ЗапросОстатков.Выполнить().Выбрать();
	РезультатЗапроса.Следующий();
	ОстаткиСтруктура.Вставить("МСК", РезультатЗапроса.КолвоОстаток);
	
	
	ЗапросОстатков.УстановитьПараметр("Склад", ЕКБ);
	РезультатЗапроса = ЗапросОстатков.Выполнить().Выбрать();
	РезультатЗапроса.Следующий();
	ОстаткиСтруктура.Вставить("ЕКБ", РезультатЗапроса.КолвоОстаток);	
