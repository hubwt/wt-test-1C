...
Процедура КраткоеОписание(Команда)
	...
	Если Объект.Товары.Количество() > 0 тогда
	Для каждого Строка Из Объект.Товары Цикл 
		Товары = Товары + Строка.Номенклатура; 
		Прервать; 
	КонецЦикла; 
	конецЕсли; 
	...
	Текст =Текст + "№" +  объект.Номер + Символы.ПС; 
	Текст =Текст  + ОтветственныйФамилия + Символы.ПС; 
	Текст =Текст + Товары +  Символы.ПС; 
	Текст =Текст + Ссылка +  Символы.ПС; 
	Текст =Текст + объект.СуммаДокумента + " - "+ объект.Состояние; 
