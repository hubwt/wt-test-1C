 ...
 Процедура СменаСтраницыСервер(Стр)
	 ...
	 Иначе
		 ...
		 ИначеЕсли стр > 10  и стр < 13 Тогда 	
			 ...
			 Список.Параметры.УстановитьЗначениеПараметра("ДатаСвязиНачало",    (НачалоДня(ТекущаяДата())+(стр-8)*24*3600));
			 Список.Параметры.УстановитьЗначениеПараметра("ДатаСвязиОкончание", (КонецДня(ТекущаяДата())+(стр-8)*24*3600));
			 Если Менеджер <> Справочники.Пользователи.ПустаяСсылка() Тогда
				 Список.Параметры.УстановитьЗначениеПараметра("Ответственный", Менеджер);	
			 КонецЕсли;
		 КонецЕсли; 	
 ...
 Процедура ФильтрЗаявокПриИзменении(Элемент)
	 ...
	 КонецЕсли;
	  ...
	  Если ФильтрЗаявок = "Дата2" Тогда
		 СменаСтраницыСервер(12);
		// ЗаполнитьПодвал(10);
		 НомерСтраницы = 12;
	 КонецЕсли;
	 
	  Если ФильтрЗаявок = "Дата3" Тогда
		 СменаСтраницыСервер(13);
		// ЗаполнитьПодвал(11);
		 НомерСтраницы = 13;
	 КонецЕсли;
