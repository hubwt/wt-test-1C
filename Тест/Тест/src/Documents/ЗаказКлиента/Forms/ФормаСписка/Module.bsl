...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Волков ИО 28.12.23 ++
	Список.Параметры.УстановитьЗначениеПараметра("ДатаНач", ДобавитьМесяц(ТекущаяДата(), -6));
	НовыйЭлемент = Элементы.Добавить("ФактРеакцияНаЗаявку", Тип("ПолеФормы"), Элементы.Список);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.Заголовок = "Факт реация на заявку";
	НовыйЭлемент.ПутьКДанным = "Список.ФактРеакцияНаЗаявку"; 
	//Волков ИО 28.12.23 --
	...
	/// Комлев 23/05/24 +++
	// Меняет статус заявки на Ожидание если заявка со статусом Думает, висит дольше 2х дней.
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗаказКлиента.Ссылка,
		|	ЗаказКлиента.Дата КАК Дата,
		|	ЗаказКлиента.ДатаИзмененияСостояния
		|ИЗ
		|	Документ.ЗаказКлиента КАК ЗаказКлиента
		|ГДЕ
		|	ЗаказКлиента.Состояние = ЗНАЧЕНИЕ(Перечисление.дт_СостоянияЗаказовКлиента.Думает)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Разница = ТекущаяДата() - ВыборкаДетальныеЗаписи.ДатаИзмененияСостояния;
		Если Разница >= 172800 Тогда
			Док = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
			Док.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Ожидание;
			Попытка
				Док.Записать();
			Исключение
				Док.РазрешитьИзменениеПрограммно = Истина;
				Док.Записать();
			КонецПопытки;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	//Волков ИО 28.12.23 ++
	ПолучитьПланРеакцииНаЗаявку();
	//Волков ИО 28.12.23 -- 
	
КонецПроцедуры

Процедура	ПолучитьПланРеакцииНаЗаявку()

   //ЭтотОбъект.ПланРеакцияНаЗаявку.ПланРеакцияНаЗаявку = Константы.ПланРеакцияНаЗаявку.Получить();
...
//Волков ИО 28.12.23 ++
&НаСервере
Процедура ЗаписатьПланРеакцииНаЗаявкуНаСервере()

	//Константы.ПланРеакцияНаЗаявку.Установить(ПланРеакцияНаЗаявку);
	//Константы.ПланРеакцияНаЗаявку.Установить(ПланРеакцияНаЗаявку.ПланРеакцияНаЗаявку);
	//Элементы.Список.Обновить();
...
&НаКлиенте
Процедура ЗаписатьПланРеакцииНаЗаявку(Команда)
	//ЗаписатьПланРеакцииНаЗаявкуНаСервере();
КонецПроцедуры
