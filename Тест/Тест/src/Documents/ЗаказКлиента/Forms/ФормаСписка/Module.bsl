...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ПродажаЗапчастей.ЗаказКлиента.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ПродажаЗапчастей КАК ПродажаЗапчастей
		|ГДЕ
		|	ПродажаЗапчастей.Оплачено = ИСТИНА
		|	И ПродажаЗапчастей.ЗаказКлиента.Состояние = ЗНАЧЕНИЕ(Перечисление.дт_СостоянияЗаказовКлиента.ЖдёмДенег)";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Заявка.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Продажа;
			Заявка.Записать();
...
Процедура ПерейтиВЗаявкуWT10(Команда)
	...
	ОбщДействия.ОткрытьЗаявкуНаСайтеWT10(ЭтаФорма.Элементы.Список.ТекущиеДанные.Номер);
	// -- МазинЕС 22/07/2024	
