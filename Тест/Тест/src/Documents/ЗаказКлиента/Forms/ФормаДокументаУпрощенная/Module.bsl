...
Процедура ЭтапыПродажиПриИзменении(Элемент)
	Если НЕ Объект.ЭтапыПродажи = ПредопределенноеЗначение("Перечисление.ЭтапыПродажи.Отказ") ИЛИ (НЕ Объект.ЭтапыПродажи = ПредопределенноеЗначение("Перечисление.ЭтапыПродажи.Сборка")) ТОгда 
		ОпределениеЭтапаПродаж(); 
	КонецЕсли; 
КонецПроцедуры
...
//	ОбработкаИзмененияСтроки("Товары", "Цена"); МазинПравки
КонецПроцедуры
...
&НаСервере
Функция ОпределениеЭтапаПродаж()
	ЭтапаПродажСобрано(); // Собран
КонецФункции

&НаСервере
Функция ЭтапаПродажСобрано()
	...
	Запрос.Текст =
		...
		|	ИЛИ ЗаказКлиентаТовары.Выдан = Ложь)";
	
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() ТОгда 
		Объект.ЭтапыПродажи = Перечисления.ЭтапыПродажи.Собрано; 
	конецЕсли; 
