...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ТипСделки = Перечисления.ТипСделки.Заявка И Объект.ПродажаНайденная <> Документы.ПродажаЗапчастей.ПустаяСсылка() Тогда
		ОтменитьПродажу();
	КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ТекДанные.Цена 		 	= дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьРекомендованнуюЦену(ТекДанные.Номенклатура, ТекущаяДата());
