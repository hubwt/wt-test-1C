...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.Товары.ТекущаяСтрока = Неопределено Тогда 
		СписокПартий.Параметры.УстановитьЗначениеПараметра("Товар",Справочники.Номенклатура.ПустаяСсылка());
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПродажаНайденная = ЕстьПродажаВЗаявке(ТекущийОбъект.Ссылка);
	Если Объект.ТипСделки = Перечисления.ТипСделки.Продажа И ПродажаНайденная = Документы.ПродажаЗапчастей.ПустаяСсылка() Тогда
		СоздатьПродажу(ТекущийОбъект);  
		Возврат ;
	КонецЕсли;

	Если Объект.Продажа <> Документы.ПродажаЗапчастей.ПустаяСсылка() Тогда
		ОбновитьПродажу(ТекущийОбъект);
	КонецЕсли;
...
Процедура ЭтапыПродажиПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ЭтапыПродажи = ПредопределенноеЗначение("Перечисление.ЭтапыПродажи.Выдать") и НЕ ЗначениеЗаполнено(объект.Продажа) ТОгда 
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Не создан документ продажа. Код ошибки: 123554"; 
		Сообщение.Сообщить();	
...
//	Этап = ЭтапированиеЗаявки.ЭтапированиеЗаявки(Объект); 
//	Если ЗначениеЗаполнено(Этап) Тогда 
//		Объект.ЭтапыПродажи = Этап; 
//	КонецЕСли; 
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	Этап = ДокументОбъект.ЭтапированиеЗаявки(); 
	Если ЗначениеЗаполнено(Этап) Тогда 
		Объект.ЭтапыПродажи = Этап; 
	КонецЕСли; 
...
Процедура СсылкаНаОтслеживаниеДоставкиНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ЗапуститьПриложение(СсылкаНаОтслеживаниеДоставки); 
...
Процедура НарядНаДоставку(Команда)
	...
	Записать();
	Если Объект.ЭтапыПродажи = ПредопределенноеЗначение("Перечисление.ЭтапыПродажи.Выдано") 
	ИЛИ Объект.ЭтапыПродажи = ПредопределенноеЗначение("Перечисление.ЭтапыПродажи.Оформлено")
	ИЛИ Объект.ЭтапыПродажи = ПредопределенноеЗначение("Перечисление.ЭтапыПродажи.Отправлено") Тогда 
	
	Иначе	
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Ошибка!!! Товар не выдан. Код ошибки 010567"; 
		Сообщение.Сообщить();
		Возврат; 
	КонецЕсли; 
...
&НаКлиенте
Процедура КраткоеОписание(Команда)
	Товары = ""; 
	Если Объект.Товары.Количество() > 0 тогда
	Счетчик =0; 
	Для каждого Строка Из Объект.Товары Цикл 
		Товары = Товары + Строка.Номенклатура + Символы.ПС; 
		Счетчик = Счетчик + 1; 
	...
	Ответственный  = СтрЗаменить(Ответственный," ",Символы.ПС);
	ОтветственныйФамилия = СтрПолучитьСтроку(Ответственный,1);
	Ссылка = "https://wt10.ru/application-details/"+объект.Номер; 
	
	Текст =Текст + "№" +  объект.Номер + Символы.ПС + ОтветственныйФамилия + Символы.ПС 
	+ Товары  + Ссылка +  Символы.ПС + объект.СуммаДокумента + " - "+ объект.ЭтапыПродажи ; 
...
&НаСервере
Процедура ОбновитьПродажу(ТекущийОбъект)
	Попытка
	Заблокирована = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийОбъект.Продажа, "Заблокирована");
	Если Не Заблокирована Тогда
		ОбъектПродажа = ТекущийОбъект.Продажа.ПолучитьОбъект();
		ОбъектПродажа.Организация = ТекущийОбъект.Организация;
		ОбъектПродажа.Клиент = ТекущийОбъект.Клиент;
		ОбъектПродажа.КтоПродал = ТекущийОбъект.Ответственный;
	...
	// Обновить данные доставки
		...
		ОбъектПродажа.Телефон = Строка(ТекущийОбъект.НомерТелефона);
...
//		КонецЕсли;
		...
		ОбъектПродажа.Заблокирована =  ТекущийОбъект.Заблокирована; 
		...
		НаборЗаписей.Отбор.Регистратор.Установить (ТекущийОбъект.Продажа);
		НаборЗаписей.Прочитать();
		НаборЗаписей.Очистить();
		НаборЗаписей.Записать();   
		// --- 
		ОбъектПродажа.Таблица.Очистить();
		Для Каждого СтрокаТЧЗаявки Из ТекущийОбъект.Товары Цикл
	...
	Исключение
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Ошибка!!! не получилось синхронизовать продажу. Пишите в тех поддержку! Код Ошибки: 03121994"; 
		Сообщение.Сообщить();
	КонецПопытки; 
КонецПроцедуры

&НаСервере
Процедура СоздатьПродажу(ТекущийОбъект)
	
	НовыйДокумент = Документы.ПродажаЗапчастей.СоздатьДокумент();
	НовыйДокумент.Дата = ТекущаяДатаСеанса();
	НовыйДокумент.Клиент = ТекущийОбъект.Клиент;
	НовыйДокумент.ЗаказКлиента = ТекущийОбъект.Ссылка;
...
&НаСервереБезКонтекста
Функция ПолучитьЦенуПоРасспродажеИлиРекЦену(Номенклатура, Дата)
	Возврат Документы.ЗаказКлиента.ПолучитьЦенуПоРасспродажеИлиРекЦену(Номенклатура, Дата);
КонецФункции
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	ТекДанные = Элементы.Товары.ТекущиеДанные;
	ТекДанные.Количество 	= 1;
	ТекДанные.Цена 		 	= ПолучитьЦенуПоРасспродажеИлиРекЦену(ТекДанные.Номенклатура, Объект.Дата);
	//ТекДанные.Сумма 		= ТекДанные.Цена * ТекДанные.Количество;
...
/// +++ Добавляем динамический список партий----------------------------------------------------------------------------

&НаКлиенте
Процедура ОбновитьОтборПартий()
	
	ТекДанные = Элементы.Товары.ТекущиеДанные;
	СписокПартий.Параметры.УстановитьЗначениеПараметра("Товар", ТекДанные.Номенклатура);
...
//
//&НаСервере
//Функция ПередЗаписьюНаСервере2() 
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ДатаЗаявки", Объект.Дата); 
...
//&НаСервере
//Функция ВызовССервера() 
//	Этап = ЭтапированиеЗаявки.ЭтапированиеЗаявки(,Объект.Ссылка);
//	вОЗВРАТ Этап;
//КонецФункции
//&НаСервере                                                
