...
Если ХанаБонусам = Истина Тогда 
	...
	СтрокаМестоРаботы =  ВернутьМестоРаботыпользователя(СтрокаТЧ.Сотрудник); 
		Если СтрокаМестоРаботы = МестоРаботыННКод И Структура.ДоляКладовщикаНН <> 0 Тогда
			...
			ЗаписьВРегистреСведений.Бонус 		= СуммаДокументПродажа * Структура.ДоляКладовщикаНН * 0.005;
		...
		Если СтрокаМестоРаботы = МестоРаботыМСККод И Структура.ДоляКладовщикаМСК <> 0 Тогда
			...
			ЗаписьВРегистреСведений.Продажа 	= ссылка;
			ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
			ЗаписьВРегистреСведений.Бонус 		= СуммаДокументПродажа * Структура.ДоляКладовщикаМСК * 0.005;
			ЗаписьВРегистреСведений.Записать();
			Итер = Итер + 1;
		КонецЕсли;
		...
		Если СтрокаМестоРаботы = МестоРаботыЕКБКод И Структура.ДоляКладовщикаЕКБ <> 0 Тогда
			...
			ЗаписьВРегистреСведений.Продажа 	= ссылка;
			ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
			ЗаписьВРегистреСведений.Бонус 		= СуммаДокументПродажа * Структура.ДоляКладовщикаЕКБ * 0.005;
			ЗаписьВРегистреСведений.Записать();
			Итер = Итер + 1;
		КонецЕсли;
	КонецЕсли;
...
КонецФункции
	...
	Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказКлиента") И НЕ ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("МенеджерПоПродажам"))  Тогда
