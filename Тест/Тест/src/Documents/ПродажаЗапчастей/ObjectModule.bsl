#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ОбработкаПроведения(Отказ, Режим)
		...
		//Добавление бонусов в карточку Клиенты
			...
			ЕстьБонусыЗаЭтуПродажуИСуммаНеИзменилась = Ложь;
			
			 Для каждого Строка Из КлиентОбъект.БонусыКлиента Цикл 
			 	Если Строка.Продажа = Ссылка И Строка.СуммаПродажи =  ИтогоБезнал Тогда
			 		ЕстьБонусыЗаЭтуПродажуИСуммаНеИзменилась = Истина;
			 	КонецЕсли;
			 КонецЦикла; 
			 
			 Если Не ЕстьБонусыЗаЭтуПродажуИСуммаНеИзменилась  Тогда
			 	НоваяСтрокаТЧ = КлиентОбъект.БонусыКлиента.Добавить();
				НоваяСтрокаТЧ.Дата = Дата;
				НоваяСтрокаТЧ.Продажа = Ссылка;
			
				НоваяСтрокаТЧ.СуммаПродажи = ИтогоБезнал;
				СуммаБонусов = ИтогоБезнал * 0.05;
