...
Процедура ПосчитатьИтоги()
	...
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		...
		ИтогоРекв = ИтогоРекв + СтрокаТаблицы.Сумма;
		СуммаДокументаНДС = СуммаДокументаНДС + СтрокаТаблицы.СуммаНДС;

	КонецЦикла;

	Если ЗначениеЗаполнено(ТМЦ) И ВклВыклРасходникиВСтоимость = Перечисления.ВклВыклРасходниковВСтоимость.ВключитьРасходникиВСтоимость Тогда
		Для каждого СтрокаТМЦ Из ТМЦ Цикл
			Если СтрокаТМЦ.Сумма = 0 Тогда
				СтрокаТМЦ.Сумма = СтрокаТМЦ.Количество * СтрокаТМЦ.Цена;
			КонецЕсли;
			
			ИтогоРекв = ИтогоРекв + СтрокаТМЦ.Сумма;
		КонецЦикла;
	КонецЕсли;
	рассчётКоличества();
	ИтогоБезнал = ИтогоРекв;
