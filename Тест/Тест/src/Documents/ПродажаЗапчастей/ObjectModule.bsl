...
Процедура дт_ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСписанныеТовары",
		ПараметрыПроведенияСклад.ТаблицаСписанныеТовары);
			...
			Если Оплачено = Истина Тогда
				...
				Для Каждого ТекСтрокаТаблица Из Таблица Цикл
					...
					Иначе
						...
						СкидкаОбщ = СкидкаОбщ + (ТекСтрокаТаблица.Цена * ТекСтрокаТаблица.Количество) * (Клиент.Скидка
							/ 100);
					КонецЕсли;
	...
	Если Таблица.Количество() <> 0 Тогда
		Если Ответственные.Количество() <> 0 Тогда
			...
			Для Каждого СтрокаТовары Из Таблица Цикл
				Если СтрокаТовары.Склад.Код = СкладНН И СтрокаТовары.СтатусТовара = Истина Тогда
					СуммаСтрокиТовараНН 		= СуммаСтрокиТовараНН + СтрокаТовары.Сумма - СтрокаТовары.СуммаНДС;
				КонецЕсли;
				Если СтрокаТовары.Склад.Код = СкладМСК И СтрокаТовары.СтатусТовара = Истина Тогда
					СуммаСтрокиТовараМСК		= СуммаСтрокиТовараМСК + СтрокаТовары.Сумма - СтрокаТовары.СуммаНДС;
				КонецЕсли;
				Если СтрокаТовары.Склад.Код = СкладЕКБ И СтрокаТовары.СтатусТовара = Истина Тогда
					СуммаСтрокиТовараЕКБ		= СуммаСтрокиТовараЕКБ + СтрокаТовары.Сумма - СтрокаТовары.СуммаНДС;
				КонецЕсли;
				СуммаДокументПродажа			= СуммаДокументПродажа + СтрокаТовары.Сумма - СтрокаТовары.СуммаНДС;
				МассивДатаВыдачи.Добавить(СтрокаТовары.ДатаВыдачи);

				Если СтрокаТовары.Отменено = Ложь И СтрокаТовары.СтатусТовара = Ложь Тогда
					ХанаБонусам = Ложь;
				КонецЕсли;
			КонецЦикла;

			ДатаВыдачиМини = МассивДатаВыдачи[0];
			Для Каждого Стрдата Из МассивДатаВыдачи Цикл
				Если Стрдата > ДатаВыдачиМини Тогда
					ДатаВыдачиМини = Стрдата;
				КонецЕсли;
			КонецЦикла;
			...
			Если СуммаСтрокиТовараНН <> 0 И КоличествоКладовщиковНН <> 0 Тогда
				Структура.Вставить("ДоляКладовщикаНН", СуммаСтрокиТовараНН / СуммаДокументПродажа
					/ КоличествоКладовщиковНН);
			Иначе
				Структура.Вставить("ДоляКладовщикаНН", 0);
			КонецЕсли;

			Если СуммаСтрокиТовараМСК <> 0 И КоличествоКладовщиковМСК <> 0 Тогда
				Структура.Вставить("ДоляКладовщикаМСК", СуммаСтрокиТовараМСК / СуммаДокументПродажа
					/ КоличествоКладовщиковМСК);
			Иначе
				Структура.Вставить("ДоляКладовщикаМСК", 0);
			КонецЕсли;

			Если СуммаСтрокиТовараЕКБ <> 0 И КоличествоКладовщиковЕКБ <> 0 Тогда
				Структура.Вставить("ДоляКладовщикаЕКБ", СуммаСтрокиТовараЕКБ / СуммаДокументПродажа
					/ КоличествоКладовщиковЕКБ);
			Иначе
				Структура.Вставить("ДоляКладовщикаЕКБ", 0);
			КонецЕсли;

			Итер = 1;
			Если ХанаБонусам = Истина Тогда
				Для Каждого СтрокаТЧ Из Ответственные Цикл
					...
					Если СтрокаТЧ.Роль = Перечисления.дт_РолиВПродаже.Получить(2) Тогда
						...
						Если СтрокаМестоРаботы = МестоРаботыННКод И Структура.ДоляКладовщикаНН <> 0 Тогда
							ЗаписьВРегистреСведений 			= РегистрыСведений.БонусыСотрудниковОтПродажи.СоздатьМенеджерЗаписи();
							ЗаписьВРегистреСведений.Период 		= ДатаВыдачиМини + Итер;
							...
							ЗаписьВРегистреСведений.Бонус 		= СуммаДокументПродажа * Структура.ДоляКладовщикаНН
								* 0.005;
							ЗаписьВРегистреСведений.Записать();
							Итер = Итер + 1;
						КонецЕсли;
						Если СтрокаМестоРаботы = МестоРаботыМСККод И Структура.ДоляКладовщикаМСК <> 0 Тогда
							ЗаписьВРегистреСведений 			= РегистрыСведений.БонусыСотрудниковОтПродажи.СоздатьМенеджерЗаписи();
							ЗаписьВРегистреСведений.Период 		= ДатаВыдачиМини + Итер;
							ЗаписьВРегистреСведений.Сотрудник 	= СтрокаТЧ.Сотрудник;
							...
							ЗаписьВРегистреСведений.Продажа 	= ссылка;
							ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
							ЗаписьВРегистреСведений.Бонус 		= СуммаДокументПродажа * Структура.ДоляКладовщикаМСК
								* 0.005;
							ЗаписьВРегистреСведений.Записать();
							Итер = Итер + 1;
						КонецЕсли;

						Если СтрокаМестоРаботы = МестоРаботыЕКБКод И Структура.ДоляКладовщикаЕКБ <> 0 Тогда
							ЗаписьВРегистреСведений 			= РегистрыСведений.БонусыСотрудниковОтПродажи.СоздатьМенеджерЗаписи();
							ЗаписьВРегистреСведений.Период 		= ДатаВыдачиМини + Итер;
							ЗаписьВРегистреСведений.Сотрудник 	= СтрокаТЧ.Сотрудник;
							...
							ЗаписьВРегистреСведений.Продажа 	= ссылка;
							ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
							ЗаписьВРегистреСведений.Бонус 		= СуммаДокументПродажа * Структура.ДоляКладовщикаЕКБ
								* 0.005;
							ЗаписьВРегистреСведений.Записать();
							Итер = Итер + 1;
						КонецЕсли;
...
////==================================================================================================================
	...
	УчаствуетВАкции10 = ЗаявкаУчаствуетВАкции(); 
	Если УчаствуетВАкции10 = Ложь Тогда 
		...
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ПродажаЗапчастей.Товар КАК Товар,
			|	ПродажаЗапчастей.Партия КАК индкод,
			|	СУММА(ЕСТЬNULL(ПродажаЗапчастей.Количество, 0)) КАК Количество
			|ПОМЕСТИТЬ ВТ_ТЧПродажаЗапчастей
			|ИЗ
			|	Документ.ПродажаЗапчастей.Таблица КАК ПродажаЗапчастей
			|ГДЕ
			|	ПродажаЗапчастей.Ссылка = &Ссылка
			|	И ПродажаЗапчастей.Партия <> ЗНАЧЕНИЕ(Справочник.ИндКод.ПустаяСсылка)
			|СГРУППИРОВАТЬ ПО
			|	ПродажаЗапчастей.Товар,
			|	ПродажаЗапчастей.Партия
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ВТ_ТЧПродажаЗапчастей.Товар КАК Товар,
			|	ВТ_ТЧПродажаЗапчастей.индкод КАК индкод,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_ТЧПродажаЗапчастей.Товар) КАК ПредставлениеТовар,
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_ТЧПродажаЗапчастей.индкод) КАК ПредставлениеИндкод,
			|	ЕСТЬNULL(РегистрАкция10Остатки.КолвоОстаток, 0) КАК Остаток,
			|	ЕСТЬNULL(ВТ_ТЧПродажаЗапчастей.Количество, 0) КАК Количество,
			|	РегистрАкция10Остатки.Склад КАК Склад,
			|	РегистрАкция10Остатки.машина КАК Машина
			|ИЗ
			|	ВТ_ТЧПродажаЗапчастей КАК ВТ_ТЧПродажаЗапчастей
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РегистрАкция10.Остатки(&МоментВремени, (Товар, индкод) В
			|			(ВЫБРАТЬ
			|				Т.Товар,
			|				Т.индкод
			|			ИЗ
			|				ВТ_ТЧПродажаЗапчастей КАК Т)) КАК РегистрАкция10Остатки
			|		ПО ВТ_ТЧПродажаЗапчастей.индкод = РегистрАкция10Остатки.индкод";
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Если Продажа.ссылка = Неопределено Тогда
			...
			Шапка = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(РезультатЗапроса[РезультатЗапроса.Количество()
				- 2].Выгрузить()[0]);
			Организация = Справочники.Организация.НайтиПоКоду("000000010");
			СтатусДоставки = Перечисления.СтатусОтправки.Отправлен;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоУслуга Тогда
		...
		Для Каждого СтрокаТЧ Из Таблица Цикл
			Если СтрокаТЧ.Товар.Подкатегория2 <> Справочники.Категории.НайтиПоКоду("000000331") И СтрокаТЧ.Партия
				= Справочники.ИндКод.ПустаяСсылка() Тогда
				Отказ = Истина;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		...
		// Удольть бонусы кладовщикам если отменили проведение 	
		НаборЗаписей = РегистрыСведений.БонусыСотрудниковОтПродажи.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ДокументРег.Установить(Ссылка);
		НаборЗаписей.Прочитать();
	...
	КонецЕсли;
		...
		// Руководитель
	Если Не ЗначениеЗаполнено(Руководитель) И ЗначениеЗаполнено(Организация) Тогда
		Руководитель = дт_ЗаполнениеОбъектов.ПолучитьПоследнееЗначение(
			Метаданные().Имя, "Руководитель", Новый Структура("Организация", Организация));
	...
	// +++ МазинЕС проставить НДС в ТЧ если нужно, там где его нет (при добавлении товаров через Сайт)

	Если Дата < '20231010' Тогда
		НДС = 0;
	Иначе
		НДС = ПолучитьПроцентНалога();
	КонецЕсли;
	// Комлев АА +++
	СтрокиАктивные = Таблица.НайтиСтроки(Новый Структура("Отменено", Ложь));
	СуммаДокументаНДС = 0;
	// Комлев АА ---

	Для Каждого СтрокаТаблицы Из СтрокиАктивные Цикл
		// Комлев АА 13/01/25 +++
//		Если НЕ дт_ОбщегоНазначенияВызовСервераПовтИсп.ЭтоПолноправныйПользователь() 
//			И (НЕ дт_ОбщегоНазначенияВызовСервераПовтИсп.ЭтоПользовательИмя("Рожков Александр Евгеньевич") 
//			ИЛИ НЕ дт_ОбщегоНазначенияВызовСервераПовтИсп.ЭтоПользовательИмя("Едемский Анатолий Вячеславович")) Тогда
//			ЦенаДетали = Неопределено;
//			Если СтрокаТаблицы.Партия <> Справочники.ИндКод.ПустаяСсылка() Тогда
//				ЦенаДетали = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьЦенуИндКода(СтрокаТаблицы.Партия)
//			Иначе
//				Если СтрокаТаблицы.Товар <> Справочники.Номенклатура.ПустаяСсылка() Тогда
//					ЦенаДетали = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьРекомендованнуюЦену(СтрокаТаблицы.Товар, Дата);
//				КонецЕсли;
//			КонецЕсли;
//	
//			Если ЦенаДетали <> Неопределено Тогда
//				МинимальнаяСумма = ЦенаДетали - (ЦенаДетали / 100 * 5);
//				Если СтрокаТаблицы.Сумма < МинимальнаяСумма Тогда
//					СтрокаТаблицы.Сумма = МинимальнаяСумма;
//				КонецЕсли;
//			КонецЕсли;
//		КонецЕсли;
		// Комлев АА 13/01/25 ---
		...
		Если НДС <> 0 Тогда
			...
			СтрокаТаблицы.СуммаНДС = (СтрокаТаблицы.Цена * (НДС / 100)) * СтрокаТаблицы.Количество;
		Иначе
			СтрокаТаблицы.СуммаНДС = 0;
			СтрокаТаблицы.Цена  =  СтрокаТаблицы.Сумма / СтрокаТаблицы.Количество;
		КонецЕсли;
		СуммаДокументаНДС = СуммаДокументаНДС + СтрокаТаблицы.СуммаНДС;
	КонецЦикла;
...
// +++ МазинЕС 04-12-2024 
	...
	Запрос.УстановитьПараметр("ДокРег", Организация);
...
Процедура ОтправитьУведомлениеКлиенту() Экспорт
	...
	ИначеЕсли ДополнительныеСвойства.Свойство("СтатусДоставкиИзменен") И ДополнительныеСвойства.СтатусДоставкиИзменен Тогда
