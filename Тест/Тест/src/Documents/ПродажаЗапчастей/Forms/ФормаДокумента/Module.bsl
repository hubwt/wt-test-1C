...
Процедура СписатьБонусыНаСервере()
	...
	Если Объект.СколькоСписать <> 0  Тогда
		...
		Если Объект.СколькоСписать > МаксимальнаяСуммаСписания Тогда
			
			Сообщить("" + МаксимальнаяСуммаСписания + " Максимальная сумма списания");
			Объект.СколькоСписать = МаксимальнаяСуммаСписания;
		КонецЕсли;
...
Процедура ОтменитьСписаниеНаСервере()
	...
	Для Каждого СтрокаТЧ Из КлиентОбъект.БонусыКлиента Цикл 
		Если (СтрокаТЧ.Списаны = Истина ИЛИ  СтрокаТЧ.Бонусы < СтрокаТЧ.Бонусы - Объект.СколькоСписать) И СтрокаТЧ.Дата < Объект.Дата Тогда
			
			СтрокаТЧ.Бонусы = СтрокаТЧ.Бонусы + Объект.СколькоСписать;
