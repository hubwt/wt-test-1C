...
Процедура ПриОткрытии(Отказ)
	...
	// Смотрим есть ли у клиента бонусы.
	ПолучениеСуммыБонусовКлиента();
	// Если бонусы списывались в этом документе, то блокировать кнопку
...
Процедура СписатьБонусыНаСервере()
	...
	Сообщить("" + МаксимальнаяСуммаСписания + " Максимальная сумма списания");
	
	Сообщение = Новый СообщениеПользователю();
	
	Если Объект.СуммаБонусов <> 0 И Объект.СуммаБонусов <=  МаксимальнаяСуммаСписания Тогда
		
		СуммаСписания = Объект.СуммаБонусов / Объект.Таблица.Количество();
		
		Для каждого СтрокаТч Из Объект.Таблица Цикл 
			СтрокаТч.Сумма = СтрокаТч.Сумма - Окр(СуммаСписания, 0, РежимОкругления.Окр15как10);
		КонецЦикла;
		...
		Сообщение.Сообщить();
		ПолучениеСуммыБонусовКлиента();
	ИначеЕсли Объект.СуммаБонусов = 0 Тогда	
		Сообщение.Текст = "Бонусов нет";
		Сообщение.Сообщить();
		Возврат;
	...
	ИначеЕсли Объект.СуммаБонусов >  МаксимальнаяСуммаСписания Тогда
		...
		КлиентОбъект = Объект.Клиент.ПолучитьОбъект();
		Если КлиентОбъект.БонусыКлиента.Количество() <> 0 Тогда
			СуммаСписанияБонусов = МаксимальнаяСуммаСписания / КлиентОбъект.БонусыКлиента.Количество();
		КонецЕсли;
		
		Сообщить("" + СуммаСписанияБонусов + " Сумма списания бонусов");
		Для каждого СтрокаТЧ из КлиентОбъект.БонусыКлиента Цикл
			
			СтрокаТЧ.Бонусы = СтрокаТЧ.Бонусы - СуммаСписанияБонусов;
			Если СтрокаТЧ.Бонусы <= 0 Тогда
				СтрокаТЧ.Списаны = Истина;
				СтрокаТЧ.Бонусы = СтрокаТЧ.СуммаБонусовДоСписания;
			КонецЕсли;
		КонецЦикла;
		КлиентОбъект.Записать();
		...
		Сообщение.Текст = "Бонусами можно оплатить до 30% стоимости.";
		Сообщение.Сообщить();
		
	КонецЕсли;
	Объект.БылоСписаниеБонусов = Истина;
	Элементы.СписатьБонусы.Доступность = Ложь;
...
Процедура ОтменитьСписаниеНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Клиент) Тогда
		...
		Для каждого СтрокаТЧ из КлиентОбъект.БонусыКлиента Цикл
				СтрокаТЧ.Бонусы = СтрокаТЧ.СуммаБонусовДоСписания;
				Если СтрокаТЧ.Списаны = Истина Тогда
...
Процедура ОткрытьСайт(Команда)
	// Вставить содержимое обработчика.
	ОбщДействия.ПоискКодаНаСайте(ПолучитьКодТовара(ЭтаФорма.Элементы.Таблица.ТекущиеДанные.Товар));
КонецПроцедуры
...
Процедура ТоварПоПоступлению(Команда)
	...
	ТабДокумент = ТоварПоПоступлениюСервер();
	ТабДокумент.Показать();
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("СписаниеТоваров")) И
		...
		Не Пользователи.ТекущийПользователь() = Справочники.Пользователи.НайтиПоНаименованию("Дубнов Евгений Владимирович") Тогда
		Элементы.Группа3.ТолькоПросмотр 						= Истина;
		Элементы.Группа4.ТолькоПросмотр 						= Истина;
		Элементы.ГруппаШапка.ТолькоПросмотр 					= Истина;
...
&НаСервере
Функция ТоварПоПоступлениюСервер()
	// Вставить содержимое обработчика.
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.АвтоМасштаб = Истина;
...
Процедура СтатусПродажиПриИзмененииНаСервере()
	Если объект.СтатусПродажи = Перечисления.СтатусыПродажи.Сорвалась тогда
