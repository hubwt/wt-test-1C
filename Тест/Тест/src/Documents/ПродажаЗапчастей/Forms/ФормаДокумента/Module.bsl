...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)	
	...
	Если  ТекушийПользователь.Роли.Содержит(Метаданные.Роли.Найти("дт_ПолныеПрава"))
		Или  ТекушийПользователь.ПолноеИмя = "Вадяева Дарья Викторовна" Или  ТекушийПользователь.Роли.Содержит(
		Метаданные.Роли.Найти("ПолныеПрава")) Тогда

		Элементы.Таблица.ПодчиненныеЭлементы.ТаблицаКорректировка.Видимость = Истина;
		Если  ТекушийПользователь.ПолноеИмя = "Вадяева Дарья Викторовна" Или ТекушийПользователь.ПолноеИмя = "Мазин Евгений Сергеевич" Тогда
	...
	ЗаблокироватьРазблокировтьПродажу();
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	///+ГомзМА 04.05.2023
	Если ЭтаФорма.ТолькоПросмотр = Истина И НЕ СодержитРоли Тогда
	...
	//Проверка = эЛЕМЕНТЫ.Организация.ТолькоПросмотр И эЛЕМЕНТЫ.Организация.Доступность; 	
КонецПроцедуры  
&НаСервере
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Синхронизируем табличные части Продажи и Заявки (Синхронизация 1С -1С)
	СинхронизоватьВыданСЗаказомКлиента(объект.ЗаказКлиента.Ссылка,объект.Номер);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СинхронизоватьВыданСЗаказомКлиента(СсылкаЗаказКлиента,НомерПродажа)
	...
	ОбъектЗаказКлиента.Записать(РежимЗаписиДокумента.Проведение);	
	Исключение
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Ошибка!!! не получилось синхронизовать заявку. Пишите в тех поддержку! Код Ошибки: 01042"; 
		Сообщение.Сообщить();
	КонецПопытки;
...
Процедура ЕстьДоставкаПриИзменении(Элемент)
	...
	УправлениеФормой(ЭтаФорма);
	 //УстановитьСтрануПолучения();
КонецПроцедуры

&НаКлиенте
Процедура РегионПолученияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	РегионПолученияНачалоВыбораСервер();
КонецПроцедуры

&НаСервере
Процедура РегионПолученияНачалоВыбораСервер()
	/// Комлев 13/9/24 +++
	Элементы.РегионПолучения.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);
	Если ЗначениеЗаполнено(Объект.СтранаПолучения) Тогда
		НоваяСвязь = Новый СвязьПараметраВыбора("Отбор.Владелец", "Объект.СтранаПолучения");
...
Процедура ГородПолученияНачалоВыбораСервер()
	...
	Если ЗначениеЗаполнено(Объект.РегионПолучения) Тогда
		НоваяСвязь = Новый СвязьПараметраВыбора("Отбор.Владелец", "Объект.РегионПолучения");
		НовыйМассив = Новый Массив;
		НовыйМассив.Добавить(НоваяСвязь);
...
Процедура НарядДоставка(Команда)
	...
	///ОбъектПродажа = Объект.Ссылка.ПолучитьОбъект(); 	
	ТабДокумент = дт_Продажи.НарядДоставка(Объект.Ссылка);
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "Таблица");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
	"Объект.Таблица.Корректировка", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаПоля );
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаКорректировкаПриИзменении(Элемент)
	УстановитьУсловноеОформление();
КонецПроцедуры
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.Группа23.Видимость = Объект.ЕстьДоставка;
...
Процедура ЗаменитьЗначениеСтатусКлиентаВСправочникеКлиенты()
	...
	///+ГомзМА 23.01.2023
	Если ТипЗнч(Объект.Клиент) = Тип("СправочникСсылка.УчастникиАкции10") Тогда
		Возврат; 
	КонецЕсли; 
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("СписаниеТоваров")) И
		НЕ (ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("ПолныеПрава")) ИЛИ
		ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("дт_ПолныеПрава")))
		И НЕ (ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("СписаниеТоваров")) И ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("МенеджерПоПродажам")))
		
		И Не Пользователи.ТекущийПользователь() = Справочники.Пользователи.НайтиПоНаименованию("Урунов Муминджон Мойдинович") И
		...
		Не Пользователи.ТекущийПользователь() = Справочники.Пользователи.НайтиПоНаименованию("Кузмичёв Иван Сергеевич") 
		...
		Элементы.ТаблицаКоличество.ТолькоПросмотр 				= Истина;
		Элементы.ТаблицаЦена.ТолькоПросмотр 					= Истина;
		Элементы.ТаблицаСуммаНДС.ТолькоПросмотр 				= Истина; // Поправил было Истина //МазинПравка
		Элементы.ТаблицаСумма.ТолькоПросмотр 					= Истина;
...
Запрос.Текст =
	...
	|ИЗ
	|	РегистрСведений.НДС.СрезПоследних(&Дата, ДокРег = &ДокРег) КАК НДССрезПоследних";

Запрос.УстановитьПараметр("ДокРег", Объект.Организация);
...
Если НЕ Запрос.Выполнить().Пустой()  Тогда 
		...
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		РезультатЗапроса.Следующий();
		Результат = РезультатЗапроса.НДС; 
...
Функция ПользовательСодержитРоли()
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("СписаниеТоваров")) 
		И  ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("МенеджерПоПродажам")) Тогда
		Возврат  Истина;
	Иначе 
		Возврат  Ложь;
	КонецЕсли;
...
Функция ПересчитатьЦену()
	...
	///+ГомзМА 20.10.2023
		ТекСтрокаТаблица.Цена    	= ?(ТекСтрокаТаблица.Количество = 0, ТекСтрокаТаблица.Сумма, ТекСтрокаТаблица.Сумма/ТекСтрокаТаблица.Количество/((НДС+100)/100));
   		ТекСтрокаТаблица.СуммаНДС   = ?(ТекСтрокаТаблица.Количество = 0, 0, ТекСтрокаТаблица.Сумма/ТекСтрокаТаблица.Количество/((НДС+100)/100)*(НДС/100)*ТекСтрокаТаблица.Количество);	
	КонецЕсли;
	///-ГомзМА 20.10.2023
...
Функция ПолучитьТабличныйДокументТоварныйЧек()
	...
	ПокупательПредставление = ?(СтрДлина(ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование")) > 15, ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование"), Объект.Клиент );
	ПараметрыШапки.Вставить("ПокупательНаименование",	ПокупательПредставление);
...
&НаСервере
Функция НарядНаСборкуСервер(Сортировка = Неопределено) Экспорт
	Записать();
	// Вставить содержимое обработчика.
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.АвтоМасштаб = Истина;
...
//	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Объект.Таблица Цикл
		Если СтрокаТаблицы.Отменено = Истина Тогда
			Продолжить;
		КонецЕсли;
...
&НаКлиенте
Процедура ПечатьСписания() Экспорт
...
Процедура ЗакрытаПриИзмененииНаСервере()
	...
	ЕСли Объект.Закрыта = Истина Тогда
		объект.ДатаЗакрытия = ТекущаяДатаСеанса(); 
	КонецЕСли; 
...
Процедура ОрганизацияПриИзменении1(Элемент)
	...
	Для Каждого СтрокаТаблицы Из Объект.Таблица Цикл
		Если НДС <> 0 Тогда			
			СтрокаТаблицы.Цена  =  СтрокаТаблицы.Сумма / ((НДС+100)/100) / СтрокаТаблицы.Количество;
			СтрокаТаблицы.СуммаНДС = Цел(СтрокаТаблицы.Цена * (НДС / 100) ) * СтрокаТаблицы.Количество;
		Иначе
...
Процедура ГородПолученияПриИзменении(Элемент)
	Если НЕ ЗначениеЗаполнено(Объект.РегионПолучения) Тогда
		Объект.РегионПолучения = ПолучитьРегионГорода(); 
	КонецЕсли;	
	
	
	Если НЕ ЗначениеЗаполнено(Объект.СтранаПолучения) Тогда
		Объект.СтранаПолучения = ПолучитьСтрануРегиона(); 
	КонецЕсли;
...
Функция ПолучитьРегионГорода()
  /// Комлев 03/09/24 +++ 
	Попытка
		Возврат Объект.ГородПолучения.Владелец;
	Исключение
...
Функция ПолучитьСтрануРегиона()
  /// Комлев 03/09/24 +++ 
	Попытка
		Возврат Объект.РегионПолучения.Владелец;
	Исключение
...
Процедура ЗаблокироватьРазблокировтьПродажу()
	// Комлев АА +++
	Если Объект.Заблокирована = Истина Тогда
		Элементы.Таблица.КоманднаяПанель.Доступность = Ложь;
		Если ЕстьДолжностьКладовщик(Пользователи.ТекущийПользователь()) Тогда
			Для Каждого ПодчЭлемент Из Элементы.Таблица.ПодчиненныеЭлементы Цикл
				Если ПодчЭлемент.Имя = "ТаблицаПартия" Или ПодчЭлемент.Имя = "ТаблицаСтатусТовара" Тогда
					Продолжить;
				Иначе
					ПодчЭлемент.ТолькоПросмотр = Истина;
				КонецЕсли;
			КонецЦикла;
		Иначе
		...
		Элементы.ГруппаЗаказНаряд.ТолькоПросмотр = Истина;

		Элементы.ГруппаСчет.ТолькоПросмотр = Истина;
		Элементы.ОбновитьСчет.Доступность = Ложь;
		Элементы.СоздатьСчет.Доступность = Ложь;

		Элементы.ГруппаДоговорДатаОжидаемойВыплаты.ТолькоПросмотр = Истина;
		...
		Элементы.Расходы.ТолькоПросмотр = Истина;
		Элементы.ВыводДенег.ТолькоПросмотр = Истина;
		Элементы.Ответственные.ТолькоПросмотр = Истина;
		Элементы.КтоПродал.ТолькоПросмотр = Истина;
	ИначеЕсли Объект.Заблокирована = Ложь Тогда
		Элементы.Таблица.КоманднаяПанель.Доступность = Истина;
		Если ЕстьДолжностьКладовщик(Пользователи.ТекущийПользователь()) Тогда
			Для Каждого ПодчЭлемент Из Элементы.Таблица.ПодчиненныеЭлементы Цикл
				Если ПодчЭлемент.Имя = "ТаблицаПартия" Или ПодчЭлемент.Имя = "ТаблицаСтатусТовара" Тогда
					Продолжить;
				Иначе
					ПодчЭлемент.ТолькоПросмотр = Ложь;
				КонецЕсли;
			КонецЦикла;
		Иначе
			Элементы.Таблица.ТолькоПросмотр = Ложь;
		КонецЕсли;
		Элементы.ГруппаШапкаЛево.ТолькоПросмотр = Ложь;
		Элементы.ГруппаНомера.ТолькоПросмотр = Ложь;
		Элементы.ГруппаЗаказНаряд.ТолькоПросмотр = Ложь;
		Элементы.ГруппаСчет.ТолькоПросмотр = Ложь;
		Элементы.ОбновитьСчет.Доступность = Истина;
		Элементы.СоздатьСчет.Доступность = Истина;

		Элементы.ГруппаДоговорДатаОжидаемойВыплаты.ТолькоПросмотр = Ложь;

		Элементы.БухгалтерскиеДокументы.ТолькоПросмотр = Ложь;

		Элементы.ОтправитьСообщение.Доступность = Истина;
		Элементы.СтраницаВТСпасибо.ТолькоПросмотр = Ложь;
		Элементы.СтраницаРасходники.ТолькоПросмотр = Ложь;
		Элементы.Расходы.ТолькоПросмотр = Ложь;
		Элементы.ВыводДенег.ТолькоПросмотр = Ложь;
		Элементы.Ответственные.ТолькоПросмотр = Ложь;
		Элементы.КтоПродал.ТолькоПросмотр = Ложь;
	КонецЕсли;
	Элементы.БезДоговора.ТолькоПросмотр = Ложь;
	...
	Если ТекушийПользователь.Роли.Содержит(Метаданные.Роли.Найти("дт_ПолныеПрава")) Или ТекушийПользователь.ПолноеИмя
		...
		Элементы.Заблокирована.Доступность			 = Истина;
		Элементы.ДатаБлокировкиПродажи.Доступность	 = Истина; 
...
&НаСервереБезКонтекста
Функция ЕстьДолжностьКладовщик(Пользователь)
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДолжностиСотрудниковСрезПоследних.Должность КАК Должность
	|ИЗ
	|	РегистрСведений.ДолжностиСотрудников.СрезПоследних(&ТекДата, Сотрудник.Пользователь = &Пользователь
	|	И Должность = &Должность1
	|	ИЛИ Должность = &Должность2
	|	ИЛИ Должность = &Должность3) КАК ДолжностиСотрудниковСрезПоследних
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДолжностиСотрудниковСрезПоследних.Период УБЫВ";

	Запрос.УстановитьПараметр("Должность1", Справочники.ДолжностиПредприятия.НайтиПоКоду("000000159"));
	Запрос.УстановитьПараметр("Должность2", Справочники.ДолжностиПредприятия.НайтиПоКоду("000000122"));
	Запрос.УстановитьПараметр("Должность3", Справочники.ДолжностиПредприятия.НайтиПоКоду("000000076"));
...
КонецФункции
&НаКлиенте
Процедура ЗаблокированаПриИзменении(Элемент)
	// Комлев АА +++
	ЗаблокироватьРазблокировтьПродажу();
	// Комлев АА +++
	// +++ МазинЕС (продажу считаем закрытой когда ее заблокировали, дата блокировки становится основным периодом)
	Если НЕ ЗначениеЗаполнено(Объект.ДатаБлокировкиПродажи ) Тогда 
		Объект.ДатаБлокировкиПродажи = ТекущаяДата(); 
	КонецЕсли; 
	/// --- МазинЕС (продажу считаем закрытой когда ее заблокировали, дата блокировки становится основным периодом)
КонецПроцедуры
