...
Процедура ТаблицаСуммаПриИзменении(Элемент)
	...
	Если НЕ дт_ОбщегоНазначенияВызовСервераПовтИсп.ЭтоПолноправныйПользователь() 
		...
		Если ЗначениеЗаполнено(ТекДанные.Партия) Тогда
			Цена = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьЦенуИндКода(ТекДанные.Партия)
		ИначеЕсли Цена = 0 Тогда
			Если ЗначениеЗаполнено(ТекДанные.Товар) Тогда
				Цена = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьРекомендованнуюЦену(ТекДанные.Товар, Объект.Дата);
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ТекДанные.Товар) Тогда
				Цена = дт_ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьРекомендованнуюЦену(ТекДанные.Товар, Объект.Дата);
		...
		Если Цена <> Неопределено Тогда
			МинимальнаяСумма = Цена - (Цена / 100 * 5);
			Если ТекДанные.Сумма < МинимальнаяСумма Тогда
				ТекДанные.Сумма = МинимальнаяСумма;
				ТекстСообщения = "Минимальная сумма для " + ТекДанные.Товар + " - " + МинимальнаяСумма;
