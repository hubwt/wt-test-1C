...
//	КонецЕсли;
	...
	/// Комлев АА 29/11/24+++
	ЗаблокироватьЭлементыВсе();
	ПоказатьКнопкуБлокировкиИКоманднуюСтроку();
	/// Комлев АА 29/11/24---
...
&НаКлиенте
Процедура ПродажаЗаблокированаДляИзмененияПриИзменении(Элемент)
	ЗаблокироватьЭлементыВсе();
	ПоказатьКнопкуБлокировкиИКоманднуюСтроку();
	Если Объект.ПродажаЗаблокированаДляИзменения = Ложь Тогда
...
Процедура СформироватьУПД(Команда)
	...
	// Комлев АА 29/11/24 +++
	Объект.ПродажаЗаблокированаДляИзменения = Истина;
	ЗаблокироватьЭлементыВсе();
	ПоказатьКнопкуБлокировкиИКоманднуюСтроку();
	ЭтотОбъект.Записать();
	// Комлев АА 29/11/24 ---
...
Процедура ЗаблокироватьЭлементыВсе()
	/// Комлев АА 29/11/24 +++
	Если Объект.ПродажаЗаблокированаДляИзменения = Истина Тогда
		Для Каждого Элемент Из Элементы Цикл
			Если ТипЗнч(Элемент) <> Тип("ДекорацияФормы") Тогда
				Элемент.Доступность = Ложь;
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли Объект.ПродажаЗаблокированаДляИзменения = Ложь Тогда
		Для Каждого Элемент Из Элементы Цикл
			Если ТипЗнч(Элемент) <> Тип("ДекорацияФормы") Тогда
...
Процедура ПоказатьКнопкуБлокировкиИКоманднуюСтроку()
	///Комлев АА 29/11/24 +++
	КоманднаяПанель.Доступность = Истина;
	Для каждого Элемент Из КоманднаяПанель.ПодчиненныеЭлементы Цикл
		Если Элемент.Имя = "ПодменюПечать" Тогда
			Для каждого Подчиненный из Элемент.ПодчиненныеЭлементы Цикл
				Если Подчиненный.Имя = "ПодменюПечатьОбычное" Тогда
					Для каждого Кнопка из Подчиненный.ПодчиненныеЭлементы Цикл
						Кнопка.Доступность = Истина;
					КонецЦикла;
				КонецЕсли;
				Подчиненный.Доступность = Истина;
			КонецЦикла;
		КонецЕсли;
		Если Элемент.Имя = "ПодменюНарядНаСборку" Тогда
			Для каждого КнопкаПодменюНарядНаСборку из Элемент.ПодчиненныеЭлементы Цикл
				КнопкаПодменюНарядНаСборку.Доступность = Истина;
			КонецЦикла;
		КонецЕсли;
		
		Если Элемент.Имя = "ФормаСоздатьНаОсновании" Тогда
			Для каждого КнопкаСоздатьНаОсновании из Элемент.ПодчиненныеЭлементы Цикл
				КнопкаСоздатьНаОсновании.Доступность = Истина;
			КонецЦикла;
		КонецЕсли;
		Элемент.Доступность = Истина;
	КонецЦикла;
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Вадяева Дарья Викторовна"
		Или ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(
		Метаданные.Роли.Найти("дт_ПолныеПрава")) Тогда
		Элементы.ПродажаЗаблокированаДляИзменения.Видимость = Истина;
		Элементы.ПродажаЗаблокированаДляИзменения.Доступность = Истина;
	КонецЕсли;
	///Комлев АА 29/11/24 ---
КонецПроцедуры
