...
Процедура ПолучениеСуммыБонусовКлиента() 
	...
	Иначе 
		Для Счетчик = 0 По КлиентОбъект.БонусыКлиента.Количество() -1 Цикл
			Если КлиентОбъект.БонусыКлиента[Счетчик].Продажа = Объект.Ссылка Тогда
				Продолжить;
			Иначе 
...
Процедура СписатьБонусыНаСервере()
	...
	Сообщить(МаксимальнаяСуммаСписания);
	...
	Если Объект.СуммаБонусов <> 0 И Объект.СуммаБонусов <=  МаксимальнаяСуммаСписания Тогда
		...
		Для каждого СтрокаТч Из Объект.Таблица Цикл 
			СтрокаТч.Сумма = СтрокаТч.Сумма - Окр(СуммаСписания);
		КонецЦикла;
		Объект.ИтогоРекв = Объект.ИтогоРекв - Объект.СуммаБонусов;
		
		КлиентОбъект = Объект.Клиент.ПолучитьОбъект();
		КлиентОбъект.БонусыКлиента.Очистить();
