...
Процедура ПерейтиВЗаявкуWT10(Команда)
	ПерейтиВЗаявкуWT10НаСервере(ЭтаФорма.Элементы.Список.ТекущиеДанные.Номер);
КонецПроцедуры

&НаСервере
Процедура ПерейтиВЗаявкуWT10НаСервере(НомерЗ)
	...
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ПродажаЗапчастей.ЗаказКлиента.Номер КАК Номер
		|ИЗ
		|	Документ.ПродажаЗапчастей КАК ПродажаЗапчастей
		|ГДЕ
		|	ПродажаЗапчастей.Номер = &Номер";
	
	Запрос.УстановитьПараметр("Номер", НомерЗ);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НомерЗаявки = ВыборкаДетальныеЗаписи.Номер;
	КонецЦикла;
