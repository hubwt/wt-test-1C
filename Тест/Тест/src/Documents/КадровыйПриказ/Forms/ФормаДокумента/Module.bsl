...
&НаСервере
 Процедура ПередЗаписьюНаСервере()
 	...
 	///+ТатарМА 27.09.2024
 	Если Объект.Вакансия <> Справочники.ВакансииКампании.ПустаяСсылка() И Объект.Вакансия.Статус = Истина Тогда//И Объект.Ссылка.Проведен Тогда
 		...
 		ОбъектВакансии.Сотрудник = Объект.Сотрудник;
...
Процедура ПослеЗаписиНаСервере()
	...
	ЗаписьВРегистреСведений.Прочитать();
		Если ЗаписьВРегистреСведений.Выбран() И ЗаписьВРегистреСведений.КадровыйПриказ = Документы.КадровыйПриказ.ПустаяСсылка() Тогда
			...
			ЗаписьВРегистреСведений.ПериодПоиска 					= РаботаСДокументамиАналитикаВызовСервера.ПолучитьРазницуДат(ЗаписьВРегистреСведений.ДатаСоздания, ЗаписьВРегистреСведений.ДатаЗакрытия);
			ЗаписьВРегистреСведений.ОкончаниеИспытательногоСрока 	= ДобавитьМесяц(Объект.Дата, 3);
