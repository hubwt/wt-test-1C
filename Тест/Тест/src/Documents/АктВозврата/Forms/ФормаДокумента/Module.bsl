...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//==================================================================================================================
	Если  ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("СогласованиеАктВозврата")) 
		ИЛИ ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("дт_ПолныеПрава")) 
			ИЛИ ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("ПолныеПрава")) Тогда 
				Элементы.КомандаСогласовать.Видимость = истина;		
		КонецЕсли; 
	
	ЕСли  Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.Количество() > 0  Тогда	                                                
		Если ЗначениеЗаполнено(Объект.Согласовал)  Тогда 
			Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.
			ФормаДокументЗаявкаНаРасходСоздатьНаОсновании.Доступность = Истина; 
		Иначе 
			Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.
			ФормаДокументЗаявкаНаРасходСоздатьНаОсновании.Доступность = Ложь;
		КонецЕсли;
	КонецЕСли; 
	//==================================================================================================================
...
Процедура ПриОткрытии(Отказ)
	...
	ПроверитьРоль();  
	//Волков ИО 29.12.23 --  
	Если ПроверитьТекущегоПользователя("Вадяева Дарья Викторовна") ИЛИ ПроверитьТекущегоПользователя("Лазарева Алёна Владимировна") Тогда
		Элементы.ОбщаяСумма.ТолькоПросмотр = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПроверитьТекущегоПользователя(ПолноеИмя)
	Если Пользователи.ТекущийПользователь().ПолноеНаименование() = ПолноеИмя Тогда
		Возврат Истина;
	Иначе 
		Возврат Ложь;
	КонецЕсли;
...
Функция ПечатьНаСервере(ТабДок, СсылкаНаДокумент)
	...
	ОбластьЗаголовок.Параметры.Комментарий = Объект.Комментарий;  
	ОбластьЗаголовок.Параметры.ПричинаВозврата = Объект.ПричинаВозврата;  
...
Процедура КомандаСогласовать(Команда)
	...
	ДоступСоздатьНаОсновании();
КонецПроцедуры

&НаСервере 
Функция ДоступСоздатьНаОсновании()
ЕСли  Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.Количество() > 0  Тогда	
	
	Если Объект.Согласовал <> Справочники.Пользователи.ПустаяСсылка() Тогда 
		Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.
		ФормаДокументЗаявкаНаРасходСоздатьНаОсновании.Доступность = Истина; 
	Иначе 
		Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.
