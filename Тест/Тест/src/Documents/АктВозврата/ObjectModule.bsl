...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПродажаЗапчастей") Тогда
		...
		Инициатор 	   = ПользователиИнформационнойБазы.ТекущийПользователь();
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказНаряд") Тогда
		...
		СлужбаДоставки = Справочники.Доставка.ПустаяСсылка();
		Инициатор 	   = ПользователиИнформационнойБазы.ТекущийПользователь();
		...
		Для Каждого ТекСтрокаТовары Из ДанныеЗаполнения.Товары Цикл
			Если ТекСтрокаТовары.Отменено Тогда
				...
				//НоваяСтрока.ПродНак 	= ТекСтрокаТовары.ПродНак;
				НоваяСтрока.машина 		= ТекСтрокаТовары.Автомобиль;
				НоваяСтрока.Партия 		= ТекСтрокаТовары.Партия;
				//НоваяСтрока.Скидка 		= ТекСтрокаТовары.Скидка;
				НоваяСтрока.Склад 		= ДанныеЗаполнения.Склад;
				НоваяСтрока.Сумма 		= ТекСтрокаТовары.Сумма;
				НоваяСтрока.Товар 		= ТекСтрокаТовары.Номенклатура;
				НоваяСтрока.укод 		= ТекСтрокаТовары.Номенклатура.Код; 
				НоваяСтрока.Цена		= ТекСтрокаТовары.Цена;	
				НоваяСтрока.Артикул     = ТекСтрокаТовары.Номенклатура.Артикул;  
			КонецЕсли;
		КонецЦикла;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Налог = 1; 
	Налог = Налог +  РезультатЗапроса.Налог/100; 
	
	/// - Есть НДС у Продажи или нет ИЛИ нет
	...
	Если ЗначениеЗаполнено(Продажа) И ТипЗнч(Продажа) = Тип("ДокументСсылка.ПродажаЗапчастей") Тогда
		...
		Если Стуктура <> Неопределено Тогда 
			...
			Если НаборЗаписей.Количество() = 0 Тогда
				...
				ЗаписьВРегистреСведений.Сумма 		= 0 -(ОбщаяСумма / Налог);
				...
				ЗаписьВРегистреСведений.Бонус 		= 0 - ((ОбщаяСумма / Налог) * 0.03);
				...
				Для Каждого СтрокаТЧ Из СписокУчастниковРаботы Цикл 
					...
					Если СтрокаТЧ.МестоРаботыКод 			= МестоРаботыННКод И Стуктура.ДоляКладовщикаНН <> 0 Тогда 
						...
						ЗаписьВРегистреСведений.Сумма 		= 0 - ((ОбщаяСумма / Налог) * Стуктура.ДоляКладовщикаНН);
						ЗаписьВРегистреСведений.ДокументРег = ссылка;
						ЗаписьВРегистреСведений.Продажа 	= Продажа;
						ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
						ЗаписьВРегистреСведений.Бонус 		=  0 - ((ОбщаяСумма / Налог) *  Стуктура.ДоляКладовщикаНН * 0.005);
						ЗаписьВРегистреСведений.Записать();
						Итер = Итер +1;
					КонецЕсли;
					Если СтрокаТЧ.МестоРаботыКод 			= МестоРаботыМСККод И Стуктура.ДоляКладовщикаМСК <> 0 Тогда
						...
						ЗаписьВРегистреСведений.Период 		= Дата + Итер;
						ЗаписьВРегистреСведений.Сотрудник 	= СтрокаТЧ.Сотрудник;
						ЗаписьВРегистреСведений.Сумма 		= 0 - ((ОбщаяСумма / Налог) * Стуктура.ДоляКладовщикаМСК);
						ЗаписьВРегистреСведений.ДокументРег = ссылка;
						ЗаписьВРегистреСведений.Продажа 	= Продажа;
						ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
						ЗаписьВРегистреСведений.Бонус 		=  0 - ((ОбщаяСумма / Налог) * Стуктура.ДоляКладовщикаМСК * 0.005); 
						ЗаписьВРегистреСведений.Записать();
						Итер = Итер +1;	
					КонецЕсли;
					...
					Если СтрокаТЧ.МестоРаботыКод 			= МестоРаботыЕКБКод И Стуктура.ДоляКладовщикаЕКБ <> 0 Тогда
						...
						ЗаписьВРегистреСведений.Период 		= Дата + Итер;
						ЗаписьВРегистреСведений.Сотрудник 	= СтрокаТЧ.Сотрудник;
						ЗаписьВРегистреСведений.Сумма 		= 0 - ((ОбщаяСумма / Налог) * Стуктура.ДоляКладовщикаЕКБ);
						ЗаписьВРегистреСведений.ДокументРег = ссылка;
						ЗаписьВРегистреСведений.Продажа 	= Продажа;
						ЗаписьВРегистреСведений.Роль 		= Справочники.ДолжностиДляУК.НайтиПоКоду("000000003"); //Кладовщик
						ЗаписьВРегистреСведений.Бонус 		=  0 - ((ОбщаяСумма / Налог) * Стуктура.ДоляКладовщикаЕКБ * 0.005);
						ЗаписьВРегистреСведений.Записать();
						Итер = Итер +1;	
					КонецЕсли;
