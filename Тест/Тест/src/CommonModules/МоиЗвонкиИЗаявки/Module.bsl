...
Процедура СоздатьЗаявкиПоЗвонкамНаСервере() Экспорт 
	...
	Пока РезультатЗапросаТелефонов.Следующий() Цикл 
		...
		Если   (ОткрытаяЗаявкаПоЗвонку = Неопределено) И (НЕ РезультатЗапросаЗаявок.Менеджер = Справочники.Пользователи.НайтиПоНаименованию("Муравьев Андрей Александрович")) Тогда
