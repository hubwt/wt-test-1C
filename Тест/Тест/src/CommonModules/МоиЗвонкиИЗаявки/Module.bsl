...
Процедура СоздатьЗаявкиПоЗвонкамНаСервере() Экспорт 
	...
	Пока РезультатЗапросаТелефонов.Следующий() Цикл 
		...
		Если  НЕ ЗначениеЗаполнено(ОткрытаяЗаявкаПоЗвонку) ИЛИ ОткрытаяЗаявкаПоЗвонку = Неопределено  Тогда
		...
		Пока РезультатЗапросаЗаявок.Следующий() Цикл
			...
			ИначеЕСЛИ  НоваяЗаявка <> Неопределено Тогда
				 ...
				 Попытка
				 	...
				 	Каширин = Справочники.Пользователи.НайтиПоНаименованию("Каширин Владислав Александрович");
				 	РомановИгорь = Справочники.Пользователи.НайтиПоНаименованию("Романов Игорь Леонидович");
				 	...
				 	ИначеЕсли РезультатЗапросаЗаявок.Менеджер = РомановИгорь Тогда
				 		ОбъектЗаявки.Ответственный = Чувилев;
					Иначе 
				 		ОбъектЗаявки.Ответственный = РезультатЗапросаЗаявок.Менеджер;
				 	КонецЕсли;
					Если НЕ РезультатЗапросаЗаявок.Менеджер = Каширин Тогда
						ОбъектЗаявки.Записать();
					КонецЕсли;
				 
					Если  РезультатЗапросаЗаявок.Менеджер = Справочники.Пользователи.НайтиПоНаименованию("Муравьев Андрей Александрович") 
						И ЗначениеЗаполнено(НоваяЗаявка)  Тогда 
						НоваяЗаявка.Удалить();
					КонецЕсли;
				 	
				 	Если  РезультатЗапросаЗаявок.Менеджер = Справочники.Пользователи.ПустаяСсылка() ИЛИ НЕ ЗначениеЗаполнено(РезультатЗапросаЗаявок.Менеджер) Тогда 
						НоваяЗаявка.Удалить();
					КонецЕсли;
...
Функция ПолучитьОткрытуюЗаявкуПоЗвонку(НомерТелефона)
	...
	|	ЗаказКлиента.Ссылка КАК Ссылка,
	|	ЗаказКлиента.Клиент КАК Клиент,
	|	Клиенты.Ссылка КАК СсылкаКлиент,
	|	ЕСТЬNULL(Клиенты.Целевой,0) КАК Целеыой
	|ИЗ
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Клиенты КАК Клиенты
	|		ПО ЗаказКлиента.Клиент = Клиенты.Ссылка
	|ГДЕ
	|	ЗаказКлиента.НомерТелефона = &НомерТелефона
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда 
		...
		Попытка
		Если Выборка.Целевой <> 0  ТОгда 
			Если Выборка.Целевой = Ложь  ТОгда
  				КлиентОбъект = 	Выборка.СсылкаКлиент.ПолучитьОбъект(); 
  				КлиентОбъект.Целевой = Истина; 
		...
		Если (ОбъектЗаявка.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Ожидание
				ИЛИ ОбъектЗаявка.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.Думает 
				ИЛИ ОбъектЗаявка.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.ЖдёмДенег 
				ИЛИ ОбъектЗаявка.Состояние = Перечисления.дт_СостоянияЗаказовКлиента.ВРаботе) 
				И ( ДеньГода(ТекущаяДата()) - ДеньГода(ОбъектЗаявка.Дата) < 14 ) Тогда 
					Возврат Ссылка;  
		КонецЕсли; 
	КонецЕсли;
...
Функция СоздатьНовуюЗаявку(Телефон)
  ...
  Если ДанныеКлиента.ссылка.СтатусКлиента <> Справочники.СтатусыКлиентов.НайтиПоКоду("000000007")   Тогда
	Если ДанныеКлиента.Целевой = Ложь ТОгда 
  		Попытка
  		КлиентОбъект = 	ДанныеКлиента.Ссылка.ПолучитьОбъект(); 
  		КлиентОбъект.Целевой = Истина; 
  		КлиентОбъект.Записать();
  		Исключение 
  			т=78;
   ...
   НовыйДокумент.ЭтапыПродажи = Перечисления.ЭтапыПродажи.НоваяЗаявка; // МазинПравка
