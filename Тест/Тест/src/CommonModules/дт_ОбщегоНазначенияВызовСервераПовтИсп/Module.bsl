...
Функция СоздатьXMLФайл(Каталог, СсылкаНаДокумент, Менеджер) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ЗаписьXML.ЗаписатьАтрибут("ВерсФорм", "5.03"); // 5.01 - было
		...
		ЗаписьXML.ЗаписатьАтрибут("НомерДок", НомерСчетаФактурыНаПечать(ДанныеПечати.Номер,
			ДанныеПечати.ИндексПодразделения));
		ЗаписьXML.ЗаписатьАтрибут("ДатаДок", СтрРазделить(ДанныеПечати.Дата, " ")[0]);
...
//			ЗаписьXML.ЗаписатьАтрибут("КПП", ДанныеПечати.Грузополучатель.КПП); // Добавил
		Иначе
			ЗаписьXML.ЗаписатьНачалоЭлемента("СвИП");
		...
		Иначе 
			...
			ЗаписьXML.ЗаписатьАтрибут("Фамилия", "Индивидуальный");
			ЗаписьXML.ЗаписатьАтрибут("Имя", "предприниматель");
			МассивСлов = СтрРазделить(ДанныеПечати.Грузополучатель.ПолноеНаименование, " ");	// Добавил 
			КрайнийИндексМассива  = МассивСлов.ВГраница();											// Добавил 
			ЗаписьXML.ЗаписатьАтрибут("Отчество", МассивСлов[КрайнийИндексМассива - 2] + " " 
																+ МассивСлов[КрайнийИндексМассива - 1] + " " 
																+ МассивСлов[КрайнийИндексМассива]);
			ЗаписьXML.ЗаписатьКонецЭлемента(); // Закрываем ФИО
		...
		Пока СтрокаТовары.Следующий() Цикл
			...
			ЗаписьXML.ЗаписатьНачалоЭлемента("СумНал");
// МазинПравка	ЗаписьXML.ЗаписатьТекст(СуммаНДС); 
//			ЗаписьXML.ЗаписатьТекст(Строка(СтрокаТовары.СуммаНДС));
			ЗаписьXML.ЗаписатьТекст( СтрЗаменить(Формат(СтрокаТовары.СуммаНДС, "ЧДЦ=2; ЧРГ=.; ЧГ=0"), ",", "."));
