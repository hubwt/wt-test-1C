...
Функция ПолучитьРекомендованнуюЦену(Номенклатура, Дата) Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрСведений.дт_ЦеныНоменклатуры.СрезПоследних(&Дата, Номенклатура = &Номенклатура
		|	И ТипЦен = ЗНАЧЕНИЕ(Справочник.ТипыЦен.Фиксированная)) КАК дт_ЦеныНоменклатурыСрезПоследних";
