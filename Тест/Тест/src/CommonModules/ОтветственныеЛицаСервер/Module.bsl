...
Функция ПолучитьДанныеОтветственногоЛица(Организация, Дата = Неопределено, ОтветственноеЛицо = Неопределено) Экспорт
	...
	Для Каждого МетаВидОтветственного Из Метаданные.Перечисления.ОтветственныеЛицаОрганизаций.ЗначенияПеречисления Цикл
		...
		Если ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций[Ключ] Тогда
			...
			Результат.Вставить("ОснованиеПраваПодписи", 	 СтруктураОтветственных[Ключ + "ОснованиеПраваПодписи"]);
			Прервать;
		КонецЕсли;
...
Функция ПолучитьОтветственныеЛицаОрганизации(Организация, Дата = Неопределено) Экспорт
	...
	Для Каждого МетаВидОтветственного Из Метаданные.Перечисления.ОтветственныеЛицаОрганизаций.ЗначенияПеречисления Цикл
		...
		Если СтрокаТаблицы = Неопределено Тогда
			...
			СтруктураОтветственных.Вставить(Ключ, 				         Неопределено);//Справочники.ФизическиеЛица.ПустаяСсылка());
			...
			//+
			СтруктураОтветственных.Вставить(Ключ + "ОснованиеПраваПодписи",       "");
		...
		Иначе
			...
			КадровыеДанные = дт_Зарплата.ПолучитьКадровыеДанные(СтрокаТаблицы.ФизическоеЛицо, Дата, Организация);
			...
			СтруктураОтветственных.Вставить(Ключ + "Должность",          Строка(Должность));
			СтруктураОтветственных.Вставить(Ключ + "ДолжностьСсылка",    Должность);
			...
			СтруктураОтветственных.Вставить(Ключ + "ОснованиеПраваПодписи",       СтрокаТаблицы.ОснованиеПраваПодписи);
		КонецЕсли;
...
Функция ПолучитьТаблицуОтветственныхЛицПоОтбору(Знач Отбор, ДопустимыПомеченныеНаУдаление = Ложь, СтрокаПоиска = "") Экспорт
	...
	ПоляОтбора = дт_ОбщегоНазначенияПовтИсп.РеквизитыСправочника("ОтветственныеЛицаОрганизаций");
...
Процедура дт_СформироватьВременнуюТаблицуОтветственныхЛицДокументов(МассивДокументов, МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	ТаблицаДанныхДокументов КАК Док
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОтветственныеЛицаОрганизаций КАК ОтветственныеЛицаОрганизаций
		|		ПО Док.Руководитель = ОтветственныеЛицаОрганизаций.Ссылка
		|			И Док.Организация = ОтветственныеЛицаОрганизаций.Владелец
		|ГДЕ
		...
		|	втИсточники.Ссылка
		|ПОМЕСТИТЬ втПериодыАктуальности
		|ИЗ
		|	РегистрСведений.ДолжностиСотрудников КАК ДолжностиСотрудников
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втИсточники КАК втИсточники
		|		ПО ДолжностиСотрудников.Сотрудник = втИсточники.ФизическоеЛицо
		|		И ДолжностиСотрудников.Период <= втИсточники.Дата
		|		И ДолжностиСотрудников.Организация = втИсточники.Организация
		|СГРУППИРОВАТЬ ПО
		|	ДолжностиСотрудников.Сотрудник,
		|	втИсточники.Ссылка
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втПериодыАктуальности.Ссылка,
		|	втПериодыАктуальности.Сотрудник,
		|	ДолжностиСотрудников.Должность
		|ПОМЕСТИТЬ втДолжности
		|ИЗ
		|	РегистрСведений.ДолжностиСотрудников КАК ДолжностиСотрудников
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втПериодыАктуальности КАК втПериодыАктуальности
		|		ПО втПериодыАктуальности.Период = ДолжностиСотрудников.Период
		|		И втПериодыАктуальности.Сотрудник = ДолжностиСотрудников.Сотрудник
		|;
		|
		|
		|
		...
		|ВЫБРАТЬ
		|	втИсточники.Ссылка,
		|	втИсточники.Руководитель КАК Руководитель,
		|	ЕСТЬNULL(втИсточники.Руководитель.Наименование, """") КАК РуководительНаименование,
		|	ЕСТЬNULL(втДолжности.Должность, """") КАК РуководительДолжность,
		|	ЕСТЬNULL(втИсточники.ФизическоеЛицо.Наименование, """") КАК РуководительФИО,
		|	ЕСТЬNULL(втИсточники.Руководитель.ОснованиеПраваПодписи, """") КАК РуководительОснование
		|ПОМЕСТИТЬ ТаблицаОтветственныеЛица
		|ИЗ
		|	втИсточники КАК втИсточники
		|		ЛЕВОЕ СОЕДИНЕНИЕ втДолжности КАК втДолжности
		|		ПО втИсточники.Ссылка = втДолжности.Ссылка
		|		И втДолжности.Сотрудник = втИсточники.ФизическоеЛицо";
