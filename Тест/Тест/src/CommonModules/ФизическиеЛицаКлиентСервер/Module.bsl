...
Функция ФамилияИнициалы(Знач ФамилияИмяОтчество) Экспорт
	...
	Если Не ЗначениеЗаполнено(ФамилияИмяОтчество) Тогда
		Возврат "";
	КонецЕсли;
	
	Если ТипЗнч(ФамилияИмяОтчество) = Тип("СправочникСсылка.Пользователи") Тогда
		Объект = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ФамилияИмяОтчество, "ФизическоеЛицо,Наименование");
		Если ЗначениеЗаполнено(Объект.ФизическоеЛицо) Тогда
			Возврат ФамилияИнициалы(Объект.ФизическоеЛицо);
		Иначе
			Возврат ФамилияИнициалы(Объект.Наименование);
		КонецЕсли;
	КонецЕсли;
