...
Процедура ЗапуститьНаСервере(СсылкаВызывДок = Неопределено) 
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПриходДенегНаСчет.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&ДатаНач, ДЕНЬ) И КОНЕЦПЕРИОДА(&ДатаКон, ДЕНЬ)
		|	И ПриходДенегНаСчет.Проведен = ИСТИНА";
	
	Сегодня = ТекущаяДата();
	ТриМесяцаНазад = ДобавитьМесяц(Сегодня,-3); 
	Запрос.УстановитьПараметр("ДатаНач", ТриМесяцаНазад);
	Запрос.УстановитьПараметр("ДатаКон", ТекущаяДата());
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Текст = "Установить период";
	Успешно = 0; 
	Всего = 0; 
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если СсылкаВызывДок <> ВыборкаДетальныеЗаписи.Ссылка Тогда 
			СсылкаДок = ВыборкаДетальныеЗаписи.Ссылка;
			ДокОбъект = СсылкаДок.ПолучитьОбъект();
			Текст = "Перезаписанно успешно: " +Успешно+Символы.ПС +"Из: " + Всего; 
			Попытка
...
Процедура ПерезаписатьПриходДенегНаСчет(СсылкаВызывДок = Неопределено) Экспорт
	ЗапуститьНаСервере(СсылкаВызывДок = Неопределено); 
КонецПроцедуры
