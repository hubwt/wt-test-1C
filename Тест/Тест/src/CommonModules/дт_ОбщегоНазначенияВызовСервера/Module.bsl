...
// КомлевАА +_++
	...
	|	ЕСТЬNULL(РегистрНакопления1Остатки.машина.Серия, """") КАК Серия
			...
			Для Каждого Строка Из ФоткиИндкод Цикл
				...
				Если Строка.indCode = Строка(Выборка.индкод) Тогда
					...
					Если Строка.urls.Количество() > 0 Тогда
						...
						СтрокаНомерСерии = СтрРазделить(Выборка.Серия, " ")[0]; 
						МассивЦифр  = СтрРазделить(СтрокаНомерСерии,"/" );
						СтруктураТоваров.Вставить("series", МассивЦифр);
						Прервать;
					КонецЕсли;
				КонецЕсли;
