...
Функция НарядДоставка(Ссылка) Экспорт
	...
	ОбластьМакета.Параметры.заголовок = "Наряд на доставку №" + ОбъектПродажи.Номер + " от " + Формат(ТекущаяДата(),"ДЛФ=D" );
	...
	/// Комлев 9/9/24 +++
	Если ОбъектПродажи.ЕстьТрекНомер Тогда
		ОбластьМакета.Параметры.ТрекНомер = ОбъектПродажи.ТрекНомер;
	Иначе
		ОбластьМакета.Параметры.ТрекНомер = "Нет";
	КонецЕсли;
	/// Комлев 9/9/24 ---
...
Процедура дт_РасчетПоказателейАвтомобилей() Экспорт
	...
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ((ВТ_ПоказателиРасчетные.СуммаВыручкаОплаченная - Машины.Сумма) / Машины.Сумма * 100 КАК ЧИСЛО(15, 2))
