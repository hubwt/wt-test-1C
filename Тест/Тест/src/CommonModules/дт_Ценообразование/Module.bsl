...
Функция ПолучитьЦену(Отбор, Дата, Знач ТипЦены = Неопределено) Экспорт
	...
	//Если ТипЦены = Неопределено Тогда
		ТипЦены = ПредопределенноеЗначение("Справочник.ТипыЦен.Фиксированная");
	//КонецЕсли;
...
Процедура УстановитьЦену(ДанныеЗаполнения, Дата, Цена, Отказ = Ложь, АвтоИзменение = Ложь) Экспорт
	...
	Док.БылоАвтоИзменениеЦены = АвтоИзменение;
