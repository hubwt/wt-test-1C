...
Процедура конецРабочегоДня() экспорт
	...
	Запрос.УстановитьПараметр("ЗаказНаряд", выборка.Ссылка);
	//@skip-check query-in-loop
	ВыборкаВремени = Запрос.Выполнить().Выбрать();
	Если ВыборкаВремени.Количество() > 0 Тогда
		Пока ВыборкаВремени.Следующий() Цикл
		НаборЗаписей = РегистрыСведений.ВремяВыполненияЗН.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ЗаказНаряд.Установить(ВыборкаВремени.Задача);
		НаборЗаписей.Отбор.Период.Установить(ВыборкаВремени.ДатаСреза);
		НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() = 1 Тогда
			НашЗамер = НаборЗаписей[0];
			НашЗамер.КонецЗамера = ТекущаяДата();
