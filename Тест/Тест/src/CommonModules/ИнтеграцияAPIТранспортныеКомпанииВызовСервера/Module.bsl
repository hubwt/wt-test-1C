		...
		Если РезультатЗапроса.Количество() > 0 Тогда
				...
				Если ОтветЗапроса <> Неопределено Тогда
					Для каждого ДанныеТрекНомеров Из ОтветЗапроса Цикл
						...
						Если СтатусЗаказаВТК = "Вручен" Тогда
							...
							ДатаПолучения = СтрЗаменить(Лев(ДанныеТрекНомеров.Значение[ИндексПоследнейЗаписиВИстории].Получить("stateDate"), 19), "T", "");
							ДатаПолучения = СтрЗаменить(ДатаПолучения, "-" ,"");
							ДокументОбъект.ДатаПолучения = Дата(СтрЗаменить(ДатаПолучения, ":" ,""));
						КонецЕсли;
						...
						НаборЗаписей = РегистрыСведений.Telegram_Пользователи.СоздатьНаборЗаписей();
      					...
      					Если НаборЗаписей.Количество() = 1 Тогда
    						Телега = НаборЗаписей[0].ID_Пользователя;    ТекстЛога = "Ваша продажа № " +ДокументОбъект.номер +" для клиента "+ДокументОбъект.Клиент +" получила статус ТК " +Строка(ДокументОбъект.СтатусЗаказаВТК);
			...
			ИначеЕсли ТранспортнаяКомпания = Справочники.ТранспротнаяКомпания.НайтиПоКоду("000000020") Тогда //СДЭК
				...
				Для каждого ТрекНомер Из МассивТрекНомеров Цикл
					...
					Если ОтветЗапроса <> Неопределено Тогда
						...
						Если СтатусЗаказаВТК = "Вручен" Тогда
							ДокументОбъект.WTpanel = Справочники.СтатусыWT.НайтиПоКоду("000000008"); //Заказ получен
							ДатаПолучения = СтрЗаменить(Лев(ОтветЗапроса[0].date_time, 19), "T", "");
							ДатаПолучения = СтрЗаменить(ДатаПолучения, "-" ,"");
							ДокументОбъект.ДатаПолучения = Дата(СтрЗаменить(ДатаПолучения, ":" ,""));
						КонецЕсли;
			...
			ИначеЕсли ТранспортнаяКомпания = Справочники.ТранспротнаяКомпания.НайтиПоКоду("000000003") Тогда //ПЭК
				...
				Для каждого ДанныеПоТрекНомеру Из ДанныеПоТрекНомерам Цикл
					...
					Если СтатусЗаказаВТК = "Вручен" Тогда
						ДокументОбъект.WTpanel = Справочники.СтатусыWT.НайтиПоКоду("000000008"); //Заказ получен
						ДатаПолучения = СтрЗаменить(Статусы[ИндексПоследнейЗаписиВИстории].Получить("date"), "T", "");
						ДатаПолучения = СтрЗаменить(ДатаПолучения, "-" ,"");
						ДокументОбъект.ДатаПолучения = Дата(СтрЗаменить(ДатаПолучения, ":" ,""));
					КонецЕсли;
