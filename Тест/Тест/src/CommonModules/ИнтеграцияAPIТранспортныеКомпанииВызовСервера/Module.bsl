				...
				Для каждого ТрекНомер Из МассивТрекНомеров Цикл
					...
					Если ОтветЗапроса <> Неопределено Тогда
						...
						Если ЕстьСтруктура Тогда
							...
							Если СтатусЗаказаВТК = "Вручен" Тогда
								ДокументОбъект.WTpanel = Справочники.СтатусыWT.НайтиПоКоду("000000008"); //Заказ получен
								ДатаПолучения = Прав(ОтветЗапроса.status.text, 11);
								ДеньПолучения = Сред(ДатаПолучения, 1, 2);
								МесяцПолучения = Сред(ДатаПолучения, 4, 2);
								ГодПолучения = Сред(ДатаПолучения, 7, 4);
								ДокументОбъект.ДатаПолучения = Дата(ГодПолучения + МесяцПолучения + ДеньПолучения);
							КонецЕсли;
			...
			ИначеЕсли ТранспортнаяКомпания = Справочники.ТранспротнаяКомпания.НайтиПоКоду("000000012") Тогда //Энегрия
				...
				Для каждого ТрекНомер Из МассивТрекНомеров Цикл
					...
					Если ОтветЗапроса <> Неопределено Тогда
						...
						Если СтатусЗаказаВТК = "Вручен" Тогда
							ДокументОбъект.WTpanel = Справочники.СтатусыWT.НайтиПоКоду("000000008"); //Заказ получен
							ДатаПолучения = ОтветЗапроса.states[0].movingDateFormatted;
							ДеньПолучения = Сред(ДатаПолучения, 1, 2);
							МесяцПолучения = Сред(ДатаПолучения, 4, 2);
							ГодПолучения = Сред(ДатаПолучения, 7, 4);
							ДокументОбъект.ДатаПолучения = Дата(ГодПолучения + МесяцПолучения + ДеньПолучения);
