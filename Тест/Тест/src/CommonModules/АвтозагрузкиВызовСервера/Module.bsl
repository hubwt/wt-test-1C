...
Функция ВыгрузитьДанныеНаСервереАвитоМенее19кг(ТЗРезультат)
	...
	Если Фотки <> Неопределено Тогда
		Для каждого РезультатЗапроса Из ТЗРезультат Цикл
			...
			СтепеньИзноса = ПолучитьСтепеньИзноса(РезультатЗапроса.Товар.Код);
...
Функция ПолучитьСтепеньИзноса(Код)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	РегистрНакопления1Остатки.Товар КАК КоличествоТовара,
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		...
		Попытка
			Количество = РезультатЗапроса.КолвоОстаток;
		Исключение
			Количество = 0;
		КонецПопытки;
