...
Функция ПолучитьТекстДляТоваровВК()

///+ТатарМА 23.09.2024
	Результат = "ВЫБРАТЬ
	|	ИндНомер.индкод.Владелец КАК Товар,
	|	МАКСИМУМ(ИндНомер.индкод) КАК индкод
	|ПОМЕСТИТЬ ВТ_Товары
	|ИЗ
	|	РегистрСведений.ИндНомер КАК ИндНомер
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РегистрНакопления1.Остатки КАК РегистрНакопления1Остатки
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РегистрНакопления1.Остатки КАК РегистрНакопления1Остатки
	|			ПО ИндНомер.индкод = РегистрНакопления1Остатки.индкод
	|		ПО ВТ_Товары.индкод = ИндНомер.индкод
	|ГДЕ
	|	ИндНомер.АвитоЧастник
	|	И РегистрНакопления1Остатки.КолвоОстаток > 0
	|	И ИндНомер.Цена > 0
...
Функция ПолучитьТекстДляТоваровХодовыеАвито()
	...
	|	И ВЫБОР
	|		КОГДА НЕ (ИндНомер.индкод.Владелец.Подкатегория2 В (&Категории)
	|		И ИндНомер.индкод.Владелец.Подкатегория2 В (&Категории2))
	|			ТОГДА НЕ РегистрНакопления1Остатки.машина.Серия = ЗНАЧЕНИЕ(Справочник.Серии.ПустаяСсылка)
	|		ИНАЧЕ
	|		НЕ (ИндНомер.индкод.Владелец.Бренд = ЗНАЧЕНИЕ(Справочник.Бренд.ПустаяСсылка)
	|		ИЛИ ИндНомер.индкод.Владелец.МодельШин = ЗНАЧЕНИЕ(Справочник.МоделиШин.ПустаяСсылка)
...
Процедура АвтозагрузкаОзон() Экспорт
...
Процедура АвтозагрузкаВК() Экспорт

	///+ТатарМА 23.09.2024
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьТекстДляТоваровВК();
	...
	Запрос.УстановитьПараметр("Стеллаж", 	Справочники.Стеллаж.НайтиПоКоду("000001072")); //2_ANGAR
	Запрос.УстановитьПараметр("Категории", 	МассивКатегорийШин);
	Запрос.УстановитьПараметр("Склад", 		МассивСкладов);
	Запрос.УстановитьПараметр("Поддон", 	Справочники.Поддоны.НайтиПоКоду("000000003")); // П0003 потеряшки
	...
	ДеталиТЗ.Колонки.Добавить("НомерЗаписи");
	ДеталиТЗ.Колонки.Добавить("ПустаяСтрока");
	
	НомерЗаписи = 1;
	Для каждого СтрокаТЗ Из ДеталиТЗ Цикл
	...
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.ОткрытьФайл("\\192.168.0.245\shared_folder\Autoloads_MarketPlace\Autoload_VK\vk.xml", "UTF-8");
	...
	"<currencies>" + Символы.ПС + 
	"<currency id=""RUB"" rate=""1""/>" + Символы.ПС + 
	"</currencies>" + Символы.ПС + 
	"<categories>" + Символы.ПС + 
	"<category id=""1"">Транспорт</category>" + Символы.ПС + 
	...
	Для НомерЗаписи = 0 По КонечныйНомер Цикл
		...
		КонецЦикла;
		КонечноеЗначение = НомерЗаписи + 1;
		ТекстФайла = ВыгрузитьДанныеНаСервереВК(ЧастьДеталей);
...
Функция ВыгрузитьДанныеНаСервереВК(ТЗРезультат)
	...
	Если Фотки <> Неопределено Тогда
		Для каждого РезультатЗапроса Из ТЗРезультат Цикл
			Для каждого СтрокаФото ИЗ Фотки Цикл 
				Если СтрокаФото.indCode = РезультатЗапроса.индкод.наименование Тогда
					НайденныеФотки = СтрокаФото.xml;
					//Обрезать Фотки до 5 штук
					КоличествоСтрок = СтрЧислоСтрок(НайденныеФотки);
					Если КоличествоСтрок - 2 > 5 Тогда
						Текст = "";
			...
			Айдишник = Строка(Айдишник) + "_" + Формат(РезультатЗапроса.НомерЗаписи, "ЧГ=");
			...
			Если РезультатЗапроса.Категории = Справочники.Категории.НайтиПоКоду("000000501") ИЛИ
				...
				|<model>" + РезультатЗапроса.Модель + "</model>
				|<vendor>" + Бренд + "</vendor>
				...
				|<description>
				|<![CDATA[          
				|<p>" + РезультатЗапроса.Артикул + " " + РезультатЗапроса.Товар + " scania" + "</p>
				|<p>Оригинал</p>
				|<p>Состояние " + РезультатЗапроса.Состояние + "<br>
				|Размеры " + РазмерыШин + "<br>
				|Вес " + РезультатЗапроса.Вес + "</p>
				|<p>Свяжитесь с нами, если у вас остались вопросы, запчасть в наличии!</p>
				|]]>
				|</description>                               
				|<price>" + Формат(РезультатЗапроса.Цена, "ЧДЦ=0; ЧГ=0") + "</price>
				|</offer>";
...
#Область АвитоХодовые

Процедура АвтозагрузкаАвитоХодовые() Экспорт
	
	///+ТатарМА 09.12.2024
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьТекстДляТоваровХодовыеАвито();
	...
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.ОткрытьФайл("\\192.168.0.245\shared_folder\Autoloads_MarketPlace\Autoload_avito_marketable\avito_marketable.xml", "UTF-8");
	ЗаписьXML.ЗаписатьБезОбработки("<Ads formatVersion=""3"" target=""Avito.ru"">");
