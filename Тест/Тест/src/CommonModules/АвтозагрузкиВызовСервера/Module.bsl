...
Функция ПолучитьТекстДляТоваровАвитоМенее15кг()
	...
	|	И ИндНомер.индкод.Владелец.Вес < 15
...
Функция ПолучитьТекстДляТоваровАвитоБолее15кг()
	...
	|						ИЛИ ИндНомер.индкод.Владелец.Состояние = НЕОПРЕДЕЛЕНО)
	|		КОНЕЦ
	|	И (ИндНомер.Цена > 149999 ИЛИ (ИндНомер.индкод.Владелец.Вес >= 15
	...
	Если Фотки <> Неопределено Тогда
		Для каждого РезультатЗапроса Из ТЗРезультат Цикл
			...
			Если РезультатЗапроса.Категории = Справочники.Категории.НайтиПоКоду("000000501") ИЛИ
				...
				|<Description><![CDATA[<p>" + РезультатЗапроса.НомерПроизводителя + " " + РезультатЗапроса.Товар + "</p>" + Комментарий + " 
			...
			Иначе
				...
				|<p>" + РезультатЗапроса.НомерПроизводителя + " " + РезультатЗапроса.Товар + "</p>" + Комментарий + " 
...
#Область АвитоМенее15кг

Процедура АвтозагрузкаАвитоМенее15кг() Экспорт
	
	///+ГомзМА 15.02.2024
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьТекстДляТоваровАвитоМенее15кг();
	...
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.ОткрытьФайл("\\192.168.0.245\shared_folder\Autoloads_MarketPlace\Autoload_avito-15kg\avito-15kg.xml", "UTF-8");
	ЗаписьXML.ЗаписатьБезОбработки("<Ads formatVersion=""3"" target=""Avito.ru"">");
	...
	Для НомерЗаписи = 0 По КонечныйНомер Цикл
		...
		КонецЦикла;
		КонечноеЗначение = НомерЗаписи + 1;
		ТекстФайла = ВыгрузитьДанныеНаСервереАвитоМенее15кг(ЧастьДеталей);
...
Функция ВыгрузитьДанныеНаСервереАвитоМенее15кг(ТЗРезультат)
...
#Область АвитоБолее15кг

Процедура АвтозагрузкаАвитоБолее15кг() Экспорт
	
	///+ГомзМА 15.02.2024
	Запрос = Новый Запрос;
	Запрос.Текст = ПолучитьТекстДляТоваровАвитоБолее15кг();
	...
	Для НомерЗаписи = 0 По КонечныйНомер Цикл
		...
		КонецЦикла;
		КонечноеЗначение = НомерЗаписи + 1;
		ТекстФайла = ВыгрузитьДанныеНаСервереАвитоБолее15кг(ЧастьДеталей);
...
Функция ВыгрузитьДанныеНаСервереАвитоБолее15кг(ТЗРезультат)
	...
	Если Фотки <> Неопределено Тогда
		Для каждого РезультатЗапроса Из ТЗРезультат Цикл
			...
			Если РезультатЗапроса.Категории = Справочники.Категории.НайтиПоКоду("000000501") ИЛИ
				...
				|" + НайденныеФотки + "
				|<Description><![CDATA[<p>" + РезультатЗапроса.НомерПроизводителя + " " + РезультатЗапроса.Товар + "</p>
				|<p>️Состояние: " + Состояние + "<br>
				|Размеры: " + РазмерыШин + "</p>
				|<p>Оригинальный номер: " + ОригинальныйНомер + "<br>
				|Аналоги: " + Аналог + "<br>
...
Процедура АвтозагрузкаИззап() Экспорт
	...
	ДеталиТЗ = Запрос.Выполнить().Выгрузить();
	МассивИндкодов = ДеталиТЗ.ВыгрузитьКолонку("ИндкодНаименование");
	...
	Лист.Columns(1).ColumnWidth = 17;
	Лист.Columns(2).ColumnWidth = 17;
	Лист.Columns(3).ColumnWidth = 23;
	Лист.Columns(4).ColumnWidth = 17;
	Лист.Columns(5).ColumnWidth = 17;
	Лист.Columns(6).ColumnWidth = 80;
	Лист.Columns(7).ColumnWidth = 7;
	Лист.Columns(8).ColumnWidth = 13;
	Лист.Columns(9).ColumnWidth = 80;
	Лист.Columns(10).ColumnWidth = 11;
	Лист.Columns(11).ColumnWidth = 59;
	Лист.Columns(12).ColumnWidth = 80;
	Лист.Columns(13).ColumnWidth = 80;
	Лист.Columns(14).ColumnWidth = 80;
	
	//Заполняем ячейки данными
	//Заполняем заголовки
	Лист.Cells(1, 1).Value = "Id1c";
	Лист.Cells(1, 2).Value = "VendorCode";
	Лист.Cells(1, 3).Value = "nproizv";
	Лист.Cells(1, 4).Value = "brand";
	Лист.Cells(1, 5).Value = "Vendor";
	Лист.Cells(1, 6).Value = "Title";
	Лист.Cells(1, 7).Value = "kolvo";
	Лист.Cells(1, 8).Value = "Price";
	Лист.Cells(1, 9).Value = "analogs";
	Лист.Cells(1, 10).Value = "condition";
	Лист.Cells(1, 11).Value = "category";
	Лист.Cells(1, 12).Value = "url";
	Лист.Cells(1, 13).Value = "Image_path";
	Лист.Cells(1, 14).Value = "Image";
	
	//Заполняем строки
	Строка = 2;
	Фотки = ПолучитьФото(МассивИндкодов);
	Итер = 0;
	Для каждого СтрокаТЧ Из ДеталиТЗ Цикл
		Если Фотки <> Неопределено Тогда
		...
		Иначе
			НайденноеФото = "";
		КонецЕсли;
		Лист.Cells(Строка, 1).Value  = Строка(СтрокаТЧ.индкод);
		Лист.Cells(Строка, 2).Value  = Строка(СтрокаТЧ.Артикул);
		Лист.Cells(Строка, 3).Value  = Строка(СтрокаТЧ.НомерПроизводителя);
		Лист.Cells(Строка, 4).Value  = Строка(СтрокаТЧ.Бренд);
		Лист.Cells(Строка, 5).Value  = Строка(СтрокаТЧ.Производитель);
		Лист.Cells(Строка, 6).Value  = Строка(СтрокаТЧ.Товар);
		Лист.Cells(Строка, 7).Value  = СтрокаТЧ.КолвоОстаток;
		Лист.Cells(Строка, 8).Value  = СтрокаТЧ.Цена;
		Лист.Cells(Строка, 9).Value  = ПолучитьАналогиТовара(СтрокаТЧ.Товар);
		Лист.Cells(Строка, 10).Value = Строка(СтрокаТЧ.Состояние);
