...
Процедура ОтменаРаспродажи() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	Номенклатура.Распродажа = ИСТИНА
		|	И НАЧАЛОПЕРИОДА(Номенклатура.ДатаОкончанияРаспродажи, ДЕНЬ) <= НАЧАЛОПЕРИОДА(&ТекДата, ДЕНЬ)";
	...
	Если  НЕ РезультатЗапроса.Пустой() Тогда
		...
		НоменклатураОбъект = Выборка.НоменклатураСсылка.ПолучитьОбъект();
		НоменклатураОбъект.РекомендованаяЦена = НоменклатураОбъект.ЦенаДоРаспродажи;
		НоменклатураОбъект.ДатаОкончанияРаспродажи = '00010101';
		НоменклатураОбъект.Распродажа = Ложь;
		НоменклатураОбъект.БылаОтменаРаспродажи = Истина;
		
		ДанныеЗаписи = Новый Структура("Номенклатура, ТипЦен", НоменклатураОбъект.Ссылка, Справочники.ТипыЦен.Фиксированная);
		дт_Ценообразование.УстановитьЦену(ДанныеЗаписи, ТекущаяДата(), НоменклатураОбъект.РекомендованаяЦена);
	
		НоменклатураОбъект.Записать();
		Сообщить("Снята с распродажи " + Выборка.НоменклатураСсылка);
	КонецЦикла;
...
Функция ПросклонятьФИО(ФИО, Падеж) Экспорт
	Рез = СклонениеПредставленийОбъектов.ПросклонятьФИО(ФИО, Падеж);
	Возврат Рез;
КонецФункции
