...
Процедура ПроставитьВЗаявкеСтатусЖдемДенегЕслиПродажаОплачена() Экспорт
/// Комлев 19/06/24 +++
...
Процедура ЗаблокироватьПродажи() Экспорт
	// КомлевАА +++
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	ПродажаЗапчастей.Заблокирована КАК Заблокирована,
	...
	Пока Выборка.Следующий() Цикл
		Если (Месяц(Выборка.Дата) <> Месяц(ТекущаяДата()) Или Строка(Выборка.НомерУПД) <> "") И Выборка.Заблокирована
			= Ложь Тогда
			Продажа = Выборка.Ссылка.ПолучитьОбъект();
			Продажа.Заблокирована = Истина;
			Продажа.Записать();
		Иначе
			Продолжить;
		КонецЕсли;
...
Процедура ЗаблокироватьЗаказНаряды() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗаказНаряд.Ссылка,
		|	ЗаказНаряд.Заблокирован,
		|	ЗаказНаряд.НомерУПД
		|ИЗ
		|	Документ.ЗаказНаряд КАК ЗаказНаряд
		|ГДЕ
		|	ЗаказНаряд.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
		|	И ЗаказНаряд.НомерУПД <> """"";
	
	ДатаНачала = ДобавитьМесяц(ТекущаяДата(), -15);
	Запрос.УстановитьПараметр("ДатаНачала", ДатаНачала);
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Заблокирован = Ложь Тогда
			ЗаказНаряд = Выборка.Ссылка.ПолучитьОбъект();
			ЗаказНаряд.Заблокирован = Истина;
			ЗаказНаряд.Записать();
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
