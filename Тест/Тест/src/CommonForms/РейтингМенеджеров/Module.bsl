...
&НаКлиенте
Процедура ДатаГруппКлиентовПриИзменении1(Элемент)
	ОбновитьСписокГруппКлиентовПоМенеджерам();
КонецПроцедуры


Процедура РассчитатьИтоги()
	// Комлев АА 16/10/24 +++
	Схема = Элементы.СписокГруппыКлиентовПоМенеджерам.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	Настройки = Элементы.СписокГруппыКлиентовПоМенеджерам.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	...
	//ЭтотОбъект.ЭтаФорма.Элементы.СписокГруппыКлиентовПоМенеджерамКоличествоАктивных.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.СписокГруппыКлиентовПоМенеджерамКоличествоАктивных.ТекстПодвала =  "Итого: " + Формат(Результат.Итог(
		"КоличествоАктивных"), "ЧДЦ=2; ЧН=-");
		
		ЭтотОбъект.ЭтаФорма.Элементы.СписокГруппыКлиентовПоМенеджерамКоличествоТеплых.ТекстПодвала =  "Итого: " + Формат(Результат.Итог(
		"КоличествоТеплых"), "ЧДЦ=2; ЧН=-");
		
		ЭтотОбъект.ЭтаФорма.Элементы.СписокГруппыКлиентовПоМенеджерамКоличествоПотенциальных.ТекстПодвала =  "Итого: " + Формат(Результат.Итог(
		"КоличествоПотенциальных"), "ЧДЦ=2; ЧН=-");
		
		ЭтотОбъект.ЭтаФорма.Элементы.СписокГруппыКлиентовПоМенеджерамКоличествоХолодных.ТекстПодвала =  "Итого: " + Формат(Результат.Итог(
		"КоличествоХолодных"), "ЧДЦ=2; ЧН=-");
		
		ЭтотОбъект.ЭтаФорма.Элементы.СписокГруппыКлиентовПоМенеджерамИтогоПоМенеджеру.ТекстПодвала =  "Итого: " + Формат(Результат.Итог(
		"ИтогоПоМенеджеру"), "ЧДЦ=2; ЧН=-");
// Комлев АА 16/10/24 ---
КонецПроцедуры


Процедура ОбновитьСписокГруппКлиентовПоМенеджерам()
	// Комлев АА 16/10/24 +++
	
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("Общий", Справочники.Пользователи.НайтиПоНаименованию("Общий Клиент"));
	ДатаНачалаАКБ = НачалоДня(Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 30);
	ДатаОкончанияАКБ = КонецДня(Дата(ДатаГруппКлиентов));
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("ДатаНачалаАКБ", ДатаНачалаАКБ);
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("ДатаОкончанияАКБ", ДатаОкончанияАКБ);

	ДатаНачалаТеплые = НачалоДня(Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 90);
	ДатаОкончанияТеплые = КонецДня(Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 31);
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("ДатаНачалаТеплые", ДатаНачалаТеплые);
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("ДатаОкончанияТеплые", ДатаОкончанияТеплые);

	ДатаНачалаПотенциальные = НачалоДня(Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 91);
	ДатаОкончанияПотенциальные = КонецДня(Дата(ДатаГруппКлиентов));
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("ДатаНачалаПотенциальные",
		ДатаНачалаПотенциальные);
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("ДатаОкончанияПотенциальные",
		ДатаОкончанияПотенциальные);

	ДатаХолодные = НачалоДня(Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 90);
	СписокГруппыКлиентовПоМенеджерам.Параметры.УстановитьЗначениеПараметра("ДатаХолодные", ДатаХолодные);
	РассчитатьИтоги();
	// Комлев АА 16/10/24 ---
КонецПроцедуры
...
Процедура ПодстраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	//Комлев 16/10/24 +++
	Если ТекущаяСтраница = Элементы.ГруппыКлиентовПоМенеджерам Тогда
		ДатаГруппКлиентов = ТекущаяДата();
		ОбновитьСписокГруппКлиентовПоМенеджерам();
	КонецЕсли;
