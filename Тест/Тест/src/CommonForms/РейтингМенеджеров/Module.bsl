...
Процедура ДатаДиаграммыКлиентыПриИзменении(Элемент)
	ОбновитьДиаграммуГруппыКлиентов(ДатаДиаграммыКлиенты);
КонецПроцедуры

Процедура ОбновитьДиаграммуГруппыКлиентов(ДатаДиаграммыКлиенты)
	/// +++ Комлев 15/10/24 +++
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	ЗаказКлиента.Клиент
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(ЗаказКлиента.Клиент.Код) > 1
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЗаказКлиента.Клиент
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(ЗаказКлиента.Клиент.Код) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Потннциальные.Клиент) КАК КоличествоПотенциальных,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Холодные.Клиент) КАК КоличествоХолодных
	|ИЗ
	|	АКБ КАК АКБ,
	|	Теплые КАК Теплые,
	|	Потннциальные КАК Потннциальные,
	...
	Серия1 = Диаграмма.Серии.Добавить();
	...
	Выборка.Следующий();
	Диаграмма.УстановитьЗначение(Точка, Серия1, Выборка.КоличествоАктивных);
	Диаграмма.УстановитьЗначение(Точка, Серия2, Выборка.КоличествоТеплых);
	Диаграмма.УстановитьЗначение(Точка, Серия3, Выборка.КоличествоПотенциальных);
	Диаграмма.УстановитьЗначение(Точка, Серия4, Выборка.КоличествоХолодных);
