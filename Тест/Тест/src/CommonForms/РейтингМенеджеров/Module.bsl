...
Процедура ДатаДиаграммыКлиентыПриИзменении(Элемент)
	Данные = ПолучитьДанныеПоГруппамКлиентов(ДатаДиаграммыКлиенты);
	ОбновитьДиаграммуКлиенты(Данные);
КонецПроцедуры
...
Функция ПолучитьДанныеПоГруппамКлиентов(ДатаДиаграммыКлиенты)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаказКлиента.Клиент КАК КоличествоХолодных
	|ПОМЕСТИТЬ ХОЛОДНЫЕ
	|ИЗ
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КОЛИЧЕСТВО(ХОЛОДНЫЕ.КоличествоХолодных) КАК КоличествоХолодных
	|ИЗ
	|	ХОЛОДНЫЕ КАК ХОЛОДНЫЕ";
	
	ДатаНачалаАКБ = Дата(ДатаДиаграммыКлиенты) - (24 * 60 * 60) * 30;
	ДатаОкончанияАКБ = Дата(ДатаДиаграммыКлиенты);
	Запрос.УстановитьПараметр("ДатаНачалаАКБ", ДатаНачалаАКБ);
	Запрос.УстановитьПараметр("ДатаОкончанияАКБ", ДатаОкончанияАКБ);
	
	ДатаНачалаТеплые = Дата(ДатаДиаграммыКлиенты) - (24 * 60 * 60) * 90;
	ДатаОкончанияТеплые = Дата(ДатаДиаграммыКлиенты) - (24 * 60 * 60) * 31;
	...
	ДатаНачалаПотенциальные = Дата(ДатаДиаграммыКлиенты) - (24 * 60 * 60) * 91;
	ДатаОкончанияПотенциальные = Дата(ДатаДиаграммыКлиенты);
	...
	ДатаХолодные = Дата(ДатаДиаграммыКлиенты) - (24 * 60 * 60) * 90;
	...
	ВыборкаХолодные = Пакеты[7].Выбрать();
...
Процедура ОбновитьСписокГруппКлиентовПоМенеджерам()
	// Комлев АА 16/10/24 +++
	ДатаНачалаАКБ = Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 30;
	ДатаОкончанияАКБ = Дата(ДатаГруппКлиентов);
	...
	ДатаНачалаТеплые = Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 90;
	ДатаОкончанияТеплые = Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 31;
	...
	ДатаНачалаПотенциальные = Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 91;
	ДатаОкончанияПотенциальные = Дата(ДатаГруппКлиентов);
	...
	ДатаХолодные = Дата(ДатаГруппКлиентов) - (24 * 60 * 60) * 90;
...
КонецПроцедуры
 ...
 Процедура ОбновитьДиаграммуКлиенты(ДанныеОКлиентах)
 	/// --- Комлев 15/10/24 ---
	...
	Диаграмма.ОбластьЗаголовка.Текст = "КоличествоКлиентовПоГруппам";
