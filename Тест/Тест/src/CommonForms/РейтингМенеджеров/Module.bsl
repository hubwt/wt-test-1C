&НаКлиенте
Процедура ДатаДиаграммыКлиентыПриИзменении(Элемент)
	ОбновитьДиаграммуКлиенты();
КонецПроцедуры

&НаКлиенте
Процедура ПериодГрафикаПриростКлинтовПриИзменении(Элемент)
	ОбновитьГрафикПриростАктивныхКлиентов();
КонецПроцедуры

&НаСервере
Процедура ОбновитьГрафикПриростАктивныхКлиентов() Экспорт
	// Комлев АА 23/10/24 +++
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПоказателиКлиенскойБазы.Период КАК Период,
	|	ПоказателиКлиенскойБазы.КоличествоАктивных,
	|	ПоказателиКлиенскойБазы.КоличествоТеплых,
	|	ПоказателиКлиенскойБазы.КоличествоПотенциальных,
	|	ПоказателиКлиенскойБазы.КоличествоХолодных
	|ИЗ
	|	РегистрСведений.ПоказателиКлиенскойБазы КАК ПоказателиКлиенскойБазы
	|ГДЕ
	|	ПоказателиКлиенскойБазы.Период МЕЖДУ НачалоПериода(&ДатаНачала, День) И КонецПериода(&ДатаОкончания, День)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период";

	Запрос.УстановитьПараметр("ДатаНачала", ПериодГрафикаПриростКлинтов.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", ПериодГрафикаПриростКлинтов.ДатаОкончания);
	...
	Диаграмма = ГрафикПриростаАктивныхКлиентов;
	Диаграмма.Очистить();
	...
	Активные = Диаграмма.Серии.Добавить();
	Теплые = Диаграмма.Серии.Добавить();
	Потенциальные = Диаграмма.Серии.Добавить();

	Пока Выборка.Следующий() Цикл
		Точка = Диаграмма.УстановитьТочку(Формат(Выборка.Период, "ДЛФ=D;"));
		Диаграмма.УстановитьЗначение(Точка, Активные, Выборка.КоличествоАктивных);
		Активные.Текст = "Активные " + Выборка.КоличествоАктивных;
		Диаграмма.УстановитьЗначение(Точка, Теплые, Выборка.КоличествоТеплых);
		Теплые.Текст = "Теплые " + Выборка.КоличествоТеплых;
		Диаграмма.УстановитьЗначение(Точка, Потенциальные, Выборка.КоличествоПотенциальных);
		Потенциальные.Текст = "Потенциальные " + Выборка.КоличествоПотенциальных;
...
КонецФункции
 ...
 Процедура ОбновитьДиаграммуКлиенты() Экспорт
 	/// --- Комлев 15/10/24 ---
 	ДанныеОКлиентах = ПолучитьДанныеПоГруппамКлиентов(ДатаДиаграммыКлиенты);
	Диаграмма = ГруппыКлиентовДиаграмма;
	...
	Холодные = Диаграмма.Серии.Добавить();
	Точка = Диаграмма.УстановитьТочку("Количество");
	Диаграмма.УстановитьЗначение(Точка, Активные, ДанныеОКлиентах.КоличествоАктивных);
	Активные.Текст = "Активные " + ДанныеОКлиентах.КоличествоАктивных;
	Диаграмма.УстановитьЗначение(Точка, Теплые, ДанныеОКлиентах.КоличествоТеплых);
	Теплые.Текст = "Теплые " + ДанныеОКлиентах.КоличествоТеплых;
	Диаграмма.УстановитьЗначение(Точка, Потенциальные, ДанныеОКлиентах.КоличествоПотенциальных);
	Потенциальные.Текст = "Потенциальные " + ДанныеОКлиентах.КоличествоПотенциальных;
	Диаграмма.УстановитьЗначение(Точка, Холодные, ДанныеОКлиентах.КоличествоХолодных);
	Холодные.Текст = "Холодные " + ДанныеОКлиентах.КоличествоХолодных;
	Диаграмма.ВидПодписей =  ВидПодписейКДиаграмме.Значение;
	Диаграмма.АвтоТранспонирование 	= Истина;
	Диаграмма.Обновление		= Истина;
	/// --- Комлев 15/10/24 ---
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДатаОкончания = '00010101' Тогда
		ДатаОкончания = '30030101';
	КонецЕсли;
	
	РейтингМенеджеров.Параметры.УстановитьЗначениеПараметра("ВидРасхода",Справочники.ВидыРасходов.НайтиПоКоду("000000041"));
	РейтингМенеджеров.Параметры.УстановитьЗначениеПараметра("ДатаНачалаБезналБезНДС", '20241101 000000');
...
//&НаСервере
//Процедура ОбновитьСтатистикуПоКлинтам(ДатаПарам)
//	///  +++ Комлев 11/10/24 +++
//	ПериодОтчетаКлинта = ДатаПарам;
//	ДатаМесяцНазад = ПериодОтчетаКлинта - 2678400;
//	Клинты.Параметры.УстановитьЗначениеПараметра("ДатаНачала", ДатаМесяцНазад);
//	Клинты.Параметры.УстановитьЗначениеПараметра("ДатаОкончания", ПериодОтчетаКлинта);
//	
//	
//	Запрос = Новый Запрос;
//	Запрос.Текст =
//		"ВЫБРАТЬ
//		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Клиенты.Ссылка) КАК КоличествоАктивных
//		|ИЗ
//		|	Справочник.Клиенты КАК Клиенты
//		|ГДЕ
//		|	Клиенты.Ссылка В
...
//		|ВЫБРАТЬ
//		|
//		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Клиенты.Ссылка) КАК КоличествоНеАктивных
//		|ИЗ
//		|	Справочник.Клиенты КАК Клиенты
//		|ГДЕ
//		|		НЕ Клиенты.Ссылка В
...
//		|ВЫБРАТЬ
//		|
//		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Клиенты.Ссылка) КАК КоличествоЦелевых
//		|ИЗ
//		|	Справочник.Клиенты КАК Клиенты
//		|ГДЕ
//		|	Клиенты.Целевой
...
//		|////////////////////////////////////////////////////////////////////////////////
//		|ВЫБРАТЬ
//		|	КОЛИЧЕСТВО(Клиенты.Ссылка) КАК КоличествоПоМенеджерам
//		|ИЗ
//		|	Справочник.Клиенты КАК Клиенты
//		|ГДЕ
//		|	Клиенты.Ответственный <> Значение(Справочник.Пользователи.ПустаяСсылка)
//		|	И Клиенты.Ответственный <> &ОбщийКлиент
//		|;
//		|
//		|////////////////////////////////////////////////////////////////////////////////
//		|ВЫБРАТЬ
//		|	КОЛИЧЕСТВО(Клиенты.Ссылка) КАК КоличествоОбщих
//		|ИЗ
//		|	Справочник.Клиенты КАК Клиенты
//		|ГДЕ
//		|	Клиенты.Ответственный = Значение(Справочник.Пользователи.ПустаяСсылка)
...
//	
//	
//	Запрос.УстановитьПараметр("ДатаНачала", НачалоДня(ДатаМесяцНазад));
//	Запрос.УстановитьПараметр("ДатаОкончания", КонецДня(ПериодОтчетаКлинта));
//	Запрос.УстановитьПараметр("ОбщийКлиент", Справочники.Пользователи.НайтиПоНаименованию("Общий Клиент"));
//	
//	Пакеты = Запрос.ВыполнитьПакет();
//	
//	КоличествоАктивныхВыб = Пакеты[0].Выбрать();
//	КоличествоАктивныхВыб.Следующий();
//	КоличествоАктивных = КоличествоАктивныхВыб.КоличествоАктивных;
//
//	КоличествоНеАктивныхВыб = Пакеты[1].Выбрать();
//	КоличествоНеАктивныхВыб.Следующий();
//	КоличествоПассивных = КоличествоНеАктивныхВыб.КоличествоНеАктивных;
//	
//	КоличествоЦелевыхВыб = Пакеты[2].Выбрать();
//	КоличествоЦелевыхВыб.Следующий();
//	Целевые = КоличествоЦелевыхВыб.КоличествоЦелевых;
//	
//	КоличествоНЕЦелевыхВыб = Пакеты[3].Выбрать();
//	КоличествоНЕЦелевыхВыб.Следующий();
//	НеЦелевые = КоличествоНЕЦелевыхВыб.КоличествоНеЦелевых;
//	
//	КоличествоПоМенеджерамВыб = Пакеты[4].Выбрать();
//	КоличествоПоМенеджерамВыб.Следующий();
//	ПоМенеджерам = КоличествоПоМенеджерамВыб.КоличествоПоМенеджерам;
//	
//	
//	КоличествоОбщихВыб = Пакеты[5].Выбрать();
//	КоличествоОбщихВыб.Следующий();
//	ОбщиеКлинты = КоличествоОбщихВыб.КоличествоОбщих;
//	///  --- Комлев 11/10/24 ---
//КонецПроцедуры
//
//&НаКлиенте
//Процедура ПериодОтчетаКлинтаПриИзменении(Элемент)
//	ОбновитьСтатистикуПоКлинтам(ПериодОтчетаКлинта);
//КонецПроцедуры
...
Процедура ПериодПриИзменении(Элемент)
	...
	ОбновитьСпискиВЦелом(Период.ДатаНачала, ДатаОкончания);
	ОбновитьСпискиФинОтделВЧислах(Период.ДатаНачала, ДатаОкончания);
	ОбновитьСпискиотделаКадров(Период.ДатаНачала, ДатаОкончания);
    ОбновитьСпискиФинОтделВДинамике(Период.ДатаНачала, ДатаОкончания); 
	ОбновитьСпискиОКК(Период.ДатаНачала, ДатаОкончания);
	ЗаполнитьПодвалРейтингМенеджеров();
	ЗаполнитьПодвалРейтингМенеджеров1();
	ЗаполнениеПодвала();
...
Процедура ЗаполнитьПодвалРейтингМенеджеров()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровБезналБезНДС.ГоризонтальноеПоложениеВПодвале 			= ГоризонтальноеПоложениеЭлемента.Право;
...
//	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1ДоПлана.ГоризонтальноеПоложениеВПодвале 				= ГоризонтальноеПоложениеЭлемента.Право;
	...
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКоличествоАвтомобилей.ТекстПодвала =  Формат(Результат.Итог("КоличествоАвтомобилей"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКлиентыобработанные.ТекстПодвала   =  Формат(Результат.Итог("Клиентыобработанные"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКоличествоКлиентов.ТекстПодвала    =  Формат(Результат.Итог("КоличествоКлиентов"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКоличествоЗвонков.ТекстПодвала 	 =  Формат(Результат.Итог("КоличествоЗвонков"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаПоступлений.ТекстПодвала 	 =  Формат(Результат.Итог("СуммаПоступлений"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровДлительность.ТекстПодвала 		 =  Формат(Результат.Итог("Длительность"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолвоПродаж.ТекстПодвала 			 =  Формат(Результат.Итог("КолвоПродаж"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаПродаж.ТекстПодвала 			 =  Формат(Результат.Итог("СуммаПродаж"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолвоЗаявок.ТекстПодвала 		 	 =  Формат(Результат.Итог("КолвоЗаявок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровЗПМенеджера.ТекстПодвала 			 =  Формат(Результат.Итог("ЗПМенеджера"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровИсходящие.ТекстПодвала 			 =  Формат(Результат.Итог("Исходящие"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровВходящие.ТекстПодвала 			 =  Формат(Результат.Итог("Входящие"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровНаличные.ТекстПодвала 			 =  Формат(Результат.Итог("Наличные"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровБезнал.ТекстПодвала 				 =  Формат(Результат.Итог("Безнал"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровБезналБезНДС.ТекстПодвала 		 =  Формат(Результат.Итог("БезналБезНДС"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровДельта.ТекстПодвала 				 =  Формат(Результат.Итог("Дельта"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	//Если ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаРасходом <> Неопределено Тогда
	//ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаРасходом.ТекстПодвала 	 =  Формат(Результат.Итог("СуммаРасходом"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	//КонецЕсли;
	//ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1СуммаПоступлений.ТекстПодвала 	 =  Формат(Результат.Итог("СуммаПоступлений"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	//ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1ДоПлана.ТекстПодвала 	     	 =  Формат(Результат.Итог("ДоПлана"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	
КонецПроцедуры

Процедура ЗаполнитьПодвалРейтингМенеджеров1()
	
	Схема = Элементы.РейтингМенеджеров1.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	Настройки = Элементы.РейтингМенеджеров1.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(Схема, Настройки,,,Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
	ПроцессорКомпоновки = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновки.Инициализировать(МакетКомпоновки);
	...
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1СуммаПоступлений.ГоризонтальноеПоложениеВПодвале 	= ГоризонтальноеПоложениеЭлемента.Право;
//	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1ДоПлана.ГоризонтальноеПоложениеВПодвале 				= ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаЗаявок.ГоризонтальноеПоложениеВПодвале 			= ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолПроиграноСделок.ГоризонтальноеПоложениеВПодвале 	= ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаПроигранноСделок.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолВсегоЗаявок.ГоризонтальноеПоложениеВПодвале 		= ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолВсегоВыйграноСделок.ГоризонтальноеПоложениеВПодвале 	= ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1ДоПлана.ГоризонтальноеПоложениеВПодвале 	= ГоризонтальноеПоложениеЭлемента.Право;
	...
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1СуммаПоступлений.ТекстПодвала 	 	=  Формат(Результат.Итог("СуммаПоступлений"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1ДоПлана.ТекстПодвала 	     		=  Формат(Результат.Итог("ДоПлана"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаЗаявок.ТекстПодвала 				=  Формат(Результат.Итог("СуммаЗаявок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолПроиграноСделок.ТекстПодвала 		=  Формат(Результат.Итог("КолПроиграноСделок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаПроигранноСделок.ТекстПодвала 	=  Формат(Результат.Итог("СуммаПроигранноСделок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолВсегоЗаявок.ТекстПодвала 			=  Формат(Результат.Итог("КолВсегоЗаявок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКолВсегоВыйграноСделок.ТекстПодвала 	=  Формат(Результат.Итог("КолВсегоВыйграноСделок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	Структура = ДляПодвалаПланРейтингМенеджеров1();
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1ДоПлана.ТекстПодвала 				=  Формат(Структура.Результат,"ЧДЦ=0; ЧН=-; ЧЦ=30");


КонецПроцедуры
	
	Функция ДляПодвалаПланРейтингМенеджеров1()
		
		Запрос = Новый Запрос;
		Запрос.Текст =
		
			"ВЫБРАТЬ
			|	СУММА(ЕСТЬNULL(ДополнительныеНомера.ИндивидуальныйПланНаМесяц,0)) КАК ИндивидуальныйПланНаМесяц
			|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Структура.Вставить("Результат",ВыборкаДетальныеЗаписи.ИндивидуальныйПланНаМесяц);
...
Процедура ЗаполнениеПодвала()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСчетаСуммаСчёта.ТекстПодвала    =  Формат(Результат.Итог("СуммаСчёта"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСчетаСуммаЗаявки.ТекстПодвала   =  Формат(Результат.Итог("СуммаЗаявки"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСчетаСуммаПродажи.ТекстПодвала  =  Формат(Результат.Итог("СуммаПродажи"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСчетаСуммаПрихода.ТекстПодвала  =  Формат(Результат.Итог("СуммаПрихода"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 
...
//	
//	
//	ЭтотОбъект.ЭтаФорма.Элементы.ПоказателиКладовщиковКомплектовка.ТекстПодвала =  Формат(Результат.Итог("Комплектовка"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//	ЭтотОбъект.ЭтаФорма.Элементы.ПоказателиКладовщиковПоступления.ТекстПодвала  =  Формат(Результат.Итог("Поступления"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//	ЭтотОбъект.ЭтаФорма.Элементы.ПоказателиКладовщиковКарточки.ТекстПодвала     =  Формат(Результат.Итог("Карточки"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//	ЭтотОбъект.ЭтаФорма.Элементы.ПоказателиКладовщиковСрЦена.ТекстПодвала 	    =  Формат(Результат.Итог("СрЦена"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//	ЭтотОбъект.ЭтаФорма.Элементы.ПоказателиКладовщиковЦена.ТекстПодвала 	    =  Формат(Результат.Итог("Цена"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//	
	...
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиКоличестоОбщее.ТекстПодвала    		= Формат(Результат.Итог("КоличестоОбщее"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСуммаОбщая.ТекстПодвала    			= Формат(Результат.Итог("СуммаОбщая"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиКоличествоДумает.ТекстПодвала    	= Формат(Результат.Итог("КоличествоДумает"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСуммаДумает.ТекстПодвала    			= Формат(Результат.Итог("СуммаДумает"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиКоличествоЖдёмДенег.ТекстПодвала  	= Формат(Результат.Итог("КоличествоЖдёмДенег"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСуммаЖдёмДенег.ТекстПодвала    		= Формат(Результат.Итог("СуммаЖдёмДенег"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиКоличествоНетВНаличии.ТекстПодвала   = Формат(Результат.Итог("КоличествоНетВНаличии"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСуммаНетВНаличии.ТекстПодвала  		= Формат(Результат.Итог("СуммаНетВНаличии"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиКоличествоОжидание.ТекстПодвала    	= Формат(Результат.Итог("КоличествоОжидание"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСуммаОжидание.ТекстПодвала    		= Формат(Результат.Итог("СуммаОжидание"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиКоличествоОтказ.ТекстПодвала    		= Формат(Результат.Итог("КоличествоОтказ"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСуммаОтказ.ТекстПодвала    			= Формат(Результат.Итог("СуммаОтказ"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиКоличествоПродажа.ТекстПодвала    	= Формат(Результат.Итог("КоличествоПродажа"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСуммаПродажа.ТекстПодвала    		= Формат(Результат.Итог("СуммаПродажа"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ЗаявкиСостояние.ТекстПодвала    			= Формат(Результат.Итог("Состояние"),"ЧДЦ=0; ЧН=-; ЧЦ=30");   
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансКолвоПриход.ТекстПодвала    		= Формат(Результат.Итог("КолвоПриход"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансСуммаПрихода.ТекстПодвала    		= Формат(Результат.Итог("СуммаПрихода"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансКоличество.ТекстПодвала    		= Формат(Результат.Итог("Количество"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансСумма.ТекстПодвала    				= Формат(Результат.Итог("Сумма"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансБалансТовара.ТекстПодвала  		= Формат(Результат.Итог("БалансТовара"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансБалансДеньги.ТекстПодвала    		= Формат(Результат.Итог("БалансДеньги"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	//ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансБалансПоПоступлению.ТекстПодвала   = Формат(Результат.Итог("БалансПоПоступлению"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокБалансКолвоПоПоступлению.ТекстПодвала    = Формат(Результат.Итог("КолвоПоПоступлению"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокДоговоровколичество.ТекстПодвала    = Формат(Результат.Итог("Количество"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеров1СуммаПоступлений.ТекстПодвала 	 =  Формат(Результат.Итог("СуммаПоступлений"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	...
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровВРаботе1.ТекстПодвала 		 	 =  Формат(Результат.Итог("ВРаботе"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровСуммаВРаботе1.ТекстПодвала 			 =  Формат(Результат.Итог("СуммаВРаботе"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровперерыв1.ТекстПодвала 			 =  Формат(Результат.Итог("перерыв"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровКоличествоЗвонков1.ТекстПодвала 			 =  Формат(Результат.Итог("КоличествоЗвонков"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровВходящие1.ТекстПодвала 			 =  Формат(Результат.Итог("Входящие"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровДлительностьВходящих1.ТекстПодвала 				 =  Формат(Результат.Итог("ДлительностьВходящих"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровИсходящие1.ТекстПодвала 				 =  Формат(Результат.Итог("Исходящие"),"ЧДЦ=0; ЧН=-; ЧЦ=30");  
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровДлительностьИсходящих1.ТекстПодвала 				 =  Формат(Результат.Итог("ДлительностьИсходящих"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.РейтингМенеджеровДлительность1.ТекстПодвала 				 =  Формат(Результат.Итог("Длительность"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
...
Процедура ЗаполнениеПодвалаСотрудники()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСотрудникиВыручка.ТекстПодвала               = Формат(Результат.Итог("Выручка"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСотрудникиКоличествоСотрудников.ТекстПодвала = Формат(Результат.Итог("КоличествоСотрудников"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСотрудникиПроизводительность.ТекстПодвала    = Формат(Результат.Итог("Производительность"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 
/////////////////////////////////////////////////////////////////////////
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСотрудники1Выручка.ТекстПодвала                = Формат(Результат.Итог("Выручка"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСотрудники1Мощность.ТекстПодвала               = Формат(Результат.Итог("Мощность"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокСотрудники1ПроизводительностьВЧас.ТекстПодвала = Формат(Результат.Итог("ПроизводительностьВЧас"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 	
КонецПроцедуры
...
Процедура ЗаполнениеПодвалаОКК()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК1КоличествоЗвонков.ТекстПодвала   = Формат(Результат.Итог("КоличествоЗвонков"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК1КоличествоСообщений.ТекстПодвала = Формат(Результат.Итог("КоличествоСообщений"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК1Суммаобращений.ТекстПодвала      = Формат(Результат.Итог("Суммаобращений"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 
/////////////////////////////////////////////////////////////////////////
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК2КоличествоЗаявок.ТекстПодвала  = Формат(Результат.Итог("КоличествоЗаявок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК2КоличествоОтказов.ТекстПодвала = Формат(Результат.Итог("КоличествоОтказов"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК2КоличествоПродаж.ТекстПодвала  = Формат(Результат.Итог("КоличествоПродаж"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК2Суммаобращений.ТекстПодвала    = Формат(Результат.Итог("Суммаобращений"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
//////////////////////////////////////////////////////////////////////////	
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК3СуммаЗаявок.ТекстПодвала    = Формат(Результат.Итог("СуммаЗаявок"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК3СуммаОтказов.ТекстПодвала   = Формат(Результат.Итог("СуммаОтказов"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК3СуммаПродаж.ТекстПодвала    = Формат(Результат.Итог("СуммаПродаж"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОтделКК3Суммаобращений.ТекстПодвала = Формат(Результат.Итог("Суммаобращений"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
...
Процедура ЗаполнениеПодвалаОборот()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокДинамикиФинОтделаБалансПриход.ТекстПодвала = Формат(Результат.Итог("БалансПриход"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокДинамикиФинОтделаБалансРасход.ТекстПодвала = Формат(Результат.Итог("БалансРасход"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокДинамикиФинОтделаБалансОборот.ТекстПодвала = Формат(Результат.Итог("БалансОборот"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 
/////////////////////////////////////////////////////////////////////////
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокДинамикиФинОтдела1БалансПриход.ТекстПодвала = Формат(Результат.Итог("БалансПриход"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокДинамикиФинОтдела1БалансРасход.ТекстПодвала = Формат(Результат.Итог("БалансРасход"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокДинамикиФинОтдела1БалансОборот.ТекстПодвала = Формат(Результат.Итог("БалансОборот"),"ЧДЦ=0; ЧН=-; ЧЦ=30");	
КонецПроцедуры 
...
Процедура ЗаполнениеПодвалаТовароОборот()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокТоварооборотСумма.ТекстПодвала             = Формат(Результат.Итог("Сумма"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокТоварооборотСуммаПроизводство.ТекстПодвала = Формат(Результат.Итог("СуммаПроизводство"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокТоварооборотСуммаРеализация.ТекстПодвала   = Формат(Результат.Итог("СуммаРеализация"),"ЧДЦ=0; ЧН=-; ЧЦ=30"); 
/////////////////////////////////////////////////////////////////////////
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокТоварооборот1КолвоКонечныйОстаток.ТекстПодвала = Формат(Результат.Итог("КолвоКонечныйОстаток"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокТоварооборот1КолвоПроизводство.ТекстПодвала    = Формат(Результат.Итог("КолвоПроизводство"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокТоварооборот1КоличествоРеализация.ТекстПодвала = Формат(Результат.Итог("КоличествоРеализация"),"ЧДЦ=0; ЧН=-; ЧЦ=30");	
КонецПроцедуры
...
Процедура ЗаполнениеПодвалаКлиенты()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.ТаблицаКлиентыЦелевые.ТекстПодвала        = Формат(Результат.Итог("Целевые"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ТаблицаКлиентыАктивные.ТекстПодвала       = Формат(Результат.Итог("Активные"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ТаблицаКлиентыЛояльные.ТекстПодвала       = Формат(Результат.Итог("Лояльные"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.ТаблицаКлиентыПотенциальные.ТекстПодвала  = Формат(Результат.Итог("Потенциальные"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
КонецПроцедуры  
...
Процедура ЗаполнениеПодвалаЗарплата()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокЗпПоОтделамСумма.ТекстПодвала = Формат(Результат.Итог("сумма"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
КонецПроцедуры
...
Процедура ЗаполнитьПодвалВкладкиФинансовыйОтдел()
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОстатковНаСчётеБалансОстаток.ТекстПодвала  		=  Формат(Результат.Итог("БалансОстаток"), "ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОстатковНаСчётеБалансОстатокНаДату.ТекстПодвала  =  Формат(Результат.Итог("БалансОстатокНаДату"), "ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОстатковНаСчётеСуммаДокумента.ТекстПодвала 		=  Формат(Результат.Итог("СуммаДокумента"), "ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОстатковНаСчётеРасход.ТекстПодвала  		  		=  Формат(Результат.Итог("Расход"), "ЧДЦ=0; ЧН=-; ЧЦ=30");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокОстатковНаСчётеПрибыль.ТекстПодвала  		  	=  Формат(Результат.Итог("Прибыль"), "ЧДЦ=0; ЧН=-; ЧЦ=30");
	...
	ЭтотОбъект.ЭтаФорма.Элементы.РасходыСумма.ТекстПодвала  =  Формат(Результат.Итог("Сумма"), "ЧДЦ=0; ЧН=-; ЧЦ=30");
	...
	ЭтотОбъект.ЭтаФорма.Элементы.ВидыРасходаСумма.ТекстПодвала 	=  Формат(Результат.Итог("Сумма"), "ЧДЦ=0; ЧН=-; ЧЦ=30");
	///-ГомзМА 05.06.2023
...
Функция ПолучитьАктуальныйДолг()
	Запрос = Новый запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	СУММА(ЕСТЬNULL(ФиксацияЗаказаМашин.СтоимостьАвтоЕвро, 0)) КАК СтоимостьАвтоЕвро
	|ИЗ
	...
	Если Выборка.Количество() > 0 Тогда
		выборка.Следующий();
		Если ТипЗнч(Выборка.СтоимостьАвтоЕвро) = Тип("Null") Тогда
			СтоимостьАвтоЕвро = 0;
		Иначе
			СтоимостьАвтоЕвро = Выборка.СтоимостьАвтоЕвро;
		КонецЕсли;
		
		Возврат (СтоимостьАвтоЕвро * ПолучитьКурсыВалют(ТекущаяДата()));
	Иначе
		Возврат 0;
	КонецЕсли;	
КонецФункции
...
Процедура ВкладкиПриСменеСтраницыНаСервере()
	...
	//
	//
	//ЭтотОбъект.ЭтаФорма.Элементы.СписокОстатковНаСчётеБалансОстаток.ТекстПодвала  =  Формат(Результат.Итог("Сумма"),"ЧДЦ=0; ЧН=-; ЧЦ=30");
	//КонецЕсли;
	///-ГомзМА 04.07.2023
...
//	КонецЕсли;	
	...
	Если ТекущаяСтраница = Элементы.КоличествоКлиентовПоГруппам Тогда
		ДатаДиаграммыКлиенты = ТекущаяДата();
		ОбновитьДиаграммуКлиенты();
		ОбновитьГрафикПриростАктивныхКлиентов();
	КонецЕсли;
	
	
//	Если ТекущаяСтраница = Элементы.ОтчетПоКлиентам Тогда
//		ОбновитьСтатистикуПоКлинтам(ТекущаяДата());
//	КонецЕсли;
