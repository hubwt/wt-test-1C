...
Процедура ПоискНаСервере()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(РасспродажаТовараСрезПоследних.Номенклатура) КАК Номенклатура,
	|	МАКСИМУМ(РасспродажаТовараСрезПоследних.Период) КАК Период
	|ПОМЕСТИТЬ ВТ_ЦенаПоАкции_Предв
	|ИЗ
	|	РегистрСведений.РасспродажаТовара.СрезПоследних(, ДатаНачала <= &ТекДата
	|	И ДатаОкончания >= &ТекДата) КАК РасспродажаТовараСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ЦенаПоАкции_Предв.Номенклатура,
	|	РасспродажаТовараСрезПоследних.Цена КАК ЦенаПоАкции
	|ПОМЕСТИТЬ ВТ_ЦенаПоАкции
	|ИЗ
	|	ВТ_ЦенаПоАкции_Предв КАК ВТ_ЦенаПоАкции_Предв
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РасспродажаТовара.СрезПоследних(, ДатаНачала <= &ТекДата
	|		И ДатаОкончания >= &ТекДата) КАК РасспродажаТовараСрезПоследних
	|		ПО ВТ_ЦенаПоАкции_Предв.Номенклатура = РасспродажаТовараСрезПоследних.Номенклатура
	|		И РасспродажаТовараСрезПоследних.Период = ВТ_ЦенаПоАкции_Предв.Период
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(ВТ_ЦенаПоАкции.ЦенаПоАкции, 0) КАК ЦенаПоАкции
	|ИЗ
	|	Справочник.Номенклатура КАК СправочникНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РегистрНакопления1.Остатки(,
	|		НЕ Склад = &Склад) КАК РегистрНакопления1Остатки
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЦенаПоАкции КАК ВТ_ЦенаПоАкции
	|		ПО ВТ_ЦенаПоАкции.Номенклатура = СправочникНоменклатура.Ссылка";
	...
	//-- МазинЕС Не считать количество с учетом склада востановленя
	СписокНоменклатуры.Параметры.УстановитьЗначениеПараметра("ТекДата",ТекущаяДатаСеанса());
КонецПроцедуры
