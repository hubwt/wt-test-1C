...
Процедура ОбновитьПоказателиАвтовыгрузки()
	...
	Запрос.Текст =
		...
		|	ИндНомер.индкод.Владелец.ВАвтозагрузкеАвито КАК ВАвтозагрузкеАвито,
		|	СУММА(ВЫБОР
		|	КОГДА ИндНомер.ВАвтозагрузкеАвито
		|		ТОГДА 1
		|	ИНАЧЕ 0
		|	КОНЕЦ) КАК КоличествоВАвито
		|ИЗ
		...
		|	ИндНомер.индкод.Владелец.ВАвтозагрузкеАвито";
	...
	//ПараметрыОтбораБезУбранных = Новый Структура("ВАвтозагрузкеАвито", Истина);
	...
	ПостроительЗапр.ИсточникДанных = Новый ОписаниеИсточникаДанных(Выборка);
	
	ВыборкаОтбор = ПостроительЗапр.Отбор.Добавить("КоличествоВАвито");
	ВыборкаОтбор.ВидСравнения = ВидСравнения.Больше;
	ВыборкаОтбор.Значение = 0; 
	ВыборкаОтбор.Использование = Истина; 
	
	ПостроительЗапр.Выполнить();
	ВсегоКарточекТовараВФайле = ПостроительЗапр.Результат.Выгрузить().Количество();
	...
	Запрос.Текст =
		...
		|	ИндНомер.ВАвтозагрузкеАвито КАК ВАвтозагрузкеАвито
		|ИЗ
		|	РегистрСведений.ИндНомер КАК ИндНомер
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТоварыИзЗаявок КАК ТоварыИзЗаявок
		|		ПО (ИндНомер.индкод.Владелец = ТоварыИзЗаявок.Номенклатура)
...
&НаКлиенте
Процедура ОткрытьФотоНаСайте(Команда)
	
	ИндКод = ЭтаФорма.Элементы.ТоварыАвитоЧасть1.ТекущиеДанные.индкод;
...
&НаКлиенте
Процедура ВыгрузитьДеталиВФайлАвито(Команда)
	АвтозагрузкиВызовСервера.АвтозагрузкаАвитоХодовые();
КонецПроцедуры
