...
Функция ОпределитьДанныеОтдела(ОтделКод,ЗарплатаКод,ОтделКодДоход,РегионКод = Неопределено) 
	...
	ПланДельта	= ПланОтдел - ИтогоРасходы;
	///---План Отдел
...
// ++ Сумма всех товаров в ЗаказНаряде после вычета Налога
	...
	Запрос.Текст =
		...
		|	И ЗаказНарядТовары.Ссылка.Дата МЕЖДУ &Дата1 И КОНЕЦПЕРИОДА(&Дата2, ДЕНЬ)
...
// ++ Сумма всех работ в ЗаказНаряде после вычета Налога
	...
	Запрос.Текст =
		...
		|	И ЗаказНарядРаботы.Ссылка.Дата МЕЖДУ &Дата1 И КОНЕЦПЕРИОДА(&Дата2, ДЕНЬ)
