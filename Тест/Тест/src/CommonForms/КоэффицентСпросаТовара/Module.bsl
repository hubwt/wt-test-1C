...
Процедура СписокНоменклатурыКоэффицентВыбор1(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	/// Комлев АА 07/12/24 +++
	Если Поле = Элементы.СписокНоменклатурыКоэффицентОстатокСовпадаетМСК Тогда
		СтандартнаяОбработка = Ложь;
		ТекущиеДанные = Элемент.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
			Если ТекущиеДанные.ОписаниеЗаполнено = Ложь Тогда
				ПроставитьГалочкуОстатокСовпадаетМСК(ТекущиеДанные.Товар);
			Иначе
				СнятьГалочкуОстатокСовпадаетМСК(ТекущиеДанные.Товар);
			КонецЕсли;
		КонецЕсли;
		Элементы.СписокНоменклатурыКоэффицент.Обновить();
	КонецЕсли;
	
	Если Поле = Элементы.СписокНоменклатурыКоэффицентОстатокСовпадаетЕКБ Тогда
		СтандартнаяОбработка = Ложь;
		ТекущиеДанные = Элемент.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
			Если ТекущиеДанные.ОписаниеЗаполнено = Ложь Тогда
				ПроставитьГалочкуОстатокСовпадаетЕКБ(ТекущиеДанные.Товар);
			Иначе
				СнятьГалочкуОстатокСовпадаетЕКБ(ТекущиеДанные.Товар);
			КонецЕсли;
		КонецЕсли;
		Элементы.СписокНоменклатурыКоэффицент.Обновить();
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КоличествоТоваровВКатегрии.Параметры.УстановитьЗначениеПараметра("Склад", Справочники.Склады.НайтиПоКоду("000000002"));
...
&НаКлиенте
Процедура ГородаПриИзменении(Элемент)
	// Комлев АА 05/12/24 +++
	Если Города = 1 Тогда
		СписокНоменклатурыКоэффицент.Параметры.УстановитьЗначениеПараметра("Склад", ПолучитьСклад("000000002") );
		КоличествоТоваровВКатегрии.Параметры.УстановитьЗначениеПараметра("Склад", ПолучитьСклад("000000002") );
	ИначеЕсли Города = 2 Тогда 
		СписокНоменклатурыКоэффицент.Параметры.УстановитьЗначениеПараметра("Склад", ПолучитьСклад("000000005") );
		КоличествоТоваровВКатегрии.Параметры.УстановитьЗначениеПараметра("Склад", ПолучитьСклад("000000005") );
	ИначеЕсли Города = 3 Тогда 
		СписокНоменклатурыКоэффицент.Параметры.УстановитьЗначениеПараметра("Склад", ПолучитьСклад("000000008") );
		КоличествоТоваровВКатегрии.Параметры.УстановитьЗначениеПараметра("Склад", ПолучитьСклад("000000008") );
	КонецЕсли;
	// Комлев АА 05/12/24 ---
КонецПроцедуры
...
Функция ПолучитьСклад(НомерСклада)
	Возврат Справочники.Склады.НайтиПоКоду(НомерСклада);
КонецФункции
...
Процедура ПроставитьГалочкуОстатокСовпадает(СсылкаНаНоменклатуру)
	...
	НоменклатураОбъект.ДатаИнвентаризацииНН = ТекущаяДатаСеанса();
	НоменклатураОбъект.Записать();
	/// Комлев АА 25/11/24 ---
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПроставитьГалочкуОстатокСовпадаетМСК(СсылкаНаНоменклатуру)
	/// Комлев АА 25/11/24 +++
	НоменклатураОбъект = СсылкаНаНоменклатуру.ПолучитьОбъект();
	НоменклатураОбъект.ОстатокСовпадаетМСК  = Истина;
	НоменклатураОбъект.ДатаИнвентаризацииМСК = ТекущаяДатаСеанса();
	НоменклатураОбъект.Записать();
	/// Комлев АА 25/11/24 ---
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СнятьГалочкуОстатокСовпадаетМСК(СсылкаНаНоменклатуру)
	/// Комлев АА 25/11/24 +++
	НоменклатураОбъект = СсылкаНаНоменклатуру.ПолучитьОбъект();
	НоменклатураОбъект.ОстатокСовпадаетМСК  = Ложь;
	НоменклатураОбъект.Записать();
	/// Комлев АА 25/11/24 ---
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПроставитьГалочкуОстатокСовпадаетЕКБ(СсылкаНаНоменклатуру)
	/// Комлев АА 25/11/24 +++
	НоменклатураОбъект = СсылкаНаНоменклатуру.ПолучитьОбъект();
	НоменклатураОбъект.ОстатокСовпадаетЕКБ  = Истина;
	НоменклатураОбъект.ДатаИнвентаризацииЕКБ = ТекущаяДатаСеанса();
	НоменклатураОбъект.Записать();
	/// Комлев АА 25/11/24 ---
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СнятьГалочкуОстатокСовпадаетЕКБ(СсылкаНаНоменклатуру)
	/// Комлев АА 25/11/24 +++
	НоменклатураОбъект = СсылкаНаНоменклатуру.ПолучитьОбъект();
	НоменклатураОбъект.ОстатокСовпадаетЕКБ  = Ложь;
	НоменклатураОбъект.Записать();
	/// Комлев АА 25/11/24 ---
КонецПроцедуры
