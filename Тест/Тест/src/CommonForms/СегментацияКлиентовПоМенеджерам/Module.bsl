...
Процедура РассчитатьИтогиСписокСегментацияКлиентовПоМенеджерам() 
	...
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоГорячих.ТекстПодвала =  РезультатКД_ТЗ.Итог(
		"КоличествоГорячих");
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоГорячих.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоАктивных.ТекстПодвала =  РезультатКД_ТЗ.Итог(
		"КоличествоАктивных");
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоАктивных.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
		
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоТеплых.ТекстПодвала =  РезультатКД_ТЗ.Итог(
		"КоличествоТеплых");
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоТеплых.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;	
	...
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоПотенциальных.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;	
		
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоХолодных.ТекстПодвала =  РезультатКД_ТЗ.Итог(
		"КоличествоХолодных");
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоХолодных.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;	
		
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоДолжников.ТекстПодвала =  РезультатКД_ТЗ.Итог(
		"КоличествоДолжников");
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоДолжников.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;		
		
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоНеЦелевых.ТекстПодвала =  РезультатКД_ТЗ.Итог(
		"КоличествоНеЦелевых");
	Элементы.СписокСегментацияКлиентовПоМенеджерамКоличествоНеЦелевых.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;	
		
	Элементы.СписокСегментацияКлиентовПоМенеджерамИтогоПоМенеджеру.ТекстПодвала =  РезультатКД_ТЗ.Итог(
		"ИтогоПоМенеджеру");
	Элементы.СписокСегментацияКлиентовПоМенеджерамИтогоПоМенеджеру.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
...
Процедура ТекстЗапросаСегментацияКлиентскойБазыФильтрКураторы()
	...
	|	ЕСТЬNULL(АктивныеКлиентыПоМенеджерам.КоличествоАктивных, 0) + ЕСТЬNULL(ТеплыеКлиентыПоменеджерам.КоличествоТеплых, 0)
	|		+ ЕСТЬNULL(ПотенциальныеКлиентыПоМенеджерам.КоличествоПотенциальных, 0) +
	|		ЕСТЬNULL(ХолодныеКлиентыПоМенеджерам.КоличествоХолодных, 0) КАК ИтогоПоМенеджеру
...
Процедура ТекстЗапросаСегментацияКлиентскойБазыФильтрНеПроставленГород()
	...
	|	Долги КАК Долги
	|ГДЕ
	|	Долги.Клиент.Город2 = ЗНАЧЕНИЕ(Справочник.Город.ПустаяСсылка)
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА Долги.Клиент.Ответственный <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|			ТОГДА Долги.Клиент.Ответственный
...
Процедура ТекстЗапросаСегментацияКлиентскойБазыФильтрНеПроставленыМашины()
	...
	|	Долги КАК Долги
	|ГДЕ
	|	Долги.Клиент.КоличествоАвтомобилей = 0
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА Долги.Клиент.Ответственный <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|			ТОГДА Долги.Клиент.Ответственный
...
Процедура ФильтрПоЗаполненнымРеквизитамКлиентаПриИзменении(Элемент)
	...
	ИначеЕсли ФильтрПоЗаполненнымРеквизитамКлиента = 2 Тогда
