...
Процедура ГруппыКлиентовПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Потенциальные Тогда
		ДатаНачалаПотенциальные = НачалоДня(Дата(ТекущаяДата()) - (24 * 60 * 60) * 91);
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Холодные Тогда
		ДатаХолодные = НачалоДня(Дата(ТекущаяДата()) - (24 * 60 * 60) * 90);
		СписокХолодные.Параметры.УстановитьЗначениеПараметра("ДатаХолодные", ДатаХолодные);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СсылкаНаФото = ПолучитьНавигационнуюСсылку(Справочники.Сотрудники.НайтиПоНаименованию(ИмяТекПользователя).Ссылка, "Фото");
...
ЕСли Пользователь Тогда Возврат СсылкаТекПользователяПользователь; 
Иначе Возврат СсылкаТекПользователя;
КонецЕсли;
