...
//		РассчитатьИтоги("СписокАктивные", "СписокАктивныеКоличествоАвтомобилей", "КоличествоАвтомобилей");
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Теплые Тогда
		...
		РассчитатьИтоги("СписокТеплые", "СписокТеплыеКоличествоАвтомобилей", "КоличествоАвтомобилей");
	ИначеЕсли ТекущаяСтраница = Элементы.Потенциальные Тогда
		...
		РассчитатьИтоги("СписокПотенциальные", "СписокПотенциальныеКоличествоАвтомобилей", "КоличествоАвтомобилей");
	ИначеЕсли ТекущаяСтраница = Элементы.Холодные Тогда
		...
		РассчитатьИтоги("СписокХолодные", "СписокХолодныеКоличествоАвтомобилей", "КоличествоАвтомобилей");
	КонецЕсли;
	// --- Комлев АА --- 17/10/24 
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	///+ТатарМА 24.10.2024
	Если НЕ ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("мт_ДобавлениеИзменениеЗаказКлиента")) Тогда
...
Процедура СкрытьСтраницыДляМенеджеров()
	...
	Если  ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Вадяева Дарья Викторовна"
		...
		ИЛИ ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Панькина Ольга Сергеевна" Тогда
		Элементы.ГруппыКлиентов.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры

Процедура РассчитатьИтоги(НазваниеДинамическогоСписка, КолонкаЭлемент, НазваниеКолонки) // Строка
	// Комлев АА 21/10/24 +++
	СКД = Элементы[НазваниеДинамическогоСписка].ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	НастройкиКД = Элементы[НазваниеДинамическогоСписка].ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	...
	ЭтотОбъект.ЭтаФорма.Элементы[КолонкаЭлемент].ТекстПодвала =  "Итого: " + Формат(РезультатКД_ТЗ.Итог(
		НазваниеКолонки), "ЧДЦ=2; ЧН=-");
...
Функция КоличествоСтрокВСписке(НазваниеСписка) // Строка 
	...
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	Количество = 0;
	Для Каждого СтрокаСписка Из РезультатКД_ТЗ Цикл
		Если СтрокаСписка.Код <> Неопределено Тогда
...
КонецФункции
&НаКлиенте
Процедура СписокАктивныеПриИзменении(Элемент)

КонецПроцедуры
&НаСервере
Процедура СписокАктивныеПриЗагрузкеПользовательскихНастроекНаСервере(Элемент, Настройки,
