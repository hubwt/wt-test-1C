...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныГорячие(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьГорячиеНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
	
	Для Каждого НомерТелефона Из НормализированныеНомера Цикл
		Лист.Cells(Строка, 1).Value  = НомерТелефона;
		Строка = Строка + 1;
	КонецЦикла;
	...
	Попытка
		...
		Книга.SaveAs(ВыбратьПутьКФайлу());
		//Закрываем Excel
...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныАктивные(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьАктивныеНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныТеплые(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьТеплыеНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныПотенциальные(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьПотенциальныеНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныХолодные(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьХолодныеНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныДолжники(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьДолжникиНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныНеЦелевые(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьНеЦелевыеНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
...
&НаКлиенте
Процедура ВыгрузитьВЭксельТелефоныВсе(Команда)
	...
	//Заполняем строки
	НормализированныеНомера = ПолучитьВсеНормализированныеНомераТелефона();
	Строка = 2;
	Лист.Columns(1).ColumnWidth = 16;
	Лист.Cells(1, 1).Value  = "msisdn";
...
&НаКлиенте
Функция ВыбратьПутьКФайлу()
	
	///+ТатарМА 16.12.2024
	ПутьКФайлу = "";
	
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	Диалог.Заголовок = "Выбор пути сохранения";
...
Функция ПолучитьГорячиеНормализированныеНомераТелефона()
	...
	|	Максимум(ВЫБОР
	|		КОГДА ПродажаЗапчастей.Самовывоз = ИСТИНА
	|			ТОГДА ""Самовывоз""
	|		ИНАЧЕ ""Доставка""
	|	КОНЕЦ) КАК Доставка,
	|	Максимум(ПродажаЗапчастей.ДатаОтгрузкиСоСклада) КАК ДатаОтгрузкиСоСклада,
	|	Максимум(ВЫБОР
	|		КОГДА ПродажаЗапчастей.Самовывоз = ИСТИНА
	|			ТОГДА ПродажаЗапчастей.ДатаОтгрузкиСоСклада
	|		ИНАЧЕ ПродажаЗапчастей.ДатаПолучения
	|	КОНЕЦ) КАК ДатаПолученияЗаказа,
	|	Максимум(РАЗНОСТЬДАТ(ПродажаЗапчастей.ДатаОтгрузкиСоСклада, ВЫБОР
	|		КОГДА ПродажаЗапчастей.Самовывоз = ИСТИНА
	|			ТОГДА ПродажаЗапчастей.ДатаОтгрузкиСоСклада
	|		ИНАЧЕ ПродажаЗапчастей.ДатаПолучения
	|	КОНЕЦ, ДЕНЬ)) КАК Ожидание
	...
	|ВЫБРАТЬ
	|	ПродажаЗапчастей.Клиент КАК Клиент,
	|	КОЛИЧЕСТВО(ПродажаЗапчастей.Ссылка) КАК КоличествоПродаж,
	|	СУММА(ПродажаЗапчастей.ИтогоРекв) КАК СуммаПродаж
	|ПОМЕСТИТЬ КоличествоИСуммаПродаж
	|ИЗ
	|	Документ.ПродажаЗапчастей КАК ПродажаЗапчастей
	|СГРУППИРОВАТЬ ПО
	|	ПродажаЗапчастей.Клиент
	...
	|ВЫБРАТЬ
	|	ЗаказКлиента.Клиент,
	|	МАКСИМУМ(ЗаказКлиента.Дата) КАК ДатаЗаявки
	|ПОМЕСТИТЬ ПоследнийКонтактВсе
	|ИЗ
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|СГРУППИРОВАТЬ ПО
	|	ЗаказКлиента.Клиент
	...
	|	МАКСИМУМ(ЗаписиЗвонков.Дата) КАК ДатаЗвонка
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ПоследнийКонтактВсе.ДатаЗаявки) КАК ДатаПоследнегоКонтакта,
	|	ПоследнийКонтактВсе.Клиент
	|Поместить ПоследнийКонтакт
	|ИЗ
	|	ПоследнийКонтактВсе КАК ПоследнийКонтактВсе
	|СГРУППИРОВАТЬ ПО
	|	ПоследнийКонтактВсе.Клиент
	|;
	|
	...
	|	Клиенты.ВидКлиента,
	|	Клиенты.КоличествоАвтомобилей КАК КоличествоМашин,
	|	Клиенты.Ответственный КАК Менеджер,
	|	ВЫБОР
	|		КОГДА Клиенты.Ответственный = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|		ИЛИ Клиенты.Ответственный = &ОбщийКлиент
	|		ИЛИ Клиенты.Ответственный.Подразделение.Город = ЗНАЧЕНИЕ(Справочник.Город.ПустаяСсылка)
	|		ИЛИ Клиенты.Ответственный.Подразделение = ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка)
	|			ТОГДА ""Общие клиенты""
	|		ИНАЧЕ Клиенты.Ответственный.Подразделение.Город
	|	КОНЕЦ КАК Филиал,
	|	КоличествоЗаявок.КоличествоЗаказов КАК КоличествоЗаказов,
	|	КоличествоИСуммаПродаж.КоличествоПродаж КАК КоличествоПродаж,
	|	КоличествоИСуммаПродаж.СуммаПродаж КАК СуммаПродаж,
	|	КлиентыПолучившиеЗаказ.Доставка КАК Доставка,
	|	КлиентыПолучившиеЗаказ.ДатаОтгрузкиСоСклада КАК ДатаОтгрузкиСоСклада,
	|	КлиентыПолучившиеЗаказ.Ожидание КАК Ожидание,
	|	КлиентыПолучившиеЗаказ.ДатаПолученияЗаказа КАК ДатаПолученияЗаказа,
	|	Клиенты.ПерезвонилиПослеДоставки КАК ПерезвонилиПослеДоставки,
	|	ПоследнийКонтакт.ДатаПоследнегоКонтакта КАК ДатаПоследнейЗаявки,
	|	РАЗНОСТЬДАТ(ПоследнийКонтакт.ДатаПоследнегоКонтакта, &ТекущаяДата, День) КАК ДнейСПоследнегоКонтакта,
	|	Клиенты.Куратор,
	|	Клиенты.ДатаПоследнегоКонтакта
	|ИЗ
	|	Справочник.Клиенты КАК Клиенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КлиентыПолучившиеЗаказ КАК КлиентыПолучившиеЗаказ
	|		ПО КлиентыПолучившиеЗаказ.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоЗаявок КАК КоличествоЗаявок
	|		ПО КоличествоЗаявок.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоИСуммаПродаж КАК КоличествоИСуммаПродаж
	|		ПО Клиенты.Ссылка = КоличествоИСуммаПродаж.Клиент
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследнийКонтакт КАК ПоследнийКонтакт
	|		ПО ПоследнийКонтакт.Клиент = Клиенты.Ссылка
	|ГДЕ
	|	Клиенты.Целевой";
	
	ДатаВПрошлом  = НачалоДня(ТекущаяДатаСеанса() - (24 * 60 * 60) * 10);
	
	Запрос.УстановитьПараметр("ОбщийКлиент", 	ПолучитьОбщегоКлиентаПользователя());
	...
	Если Выборка.Количество() > 0 Тогда
		Пока Выборка.Следующий() Цикл
			Номер = СтрЗаменить(Выборка.Телефон, "+", "");
...
Функция ПолучитьАктивныеНормализированныеНомераТелефона()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ЗаказКлиента.Дата) КАК ДатаПоследнегоОбращения,
	|	ЗаказКлиента.Клиент КАК Клиент
	|ПОМЕСТИТЬ ВТ_ПослОбр
	|ИЗ
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|СГРУППИРОВАТЬ ПО
	|	ЗаказКлиента.Клиент
	...
	|ВЫБРАТЬ
	|	ЗаказКлиента.Клиент КАК Клиент,
	|	МАКСИМУМ(ЗаказКлиента.Дата) КАК ПредпоследнееОбращение
	|ПОМЕСТИТЬ ВТ_Предпоследнее
	|ИЗ
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
	|	НЕ ЗаказКлиента.Дата В
	|		(ВЫБРАТЬ
	|			ВТ_ПослОбр.ДатаПоследнегоОбращения КАК ДатаПоследнегоОбращения
	|		ИЗ
	|			ВТ_ПослОбр КАК ВТ_ПослОбр)
	|СГРУППИРОВАТЬ ПО
	|	ЗаказКлиента.Клиент
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	СпрКлиенты.Ссылка,
	|	СпрКлиенты.Телефон,
	|	СпрКлиенты.Город2 КАК Город,
	|	СпрКлиенты.ТипКлиента,
	|	СпрКлиенты.ВидКлиента,
	|	СпрКлиенты.КоличествоАвтомобилей,
	|	СпрКлиенты.Ответственный,
	|	ВЫБОР
	|		КОГДА СпрКлиенты.Ответственный = Значение(Справочник.Пользователи.ПустаяСсылка)
	|		ИЛИ СпрКлиенты.Ответственный.Подразделение.Город = Значение(Справочник.Город.ПустаяСсылка)
	|		ИЛИ СпрКлиенты.Ответственный.Подразделение = Значение(Справочник.Подразделения.ПустаяСсылка)
	|		ИЛИ СпрКлиенты.Ответственный = &ОбщийКлиент
	|			ТОГДА ""Общие клиенты""
	|		ИНАЧЕ СпрКлиенты.Ответственный.Подразделение.Город
	|	КОНЕЦ КАК Филиал,
	|	ЕСТЬNULL(КоличествоЗаявок.КоличествоЗаказов, 0) КАК КоличествоЗаказов,
	|	ЕСТЬNULL(КоличествоИСуммаПродаж.КоличествоПродаж, 0) КАК КоличествоПродаж,
	|	ЕСТЬNULL(КоличествоИСуммаПродаж.СуммаПродаж, 0) КАК СуммаПродаж,
	|	ПоследнийКонтакт.ДатаПоследнегоКонтакта КАК ДатаПоследнейЗаявки,
	|	РАЗНОСТЬДАТ(ПоследнийКонтакт.ДатаПоследнегоКонтакта, &ТекущаяДата, День) КАК ДнейСПоследнегоКонтакта,
	|	РАЗНОСТЬДАТ(ВТ_Предпоследнее.ПредпоследнееОбращение, ПоследнийКонтакт.ДатаПоследнегоКонтакта, День) КАК
	|		ДнейМеждуОбращениями,
	|	СпрКлиенты.Куратор,
	|	СпрКлиенты.ДатаПоследнегоКонтакта
	|ИЗ
	|	Справочник.Клиенты КАК СпрКлиенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ АКБ КАК АКБ
	|		ПО АКБ.Клиент = СпрКлиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоЗаявок КАК КоличествоЗаявок
	|		ПО КоличествоЗаявок.Клиент = СпрКлиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоИСуммаПродаж КАК КоличествоИСуммаПродаж
	|		ПО СпрКлиенты.Ссылка = КоличествоИСуммаПродаж.Клиент
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследнийКонтакт КАК ПоследнийКонтакт
	|		ПО ПоследнийКонтакт.Клиент = СпрКлиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Предпоследнее КАК ВТ_Предпоследнее
	|		ПО ВТ_Предпоследнее.Клиент = СпрКлиенты.Ссылка
	|ГДЕ
	|	СпрКлиенты.Целевой";
	
	ДатаНачалаАКБ = НачалоДня(Дата(ТекущаяДатаСеанса()) - (24 * 60 * 60) * 30);
	ДатаОкончанияАКБ = КонецДня(Дата(ТекущаяДатаСеанса()));
...
Функция ПолучитьТеплыеНормализированныеНомераТелефона()
	...
	|	ЕСТЬNULL(КоличествоИСуммаПродаж.КоличествоПродаж, 0) КАК КоличествоПродаж,
	|	ЕСТЬNULL(КоличествоИСуммаПродаж.СуммаПродаж, 0) КАК СуммаПродаж,
	|	РАЗНОСТЬДАТ(ВТ_Предпоследнее.ПредпоследнееОбращение, ВТ_ПослОбр.ДатаПоследнегоОбращения, День) КАК
	|		ДнейМеждуОбращениями,
	|	ПоследнийКонтакт.ДатаПоследнегоКонтакта КАК ДатаПоследнейЗаявки,
	|	РАЗНОСТЬДАТ(ПоследнийКонтакт.ДатаПоследнегоКонтакта, &ТекущаяДата, День) КАК ДнейСПоследнегоОбращения,
	|	Клиенты.Куратор,
	|	Клиенты.ДатаПоследнегоКонтакта
	|ИЗ
	|	Справочник.Клиенты КАК Клиенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТЕПЛЫЕ КАК ТЕПЛЫЕ
	|		ПО ТЕПЛЫЕ.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоЗаявок КАК КоличествоЗаявок
	|		ПО КоличествоЗаявок.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоИСуммаПродаж КАК КоличествоИСуммаПродаж
	|		ПО Клиенты.Ссылка = КоличествоИСуммаПродаж.Клиент
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Предпоследнее КАК ВТ_Предпоследнее
	|		ПО ВТ_Предпоследнее.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПослОбр КАК ВТ_ПослОбр
	|		ПО Клиенты.Ссылка = ВТ_ПослОбр.Клиент
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследнийКонтакт КАК ПоследнийКонтакт
	|		ПО ПоследнийКонтакт.Клиент = Клиенты.Ссылка
	|ГДЕ
	|	Клиенты.Целевой";
	...
	ДатаНачалаАКБ = НачалоДня(Дата(ТекущаяДатаСеанса()) - (24 * 60 * 60) * 30);
	ДатаОкончанияАКБ = КонецДня(Дата(ТекущаяДатаСеанса()));
	ДатаНачалаТеплые = НачалоДня(Дата(ТекущаяДатаСеанса()) - (24 * 60 * 60) * 90);
	ДатаОкончанияТеплые = КонецДня(Дата(ТекущаяДатаСеанса()) - (24 * 60 * 60) * 31);
...
Функция ПолучитьПотенциальныеНормализированныеНомераТелефона()
	...
	|ИЗ
	|	Справочник.Клиенты КАК Клиенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПОТЕНЦ КАК ПОТЕНЦ
	|		ПО ПОТЕНЦ.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоЗаявок КАК КоличествоЗаявок
	|		ПО КоличествоЗаявок.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоИСуммаПродаж КАК КоличествоИСуммаПродаж
	|		ПО Клиенты.Ссылка = КоличествоИСуммаПродаж.Клиент
	...
	ДатаНачалаПотенциальные = НачалоДня(Дата(ТекущаяДатаСеанса()) - (24 * 60 * 60) * 90);
	ДатаОкончанияПотенциальные = КонецДня(Дата(ТекущаяДатаСеанса()));
...
Функция ПолучитьХолодныеНормализированныеНомераТелефона()
	...
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
	|	ЗаказКлиента.Дата < &ДатаХолодные
	|СГРУППИРОВАТЬ ПО
	|	ЗаказКлиента.Клиент
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(ЗаказКлиента.Клиент) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПолучитьДолжникиНормализированныеНомераТелефона()
	...
	|ВЫБРАТЬ
	|	ОборотыДенег.Клиент КАК Клиент,
	|	СУММА(ОборотыДенег.СуммаПродажи) КАК СуммаПродаж,
	|	СУММА(ОборотыДенег.СуммаОплаты) КАК СуммаПриходов
	|ПОМЕСТИТЬ ОборотыВсе
	|ИЗ
	|	ОборотыДенег КАК ОборотыДенег
	|ГДЕ
	|	ОборотыДенег.ДолгСписан = ЛОЖЬ
	|СГРУППИРОВАТЬ ПО
	|	ОборотыДенег.Клиент
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|ВЫБРАТЬ
	|	ОборотыДенег.Клиент КАК Клиент,
	|	СУММА(-ОборотыДенег.Долг) КАК СуммиМинусов
	|ПОМЕСТИТЬ МинусовыеДолги
	|ИЗ
	|	ОборотыДенег КАК ОборотыДенег
	|ГДЕ
	|	ОборотыДенег.Долг < 0
	...
	|ВЫБРАТЬ
	|	МАКСИМУМ(ПоследнийКонтактВсе.ДатаЗаявки) КАК ДатаПоследнегоКонтакта,
	|	ПоследнийКонтактВсе.Клиент КАК Клиент
	|ПОМЕСТИТЬ ПоследнийКонтакт
	|ИЗ
	|	ПоследнийКонтактВсе КАК ПоследнийКонтактВсе
	|СГРУППИРОВАТЬ ПО
	...
	|	МАКСИМУМ(ДоговорыКонтрагентов.ОтсрочкаПлатежаВДнях) КАК ОтсрочкаВДнях,
	|	МАКСИМУМ(ДоговорыКонтрагентов.ДатаДоговора) КАК ДатаДоговора,
	|	ВЫБОР
	|		КОГДА РАЗНОСТЬДАТ(МАКСИМУМ(ДоговорыКонтрагентов.ДатаДоговора), &ТекущаяДата,
	|			ДЕНЬ) > МАКСИМУМ(ДоговорыКонтрагентов.ОтсрочкаПлатежаВДнях)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ИстекСрокДоговора
	|ПОМЕСТИТЬ Договоры
	|ИЗ
	...
	|	И (ДоговорыКонтрагентов.ВидДоговора.Код = ""000000010""
	|	ИЛИ ДоговорыКонтрагентов.ВидДоговора.Код = ""000000011""
	|	ИЛИ ДоговорыКонтрагентов.ВидДоговора.Код = ""000000002""
	|	ИЛИ ДоговорыКонтрагентов.ВидДоговора.Код = ""000000001""
	|	ИЛИ ДоговорыКонтрагентов.ВидДоговора.Код = ""000000009"")
	|СГРУППИРОВАТЬ ПО
	...
	|	Клиенты.Город2 КАК Город,
	|	Клиенты.ТипКлиента КАК ТипКлиента,
	|	Клиенты.ВидКлиента КАК ВидКлиента,
	|	Клиенты.КоличествоАвтомобилей КАК Машины,
	|	Клиенты.Ответственный КАК Менеджер,
	|	ВЫБОР
	|		КОГДА Клиенты.Ответственный = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|		ИЛИ Клиенты.Ответственный.Подразделение.Город = ЗНАЧЕНИЕ(Справочник.Город.ПустаяСсылка)
	...
	|		ИНАЧЕ Клиенты.Ответственный.Подразделение.Город
	|	КОНЕЦ КАК Филиал,
	|	КоличествоЗаявок.КоличествоЗаказов КАК КоличествоЗаявок,
	|	КоличествоИСуммаПродаж.КоличествоПродаж КАК КоличествоПродаж,
	|	ОборотыВсе.СуммаПродаж + ЕСТЬNULL(МинусовыеДолги.СуммиМинусов, 0) КАК СуммаПродаж,
	|	ОборотыВсе.СуммаПриходов КАК СуммаПриходов,
	|	Должники.Долг КАК Долг,
	|	РАЗНОСТЬДАТ(ВТ_Предпоследнее.ПредпоследнееОбращение, ВТ_ПослОбр.ДатаПоследнегоОбращения, ДЕНЬ) КАК
	|		КоличествоДнейМеждуОбращениями,
	|	ВЫБОР
	|		КОГДА Договоры.КоличествоДоговоров > 0
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Договор,
	|	Договоры.ОтсрочкаВДнях КАК ОтсрочкаВДнях,
	|	Договоры.ДатаДоговора КАК ДатаДоговора,
	|	Договоры.ИстекСрокДоговора КАК ИстекСрокДоговора,
	|	ПоследнийКонтакт.ДатаПоследнегоКонтакта КАК ДатаПоследнейЗаявки,
	|	РАЗНОСТЬДАТ(ПоследнийКонтакт.ДатаПоследнегоКонтакта, &ТекущаяДата, ДЕНЬ) КАК ДнейСПоследнегоКонтакта,
	|	Клиенты.Куратор КАК Куратор,
	|	Клиенты.ДатаПоследнегоКонтакта КАК ДатаПоследнегоКонтакта
	|ИЗ
	|	Справочник.Клиенты КАК Клиенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Должники КАК Должники
	|			ЛЕВОЕ СОЕДИНЕНИЕ КоличествоЗаявок КАК КоличествоЗаявок
	|			ПО Должники.Клиент = КоличествоЗаявок.Клиент
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Предпоследнее КАК ВТ_Предпоследнее
	|			ПО Должники.Клиент = ВТ_Предпоследнее.Клиент
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПослОбр КАК ВТ_ПослОбр
	|			ПО Должники.Клиент = ВТ_ПослОбр.Клиент
	|			ЛЕВОЕ СОЕДИНЕНИЕ КоличествоИСуммаПродаж КАК КоличествоИСуммаПродаж
	|			ПО Должники.Клиент = КоличествоИСуммаПродаж.Клиент
	|		ПО Клиенты.Ссылка = Должники.Клиент
	|		ЛЕВОЕ СОЕДИНЕНИЕ Договоры КАК Договоры
	|		ПО Договоры.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследнийКонтакт КАК ПоследнийКонтакт
	|		ПО ПоследнийКонтакт.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОборотыВсе КАК ОборотыВсе
	|		ПО ОборотыВсе.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ МинусовыеДолги КАК МинусовыеДолги
	|		ПО МинусовыеДолги.Клиент = Клиенты.Ссылка";
...
Функция ПолучитьНеЦелевыеНормализированныеНомераТелефона()
	...
	|		ИНАЧЕ Клиенты.Ответственный.Подразделение.Город
	|	КОНЕЦ КАК Филиал,
	|	КоличествоЗаявок.КоличествоЗаказов КАК Заявки,
	|	КоличествоИСуммаПродаж.КоличествоПродаж КАК Продажи,
	|	КоличествоИСуммаПродаж.СуммаПродаж,
	|	РАЗНОСТЬДАТ(ВТ_Предпоследнее.ПредпоследнееОбращение, ВТ_ПослОбр.ДатаПоследнегоОбращения, День) КАК
	|		ДнейМеждуОбращениями,
	|	ПоследнийКонтакт.ДатаПоследнегоКонтакта КАК ДатаПоследнейЗаявки,
	|	РАЗНОСТЬДАТ(ПоследнийКонтакт.ДатаПоследнегоКонтакта, &ТекущаяДата, День) КАК ДнейСПоследнегоОбращения,
	...
	|		ПО КоличествоЗаявок.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоИСуммаПродаж КАК КоличествоИСуммаПродаж
	|		ПО КоличествоИСуммаПродаж.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПослОбр КАК ВТ_ПослОбр
	|		ПО ВТ_ПослОбр.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Предпоследнее КАК ВТ_Предпоследнее
	|		ПО ВТ_Предпоследнее.Клиент = Клиенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследнийКонтакт КАК ПоследнийКонтакт
	|		ПО ПоследнийКонтакт.Клиент = Клиенты.Ссылка
...
Функция ПолучитьВсеНормализированныеНомераТелефона()
...
&НаКлиенте
Процедура СписокГорячиеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	/// Комлев АА 12/11/24 ---
	Если Поле = Элементы.СписокГорячиеПерезвонилиПослеДоставки И ПолучитьКонтролеров() Тогда
		СтандартнаяОбработка = Ложь;
		ТекущиеДанные = Элемент.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
			Если ТекущиеДанные.ПерезвонилиПослеДоставки = Ложь Тогда
				ПроставитьГалочкуПерезвонилиПослеПолучения(ТекущиеДанные.Клиент);
			Иначе
				СнятьГалочкуПерезвонилиПослеПолучения(ТекущиеДанные.Клиент);
			КонецЕсли;
		КонецЕсли;
		Элементы.СписокГорячие.Обновить();
	ИначеЕсли Поле = Элементы.СписокГорячиеПерезвонилиПослеДоставки И (Не ПолучитьКонтролеров()) Тогда
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	/// Комлев АА 12/11/24 ---
КонецПроцедуры
...
&НаКлиенте
Процедура ФильтрКураторГорячиеПриИзменении(Элемент)
	Если ФильтрКураторГорячие = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокГорячие, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторГорячие = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокГорячие, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторГорячие = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокГорячие, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторГорячие = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокГорячие, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
	ИначеЕсли ФильтрКураторГорячие = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокГорячие, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторГорячие = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокГорячие, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторГорячие = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокГорячие, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(СписокГорячие, "Куратор");
...
&НаКлиенте
Процедура ФильтрКураторАктПриИзменении(Элемент)
	Если ФильтрКураторАкт = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокАктивные, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторАкт = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокАктивные, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторАкт = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокАктивные, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторАкт = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокАктивные, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
	ИначеЕсли ФильтрКураторАкт = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокАктивные, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторАкт = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокАктивные, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторАкт = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокАктивные, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(СписокАктивные, "Куратор");
		
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ФильтрКураторТеплыеПриИзменении(Элемент)
	Если ФильтрКураторТеплые = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокТеплые, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторТеплые = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокТеплые, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторТеплые = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокТеплые, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторТеплые = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокТеплые, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
		
	ИначеЕсли ФильтрКураторТеплые = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокТеплые, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторТеплые = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокТеплые, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторТеплые = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокТеплые, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(СписокТеплые, "Куратор");
		
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ФильтрКураторПотенцПриИзменении(Элемент)
	Если ФильтрКураторПотенц = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокПотенциальные, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторПотенц = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокПотенциальные, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторПотенц = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокПотенциальные, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторПотенц = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокПотенциальные, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
		
	ИначеЕсли ФильтрКураторПотенц = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокПотенциальные, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторПотенц = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокПотенциальные, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторПотенц = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокПотенциальные, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(СписокПотенциальные, "Куратор");
...
&НаКлиенте
Процедура ФильтрКураторХолодныеПриИзменении(Элемент)
	Если ФильтрКураторХолодные = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокХолодные, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторХолодные = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокХолодные, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторХолодные = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокХолодные, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторХолодные = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокХолодные, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
		
	ИначеЕсли ФильтрКураторХолодные = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокХолодные, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторХолодные = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокХолодные, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторХолодные = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокХолодные, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(СписокХолодные, "Куратор");
		
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ФильтрКураторДолжникиПриИзменении(Элемент)
	Если ФильтрКураторДолжники = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДолжники, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторДолжники = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДолжники, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторДолжники = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДолжники, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторДолжники = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДолжники, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
		
	ИначеЕсли ФильтрКураторДолжники = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДолжники, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторДолжники = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДолжники, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторДолжники = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокДолжники, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(СписокДолжники, "Куратор");
		
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ФильтрКураторНеЦелевыеПриИзменении1(Элемент)
	Если ФильтрКураторНеЦелевые = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокНеЦелевые, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторНеЦелевые = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокНеЦелевые, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторНеЦелевые = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокНеЦелевые, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторНеЦелевые = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокНеЦелевые, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
		
	ИначеЕсли ФильтрКураторНеЦелевые = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокНеЦелевые, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторНеЦелевые = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокНеЦелевые, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторНеЦелевые = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокНеЦелевые, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(СписокНеЦелевые, "Куратор");
		
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ФильтрКураторВсеПриИзменении(Элемент)
	Если ФильтрКураторВсе = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ВсеКлиенты, "Куратор", ПолучитьПользователяВиктория(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторВсе = 2 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ВсеКлиенты, "Куратор", ПолучитьПользователяАндрей(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли ФильтрКураторВсе = 3 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ВсеКлиенты, "Куратор", ПолучитьПользователяЕвгений(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторВсе = 4 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ВсеКлиенты, "Куратор", ПолучитьПустуюСсылкуПользователь(), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.НеРавно, , Истина);
		
	ИначеЕсли ФильтрКураторВсе = 5 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ВсеКлиенты, "Куратор", ПолучитьПользователяПоИмени("Бурлака Дмитрий Владимирович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторВсе = 6 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ВсеКлиенты, "Куратор", ПолучитьПользователяПоИмени("Русов Василий Олегович"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);
		
	ИначеЕсли ФильтрКураторВсе = 7 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ВсеКлиенты, "Куратор", ПолучитьПользователяПоИмени("Трофимов Василий Юрьевич"), // Значение отбора
		
		ВидСравненияКомпоновкиДанных.Равно, , Истина);		
		
		
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(ВсеКлиенты, "Куратор");
...
Процедура ГруппыКлиентовПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.Активные Тогда
		...
		СписокАктивные.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Теплые Тогда
		...
		СписокТеплые.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Потенциальные Тогда
		...
		СписокПотенциальные.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
		ДатаНачалаТеплые = НачалоДня(Дата(ТекущаяДата()) - (24 * 60 * 60) * 90);
		ДатаОкончанияТеплые = КонецДня(Дата(ТекущаяДата()) - (24 * 60 * 60) * 31);
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Холодные Тогда
		...
		СписокХолодные.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Должники Тогда
		СписокДолжники.Параметры.УстановитьЗначениеПараметра("ОбщийКлиент", ПолучитьОбщегоКлиентаПользователя());
		СписокДолжники.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
		УсловноеОФормлениеДолжники();
		//РассчитатьИтогиДолжники();
	ИначеЕсли ТекущаяСтраница = Элементы.НеЦелевые Тогда
		СписокНеЦелевые.Параметры.УстановитьЗначениеПараметра("ОбщийКлиент", ПолучитьОбщегоКлиентаПользователя());
		СписокНеЦелевые.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
		//РассчитатьИтогиНеЦелевые();
		
	ИначеЕсли ТекущаяСтраница = Элементы.ВсеКлиенты Тогда
		ВсеКлиенты.Параметры.УстановитьЗначениеПараметра("ОбщийКлиент", ПолучитьОбщегоКлиентаПользователя());
		ВсеКлиенты.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
		//РассчитатьИтогиВсеКлиенты();
	КонецЕсли;
	// --- Комлев АА --- 17/10/24 
КонецПроцедуры
...
Процедура ОтключитьВыполнениеДинамическихСписков()
	
	Элементы.СписокАктивные.Видимость = Ложь;
	Элементы.СписокТеплые.Видимость = Ложь;
	Элементы.СписокГорячие.Видимость = Ложь;
	Элементы.СписокПотенциальные.Видимость = Ложь;
	Элементы.СписокХолодные.Видимость = Ложь;
	Элементы.Список.Видимость = Ложь;
	Элементы.СписокОбязанности.Видимость = Ложь;
	Элементы.СписокПродажи.Видимость = Ложь;
	Элементы.СписокПродажДляЗП.Видимость = Ложь;
	Элементы.ПоЧастям.Видимость = Ложь;
	Элементы.СписокОстатковНаСчёте.Видимость = Ложь;
	Элементы.Инвентарь.Видимость = Ложь;
	Элементы.ЖурналПродажи.Видимость = Ложь;
	Элементы.СписокДенежныеПоказатели.Видимость = Ложь;
	Элементы.СписокДенежныйПоток.Видимость = Ложь;
	Элементы.СписокДетализацияРасходов.Видимость = Ложь;
	Элементы.СписокОжидаемыйПриходДенег.Видимость = Ложь;
	Элементы.СписокНеоплаченныеЗаявкиНаРасход.Видимость = Ложь;
	Элементы.СписокПомощьВПродажеМой.Видимость = Ложь;
	Элементы.СписокКлиенты.Видимость = Ложь;
	Элементы.СписокПродажиПользователь.Видимость = Ложь;
	Элементы.СписокДоставки.Видимость = Ложь;
	Элементы.СписокУчетТоваров.Видимость = Ложь;
	Элементы.СписокПеремещениеТовара.Видимость = Ложь;
	Элементы.СписокПриёмкаТовара.Видимость = Ложь;
	Элементы.СписокУстановкаМестаХранения.Видимость = Ложь;
	Элементы.СписокВыполненныеРаботы.Видимость = Ложь;
	Элементы.СписокСозданныеЗакзНаряды.Видимость = Ложь;
	Элементы.СписокУщербы.Видимость = Ложь;
	Элементы.СписокТабель.Видимость = Ложь;
	Элементы.СписокВыплаты.Видимость = Ложь;
	Элементы.СприсокПоказателиНастройка.Видимость = Ложь;
	Элементы.СписокПоказатели.Видимость = Ложь;
	Элементы.БанковскиеСчетаСотрудников.Видимость = Ложь;
	Элементы.СписокДети.Видимость = Ложь;
	Элементы.СписокПоказателиЗП.Видимость = Ложь;
	Элементы.СписокМоиЗадачи.Видимость = Ложь;
	Элементы.СписокАдминистративных.Видимость = Ложь;
	Элементы.СписокОтпусков.Видимость = Ложь;
	Элементы.СписокОбязанностей.Видимость = Ложь;
	Элементы.ДолженЗнать1.Видимость = Ложь;
	Элементы.СписокКвалификаций.Видимость = Ложь;
	Элементы.СписокКадровыхДвижений.Видимость = Ложь;
	Элементы.РолиWT10.Видимость = Ложь;
	///-ТатарМА 24.10.2024
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	///+ТатарМА 24.10.2024
	Если ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Пронин Алексей Александрович" Тогда //НЕ ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("мт_ДобавлениеИзменениеЗаказКлиента")) Тогда
		СписокДетализацияРасходов.ТекстЗапроса = "";
		ОтключитьВыполнениеДинамическихСписков();
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	ПользователиСтруктура = ПолучитьПользователя();
	СсылкаТекПользователя = ПользователиСтруктура.СсылкаТекПользователя;
	СсылкаТекПользователяПользователь = ПользователиСтруктура.СсылкаТекПользователяПользователь;
	...
	Если ТекущаяСтраница = Элементы.Табель Тогда
	...
	Если ТекущаяСтраница = Элементы.Клиенты Тогда
		СписокКлиенты.Параметры.УстановитьЗначениеПараметра("Пользователь", СсылкаТекПользователяПользователь);
		СписокКлиенты.Параметры.УстановитьЗначениеПараметра("Сотрудник", СсылкаТекПользователя);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.Доставка Тогда
		СписокДоставки.Параметры.УстановитьЗначениеПараметра("Сотрудник", СсылкаТекПользователя);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.ПомощьВПродаже Тогда
	...
	Если ТекущаяСтраница = Элементы.Продажа Тогда
		СписокПродажиПользователь.Параметры.УстановитьЗначениеПараметра("Пользователь", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.ПеремещениеТовара Тогда
		СписокПеремещениеТовара.Параметры.УстановитьЗначениеПараметра	("Ответственный", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.ГруппаУчетТоваров Тогда
		СписокУчетТоваров.Параметры.УстановитьЗначениеПараметра("Ответственный", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.ПриёмкаТовара Тогда
		СписокПриёмкаТовара.Параметры.УстановитьЗначениеПараметра("Пользователь", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.УстановкаМестаХранения Тогда
		СписокУстановкаМестаХранения.Параметры.УстановитьЗначениеПараметра("Пользователь", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.ВыполненныеРаботы Тогда
		СписокВыполненныеРаботы.Параметры.УстановитьЗначениеПараметра	("Пользователь", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.СозданныеЗаказНаряды Тогда
		СписокСозданныеЗакзНаряды.Параметры.УстановитьЗначениеПараметра ("Пользователь", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.Ущербы Тогда
		СписокУщербы.Параметры.УстановитьЗначениеПараметра("Сотрудник", СсылкаТекПользователя);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.Выплаты Тогда
		СписокВыплаты.Параметры.УстановитьЗначениеПараметра("Пользователь",СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.ПоказателиНастройка Тогда
		СприсокПоказателиНастройка.Параметры.УстановитьЗначениеПараметра("Пользователь", СсылкаТекПользователяПользователь);
	КонецЕсли;
	
	
	Если ТекущаяСтраница = Элементы.ПоказателиЗначения Тогда
	...
	Если ТекущаяСтраница = Элементы.ГруппаKPI Тогда
		РолиWT10.Параметры.УстановитьЗначениеПараметра("Пользователь",СсылкаТекПользователяПользователь);
	КонецЕсли;
	...
	Если ТекущаяСтраница = Элементы.ДолженЗнать Тогда
		ДолженЗнать.Параметры.УстановитьЗначениеПараметра("Сотрудник", СсылкаТекПользователя);
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.Обязанности Тогда
	...
	Если ТекущаяСтраница = Элементы.Зарплата Тогда
		СписокПоказателиЗП.Параметры.УстановитьЗначениеПараметра("Сотрудник", СсылкаТекПользователя);
	КонецЕсли;
	
	
	Если ТекущаяСтраница = Элементы.СтраницаДенежныеПоказатели Тогда
		ОбновитьДенежныеПоказатели();
	КонецЕсли;
	
	Если ТекущаяСтраница = Элементы.ОсновнаяИнформацияСотрудника Тогда
		ВкладкаДанныеСотрудникаЗаполнение();
	КонецЕсли;
...
Функция ПолучитьСчетаИзРегистра()
	// Комлев АА 10/03/25
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Счета.Счет КАК Счет,
	|	Счета.Принадлежность КАК Принадлежность
	|ИЗ
	|	РегистрСведений.Счета КАК Счета";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Выборка = РезультатЗапроса.Выбрать();
	МассивСчетаООО_ИЗРегистра = Новый Массив; 
	МассивСчетаИП_ИЗРегистра = Новый Массив;
	МассивСчетаКарта_ИЗРегистра = Новый Массив;
	МассивСчетаНаличка_ИЗРегистра = Новый Массив;
	Пока Выборка.Следующий() Цикл
		Если Выборка.Принадлежность = ПредопределенноеЗначение("Перечисление.ПринадлежностьСчета.ООО") Тогда
			МассивСчетаООО_ИЗРегистра.Добавить(Выборка.Счет);
		КонецЕсли;
		
		Если Выборка.Принадлежность = ПредопределенноеЗначение("Перечисление.ПринадлежностьСчета.ИП") Тогда
			МассивСчетаИП_ИЗРегистра.Добавить(Выборка.Счет);
		КонецЕсли;
		
		Если Выборка.Принадлежность = ПредопределенноеЗначение("Перечисление.ПринадлежностьСчета.Карта") Тогда
			МассивСчетаКарта_ИЗРегистра.Добавить(Выборка.Счет);
		КонецЕсли;
		
		Если Выборка.Принадлежность = ПредопределенноеЗначение("Перечисление.ПринадлежностьСчета.Наличка") Тогда
			МассивСчетаНаличка_ИЗРегистра.Добавить(Выборка.Счет);
		КонецЕсли;
	КонецЦикла; 
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("МассивСчетаООО_ИЗРегистра", МассивСчетаООО_ИЗРегистра); 
	СтруктураОтвета.Вставить("МассивСчетаИП_ИЗРегистра", МассивСчетаИП_ИЗРегистра);
	СтруктураОтвета.Вставить("МассивСчетаКарта_ИЗРегистра", МассивСчетаКарта_ИЗРегистра);
	СтруктураОтвета.Вставить("МассивСчетаНаличка_ИЗРегистра", МассивСчетаНаличка_ИЗРегистра); 
	Возврат СтруктураОтвета;
	// Комлев АА 10/03/25
КонецФункции

Процедура ОбновитьДенежныеПоказатели()
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.Найти("дт_Зарплата"))
		...
		МассивыСчетов = ПолучитьСчетаИзРегистра();
		
		МассивСчетаООО = Новый Массив; 
		Если МассивыСчетов.МассивСчетаООО_ИЗРегистра.Количество() <> 0 Тогда
			МассивСчетаООО = МассивыСчетов.МассивСчетаООО_ИЗРегистра;
		Иначе 
			//Сюда ничего добавлять не надо - Это не будет работать после добавления счетов в РегистрСведений Счета
			...
			МассивСчетаООО.Добавить(Справочники.Счета.НайтиПоКоду("000000056"));
		КонецЕсли;
		
		МассивСчетаИП = Новый Массив;
		Если МассивыСчетов.МассивСчетаИП_ИЗРегистра.Количество() <> 0 Тогда
			МассивСчетаИП = МассивыСчетов.МассивСчетаИП_ИЗРегистра; 
		Иначе 
			//Сюда ничего добавлять не надо - Это не будет работать после добавления счетов в РегистрСведений Счета
			...
			МассивСчетаИП.Добавить(Справочники.Счета.НайтиПоКоду("000000055")); 
		КонецЕсли;    
		
		МассивСчетаКарта = Новый Массив; 
		Если МассивыСчетов.МассивСчетаКарта_ИЗРегистра.Количество() <> 0 Тогда
			МассивСчетаКарта = МассивыСчетов.МассивСчетаКарта_ИЗРегистра; 
		Иначе 
			//Сюда ничего добавлять не надо - Это не будет работать после добавления счетов в РегистрСведений Счета
		...
		Если МассивыСчетов.МассивСчетаНаличка_ИЗРегистра.Количество() <> 0 Тогда
			МассивСчетаНаличка = МассивыСчетов.МассивСчетаНаличка_ИЗРегистра;
		Иначе 
			//Сюда ничего добавлять не надо - Это не будет работать после добавления счетов в РегистрСведений Счета
...
&НаКлиенте
Процедура ОбновитьВсех(Команда)
	РассчитатьИтогиВсеКлиенты();
КонецПроцедуры
...
&НаКлиенте
Процедура ОбновитьДолжники(Команда)
	РассчитатьИтогиДолжники();
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьНеЦелевые(Команда)
	РассчитатьИтогиНеЦелевые();
КонецПроцедуры
...
&НаСервереБезКонтекста
Процедура ПроставитьГалочкуПерезвонилиПослеПолучения(СсылкаНаКлиента)
	/// Комлев АА 12/11/24 +++
	КлиентОбъект = СсылкаНаКлиента.ПолучитьОбъект();
	КлиентОбъект.ПерезвонилиПослеДоставки  = Истина;
	КлиентОбъект.Записать();
	/// Комлев АА 12/11/24 ---
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СнятьГалочкуПерезвонилиПослеПолучения(СсылкаНаКлиента)
	/// Комлев АА 12/11/24 +++
	КлиентОбъект = СсылкаНаКлиента.ПолучитьОбъект();
...
Функция ПолучитьКонтролеров()
	/// Комлев АА 12/11/24 +++
	Если ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Шлеенкова Виктория Алексеевна"
		Или ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Дубнов Евгений Владимирович"
		Или ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Носовский Андрей Владимирович"
		Или ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.дт_ПолныеПрава) Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьПустуюСсылкуПользователь()
...
Функция ПолучитьПользователя()
	...
	СсылкаТекПользователяПользователь = Справочники.Пользователи.НайтиПоНаименованию(ИмяТекПользователя);
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("СсылкаТекПользователя", СсылкаТекПользователя);
	СтруктураОтвета.Вставить("СсылкаТекПользователяПользователь", СсылкаТекПользователяПользователь);
	Возврат СтруктураОтвета;
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьПользователяВиктория()
	Возврат Справочники.Пользователи.НайтиПоНаименованию("Шлеенкова Виктория Алексеевна");
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьПользователяАндрей()
	Возврат Справочники.Пользователи.НайтиПоНаименованию("Носовский Андрей Владимирович");
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьПользователяЕвгений()
	Возврат Справочники.Пользователи.НайтиПоНаименованию("Дубнов Евгений Владимирович");
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьПользователяПоИмени(ПолноеИмяПользователя)
...
Процедура ОбновитьГорячиеКлиенты() Экспорт
	...
	СписокГорячие.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
	//РассчитатьИтогиГорячие();
	МассивСтрок = ПолучитьМассивСтрокСписка("СписокГорячие");
	ПолучитьЗвонкиКлиента(МассивСтрок);
	УсловноеОФормлениеГорячиеКлиенты();
	/// Комлев АА 30/10/24 ++++
КонецПроцедуры


Процедура УсловноеОФормлениеГорячиеКлиенты()
	/// Комлев АА 30/10/24 ++++
	ЭлементОформления = СписокГорячие.УсловноеОформление.Элементы.Добавить();
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ожидание");
	...
	ЭлементОформления = СписокГорячие.УсловноеОформление.Элементы.Добавить();
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПерезвонилиПослеДоставки");
...
&НаКлиенте
Процедура УсловноеОФормлениеДолжники()
	/// Комлев АА 31/10/24 ++++
	ЭлементОформления = СписокДолжники.УсловноеОформление.Элементы.Добавить();
	ЭлементОтбора = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИстекСрокДоговора");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = Истина;
	ЭлементОтбора.Использование = Истина;
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветФона", WebЦвета.СветлоКоралловый);
...
&НаСервереБезКонтекста
Процедура ПолучитьЗвонкиКлиента(МассивСтрок)
	/// Комлев АА 30/10/24 +++
	Для Каждого Строка Из МассивСтрок Цикл
		Если Строка.Клиент.ПерезвонилиПослеДоставки = Истина Тогда
			Продолжить;
		КонецЕсли;
		Запрос = Новый Запрос;
		Запрос.Текст =
		...
		|	ЗаписиЗвонков.Клиент.ПерезвонилиПослеДоставки КАК Перезвон
		|ИЗ
		|	РегистрСведений.ЗаписиЗвонков КАК ЗаписиЗвонков
		|ГДЕ
		|	ЗаписиЗвонков.ТипЗвонка
		|	И ЗаписиЗвонков.Дата > &Дата
		|	И СТРЗАМЕНИТЬ(СТРЗАМЕНИТЬ(ЗаписиЗвонков.НомерТелефона, "" "", """"), ""-"", """") = СТРЗАМЕНИТЬ(СТРЗАМЕНИТЬ(&НомерТелефона,
		|		"" "", """"), ""-"", """")
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
		
		Запрос.УстановитьПараметр("НомерТелефона", Строка.Телефон);
		Запрос.УстановитьПараметр("Дата", Строка.ДатаПолученияЗаказа);
		
		//@skip-check query-in-loop
		РезультатЗапроса = Запрос.Выполнить();
		Если Не РезультатЗапроса.Пустой() Тогда
			КлиентОбъект = Строка.Клиент.ПолучитьОбъект();
			КлиентОбъект.ПерезвонилиПослеДоставки = Истина;
			КлиентОбъект.Записать();
		КонецЕсли;
	КонецЦикла;
...
Процедура РассчитатьИтогиАктивные() 
	...
	КоличествоАктивных = РезультатКД_ТЗ.Количество();
	Элементы.СписокАктивныеКоличествоАвтомобилей.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоАвтомобилей");
	Элементы.СписокАктивныеКоличествоЗаказов.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоЗаказов");
	Элементы.СписокАктивныеКоличествоПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоПродаж");
	Элементы.СписокАктивныеСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");		
	// Комлев АА 21/10/24 ---
КонецПроцедуры
...
Процедура РассчитатьИтогиТеплые() 
	...
	ПроцессорВыводаКД = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	КоличествоТеплых = РезультатКД_ТЗ.Количество();
	Элементы.СписокТеплыеКоличествоАвтомобилей.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоАвтомобилей");
	Элементы.СписокТеплыеКоличествоЗаказов.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоЗаказов");
	Элементы.СписокТеплыеКоличествоПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоПродаж");
	Элементы.СписокТеплыеСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");		
	// Комлев АА 21/10/24 ---
КонецПроцедуры
...
Процедура РассчитатьИтогиПотенциальные() 
	...
	ПроцессорВыводаКД = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	КоличествоПотенциальных = РезультатКД_ТЗ.Количество();
	Элементы.СписокПотенциальныеКоличествоАвтомобилей.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоАвтомобилей");
	Элементы.СписокПотенциальныеКоличествоЗаказов.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоЗаказов");
	Элементы.СписокПотенциальныеКоличествоПродаж.ТекстПодвала = РезультатКД_ТЗ.Итог("КоличествоПродаж");
	Элементы.СписокПотенциальныеСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");		
	// Комлев АА 21/10/24 ---
КонецПроцедуры
...
Процедура РассчитатьИтогиХолодные() 
	...
	ПроцессорВыводаКД = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	КоличествоХолодных = РезультатКД_ТЗ.Количество();
	Элементы.СписокХолодныеКоличествоАвтомобилей.ТекстПодвала = РезультатКД_ТЗ.Итог("КоличествоАвтомобилей");
	Элементы.СписокХолодныеКоличествоЗаказов.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоЗаказов");
	Элементы.СписокХолодныеКоличествоПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоПродаж");
	Элементы.СписокХолодныеСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");		
	// Комлев АА 21/10/24 ---
КонецПроцедуры

Функция ПолучитьМассивСтрокСписка(НазваниеСписка)
...
Процедура РассчитатьИтогиГорячие() 
	...
	ПроцессорВыводаКД = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	КоличествоГорячих = РезультатКД_ТЗ.Количество();
	Элементы.СписокГорячиеКоличествоМашин.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоМашин");
	Элементы.СписокГорячиеКоличествоЗаказов.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоЗаказов");
	Элементы.СписокГорячиеКоличествоПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоПродаж");
	Элементы.СписокГорячиеСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");		
	// Комлев АА 21/10/24 ---
КонецПроцедуры

Процедура РассчитатьИтогиДолжники() 
	// Комлев АА 21/10/24 +++
	СКД = Элементы.СписокДолжники.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	НастройкиКД = Элементы.СписокДолжники.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	КомпоновщикКД = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКД = КомпоновщикКД.Выполнить(СКД, НастройкиКД, , , Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
	ПроцессорКД = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКД.Инициализировать(МакетКД);
	ПроцессорВыводаКД = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	КоличествоДолжников = РезультатКД_ТЗ.Количество();
	//Элементы.СписокДолжникиМашины.ТекстПодвала =  РезультатКД_ТЗ.Итог("Машины");
	Элементы.СписокДолжникиКоличествоЗаявок.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоЗаявок");
	Элементы.СписокДолжникиКоличествоПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоПродаж");
	Элементы.СписокДолжникиСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");
	Элементы.СписокДолжникиСуммаПриходов.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПриходов");
	Элементы.СписокДолжникиДолг.ТекстПодвала =  РезультатКД_ТЗ.Итог("Долг");	
	// Комлев АА 21/10/24 ---
КонецПроцедуры

Процедура РассчитатьИтогиНеЦелевые()
	// Комлев АА 30/10/24 +++
	СКД = Элементы.СписокНеЦелевые.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	НастройкиКД = Элементы.СписокНеЦелевые.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	КомпоновщикКД = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКД = КомпоновщикКД.Выполнить(СКД, НастройкиКД, , , Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
	ПроцессорКД = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКД.Инициализировать(МакетКД);
	ПроцессорВыводаКД = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	КоличествоНеЦелевых = РезультатКД_ТЗ.Количество();
	Элементы.СписокНеЦелевыеМашины.ТекстПодвала =  РезультатКД_ТЗ.Итог("Машины");
	Элементы.СписокНеЦелевыеЗаявки.ТекстПодвала =  РезультатКД_ТЗ.Итог("Заявки");
	Элементы.СписокНеЦелевыеПродажи.ТекстПодвала =  РезультатКД_ТЗ.Итог("Продажи");
	Элементы.СписокНеЦелевыеСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");
	// Комлев АА 30/10/24 ---
КонецПроцедуры

Процедура РассчитатьИтогиВсеКлиенты()
	
	// Комлев АА 06/11/24 +++
	СКД = Элементы.ВсеКлиенты1.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	НастройкиКД = Элементы.ВсеКлиенты1.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	КомпоновщикКД = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКД = КомпоновщикКД.Выполнить(СКД, НастройкиКД, , , Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
	ПроцессорКД = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКД.Инициализировать(МакетКД);
	ПроцессорВыводаКД = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	РезультатКД_ТЗ = ПроцессорВыводаКД.Вывести(ПроцессорКД);
	КоличествоВсехКлиентов = РезультатКД_ТЗ.Количество();
	Элементы.ВсеКлиентыКоличествоАвтомобилей.ТекстПодвала = РезультатКД_ТЗ.Итог("КоличествоАвтомобилей");
	Элементы.ВсеКлиентыКоличествоЗаказов.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоЗаказов");
	Элементы.ВсеКлиентыКоличествоПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("КоличествоПродаж");
	Элементы.ВсеКлиентыСуммаПродаж.ТекстПодвала =  РезультатКД_ТЗ.Итог("СуммаПродаж");		
	// Комлев АА 06/11/24 ---
КонецПроцедуры
