...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().ПолноеИмя = "Вадяева Дарья Викторовна" Или 
	ПользователиИнформационнойБазы.ТекущийПользователь().Роли.Содержит(Метаданные.Роли.дт_ПолныеПрава) Тогда
		Элементы.СписатьДолгПоПродаже.Доступность = Истина;
	КонецЕсли
...
Функция ВычислитьОстатокПоКлиенту()//(Баланс)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	БалансКлиента1Остатки.БалансОстаток
	|ИЗ
	|	РегистрНакопления.БалансКлиента.Остатки(, Клиент = &Клиент) КАК БалансКлиента1Остатки";
...
Процедура ПриОткрытии(Отказ)
	...
	///+ТатарМА 17.12.2024
	Если ЗначениеЗаполнено(Объект.ДатаПоследнегоКонтактаКуратор) Тогда
		ДнейСПоследнегоКонтактаКуратор = (НачалоДня(ТекущаяДата()) - Объект.ДатаПоследнегоКонтактаКуратор) / 86400;
	КонецЕсли;
	///-ТатарМА 17.12.2024
...
&НаКлиенте
Процедура ДатаПоследнегоКонтактаКураторПриИзменении(Элемент)
...
Процедура СписатьДолгПоПродаже(Команда)
	/// Комлев АА 31/10/24 +++
	МассивВыбранныхСтрок = Элементы.АнализПродаж.ВыделенныеСтроки;
	Если МассивВыбранныхСтрок = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Для Каждого Строка Из МассивВыбранныхСтрок Цикл
		ТекущиеДанные = АнализПродаж.НайтиПоИдентификатору(Строка);
		Если ТекущиеДанные.Долг <= 0 Тогда
			Продолжить;
		Иначе
			Продажа = ТекущиеДанные.Ссылка;
			СписатьДолгПоПродажеНаСервере(Продажа);
			Сообщить("Долг по продаже " + ТекущиеДанные.Номер + " списан.");
		КонецЕсли;
...
Функция СформироватьСвойМассивЗначений()
	...
	МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Бурлака Дмитрий Владимирович"));
	МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Русов Василий Олегович"));
	МассивПользователей.Добавить(Справочники.Пользователи.НайтиПоНаименованию("Трофимов Василий Юрьевич"));
