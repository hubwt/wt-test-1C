...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ДатаПоследнегоКонтакта = ДатаПоследнегоКонтакта();
	
КонецПроцедуры 



Функция ДатаПоследнегоКонтакта()
	/// Комлев АА 01/11/24 ++++
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ПоследнийКонтактИзЗаявок
		|ИЗ
		|	Документ.ЗаказКлиента КАК ЗаказКлиента
		|ГДЕ
		|	ЗаказКлиента.Клиент = &Клиент
		...
		|ПОМЕСТИТЬ ДатаПоследнегоКонтактаИзЗвонков
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДатаПоследнегоКонтактаИзЗвонков.Клиент КАК Клиент,
		|	ВЫБОР
		|		КОГДА ЕстьNULL(ДатаПоследнегоКонтактаИзЗвонков.ДатаЗвонка, ДатаВремя(1,1, 1)) >= ЕстьNULL(ПоследнийКонтактИзЗаявок.ДатаЗаявки, ДатаВремя(1,1, 1))
		|			ТОГДА ДатаПоследнегоКонтактаИзЗвонков.ДатаЗвонка
		|		ИНАЧЕ ПоследнийКонтактИзЗаявок.ДатаЗаявки
		|	КОНЕЦ КАК ДатаПоследнегоКонтакта
		|ИЗ
		|	ДатаПоследнегоКонтактаИзЗвонков КАК ДатаПоследнегоКонтактаИзЗвонков
		|		ПОЛНОЕ СОЕДИНЕНИЕ ПоследнийКонтактИзЗаявок КАК ПоследнийКонтактИзЗаявок
		|		ПО (ПоследнийКонтактИзЗаявок.Клиент = ДатаПоследнегоКонтактаИзЗвонков.Клиент)"; 
	
	Запрос.УстановитьПараметр("Клиент", Объект.Ссылка);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	Выборка.Следующий();
	Возврат Выборка.ДатаПоследнегоКонтакта;
...
//&НаКлиенте
//Процедура КураторНачалоВыбора1(Элемент, ДанныеВыбора, СтандартнаяОбработка)
//	/// Комлев АА 06/11/24 +++
//	
//	Элементы.Куратор.СписокВыбора.Очистить();
//	СвойМассивЗначений = СформироватьСвойМассивЗначений(); 
//	Элементы.Куратор.СписокВыбора.ЗагрузитьЗначения(СвойМассивЗначений);
//	/// Комлев АА 06/11/24 ---
//КонецПроцедуры
//
//&НаКлиенте
//Процедура КураторОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
