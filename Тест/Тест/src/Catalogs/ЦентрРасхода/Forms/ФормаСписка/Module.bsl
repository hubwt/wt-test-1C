...
Процедура ПериодПриИзменении(Элемент)
	...
	СписокРентабельность.Параметры.УстановитьЗначениеПараметра("Дата1",Период.ДатаНачала);
	СписокРентабельность.Параметры.УстановитьЗначениеПараметра("Дата2",Период.ДатаОкончания);
	...
	ЗаполнитьПодвал();
	ЗаполнитьПодвалСписокРентабельность();
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокРентабельность.Параметры.УстановитьЗначениеПараметра("Дата1",Дата1);
	СписокРентабельность.Параметры.УстановитьЗначениеПараметра("Дата2",Дата2); 
	ЗаполнитьПодвал(); 
	ЗаполнитьПодвалСписокФилиалы(); 
	ЗаполнитьПодвалСписокРентабельность();
КонецПроцедуры
...
Процедура ЗаполнитьПодвалСписокРентабельность()
	
	Схема = Элементы.СписокРентабельность.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	Настройки = Элементы.СписокРентабельность.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(Схема, Настройки,,,Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
	ПроцессорКомпоновки = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновки.Инициализировать(МакетКомпоновки);
	...
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьРасходОтдела.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьНагрузка.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьВыручкаОтдела.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьПрибыль.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьРасходОтдела.ТекстПодвала =  Формат(Результат.Итог("РасходОтдела"),"ЧДЦ=0; ЧН=-");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьНагрузка.ТекстПодвала =  Формат(Результат.Итог("Нагрузка"),"ЧДЦ=0; ЧН=-");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьВыручкаОтдела.ТекстПодвала =  Формат(Результат.Итог("ВыручкаОтдела"),"ЧДЦ=0; ЧН=-");
	ЭтотОбъект.ЭтаФорма.Элементы.СписокРентабельностьПрибыль.ТекстПодвала =  Формат(Результат.Итог("Прибыль"),"ЧДЦ=0; ЧН=-");
