...
Процедура Расходы2(Команда)
	КомандыСписовПодмена2(Истина,ложь,ложь);
КонецПроцедуры

&НаКлиенте
Процедура Статьи2(Команда)
	КомандыСписовПодмена2(ложь,Истина,ложь);
КонецПроцедуры

&НаКлиенте
Процедура Влияния2(Команда)
	КомандыСписовПодмена2(ложь,ложь,Истина);
КонецПроцедуры

&НаСервере
Функция КомандыСписовПодмена2(Флаг1 = Неопределено,Флаг2 = Неопределено,Флаг3 = Неопределено)
...
&НаКлиенте
Процедура Расходы6(Команда)
	КомандыСписовПодмена6(Истина,ложь,ложь);
КонецПроцедуры

&НаКлиенте
Процедура Статьи6(Команда)
	КомандыСписовПодмена6(ложь,Истина,ложь);
КонецПроцедуры

&НаКлиенте
Процедура Влияния6(Команда)
...
// 1 - Расходы, 2 - Статьи , 3 - Влияния
		...
		Элементы.ДинСписок666.Видимость = Ложь; 
	
		Элементы.Расходы6.ЦветФона 	= Новый Цвет (252,250,235);
		Элементы.Статьи6.ЦветФона 	= Новый Цвет (252,250,235);
		Элементы.Влияния6.ЦветФона 	= Новый Цвет (252,250,235);	
	
	Если Флаг1 = Истина Тогда
		Элементы.ДинСписок6.Видимость 	= Истина;
		Элементы.Расходы6.ЦветФона 		= Новый Цвет (210,160,6);
	ИначеЕСЛИ Флаг2 = Истина Тогда
		Элементы.ДинСписок66.Видимость 	= Истина;
		Элементы.Статьи6.ЦветФона 		= Новый Цвет (210,160,6);
	ИначеЕСЛИ Флаг3 = Истина Тогда
		Элементы.ДинСписок666.Видимость = Истина;
		Элементы.Влияния6.ЦветФона 		= Новый Цвет (210,160,6);	
	КонецЕсли;
...
Функция ОбновитьСписки()
	...
	Иначе 
		...
		ДинСписок111.Параметры.УстановитьЗначениеПараметра("Дата1",Дата(1,1,1));
		ДинСписок111.Параметры.УстановитьЗначениеПараметра("Дата2",Дата(1,1,1)); 
	...
	Иначе 
		...
		ДинСписок222.Параметры.УстановитьЗначениеПараметра("Дата1",Дата(1,1,1));
		ДинСписок222.Параметры.УстановитьЗначениеПараметра("Дата2",Дата(1,1,1));   
	КонецЕсли;
	...
	Иначе 
		...
		ДинСписок333.Параметры.УстановитьЗначениеПараметра("Дата1",Дата(1,1,1));
		ДинСписок333.Параметры.УстановитьЗначениеПараметра("Дата2",Дата(1,1,1));  
	КонецЕсли;
	...
	Если   ЗначениеЗаполнено(Объект.Категория4) И ЗначениеЗаполнено (Период) Тогда 
		...
		ДинСписок444.Параметры.УстановитьЗначениеПараметра("Дата1",Дата(1,1,1));
		ДинСписок444.Параметры.УстановитьЗначениеПараметра("Дата2",Дата(1,1,1));  
	КонецЕсли;
	...
	Иначе 
		...
		ДинСписок555.Параметры.УстановитьЗначениеПараметра("Дата1",Дата(1,1,1));
		ДинСписок555.Параметры.УстановитьЗначениеПараметра("Дата2",Дата(1,1,1));   
	КонецЕсли;
	...
	Если   ЗначениеЗаполнено(Объект.Категория6) И ЗначениеЗаполнено (Период) Тогда 
		...
		ДинСписок666.Параметры.УстановитьЗначениеПараметра("Категория",Объект.Категория6);
		ДинСписок666.Параметры.УстановитьЗначениеПараметра("Дата1",Период.ДатаНачала);
		ДинСписок666.Параметры.УстановитьЗначениеПараметра("Дата2",Период.ДатаОкончания);   
	...
	Иначе 
		...
		ДинСписок666.Параметры.УстановитьЗначениеПараметра("Категория",Неопределено);
		ДинСписок666.Параметры.УстановитьЗначениеПараметра("Дата1",Дата(1,1,1));
		ДинСписок666.Параметры.УстановитьЗначениеПараметра("Дата2",Дата(1,1,1));  
	КонецЕсли;
...
КонецПроцедуры
Процедура ЗаполнитьПодвалДинСписок666()
	...
	Схема = Элементы.ДинСписок666.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	Настройки = Элементы.ДинСписок666.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(Схема, Настройки,,,Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
	ПроцессорКомпоновки = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновки.Инициализировать(МакетКомпоновки);
	...
	ЭтотОбъект.ЭтаФорма.Элементы.ДинСписок666Сумма.ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Право;
	ЭтотОбъект.ЭтаФорма.Элементы.ДинСписок666Сумма.ТекстПодвала = Формат(Результат.Итог("Сумма"),"ЧДЦ=0; ЧН=-");
