	...
	Если Не ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Упаковка), " " + НСтр("ru = 'в упаковке';
																		|en = 'in package'") + " """ + НайденнаяСтрока.Упаковка + """", ""))+НСтр("ru = '. Обменять их местами?';
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	Если Объект.ТипПодключаемогоОборудования = Перечисления.ТипыОфлайнОборудования.ККМ Тогда
...
Процедура ПрименитьИзмененияНаСервере(ТекущийОбъект)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Редактирование правила обмена с ККМ Offline';
										|en = 'Edit rule to exchange data with Offline cash register'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
