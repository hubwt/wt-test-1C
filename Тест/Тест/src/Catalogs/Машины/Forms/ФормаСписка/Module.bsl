...
Процедура СортировкаМашинаАгрегатПриИзмененииНаСервере()
	...
	Если СортировкаМашинаАгрегат = 1 Тогда
		...
		|		ПО (ВТ_ТоварыИзЗаявок.Товар = Номенклатура.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТоварыПродажа КАК ВТ_ТоварыПродажа
		|		ПО (ВТ_ТоварыПродажа.Товар = Номенклатура.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТоварыИзЗаказНарядов КАК ВТ_ТоварыИзЗаказНарядов
		|		ПО (ВТ_ТоварыИзЗаказНарядов.Товар = Номенклатура.Ссылка)
		|ГДЕ
		|	ЕСТЬNULL(ВТ_ТоварыИзЗаказНарядов.КоличествоТовараЗаказНаряд, 0) +
		|		ЕСТЬNULL(ВТ_ТоварыИзЗаявок.КоличествоТовараИзЗаявокБезПродажи, 0) +
		|		ЕСТЬNULL(ВТ_ТоварыПродажа.КоличествоТовараПродажа, 0) = 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	СУММА(РегистрНакопления1Остатки.Товар.РекомендованаяЦена) КАК СуммаЛиквидногоТовара
		...
		|			Неликвид.Ссылка
		|		ИЗ
		|			Неликвид КАК Неликвид)) КАК РегистрНакопления1Остатки
		|ГДЕ
		...
		|	ВТ_КоличествоДеталейПоМашинам.СуммаЛиквидногоТовара КАК ВыручкаПотенциальная
		|ИЗ
