...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Партии.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДатаСеанса());
	// -- obrv 05.10.18
	...
	// Комлев АА 19/12/24 +++
	Если Константы.УчестьОбязательно.Получить()  = Истина Тогда
		Константы.УчестьОбязательно.Установить(Ложь);
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Партия.Вставить("Цена", Элементы.Список.ТекущиеДанные.Цена);
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("справочникСсылка.Машины") Тогда
		...
		НоваяЗапись = УчестьДетальНаСервере(ВыбранноеЗначение,элементы.Список.ТекущиеДанные.ссылка);
		Если НоваяЗапись <> Неопределено Тогда 
			ОткрытьФорму("РегистрСведений.ИндНомер.ФормаЗаписи",Новый Структура("Ключ", НоваяЗапись));
		КонецЕсли;
		ОбновитьОтборПартий();
...
&НаКлиенте
Процедура УчестьПринудительноПриИзменении(Элемент)
	УчестьПринудительноПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Процедура УчестьПринудительноПриИзмененииНаСервере()
	// Комлев АА 19/12/24 +++
	Если УчестьПринудительно = Ложь Тогда
		Константы.УчестьОбязательно.Установить(Ложь);
	Иначе
		Константы.УчестьОбязательно.Установить(Истина);
	КонецЕсли;
	// Комлев АА 19/12/24 +++
КонецПроцедуры
#КонецОбласти
...
Процедура УстановитьПараметрыСписка()
	...
	Партии.Параметры.УстановитьЗначениеПараметра("Склад", Склад);
	Партии.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДатаСеанса());
...
Процедура ОбновитьОтборПартий()
	...
	Партии.Параметры.УстановитьЗначениеПараметра("ЕстьОтборПоСкладу", ЗначениеЗаполнено(Склад));
	Партии.Параметры.УстановитьЗначениеПараметра("Склад", Склад);
	Партии.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", ТекущаяДата());
...
&НаКлиенте
Процедура ПредложенияРегулирование(Элемент, Направление, СтандартнаяОбработка)
...
Функция ПереучестьДетальНаСервере(НашаДеталь,Причина)
	...
	/// Комлев
	Списание  = ПолучитьДокументСписания(НашаДеталь);
...
Процедура ПереучестьДеталь(Команда)
	...
	ПараметрыОткрытия = Новый Структура;
	ОткрытьФорму("Справочник.ПричиныСписания.ФормаВыбора", ПараметрыОткрытия, ЭтаФорма,
...
&НаСервереБезКонтекста
Функция ПолучитьДокументСписания(НашаДеталь)
	/// Комлев 
	Запрос = новый  Запрос;
...
функция УчестьДетальНаСервере(Машина,товар)
	...
	МестоРаботы = Справочники.Сотрудники.НайтиПоНаименованию(Пользователи.ТекущийПользователь().ПолноеНаименование()).МестоРаботы;
	...
	Если МестоРаботы = Справочники.МестоРаботы.НайтиПоКоду("000000003") Тогда 
		Объектпоступления.Склад = Справочники.Склады.НайтиПоКоду("000000002"); 
	ИначеЕсли МестоРаботы = Справочники.МестоРаботы.НайтиПоКоду("000000002") Тогда 
		Объектпоступления.Склад = Справочники.Склады.НайтиПоКоду("000000005"); 
	ИначеЕсли МестоРаботы = Справочники.МестоРаботы.НайтиПоКоду("000000004") Тогда 
		Объектпоступления.Склад = Справочники.Склады.НайтиПоКоду("000000008"); 
	КонецЕсли;
	...
	ИндКодНовый = Справочники.ИндКод.СоздатьНовыйЭлемент(ДанныеЗаполнения, Отказ);
	НоваяСтрока.Партия = ИндКодНовый;
	...
	Исключение   
		ИндКодОбъект = ИндКодНовый.ПолучитьОбъект();
		ИндКодОбъект.ПометкаУдаления = Истина; 
		ИндКодОбъект.Записать();
		Возврат Неопределено;
	КонецПопытки;
	// Комлев АА 16/01/25 ----
		// ++ МазинЕС 18-06-2024
	...
	Если МестоРаботы = Справочники.МестоРаботы.НайтиПоКоду("000000003") Тогда 
		об.Стеллаж = Справочники.Стеллаж.НайтиПоНаименованию("2_ANGAR"); 
	 
	ИначеЕсли МестоРаботы = Справочники.МестоРаботы.НайтиПоКоду("000000004") Тогда 
		об.Стеллаж = Справочники.Стеллаж.НайтиПоНаименованию("Korzina_RegionEKB"); 
	КонецЕсли;
	...
	/// Комлев 09/12/24 ++++
	об.ОтветственныйЕстьВНаличии = ПользователиКлиентСервер.ТекущийПользователь();
	об.ДатаПроверкиЕстьВНаличии = ТекущаяДатаСеанса();
	/// Комлев 09/12/24 +---
	об.Записать();
...
Функция ПолучитьДокументПоступленияДляКонвейера()
	...
	|	И ПоступлениеЗапчастейТаблица.Ссылка.Дата > ДАТАВРЕМЯ(2024, 12, 04, 17, 0, 0)
	|СГРУППИРОВАТЬ ПО
