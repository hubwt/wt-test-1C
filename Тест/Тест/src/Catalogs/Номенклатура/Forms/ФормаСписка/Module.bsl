...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("справочникСсылка.ПричиныСписания") Тогда
		...
		ПереучестьДетальНаСервере(НашаДеталь,Причина);
		ОбновитьОтборПартий();
...
Процедура ОткрытьСайт(Команда)
	/// Комлев 21/05/24 +++
	ОбщДействия.ОткрытьДетальНаСайтеСписком(ЭтаФорма.Элементы.Список.ТекущиеДанные.Код);
	/// Комлев 21/05/24 +++
КонецПроцедуры
...
&НаСервере
Процедура ПереучестьДетальНаСервере(НашаДеталь,Причина)
	Отказ = Ложь;
	...
	//Новаястрокапоступления = Объектпоступления.Таблица.Добавить();
	
	//Новаястрокапоступления.ДатаДобавления = ТекущаяДата();
	//Новаястрокапоступления.Автомобиль     = НашаДеталь.Машина;
	//Новаястрокапоступления.Колво          = 1;  
	//Новаястрокапоступления.Товар          = НашаДеталь.Товар;
	//Новаястрокапоступления.Ответственный  = Пользователи.ТекущийПользователь();
	
	
	//ДанныеЗаполнения = Новый Структура();
	//ДанныеЗаполнения.Вставить("ДокументПоступления", документы.ПоступлениеЗапчастей.НайтиПоНомеру("000003695"));
	//ДанныеЗаполнения.Вставить("ДатаПоступления", документы.ПоступлениеЗапчастей.НайтиПоНомеру("000003695").Дата);	
	//ДанныеЗаполнения.Вставить("Номенклатура", НашаДеталь.Товар);
	//ДанныеЗаполнения.Вставить("Автомобиль", НашаДеталь.Машина);
	
	//НовыйИндКод = Справочники.ИндКод.СоздатьНовыйЭлемент(ДанныеЗаполнения, Отказ);
	
	//Новаястрокапоступления.Партия = НовыйИндКод;
	...
	Иначе
		...
		Сообщить("Списано в Новый документ " + СписаниеНовыйДокумент);
	КонецЕсли;
КонецПроцедуры
