...
&НаСервере
Функция ПолучениеПредыдущейЦены(Дата)
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	дт_ЦеныНоменклатурыСрезПервых.Цена КАК ЦенаДоРаспродажи
		|ИЗ
		|	РегистрСведений.дт_ЦеныНоменклатуры.СрезПервых КАК дт_ЦеныНоменклатурыСрезПервых";
	...
	Выборка.Следующий();
		ЦенаДоРаспродажи = Выборка.ЦенаДоРаспродажи;
		
		Возврат ЦенаДоРаспродажи;
КонецФункции
...
Процедура НачатьРаспродажуНаСервере()
	ЦенаДоИзменения = ПолучениеПоследнейЦены(ТекущаяДата());
	
	Если Объект.РекомендованаяЦена >= ЦенаДоИзменения   Тогда
		Сообщить("Новая цена больше предыдущей");
		Объект.РекомендованаяЦена = ЦенаДоИзменения;
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ДатаОкончанияРасрпродажи) Тогда
		Сообщить("Дата окончания распродажи не заполонена.");
		Возврат;
	ИначеЕсли Объект.ДатаОкончанияРасрпродажи <= ТекущаяДата()  Тогда 
	...
	Сообщить("Распродажа до " + Объект.ДатаОкончанияРасрпродажи);
...
Процедура ЗакончитьРаспродажуНаСервере()
	...
	ЦенаДоРаспродажи = ПолучениеПредыдущейЦены(ТекущаяДата());
	Объект.РекомендованаяЦена = ЦенаДоРаспродажи;
	Объект.ДатаОкончанияРасрпродажи = '00010101';
...
&НаСервере
Процедура ОбновитьРекомендованнуюЦену() Экспорт
