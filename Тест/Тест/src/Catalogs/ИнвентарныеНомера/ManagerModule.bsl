...
Функция СоздатьНовыйЭлемент(ДанныеЗаполнения, Отказ) Экспорт
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения.ДокументПоступления) = Тип("ДокументСсылка.ПоступлениеЗапчастей") Тогда
		КодНовый = "З-000" + Формат(КодТМЦ, "ЧЦ=10;ЧГ=0") + "_" + СвободныйНомер;	
	КонецЕсли;
