...
Если Объект.ДатаСоздания <> '00010101' Тогда
	Если Объект.ДатаЗакрытия = '00010101' Тогда
		Элементы.Таймер.Заголовок = РаботаСДокументамиАналитикаВызовСервера.ПолучитьРазницуДат(Объект.ДатаСоздания, ТекущаяДатаСеанса());
	Иначе
		Элементы.Таймер.Заголовок = РаботаСДокументамиАналитикаВызовСервера.ПолучитьРазницуДат(Объект.ДатаСоздания, Объект.ДатаЗакрытия);
	КонецЕсли
КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписьюНаСервере()
	...
	Если Объект.Статус = Ложь Тогда
		...
		Если ЗаписьВРегистреСведений.Выбран() И ЗаписьВРегистреСведений.ДатаЗакрытия = '00010101' Тогда
			ЗаписьВРегистреСведений.ДатаЗакрытия = Объект.ДатаЗакрытия;
