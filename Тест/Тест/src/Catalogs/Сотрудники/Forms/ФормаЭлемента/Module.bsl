...
Процедура Получитькомпоненту()
	...
	Штрихкод.ЗначениеКода = "{ ""id_doc"":"+ """" + Объект.Код + """" + ",""type"":" +" ""Сотрудник" + ",""FIO"":" +  Объект.Наименование + "}";
...
Процедура ОбновитьСписки()
	...
	Для Каждого Элемент Из Элементы Цикл
		...
		Если ТипЗнч(Элемент) <> Тип("ТаблицаФормы") ИЛИ Элемент.Имя = "ТаблицаСсылок" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПолучитьФайлыНаСервере()
	Попытка
        Фотки = ПолучениеФайловПриказа(Объект.Код);
...
Процедура ТаблицаСсылокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ФайлыСОблакаСсылкаНаФайл" ИЛИ Поле.Имя = "ТаблицаСсылокСсылкаНаФайл" Тогда
...
Функция ПолучениеФайловПриказа(ИндКод)
	...
	Попытка
		...
		СтрокаЗапроса = "/document/flow/sotr?tabnum=";
...
Процедура ТаблицаСсылокНаименованиеПриИзменении(Элемент)
	...
	ПереименоватьФайл(Формат(ТекСтрока.id, "ЧГ=;"), ТекСтрока.Наименование);
КонецПроцедуры
...
Функция УдалитьФайл(id)
	...
	Попытка
		...
		СтрокаЗапроса = "/document/flow/sotr?tabnum="+объект.код+"&file_id="+id;
		Заголовки = Новый Соответствие;    
		Заголовки.Вставить("Content-Type", "application/json");
		Запрос = Новый HTTPЗапрос(СтрокаЗапроса);
...
//	Доход = (Константы.ВыручкаПредприятия.Получить() * (Объект.ПроцентОтВыручки/100)) + оклад;
КонецПроцедуры
...
|////////////////////////////////////////////////////////////////////////////////
|ВЫБРАТЬ
|	КадровыйПриказ.Оклад КАК Оклад,
|	КадровыйПриказ.Отдел.ПроценОтВыручки КАК Процент,
|	ВыручкаПредприятия.Значение КАК Выручка,
...
|		ПО КадровыйПриказ.Отдел = ВТ_Сотрудники.Отдел,
...
Попытка
	Оклад = Выборка.Оклад;
	Премия = (Выборка.Выручка * (Выборка.Процент/100))/Выборка.Сотрудник;
	Доход = Оклад + Премия;
Исключение
