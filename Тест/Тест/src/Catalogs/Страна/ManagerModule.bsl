...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Иначе
		...
		ШаблонЗапроса = "
			...
			|
			|ИЗ
			|	Справочник.Страна КАК СтраныМира
			|ПОЛНОЕ СОЕДИНЕНИЕ
			|	Классификатор
			|ПО
...
Функция ДанныеСтраныМира(Знач КодСтраны = Неопределено, Знач Наименование = Неопределено) Экспорт
	...
	Запрос = Новый Запрос("
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	Ссылка, Код, Наименование, КодАльфа2, КодАльфа3
	|ИЗ
	|	Справочник.Страна
	|ГДЕ
	|	" + УсловиеПоиска + "
...
Функция СсылкаПоДаннымКлассификатора(Знач Отбор, Знач ДополнительныеДанные = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		ОбъектСтраны = Справочники.Страна.СоздатьЭлемент();
...
Функция ТаблицаКлассификатора() Экспорт
	Макет = Справочники.Страна.ПолучитьМакет("Классификатор");
...
Функция ЕстьПравоДобавления() Экспорт
	Возврат ПравоДоступа("Добавление", Метаданные.Справочники.Страна);
КонецФункции
...
Функция ПоляПоиска()
	Результат = Новый Массив;
	СписокПолей = Справочники.Страна.ПустаяСсылка().Метаданные().ВводПоСтроке;
...
Процедура ЗаполнитьСписокСтранКОбработке(Параметры) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|ИЗ
		|	СписокСтран КАК СписокСтран
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Страна КАК СтраныМира
...
Процедура ОбновитьСтраныМираПоОКСМ(Параметры) Экспорт
	...
	СтранаМираСсылка = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, "Справочник.Страна");
	...
	Пока СтранаМираСсылка.Следующий() Цикл
		Попытка
			...
			ДанныеОКСМ = ДанныеКлассификатораСтранМираПоКоду(СтранаМираСсылка.Ссылка.Код);
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Справочники.Страна, СтранаМираСсылка.Ссылка, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
	
	Параметры.ОбработкаЗавершена = ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "Справочник.Страна");
