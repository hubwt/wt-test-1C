...
Процедура НалогПриИзмененииНаСервере()
		...
		НДС     = 0; 
	 	...
	 	Если (Формат(Период,"ДДД.ММ.ГГГГ") <> Формат(ТекущаяДата(),"ДДД.ММ.ГГГГ")) И НДС <> Объект.Налог Тогда
	 	
	 		МенеджерЗаписи = РегистрыСведений.НДС.СоздатьМенеджерЗаписи(); 
			МенеджерЗаписи.ДокРег 	= Объект.Ссылка; 
			МенеджерЗаписи.НДС 		= Объект.Налог; 
