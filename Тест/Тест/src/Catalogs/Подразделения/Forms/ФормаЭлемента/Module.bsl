...
&НаСервере
Процедура ПроверкаНаУправление (ТумблерДолжностей)
// ++ Мазин 20.05.2400

		Если объект.Родитель.Код = "000000105" И ТумблерДолжностей  Тогда 
		Элементы.ОкладСуммаУправлениеиОтдел.Видимость=Истина;
		Элементы.Оклад1.Видимость=Ложь; 
		Иначе 
		Элементы.ОкладСуммаУправлениеиОтдел.Видимость=Ложь; 
		Элементы.Оклад1.Видимость=Истина; 	
		КонецЕсли;
...
&НаСервере
Процедура ОкладыУправленияИОтделов()
//++Мазин 20.05.24
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Объект.ОкладСуммаУправлениеиОтдел = ВыборкаДетальныеЗаписи.Оклад;
	КонецЦикла;
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ПодразделенияУчастники.Ссылка.Ссылка В ИЕРАРХИИ (&Ссылка)";
...
Процедура ПоказатьСкрытьДолжности(Команда)
	Если ТумблерДолжностей Тогда
		...
		// ++ МАзин  20.05.24
		ПроверкаНаУправление (ТумблерДолжностей);
		// -- МАзин  20.05.24
	Иначе
...
Процедура РассчётДохода()
	...
	Объект.Доход = (Константы.ВыручкаПредприятия.Получить() * (Объект.ПроценОтВыручки/100)) + Объект.ОкладСуммаУправлениеиОтдел;
	Объект.Премия = (Константы.ВыручкаПредприятия.Получить() * (Объект.ПроценОтВыручки/100));
	Доход = (Константы.ВыручкаПредприятия.Получить() * (Объект.ПроценОтВыручки/100)) + Объект.ОкладСуммаУправлениеиОтдел;
	Объект.ДоходПодразделения = (Константы.ВыручкаПредприятия.Получить() * (Объект.ПроценОтВыручки/100)) + Объект.Оклад;
	// -- Мазин 20-05-24
КонецПроцедуры
