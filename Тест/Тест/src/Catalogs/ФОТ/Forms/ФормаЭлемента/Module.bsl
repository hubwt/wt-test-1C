...
Пока НЕ Родитель = Справочники.ФОТ.ПустаяСсылка() Цикл
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(СУММА(ФОТ.ЗППлан), 0) КАК ЗППлан
		|ИЗ
	...
	РодительОбъект.ЗППлан 					= РезультатЗапроса.ЗППлан; 	
...
Процедура ПриОткрытии(Отказ)
	...
	ЕСли Объект.НазначениеФОТ = ПредопределенноеЗначение("Перечисление.НазначенияФОТ.Отдел") Тогда 
		Элементы.ОтделДДС.Видимость = истина; 
	Иначе 
		Элементы.ОтделДДС.Видимость = ложь; 	
	КонецЕсли; 
