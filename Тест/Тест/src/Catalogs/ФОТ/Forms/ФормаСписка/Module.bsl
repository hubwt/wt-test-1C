...
&НаСервере
Процедура ПерерасчетСумм()
		
	ЗапросДанные = Новый Запрос;
	ЗапросДанные.Текст = 
	"ВЫБРАТЬ
	|	ФОТ.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ФОТ КАК ФОТ";
	
	РезультатЗапросаДанные = ЗапросДанные.Выполнить().Выбрать();
	
	Пока РезультатЗапросаДанные.Следующий() Цикл
		Родитель = РезультатЗапросаДанные.Ссылка.Родитель;
