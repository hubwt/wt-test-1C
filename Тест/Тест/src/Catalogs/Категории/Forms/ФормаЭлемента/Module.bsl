...
Процедура ПриОткрытииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) ТОгда
		...
		Если РезультатЗапроса.Пустой() тогда 
			...
			Элементы.АвитоТехХарактеристикаЗапчасти.Видимость 	= Истина; 
			Элементы.АвитоТипЗапчасти.Видимость					= Истина;
			
			Если ЗначениеЗаполнено(Объект.АвитоТехХарактеристикаЗапчасти) Тогда 
				Элементы.АвитоТипЗапчасти.ТолькоПросмотр			= Ложь;
				АвитоТипЗапчастиСписокВыбора(); 
			Иначе  
				Элементы.АвитоТипЗапчасти.ТолькоПросмотр			= Истина;
			КонецЕсли;
		
			АвитоТехХарактеристикаЗапчастиСписокВыбора();
...
&НаСервере
Процедура АвитоТехХарактеристикаЗапчастиСписокВыбора()
	
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КатегорииАвито.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.КатегорииАвито КАК КатегорииАвито
		|ГДЕ
		|	КатегорииАвито.Родитель = ЗНАЧЕНИЕ(Справочник.КатегорииАвито.ПустаяСсылка)";
	
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	
	Элементы.АвитоТехХарактеристикаЗапчасти.СписокВыбора.Очистить();
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Элементы.АвитоТехХарактеристикаЗапчасти.СписокВыбора.Добавить(ВыборкаДетальныеЗаписи.Ссылка);
	КонецЦикла;
...
&НаСервере
Процедура АвитоТипЗапчастиСписокВыбора()
	...
	Запрос.Текст =
		...
		|	Справочник.КатегорииАвито КАК КатегорииАвито
		|ГДЕ
		|	КатегорииАвито.Родитель = &Родитель";
	
	Запрос.УстановитьПараметр("Родитель", Объект.АвитоТехХарактеристикаЗапчасти);
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	
	Элементы.АвитоТипЗапчасти.СписокВыбора.Очистить();
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Элементы.АвитоТипЗапчасти.СписокВыбора.Добавить(ВыборкаДетальныеЗаписи.Ссылка);
	КонецЦикла;
...
&НаКлиенте
Процедура АвитоТехХарактеристикаЗапчастиПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.АвитоТехХарактеристикаЗапчасти) Тогда 
		Элементы.АвитоТипЗапчасти.ТолькоПросмотр			= Ложь; 
		АвитоТипЗапчастиСписокВыбора(); 		
	КонецЕсли;
