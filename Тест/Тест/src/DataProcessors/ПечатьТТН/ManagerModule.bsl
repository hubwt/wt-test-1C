...
Функция РазложитьМассивПоТипамОбъектов(МассивОбъектов)
	...
	Для Каждого Объект Из МассивОбъектов Цикл
		...
		Если ТипЗнч(Объект) 	= Тип("ДокументСсылка.ПродажаЗапчастей") Тогда
			Если НЕ СтруктураТипов.Свойство("ПродажаЗапчастей") Тогда
				МассивРеализаций 	= Новый Массив;
				СтруктураТипов.Вставить("ПродажаЗапчастей", МассивРеализаций);
			КонецЕсли;
			СтруктураТипов.ПродажаЗапчастей.Добавить(Объект);
...
Функция СформироватьПечатнуюФормуТТН(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	дт_ПечатьДокументов.УстановитьМинимальныеПоляПечати(ТабличныйДокумент);
...
Процедура ЗаполнитьРеквизитыШапкиТТН(ДанныеПечати, Макет, ТабличныйДокумент, СведенияОКонтрагентах)
	...
	ПараметрыМакета = Новый Структура;
	ПараметрыМакета.Вставить("НомерДокумента", дт_ПрефиксацияКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Истина, Ложь));
	...
	Если ДанныеПечати.Организация = ДанныеПечати.Грузоотправитель Тогда
		ПараметрыМакета.Вставить("ПредставлениеОрганизации", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКонтрагентах.Поставщик,
																"ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны"));
	Иначе
		ПараметрыМакета.Вставить("ПредставлениеОрганизации", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКонтрагентах.Грузоотправитель,
																"ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны"));
	КонецЕсли;
	
	ПараметрыМакета.Вставить("ПредставлениеГрузополучателя", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКонтрагентах.Грузополучатель, 
																"ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет"));
	
	ПараметрыМакета.Вставить("ПредставлениеПлательщика", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКонтрагентах.Покупатель));
...
Процедура ЗаполнитьРеквизитыПодвалаТТН(ДанныеПечати, ИтоговыеСуммы, Макет, ТабличныйДокумент, ЕдиницаИзмеренияВеса = Неопределено, КоэффициентПересчетаВТонны = 0)
	...
	ПараметрыМакета.Вставить("ФИОГлавБухгалтера", ОтветственныеЛица.ГлавныйБухгалтерНаименование);
	
	ПараметрыМакета.Вставить("ФИОРуководителя", ОтветственныеЛица.РуководительНаименование);
	ПараметрыМакета.Вставить("ДолжностьРуководителя", ОтветственныеЛица.РуководительДолжность);
	
	ПараметрыМакета.Вставить("ФИОКладовщика", ОтветственныеЛица.КладовщикНаименование);
	ПараметрыМакета.Вставить("ДолжностьКладовщика", ОтветственныеЛица.КладовщикДолжность);
...
Процедура ЗаполнитьРеквизитыТранспортногоРазделаТТН(ДанныеПечати, Макет, ОбластьМакета, СведенияОКонтрагентах)
	...
	ПараметрыМакета.Вставить("НомерДокумента", дт_ПрефиксацияКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Ложь, Ложь));
	
	ПараметрыМакета.Вставить("ПредставлениеПеревозчика", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКонтрагентах.Перевозчик, 
	...
	ПараметрыМакета.Вставить("ПунктПогрузки",  ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКонтрагентах.Грузоотправитель, "ФактическийАдрес"));
	ПараметрыМакета.Вставить("ПунктРазгрузки", ДанныеПечати.АдресДоставки);
	
	ПараметрыМакета.Вставить("ПредставлениеЗаказчикаПеревозок", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОКонтрагентах.Покупатель,
																"НаименованиеДляПечатныхФорм,ЮридическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет"));
...
Процедура ЗаполнитьРеквизитыПодвалаСведенийОГрузе(ДанныеПечати, Макет, ОбластьМакета)
	...
	ПараметрыМакета.Вставить("ФИОКладовщика", ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ДанныеПечати.Кладовщик, ДанныеПечати.Дата));
	ПараметрыМакета.Вставить("ДолжностьКладовщика", ДанныеПечати.ДолжностьКладовщика);
...
Процедура ЗаполнитьТабличныйДокументТТН(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		СведенияОКонтрагентах.Вставить("Поставщик", ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Организация,      ДанныеПечати.Дата, ДанныеПечати.БанковскийСчетОрганизации));
		СведенияОКонтрагентах.Вставить("Покупатель", ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Контрагент,       ДанныеПечати.Дата));
		СведенияОКонтрагентах.Вставить("Перевозчик", ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Перевозчик,            ДанныеПечати.Дата));
		СведенияОКонтрагентах.Вставить("Грузоотправитель", ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Грузоотправитель, ДанныеПечати.Дата));
		СведенияОКонтрагентах.Вставить("Грузополучатель", ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Грузополучатель,  ДанныеПечати.Дата));
