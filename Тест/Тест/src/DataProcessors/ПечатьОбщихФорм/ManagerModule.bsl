...
Процедура ЗаполнитьРеквизитыШапкиТОРГ12(ДанныеПечати, Макет, ТабличныйДокумент)
	...
	Если ЗначениеЗаполнено(ДанныеПечати.Основание)
		И ТипЗнч(ДанныеПечати.Основание) = Тип("ДокументСсылка.ПредварительныйСчет") Тогда
		
		ДанныеОснования = Новый Структура("Номер,Дата",ДанныеПечати.ОснованиеНомер, ДанныеПечати.ОснованиеДата);
		СтруктураПараметров.Вставить("Основание", Документы.ПредварительныйСчет.СформироватьЗаголовокДокумента(ДанныеОснования, Метаданные.Документы.ПредварительныйСчет.Синоним));
		
		СтруктураПараметров.Вставить("ОснованиеНомер", дт_ПрефиксацияКлиентСервер.НомерНаПечать(ДанныеПечати.ОснованиеНомер));
...
Функция ЗаполнитьРеквизитыПодвалаТОРГ12(ДанныеПечати, ИтоговыеСуммы, Макет, КоэффициентПересчетаВТонны)
	...
	// Главный бухгалтер
	СтруктураПараметров.Вставить("ФИОГлавБухгалтера", ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ДанныеПечати.ГлавныйБухгалтер, ДанныеПечати.Дата));
	
	// Отпуск товара разрешил
	СтруктураПараметров.Вставить("ФИОРуководителя", ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ДанныеПечати.Руководитель, ДанныеПечати.Дата));
...
Процедура ЗаполнитьРеквизитыСтрокиТовара(СтрокаТовары, ОбластьМакета, НомерСтроки, ВыводитьКодНоменклатуры = Истина, ВыводитьКодТНВД = Неопределено, ДобавлятьПрефиксАртикул = Истина)
	...
	СтруктураПараметров.Вставить("СтавкаНДС", ?(СтрокаТовары.СтавкаНДС = Неопределено, "Без НДС", Строка(СтрокаТовары.СтавкаНДС) + "%"));
...
Функция СформироватьПечатнуюФормуУПД(СтруктураТипов, ОбъектыПечати, ПараметрыПечати = Неопределено, КомплектыПечати = Неопределено) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТиповНаПечать Цикл
		...
		ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, КомплектыПечати, ПараметрыПечати);
...
Процедура ЗаполнитьРеквизитыШапкиУПД(ДанныеПечати, СведенияОПоставщике, СведенияОПокупателе, Макет, ТабличныйДокумент)
	...
	ПараметрыШапки.Вставить("Номер", НомерСчетаФактурыНаПечать(ДанныеПечати.Номер, ДанныеПечати.ИндексПодразделения, Истина));
...
Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, КомплектыПечати = Неопределено, ПараметрыПечати) Экспорт
	...
	// Печать
	СПечатью = ?(ПараметрыПечати = Неопределено, 
		Ложь,
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыПечати, "СПечатью", Ложь)
	);
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если СПечатью Тогда
			...
			ДвоичныеДанныеПечати              = Справочники.Организация.ДвоичныеДанныеКартинкиОрганизации(ДанныеПечати.Организация, "ФайлПечать");
			ДвоичныеДанныеПодписьРуководителя = Справочники.Организация.ДвоичныеДанныеКартинкиОрганизации(ДанныеПечати.Организация, "ФайлПодписьРуководителя");
			ДвоичныеДанныеПодписьГлавногоБухгалтера = Справочники.Организация.ДвоичныеДанныеКартинкиОрганизации(ДанныеПечати.Организация, "ФайлПодписьГлавногоБухгалтера");
		...
		Пока СтрокаТовары.Следующий() Цикл
			...
			//Иначе
				...
				ЗаполнитьРеквизитыСтрокиТовара(СтрокаТовары, ОбластьСтрока, НомерСтроки, , ВыводитьКодыТНВЭД, ВыводитьАртикулВСтроке);
...
Функция ВыводитьКодыТНВЭД(ДанныеПечати, ДействуетПостановление981)
	...
	ПараметрыВывода.ВыводитьВСтроке = УчетНДСУТ.СтранаЯвляетсяЧленомЕАЭС(ДанныеПечати.СтранаРегистрации, ДанныеПечати.Дата)
