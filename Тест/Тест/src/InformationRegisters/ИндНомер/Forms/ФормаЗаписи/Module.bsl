...
Процедура ЗаполнитьОписаниеНаСервере()
	...
	СостояниеДляОписания = ""; 
	
	Если Запись.Состояние =Перечисления.Состояние.БУ  Тогда СостояниеДляОписания = "Б/У"; 	
	ИначеЕсли Запись.Состояние =Перечисления.Состояние.Восстановленная  Тогда СостояниеДляОписания = "Запчасть подготовлена к продаже."; 
	ИначеЕсли Запись.Состояние =Перечисления.Состояние.Новый  			Тогда СостояниеДляОписания = "Новая."; 
	ИначеЕсли Запись.Состояние =Перечисления.Состояние.Перекупка  		Тогда СостояниеДляОписания = "Б/У."; 
	ИначеЕсли Запись.Состояние =Перечисления.Состояние.ТребуетРемонта  	Тогда СостояниеДляОписания = "Запчасть требует ремонта."; 
	КонецЕсли; 
	
	Если ЗначениеЗаполнено( Запись.Наименование) И ЗначениеЗаполнено( Запись.АккаунтАвито)  Тогда 
		НаименованиеАвито  	= Строка(Запись.Наименование);
		индкодАвито 		= Строка(Запись.индкод);  
		ОпсаниеОбъявления = НайтиОписаниеАвитоМенеджера(Запись.АккаунтАвито);
		Строка = 
				НаименованиеАвито + 
				"
				| 
				|Инд Код: " +  индкодАвито + ", " + СостояниеДляОписания + 
				"
				|
				|" + ОпсаниеОбъявления;
...
Функция НайтиОписаниеАвитоМенеджера(ПользовательНаименование)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	АвитоШаблонОписанияМенеджеров.АвитоОписаниеОбъявления КАК АвитоОписаниеОбъявления
		|ИЗ
		|	Справочник.АвитоШаблонОписанияМенеджеров КАК АвитоШаблонОписанияМенеджеров
		|ГДЕ
		|	АвитоШаблонОписанияМенеджеров.Менеджер.Наименование = &Наименование
		|	И АвитоШаблонОписанияМенеджеров.ПометкаУдаления = ЛОЖЬ";
	
	Запрос.УстановитьПараметр("Наименование", ПользовательНаименование);
	
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	ВыборкаДетальныеЗаписи.Следующий();
	
	Возврат ВыборкаДетальныеЗаписи.АвитоОписаниеОбъявления; 
КонецФункции
...
МассивДолжностей.Добавить(Справочники.ДолжностиПредприятия.НайтиПоКоду("000000001")); // Аналитик по ценообразованию !
МассивДолжностей.Добавить(Справочники.ДолжностиПредприятия.НайтиПоКоду("000000122")); // Для Клодовщик, для Юношевой
