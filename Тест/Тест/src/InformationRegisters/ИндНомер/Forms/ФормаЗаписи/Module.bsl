...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Запись.АвтоВыгрНаАвитоНетВАвито = Перечисления.АвтоВыгрНаАвитоНетВАвито.ПродаетсяВАвито Тогда 
		...
		//Элементы.СсылкаНаФайл.Видимость = Истина;
	Иначе 
		Элементы.АккаунтАвито.Видимость = Ложь; 
		//Элементы.СсылкаНаФайл.Видимость = Ложь; 
	КонецЕсли;
...
Запрос.Текст =
	"ВЫБРАТЬ
	|	Сотрудники.Пользователь.Наименование КАК Пользователь,
	|	Сотрудники.ДолжностьДляУК КАК ДолжностьДляУК
	|ИЗ
	|	Справочник.Сотрудники КАК Сотрудники
	|ГДЕ
	|	Сотрудники.ДолжностьДляУК.Код = ""000000001""";
...
Пока Выборка.Следующий() Цикл
	Элементы.АккаунтАвито.СписокВыбора.Добавить(Выборка.Пользователь);
КонецЦикла;
